...
$Page->showMessage();
?>
<form name="fcont_asientoadd" id="fcont_asientoadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" autocomplete="off">
...
<span id="el_cont_asiento_cuenta">
    <select
        ...
        data-select2-id="fcont_asientoadd_x_cuenta"
...
<script>
loadjs.ready("fcont_asientoadd", function() {
    var options = { name: "x_cuenta", selectId: "fcont_asientoadd_x_cuenta" };
    if (fcont_asientoadd.lists.cuenta?.lookupOptions.length) {
        options.data = { id: "x_cuenta", form: "fcont_asientoadd" };
    } else {
        options.ajax = { id: "x_cuenta", form: "fcont_asientoadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fcont_asientoadd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fcont_asientoadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
