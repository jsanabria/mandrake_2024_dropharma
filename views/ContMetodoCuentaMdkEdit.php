...
<?= $Page->Pager->render() ?>
<?php } ?>
<form name="fcont_metodo_cuenta_mdkedit" id="fcont_metodo_cuenta_mdkedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" autocomplete="off">
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { cont_metodo_cuenta_mdk: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var fcont_metodo_cuenta_mdkedit;
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        .setId("fcont_metodo_cuenta_mdkedit")
        .setPageId("edit")
...
<?php if ($Page->id->Visible) { // id ?>
    <div id="r_id"<?= $Page->id->rowAttributes() ?>>
        <label id="elh_cont_metodo_cuenta_mdk_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->id->cellAttributes() ?>>
<span id="el_cont_metodo_cuenta_mdk_id">
<span<?= $Page->id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id->getDisplayValue($Page->id->EditValue))) ?>"></span>
<input type="hidden" data-table="cont_metodo_cuenta_mdk" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<span id="el_cont_metodo_cuenta_mdk_tipo_destino">
    <select
        ...
        data-select2-id="fcont_metodo_cuenta_mdkedit_x_tipo_destino"
        <?php } ?>
...
<?php if (!$Page->tipo_destino->IsNativeSelect) { ?>
<script>
loadjs.ready("fcont_metodo_cuenta_mdkedit", function() {
    var options = { name: "x_tipo_destino", selectId: "fcont_metodo_cuenta_mdkedit_x_tipo_destino" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fcont_metodo_cuenta_mdkedit.lists.tipo_destino?.lookupOptions.length) {
        options.data = { id: "x_tipo_destino", form: "fcont_metodo_cuenta_mdkedit" };
    } else {
        options.ajax = { id: "x_tipo_destino", form: "fcont_metodo_cuenta_mdkedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_cont_metodo_cuenta_mdk_cuenta_id">
    <select
        ...
        data-select2-id="fcont_metodo_cuenta_mdkedit_x_cuenta_id"
        data-table="cont_metodo_cuenta_mdk"
...
<script>
loadjs.ready("fcont_metodo_cuenta_mdkedit", function() {
    var options = { name: "x_cuenta_id", selectId: "fcont_metodo_cuenta_mdkedit_x_cuenta_id" };
    if (fcont_metodo_cuenta_mdkedit.lists.cuenta_id?.lookupOptions.length) {
        options.data = { id: "x_cuenta_id", form: "fcont_metodo_cuenta_mdkedit" };
    } else {
        options.ajax = { id: "x_cuenta_id", form: "fcont_metodo_cuenta_mdkedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fcont_metodo_cuenta_mdkedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fcont_metodo_cuenta_mdkedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
