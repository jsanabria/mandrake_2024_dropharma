...
<?php } ?>
<?php } ?>
<form name="falmacenview" id="falmacenview" class="ew-form ew-view-form overlay-wrapper" action="<?= CurrentPageUrl(false) ?>" method="post" autocomplete="off">
<?php if (!$Page->isExport()) { ?>
<script>
...
<?php if ($Page->movimiento->Visible) { // movimiento ?>
    <tr id="r_movimiento"<?= $Page->movimiento->rowAttributes() ?>>
