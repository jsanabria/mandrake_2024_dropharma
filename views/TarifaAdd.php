...
$Page->showMessage();
?>
<form name="ftarifaadd" id="ftarifaadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" autocomplete="off">
...
<span id="el_tarifa_patron">
    <select
        ...
        data-select2-id="ftarifaadd_x_patron"
        <?php } ?>
        data-table="tarifa"
        data-field="x_patron"
...
<?php if (!$Page->patron->IsNativeSelect) { ?>
<script>
loadjs.ready("ftarifaadd", function() {
    var options = { name: "x_patron", selectId: "ftarifaadd_x_patron" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (ftarifaadd.lists.patron?.lookupOptions.length) {
        options.data = { id: "x_patron", form: "ftarifaadd" };
    } else {
        options.ajax = { id: "x_patron", form: "ftarifaadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_tarifa_activo">
    <select
        ...
        data-select2-id="ftarifaadd_x_activo"
        <?php } ?>
        data-table="tarifa"
        data-field="x_activo"
...
<script>
loadjs.ready("ftarifaadd", function() {
    var options = { name: "x_activo", selectId: "ftarifaadd_x_activo" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (ftarifaadd.lists.activo?.lookupOptions.length) {
        options.data = { id: "x_activo", form: "ftarifaadd" };
    } else {
        options.ajax = { id: "x_activo", form: "ftarifaadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<?php
    if (in_array("tarifa_descuento_utilidad", explode(",", $Page->getCurrentDetailTable())) && $tarifa_descuento_utilidad->DetailAdd) {
?>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="ftarifaadd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="ftarifaadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
