...
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        .setId("fpagos_proveedor_facturagrid")
        .setPageId("grid")
        ...
        .setEmptyRow(
            function (rowIndex) {
                let fobj = this.getForm(),
                    fields = [["tipo_documento",false],["abono",false],["monto",false],["comprobante",false]];
...
<?php } ?>
<div id="fpagos_proveedor_facturagrid" class="ew-form ew-list-form">
<div id="gmp_pagos_proveedor_factura" class="card-body ew-grid-middle-panel <?= $Grid->TableContainerClass ?>" style="<?= $Grid->TableContainerStyle ?>">
<table id="tbl_pagos_proveedor_facturagrid" class="<?= $Grid->TableClass ?>"><!-- .ew-table -->
<thead>
...
<?php if ($Grid->tipo_documento->Visible) { // tipo_documento ?>
        <th data-name="tipo_documento" class="<?= $Grid->tipo_documento->headerCellClass() ?>"><div id="elh_pagos_proveedor_factura_tipo_documento" class="pagos_proveedor_factura_tipo_documento"><?= $Grid->renderFieldHeader($Grid->tipo_documento) ?></div></th>
<?php } ?>
<?php if ($Grid->abono->Visible) { // abono ?>
        <th data-name="abono" class="<?= $Grid->abono->headerCellClass() ?>"><div id="elh_pagos_proveedor_factura_abono" class="pagos_proveedor_factura_abono"><?= $Grid->renderFieldHeader($Grid->abono) ?></div></th>
<?php } ?>
<?php if ($Grid->monto->Visible) { // monto ?>
        <th data-name="monto" class="<?= $Grid->monto->headerCellClass() ?>"><div id="elh_pagos_proveedor_factura_monto" class="pagos_proveedor_factura_monto"><?= $Grid->renderFieldHeader($Grid->monto) ?></div></th>
<?php } ?>
<?php if ($Grid->comprobante->Visible) { // comprobante ?>
        <th data-name="comprobante" class="<?= $Grid->comprobante->headerCellClass() ?>"><div id="elh_pagos_proveedor_factura_comprobante" class="pagos_proveedor_factura_comprobante"><?= $Grid->renderFieldHeader($Grid->comprobante) ?></div></th>
<?php } ?>
<?php
...
<span id="el<?= $Grid->RowIndex == '$rowindex$' ? '$rowindex$' : $Grid->RowCount ?>_pagos_proveedor_factura_tipo_documento" class="el_pagos_proveedor_factura_tipo_documento">
<?php
if (IsRTL()) {
...
loadjs.ready("fpagos_proveedor_facturagrid", function() {
    fpagos_proveedor_facturagrid.createAutoSuggest(Object.assign({"id":"x<?= $Grid->RowIndex ?>_tipo_documento","forceSelect":false}, { lookupAllDisplayFields: <?= $Grid->tipo_documento->Lookup->LookupAllDisplayFields ? "true" : "false" ?> }, ew.vars.tables.pagos_proveedor_factura.fields.tipo_documento.autoSuggestOptions));
});
</script>
<?= $Grid->tipo_documento->Lookup->getParamTag($Grid, "p_x" . $Grid->RowIndex . "_tipo_documento") ?>
</span>
<input type="hidden" data-table="pagos_proveedor_factura" data-field="x_tipo_documento" data-hidden="1" data-old name="o<?= $Grid->RowIndex ?>_tipo_documento" id="o<?= $Grid->RowIndex ?>_tipo_documento" value="<?= HtmlEncode($Grid->tipo_documento->OldValue) ?>">
<?php } ?>
...
<input type="hidden" data-table="pagos_proveedor_factura" data-field="x_tipo_documento" data-hidden="1" data-old name="fpagos_proveedor_facturagrid$o<?= $Grid->RowIndex ?>_tipo_documento" id="fpagos_proveedor_facturagrid$o<?= $Grid->RowIndex ?>_tipo_documento" value="<?= HtmlEncode($Grid->tipo_documento->OldValue) ?>">
<?php } ?>
<?php } ?>
</td>
    <?php } ?>
...
<?php if ($Grid->RowType == RowType::ADD) { // Add record ?>
<span id="el<?= $Grid->RowIndex == '$rowindex$' ? '$rowindex$' : $Grid->RowCount ?>_pagos_proveedor_factura_abono" class="el_pagos_proveedor_factura_abono">
<template id="tp_x<?= $Grid->RowIndex ?>_abono">
    <div class="form-check">
        <input type="radio" class="form-check-input" data-table="pagos_proveedor_factura" data-field="x_abono" name="x<?= $Grid->RowIndex ?>_abono" id="x<?= $Grid->RowIndex ?>_abono"<?= $Grid->abono->editAttributes() ?>>
...
<input type="hidden" data-table="pagos_proveedor_factura" data-field="x_abono" data-hidden="1" data-old name="o<?= $Grid->RowIndex ?>_abono" id="o<?= $Grid->RowIndex ?>_abono" value="<?= HtmlEncode($Grid->abono->OldValue) ?>">
<?php } ?>
<?php if ($Grid->RowType == RowType::EDIT) { // Edit record ?>
<span id="el<?= $Grid->RowIndex == '$rowindex$' ? '$rowindex$' : $Grid->RowCount ?>_pagos_proveedor_factura_abono" class="el_pagos_proveedor_factura_abono">
<template id="tp_x<?= $Grid->RowIndex ?>_abono">
...
<input type="hidden" data-table="pagos_proveedor_factura" data-field="x_abono" data-hidden="1" data-old name="fpagos_proveedor_facturagrid$o<?= $Grid->RowIndex ?>_abono" id="fpagos_proveedor_facturagrid$o<?= $Grid->RowIndex ?>_abono" value="<?= HtmlEncode($Grid->abono->OldValue) ?>">
<?php } ?>
<?php } ?>
</td>
    <?php } ?>
...
<?php if ($Grid->RowType == RowType::ADD) { // Add record ?>
<span id="el<?= $Grid->RowIndex == '$rowindex$' ? '$rowindex$' : $Grid->RowCount ?>_pagos_proveedor_factura_monto" class="el_pagos_proveedor_factura_monto">
<input type="<?= $Grid->monto->getInputTextType() ?>" name="x<?= $Grid->RowIndex ?>_monto" id="x<?= $Grid->RowIndex ?>_monto" data-table="pagos_proveedor_factura" data-field="x_monto" value="<?= $Grid->monto->EditValue ?>" size="30" placeholder="<?= HtmlEncode($Grid->monto->getPlaceHolder()) ?>" data-format-pattern="<?= HtmlEncode($Grid->monto->formatPattern()) ?>"<?= $Grid->monto->editAttributes() ?>>
<div class="invalid-feedback"><?= $Grid->monto->getErrorMessage() ?></div>
</span>
<input type="hidden" data-table="pagos_proveedor_factura" data-field="x_monto" data-hidden="1" data-old name="o<?= $Grid->RowIndex ?>_monto" id="o<?= $Grid->RowIndex ?>_monto" value="<?= HtmlEncode($Grid->monto->OldValue) ?>">
<?php } ?>
<?php if ($Grid->RowType == RowType::EDIT) { // Edit record ?>
<span id="el<?= $Grid->RowIndex == '$rowindex$' ? '$rowindex$' : $Grid->RowCount ?>_pagos_proveedor_factura_monto" class="el_pagos_proveedor_factura_monto">
<input type="<?= $Grid->monto->getInputTextType() ?>" name="x<?= $Grid->RowIndex ?>_monto" id="x<?= $Grid->RowIndex ?>_monto" data-table="pagos_proveedor_factura" data-field="x_monto" value="<?= $Grid->monto->EditValue ?>" size="30" placeholder="<?= HtmlEncode($Grid->monto->getPlaceHolder()) ?>" data-format-pattern="<?= HtmlEncode($Grid->monto->formatPattern()) ?>"<?= $Grid->monto->editAttributes() ?>>
...
<input type="hidden" data-table="pagos_proveedor_factura" data-field="x_monto" data-hidden="1" data-old name="fpagos_proveedor_facturagrid$o<?= $Grid->RowIndex ?>_monto" id="fpagos_proveedor_facturagrid$o<?= $Grid->RowIndex ?>_monto" value="<?= HtmlEncode($Grid->monto->OldValue) ?>">
<?php } ?>
<?php } ?>
</td>
    <?php } ?>
...
<?php if ($Grid->RowType == RowType::ADD) { // Add record ?>
<span id="el<?= $Grid->RowIndex == '$rowindex$' ? '$rowindex$' : $Grid->RowCount ?>_pagos_proveedor_factura_comprobante" class="el_pagos_proveedor_factura_comprobante">
<?php
if (IsRTL()) {
...
<script>
loadjs.ready("fpagos_proveedor_facturagrid", function() {
    fpagos_proveedor_facturagrid.createAutoSuggest(Object.assign({"id":"x<?= $Grid->RowIndex ?>_comprobante","forceSelect":false}, { lookupAllDisplayFields: <?= $Grid->comprobante->Lookup->LookupAllDisplayFields ? "true" : "false" ?> }, ew.vars.tables.pagos_proveedor_factura.fields.comprobante.autoSuggestOptions));
});
</script>
<?= $Grid->comprobante->Lookup->getParamTag($Grid, "p_x" . $Grid->RowIndex . "_comprobante") ?>
</span>
<input type="hidden" data-table="pagos_proveedor_factura" data-field="x_comprobante" data-hidden="1" data-old name="o<?= $Grid->RowIndex ?>_comprobante" id="o<?= $Grid->RowIndex ?>_comprobante" value="<?= HtmlEncode($Grid->comprobante->OldValue) ?>">
<?php } ?>
<?php if ($Grid->RowType == RowType::EDIT) { // Edit record ?>
<span id="el<?= $Grid->RowIndex == '$rowindex$' ? '$rowindex$' : $Grid->RowCount ?>_pagos_proveedor_factura_comprobante" class="el_pagos_proveedor_factura_comprobante">
<?php
...
<input type="hidden" data-table="pagos_proveedor_factura" data-field="x_comprobante" data-hidden="1" data-old name="fpagos_proveedor_facturagrid$o<?= $Grid->RowIndex ?>_comprobante" id="fpagos_proveedor_facturagrid$o<?= $Grid->RowIndex ?>_comprobante" value="<?= HtmlEncode($Grid->comprobante->OldValue) ?>">
<?php } ?>
<?php } ?>
</td>
    <?php } ?>
...
loadjs.ready(["fpagos_proveedor_facturagrid","load"], () => fpagos_proveedor_facturagrid.updateLists(<?= $Grid->RowIndex ?><?= $Grid->isAdd() || $Grid->isEdit() || $Grid->isCopy() || $Grid->RowIndex === '$rowindex$' ? ", true" : "" ?>));
</script>
<?php } ?>
<?php
    }
