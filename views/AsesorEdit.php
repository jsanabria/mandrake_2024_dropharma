...
<?= $Page->Pager->render() ?>
<?php } ?>
<form name="fasesoredit" id="fasesoredit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" autocomplete="off">
<script>
...
<span id="el_asesor_ciudad">
    <select
        ...
        data-select2-id="fasesoredit_x_ciudad"
        data-table="asesor"
        data-field="x_ciudad"
...
<script>
loadjs.ready("fasesoredit", function() {
    var options = { name: "x_ciudad", selectId: "fasesoredit_x_ciudad" };
    if (fasesoredit.lists.ciudad?.lookupOptions.length) {
        options.data = { id: "x_ciudad", form: "fasesoredit" };
    } else {
        options.ajax = { id: "x_ciudad", form: "fasesoredit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_asesor_activo">
    <select
        ...
        data-select2-id="fasesoredit_x_activo"
        <?php } ?>
        data-table="asesor"
        data-field="x_activo"
...
<script>
loadjs.ready("fasesoredit", function() {
    var options = { name: "x_activo", selectId: "fasesoredit_x_activo" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fasesoredit.lists.activo?.lookupOptions.length) {
        options.data = { id: "x_activo", form: "fasesoredit" };
    } else {
        options.ajax = { id: "x_activo", form: "fasesoredit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<?php } ?>
    ...
    <input type="hidden" data-table="asesor" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
<?php
    if (in_array("asesor_cliente", explode(",", $Page->getCurrentDetailTable())) && $asesor_cliente->DetailEdit) {
?>
...
<?php } ?>
<?php
    if (in_array("asesor_fabricante", explode(",", $Page->getCurrentDetailTable())) && $asesor_fabricante->DetailEdit) {
?>
<?php if ($Page->getCurrentDetailTable() != "") { ?>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fasesoredit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fasesoredit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
