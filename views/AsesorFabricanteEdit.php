...
<?= $Page->Pager->render() ?>
<?php } ?>
<form name="fasesor_fabricanteedit" id="fasesor_fabricanteedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" autocomplete="off">
<script>
...
<span id="el_asesor_fabricante_fabricante">
    <select
        ...
        data-select2-id="fasesor_fabricanteedit_x_fabricante"
...
<script>
loadjs.ready("fasesor_fabricanteedit", function() {
    var options = { name: "x_fabricante", selectId: "fasesor_fabricanteedit_x_fabricante" };
    if (fasesor_fabricanteedit.lists.fabricante?.lookupOptions.length) {
        options.data = { id: "x_fabricante", form: "fasesor_fabricanteedit" };
    } else {
        options.ajax = { id: "x_fabricante", form: "fasesor_fabricanteedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fasesor_fabricanteedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fasesor_fabricanteedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
