...
<?php } ?>
<?php if (!$Page->IsModal) { ?>
<form name="fuserlevelssrch" id="fuserlevelssrch" class="ew-form ew-ext-search-form" action="<?= CurrentPageUrl(false) ?>" autocomplete="off">
...
<?php } ?>
        ...
        // Add fields
        .addFields([
            ["tipo_acceso", [], fields.tipo_acceso.isInvalid]
        ])
        // Validate form
        .setValidate(
            async function () {
...
<?php
if (!$Page->tipo_acceso->UseFilter) {
    $Page->SearchColumnCount++;
}
?>
    <div id="xs_tipo_acceso" class="col-sm-auto d-sm-flex align-items-start mb-3 px-0 pe-sm-2<?= $Page->tipo_acceso->UseFilter ? " ew-filter-field" : "" ?>">
        <div class="d-flex my-1 my-sm-0">
            <label for="x_tipo_acceso" class="ew-search-caption ew-label"><?= $Page->tipo_acceso->caption() ?></label>
            <div class="ew-search-operator">
<?= $Language->phrase("=") ?>
<input type="hidden" name="z_tipo_acceso" id="z_tipo_acceso" value="=">
</div>
        </div>
        <div id="el_userlevels_tipo_acceso" class="ew-search-field">
    <select
        id="x_tipo_acceso"
        name="x_tipo_acceso"
        class="form-select ew-select<?= $Page->tipo_acceso->isInvalidClass() ?>"
        <?php if (!$Page->tipo_acceso->IsNativeSelect) { ?>
        data-select2-id="fuserlevelssrch_x_tipo_acceso"
        <?php } ?>
    ...
    <div class="invalid-feedback"><?= $Page->tipo_acceso->getErrorMessage(false) ?></div>
<?php if (!$Page->tipo_acceso->IsNativeSelect) { ?>
<script>
loadjs.ready("fuserlevelssrch", function() {
    var options = { name: "x_tipo_acceso", selectId: "fuserlevelssrch_x_tipo_acceso" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fuserlevelssrch.lists.tipo_acceso?.lookupOptions.length) {
        options.data = { id: "x_tipo_acceso", form: "fuserlevelssrch" };
    } else {
        options.ajax = { id: "x_tipo_acceso", form: "fuserlevelssrch", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<?php } ?>
<?php if ($Page->tipo_acceso->Visible) { // tipo_acceso ?>
        <th data-name="tipo_acceso" class="<?= $Page->tipo_acceso->headerCellClass() ?>"><div id="elh_userlevels_tipo_acceso" class="userlevels_tipo_acceso"><?= $Page->renderFieldHeader($Page->tipo_acceso) ?></div></th>
<?php } ?>
<?php
...
<span id="el<?= $Page->RowIndex == '$rowindex$' ? '$rowindex$' : $Page->RowCount ?>_userlevels_tipo_acceso" class="el_userlevels_tipo_acceso">
