...
<?= $Page->Pager->render() ?>
<?php } ?>
<form name="fcompaniaedit" id="fcompaniaedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" autocomplete="off">
<script>
...
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        ...
        .setFields([
            ...
            ["agente_retencion", [fields.agente_retencion.visible && fields.agente_retencion.required ? ew.Validators.required(fields.agente_retencion.caption) : null], fields.agente_retencion.isInvalid],
...
<?php if ($Page->ciudad->Visible) { // ciudad ?>
    <div id="r_ciudad"<?= $Page->ciudad->rowAttributes() ?>>
        <label id="elh_compania_ciudad" class="<?= $Page->LeftColumnClass ?>"><?= $Page->ciudad->caption() ?><?= $Page->ciudad->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<span id="el_compania_ciudad">
    <select
        ...
        data-select2-id="fcompaniaedit_x_ciudad"
        data-table="compania"
        data-field="x_ciudad"
...
<script>
loadjs.ready("fcompaniaedit", function() {
    var options = { name: "x_ciudad", selectId: "fcompaniaedit_x_ciudad" };
    if (fcompaniaedit.lists.ciudad?.lookupOptions.length) {
        options.data = { id: "x_ciudad", form: "fcompaniaedit" };
    } else {
        options.ajax = { id: "x_ciudad", form: "fcompaniaedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options = Object.assign({}, ew.modalLookupOptions, options, ew.vars.tables.compania.fields.ciudad.modalLookupOptions);
...
<?php if ($Page->direccion->Visible) { // direccion ?>
    <div id="r_direccion"<?= $Page->direccion->rowAttributes() ?>>
        <label id="elh_compania_direccion" for="x_direccion" class="<?= $Page->LeftColumnClass ?>"><?= $Page->direccion->caption() ?><?= $Page->direccion->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->direccion->cellAttributes() ?>>
<span id="el_compania_direccion">
<textarea data-table="compania" data-field="x_direccion" name="x_direccion" id="x_direccion" cols="35" rows="3" placeholder="<?= HtmlEncode($Page->direccion->getPlaceHolder()) ?>"<?= $Page->direccion->editAttributes() ?> aria-describedby="x_direccion_help"><?= $Page->direccion->EditValue ?></textarea>
...
<?php if ($Page->agente_retencion->Visible) { // agente_retencion ?>
    <div id="r_agente_retencion"<?= $Page->agente_retencion->rowAttributes() ?>>
        <label id="elh_compania_agente_retencion" for="x_agente_retencion" class="<?= $Page->LeftColumnClass ?>"><?= $Page->agente_retencion->caption() ?><?= $Page->agente_retencion->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->agente_retencion->cellAttributes() ?>>
<span id="el_compania_agente_retencion">
    <select
        ...
        data-select2-id="fcompaniaedit_x_agente_retencion"
        <?php } ?>
        data-table="compania"
    ...
    <?= $Page->agente_retencion->getCustomMessage() ?>
    <div class="invalid-feedback"><?= $Page->agente_retencion->getErrorMessage() ?></div>
<?php if (!$Page->agente_retencion->IsNativeSelect) { ?>
<script>
loadjs.ready("fcompaniaedit", function() {
    var options = { name: "x_agente_retencion", selectId: "fcompaniaedit_x_agente_retencion" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fcompaniaedit.lists.agente_retencion?.lookupOptions.length) {
        options.data = { id: "x_agente_retencion", form: "fcompaniaedit" };
    } else {
        options.ajax = { id: "x_agente_retencion", form: "fcompaniaedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<?php if ($Page->logo->Visible) { // logo ?>
    <div id="r_logo"<?= $Page->logo->rowAttributes() ?>>
        <label id="elh_compania_logo" class="<?= $Page->LeftColumnClass ?>"><?= $Page->logo->caption() ?><?= $Page->logo->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<?php } ?>
    ...
    <input type="hidden" data-table="compania" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
<?php
    if (in_array("compania_cuenta", explode(",", $Page->getCurrentDetailTable())) && $compania_cuenta->DetailEdit) {
?>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fcompaniaedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fcompaniaedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
