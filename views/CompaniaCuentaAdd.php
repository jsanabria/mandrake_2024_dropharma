...
$Page->showMessage();
?>
<form name="fcompania_cuentaadd" id="fcompania_cuentaadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" autocomplete="off">
...
<span id="el_compania_cuenta_banco">
    <select
        ...
        data-select2-id="fcompania_cuentaadd_x_banco"
...
loadjs.ready("fcompania_cuentaadd", function() {
    var options = { name: "x_banco", selectId: "fcompania_cuentaadd_x_banco" };
    if (fcompania_cuentaadd.lists.banco?.lookupOptions.length) {
        options.data = { id: "x_banco", form: "fcompania_cuentaadd" };
    } else {
        options.ajax = { id: "x_banco", form: "fcompania_cuentaadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_compania_cuenta_tipo">
    <select
        ...
        data-select2-id="fcompania_cuentaadd_x_tipo"
        <?php } ?>
        data-table="compania_cuenta"
        data-field="x_tipo"
...
<script>
loadjs.ready("fcompania_cuentaadd", function() {
    var options = { name: "x_tipo", selectId: "fcompania_cuentaadd_x_tipo" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fcompania_cuentaadd.lists.tipo?.lookupOptions.length) {
        options.data = { id: "x_tipo", form: "fcompania_cuentaadd" };
    } else {
        options.ajax = { id: "x_tipo", form: "fcompania_cuentaadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_compania_cuenta_mostrar">
    <select
        ...
        data-select2-id="fcompania_cuentaadd_x_mostrar"
        <?php } ?>
        data-table="compania_cuenta"
...
<script>
loadjs.ready("fcompania_cuentaadd", function() {
    var options = { name: "x_mostrar", selectId: "fcompania_cuentaadd_x_mostrar" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fcompania_cuentaadd.lists.mostrar?.lookupOptions.length) {
        options.data = { id: "x_mostrar", form: "fcompania_cuentaadd" };
    } else {
        options.ajax = { id: "x_mostrar", form: "fcompania_cuentaadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<span id="el_compania_cuenta_cuenta">
    <select
        ...
        data-select2-id="fcompania_cuentaadd_x_cuenta"
        data-table="compania_cuenta"
        data-field="x_cuenta"
...
<script>
loadjs.ready("fcompania_cuentaadd", function() {
    var options = { name: "x_cuenta", selectId: "fcompania_cuentaadd_x_cuenta" };
    if (fcompania_cuentaadd.lists.cuenta?.lookupOptions.length) {
        options.data = { id: "x_cuenta", form: "fcompania_cuentaadd" };
    } else {
        options.ajax = { id: "x_cuenta", form: "fcompania_cuentaadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_compania_cuenta_activo">
    <select
        ...
        data-select2-id="fcompania_cuentaadd_x_activo"
        <?php } ?>
        data-table="compania_cuenta"
        data-field="x_activo"
...
<script>
loadjs.ready("fcompania_cuentaadd", function() {
    var options = { name: "x_activo", selectId: "fcompania_cuentaadd_x_activo" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fcompania_cuentaadd.lists.activo?.lookupOptions.length) {
        options.data = { id: "x_activo", form: "fcompania_cuentaadd" };
    } else {
        options.ajax = { id: "x_activo", form: "fcompania_cuentaadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fcompania_cuentaadd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fcompania_cuentaadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
