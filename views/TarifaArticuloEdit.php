...
?>
<main class="edit">
<form name="ftarifa_articuloedit" id="ftarifa_articuloedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" autocomplete="off">
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { tarifa_articulo: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var ftarifa_articuloedit;
...
<span id="el_tarifa_articulo_fabricante">
    <select
        ...
        data-select2-id="ftarifa_articuloedit_x_fabricante"
...
<script>
loadjs.ready("ftarifa_articuloedit", function() {
    var options = { name: "x_fabricante", selectId: "ftarifa_articuloedit_x_fabricante" };
    if (ftarifa_articuloedit.lists.fabricante?.lookupOptions.length) {
        options.data = { id: "x_fabricante", form: "ftarifa_articuloedit" };
    } else {
        options.ajax = { id: "x_fabricante", form: "ftarifa_articuloedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_tarifa_articulo_articulo">
    <select
        ...
        data-select2-id="ftarifa_articuloedit_x_articulo"
        data-table="tarifa_articulo"
...
<script>
loadjs.ready("ftarifa_articuloedit", function() {
    var options = { name: "x_articulo", selectId: "ftarifa_articuloedit_x_articulo" };
    if (ftarifa_articuloedit.lists.articulo?.lookupOptions.length) {
        options.data = { id: "x_articulo", form: "ftarifa_articuloedit" };
    } else {
        options.ajax = { id: "x_articulo", form: "ftarifa_articuloedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<?php } ?>
    ...
    <input type="hidden" data-table="tarifa_articulo" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
<?= $Page->IsModal ? '<template class="ew-modal-buttons">' : '<div class="row ew-buttons">' ?><!-- buttons .row -->
    <div class="<?= $Page->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="ftarifa_articuloedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="ftarifa_articuloedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
