...
<?= $Page->Pager->render() ?>
<?php } ?>
<form name="falmacenistaedit" id="falmacenistaedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" autocomplete="off">
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { almacenista: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var falmacenistaedit;
...
<div class="input-group">
    <input type="password" name="x_clave" id="x_clave" autocomplete="new-password" data-table="almacenista" data-field="x_clave" value="<?= $Page->clave->EditValue ?>" size="30" maxlength="50" placeholder="<?= HtmlEncode($Page->clave->getPlaceHolder()) ?>"<?= $Page->clave->editAttributes() ?> aria-describedby="x_clave_help">
...
<?php } ?>
    ...
    <input type="hidden" data-table="almacenista" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
<?= $Page->IsModal ? '<template class="ew-modal-buttons">' : '<div class="row ew-buttons">' ?><!-- buttons .row -->
    <div class="<?= $Page->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="falmacenistaedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="falmacenistaedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
