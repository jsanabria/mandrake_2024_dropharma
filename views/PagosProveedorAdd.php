...
$Page->showMessage();
?>
<form name="fpagos_proveedoradd" id="fpagos_proveedoradd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" autocomplete="off">
...
<span id="el_pagos_proveedor_proveedor">
    <select
        ...
        data-select2-id="fpagos_proveedoradd_x_proveedor"
...
<script>
loadjs.ready("fpagos_proveedoradd", function() {
    var options = { name: "x_proveedor", selectId: "fpagos_proveedoradd_x_proveedor" };
    if (fpagos_proveedoradd.lists.proveedor?.lookupOptions.length) {
        options.data = { id: "x_proveedor", form: "fpagos_proveedoradd" };
    } else {
        options.ajax = { id: "x_proveedor", form: "fpagos_proveedoradd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_pagos_proveedor_tipo_pago">
    <select
        ...
        data-select2-id="fpagos_proveedoradd_x_tipo_pago"
        <?php } ?>
        data-table="pagos_proveedor"
...
<script>
loadjs.ready("fpagos_proveedoradd", function() {
    var options = { name: "x_tipo_pago", selectId: "fpagos_proveedoradd_x_tipo_pago" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fpagos_proveedoradd.lists.tipo_pago?.lookupOptions.length) {
        options.data = { id: "x_tipo_pago", form: "fpagos_proveedoradd" };
    } else {
        options.ajax = { id: "x_tipo_pago", form: "fpagos_proveedoradd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_pagos_proveedor_banco">
    <select
        ...
        data-select2-id="fpagos_proveedoradd_x_banco"
        <?php } ?>
        data-table="pagos_proveedor"
        data-field="x_banco"
...
<script>
loadjs.ready("fpagos_proveedoradd", function() {
    var options = { name: "x_banco", selectId: "fpagos_proveedoradd_x_banco" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fpagos_proveedoradd.lists.banco?.lookupOptions.length) {
        options.data = { id: "x_banco", form: "fpagos_proveedoradd" };
    } else {
        options.ajax = { id: "x_banco", form: "fpagos_proveedoradd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<script>
loadjs.ready(["fpagos_proveedoradd", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fpagos_proveedoradd", "x_fecha", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
<span id="el_pagos_proveedor_moneda">
    <select
        ...
        data-select2-id="fpagos_proveedoradd_x_moneda"
        <?php } ?>
        data-table="pagos_proveedor"
        data-field="x_moneda"
...
<script>
loadjs.ready("fpagos_proveedoradd", function() {
    var options = { name: "x_moneda", selectId: "fpagos_proveedoradd_x_moneda" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fpagos_proveedoradd.lists.moneda?.lookupOptions.length) {
        options.data = { id: "x_moneda", form: "fpagos_proveedoradd" };
    } else {
        options.ajax = { id: "x_moneda", form: "fpagos_proveedoradd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fpagos_proveedoradd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fpagos_proveedoradd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
...
loadjs.ready("load", function () {
    ...
    $("#x_proveedor").change(function(){
    	...
    	$.ajax({
    	  url : "include/Proveedor_Facturas_Buscar.php",
    	  type: "GET",
