...
$accion = $_POST['accion'] ?? $_POST['action'] ?? $_GET['accion'] ?? $_GET['action'] ?? '';

if ($accion === 'buscar') {
    TAmBuscarArticulos($_POST['q'] ?? $_GET['q'] ?? '');
}
...
loadjs.ready(["jquery"], function () {
    ...
    window.almacenesTransferencia = <?= json_encode($almacenes ?: [], JSON_UNESCAPED_UNICODE) ?>;
    window.cestaTransferencia = window.cestaTransferencia || [];

    window.TAmPostData = function (data) {
        data = data || {};

        if (typeof ew !== "undefined" && ew.TOKEN_NAME && ew.ANTIFORGERY_TOKEN) {
    ...
    window.HtmlEscapeTransferencia = function(value) {
        return String(value ?? '')
            .replace(/&/g, '&amp;')
            .replace(/</g, '&lt;')
            .replace(/>/g, '&gt;')
    ...
    window.BuscarTransferenciaMultiple = function() {
        ...
        $('#ResultadoBusquedaTransferencia').html(
            '<div class="text-center p-3 text-secondary"><i class="fa fa-spinner fa-spin"></i> Buscando...</div>'
        );

        $.ajax({
            url: "TransferenciaAlmacenMultiple",
            type: 'POST',
            dataType: 'json',
            data: window.TAmPostData({
                accion: 'buscar',
                q: q
            }),
            success: function(resp) {
                if (!resp || !resp.success) {
                    $('#ResultadoBusquedaTransferencia').html(
                        ...
                        window.HtmlEscapeTransferencia((resp && resp.message) ? resp.message : 'Error buscando.') +
                        '</div>'
                    );
                    return;
                }

                window.PintarResultadosTransferencia(resp.data || []);
            },
            error: function(xhr) {
                console.log(xhr.responseText);
                $('#ResultadoBusquedaTransferencia').html(
                    '<div class="alert alert-danger py-2">Error de comunicación.</div>'
                );
            }
        });
    };

    window.PintarResultadosTransferencia = function(rows) {
        if (rows.length === 0) {
            $('#ResultadoBusquedaTransferencia').html(
                '<div class="alert alert-warning py-2">No se encontraron existencias para esa búsqueda.</div>'
            );
            return;
        }
        ...
        rows.forEach(function(r, idx) {
            const selectDestino = window.CrearSelectDestinoTransferencia('destino_bus_' + idx, r.almacen);

            html += '<tr>';
            html += '<td>' + window.HtmlEscapeTransferencia(r.codigo) + '</td>';
            html += '<td>' + window.HtmlEscapeTransferencia(r.descripcion) + '<br><small class="text-muted">' + window.HtmlEscapeTransferencia(r.fabricante) + '</small></td>';
            html += '<td>' + window.HtmlEscapeTransferencia(r.lote) + '</td>';
            // html += '<td>' + window.HtmlEscapeTransferencia(r.fecha_fmt || '') + '</td>';
            let fechaMostrar = r.fecha_fmt || '';

            if (fechaMostrar === '' && r.fecha_vencimiento) {
                const partes = String(r.fecha_vencimiento).split('-');
                if (partes.length === 3) {
                    fechaMostrar = partes[2] + '/' + partes[1] + '/' + partes[0];
                }
            }

            if (fechaMostrar === '01/01/1990' || fechaMostrar === '01/01/2027') {
                fechaMostrar = '';
            }

            html += '<td>' + HtmlEscapeTransferencia(fechaMostrar) + '</td>';

            html += '<td>' + window.HtmlEscapeTransferencia(r.almacen) + ' - ' + window.HtmlEscapeTransferencia(r.almacen_nombre) + '</td>';
    ...
    window.CrearSelectDestinoTransferencia = function(id, almacenOrigen) {
        let html = '<select id="' + id + '" class="form-select form-select-sm">';
        html += '<option value="">Seleccione...</option>';

        window.almacenesTransferencia.forEach(function(a) {
            if (a.codigo !== almacenOrigen) {
                html += '<option value="' + window.HtmlEscapeTransferencia(a.codigo) + '">' +
                    window.HtmlEscapeTransferencia(a.descripcion) +
                    '</option>';
            }
        });
    ...
    window.AgregarACestaTransferencia = function(idx) {
        ...
        const existe = window.cestaTransferencia.findIndex(x => x.key === key);

        if (existe >= 0) {
            const nuevaCantidad = parseFloat(window.cestaTransferencia[existe].cantidad) + cantidad;
            ...
            window.cestaTransferencia[existe].cantidad = nuevaCantidad;
        } else {
            window.cestaTransferencia.push({
                key: key,
        ...
        window.PintarCestaTransferencia();
    };

    window.PintarCestaTransferencia = function() {
        let html = '';

        window.cestaTransferencia.forEach(function(r, idx) {
            html += '<tr>';
            html += '<td>' + window.HtmlEscapeTransferencia(r.codigo) + '</td>';
            html += '<td>' + window.HtmlEscapeTransferencia(r.descripcion) + '</td>';
            html += '<td>' + window.HtmlEscapeTransferencia(r.lote) + '</td>';
            // html += '<td>' + window.HtmlEscapeTransferencia(r.fecha_vencimiento) + '</td>';
            let fechaCesta = r.fecha_fmt || '';

            if (fechaCesta === '' && r.fecha_vencimiento) {
                const partes = String(r.fecha_vencimiento).split('-');
                if (partes.length === 3) {
                    fechaCesta = partes[2] + '/' + partes[1] + '/' + partes[0];
                }
            }

            if (fechaCesta === '01/01/1990' || fechaCesta === '01/01/2027') {
                fechaCesta = '';
            }

            html += '<td>' + HtmlEscapeTransferencia(fechaCesta) + '</td>';

            html += '<td>' + window.HtmlEscapeTransferencia(r.almacen_origen) + ' - ' + window.HtmlEscapeTransferencia(r.almacen_origen_nombre) + '</td>';
            html += '<td>' + window.HtmlEscapeTransferencia(r.almacen_destino) + ' - ' + window.HtmlEscapeTransferencia(r.almacen_destino_nombre) + '</td>';
    ...
    window.ActualizarCantidadCestaTransferencia = function(idx, value) {
        const cantidad = parseFloat(value || 0);

        if (!window.cestaTransferencia[idx]) {
            return;
        }

        if (cantidad <= 0 || cantidad > parseFloat(window.cestaTransferencia[idx].disponible)) {
            ew.alert('Cantidad inválida.');
            window.PintarCestaTransferencia();
            return;
        }

        window.cestaTransferencia[idx].cantidad = cantidad;
    };

    window.EliminarCestaTransferencia = function(idx) {
        window.cestaTransferencia.splice(idx, 1);
        window.PintarCestaTransferencia();
    };

    window.ProcesarTransferenciaMultiple = function() {
        if (window.cestaTransferencia.length === 0) {
        ...
        $.ajax({
            ...
            data: window.TAmPostData({
                accion: 'procesar',
                items: JSON.stringify(window.cestaTransferencia)
            }),
            success: function(resp) {
                if (resp && resp.success) {
                    ...
                    ew.alert((resp && resp.message) ? resp.message : 'No se pudo procesar la transferencia.');
                }
            },
            error: function(xhr) {
                console.log(xhr.responseText);
    ...
    $(function() {
        ...
        $('#txtBuscarTransferencia').on('keypress', function(e) {
            if (e.which === 13) {
                window.BuscarTransferenciaMultiple();
                e.preventDefault();
            }
        });
    });
