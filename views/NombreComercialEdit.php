...
<?= $Page->Pager->render() ?>
<?php } ?>
<form name="fnombre_comercialedit" id="fnombre_comercialedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" autocomplete="off">
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { nombre_comercial: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var fnombre_comercialedit;
...
<input type="hidden" data-table="nombre_comercial" data-field="x_codigo" data-hidden="1" data-old name="o_codigo" id="o_codigo" value="<?= HtmlEncode($Page->codigo->OldValue ?? $Page->codigo->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fnombre_comercialedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fnombre_comercialedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
