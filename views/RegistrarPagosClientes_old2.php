        ...
        if (!preg_match('/^\d{14,}$/', $ref_ret)) {
            jsonResponse([
                ...
                "message" => "El Nro. de Comprobante de " . $tipo_ret .
                             " debe contener únicamente números y tener al menos 14 dígitos."
            ]);
            exit;
        }
...
function agregarPago(e) {
    ...
    if (["RI", "RJ", "RR"].includes(tipo)) {
        // El comprobante debe ser numérico y tener al menos 14 dígitos.
        if (!/^\d{14,}$/.test(ref)) {
            ew.alert("El Nro. de Comprobante debe contener únicamente números y tener al menos 14 dígitos.");
            $("#ref_input").focus();
            return false;
        }
