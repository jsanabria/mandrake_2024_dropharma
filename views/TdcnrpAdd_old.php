...
if(isset($_REQUEST["pedido"])) {
    ...
    if($row = ExecuteRow($sql)) {
      ...
      header("Location: ViewInTdcnrpList");
      die();
    }
} 
...
$usuarioCodigoBarra = CurrentUserName();
$puedeEditarCodigoBarra = false;
$sql = "SELECT valor1 AS usuario FROM parametro WHERE codigo = '045' AND valor1 = '" . AdjustSql($usuarioCodigoBarra) . "' LIMIT 1;";
...
<script type="text/javascript">
  function insertar(i) { 
    ...
    $.ajax({
      ...
      url: "include/tdcnrp/insertar_linea_pedido_tdcnrp.php",
      // The data to send tdcnrp/(will be converted to a query string)
      data: { 
              pedido: pedido, 
              proveedor: proveedor, 
              costoFull: costoFull, 
  ...
  function eliminar(i, id_item) {
    ...
    $.ajax({
      // The URL for the request
      url: "include/tdcnrp/eliminar_linea_pedido_tdcnrp.php",
  ...
  function vaciar(i) {
    ...
    if(i > 0) {
      ...
      $.ajax({
        // The URL for the request
        url: "include/tdcnrp/vaciar_tdcnrp.php",
        // The data to send (will be converted to a query string)
        data: { 
                pedido: pedido, 
                username: username 
  ...
  function listar_pedido(i) {
    ...
    $.ajax({
      // The URL for the request
      url: "include/tdcnrp/listar_tdcnrp_totales.php",
      // The data to send (will be converted to a query string)
      data: { 
              pedido: i 
            },
    ...
    .done(function( json ) {
        ...
        if(json.estatus == 1) {
          $("#nroPedido").html('<button type="button" class="btn btn-outline-primary btn-sm"><i class="fa-solid fa-hashtag"></i> Pedido Nro.: ' + json.pedido + ' (' + json.nro_documento + ')</button> <button type="button" onclick="js:vaciar(' + json.pedido + ')" class="btn btn-outline-danger btn-sm"><i class="fa-solid fa-trash"></i> Vaciar Toda la Cesta <i class="fa-solid fa-exclamation"></i></button> <button type="button" onclick="js:getCodigos3(' + json.pedido + ')" class="btn btn-outline-info btn-sm"><i class="fa-solid fa-cart-shopping"></i> Listar la Cesta <i class="fa-solid fa-exclamation"></i></button> <button type="button" onclick="js:sendProccess(' + json.pedido + ')" class="btn btn-outline-success btn-sm"><i class="fa-solid fa-microchip"></i> Procesar Documento </button> <button type="button" class="btn btn-outline-primary btn-sm" onclick="js: window.location.replace(\'ViewInTdcnrpList\');"><i class="fa-solid fa-list"></i> Notas </button>');
          $("#pedido").val(json.pedido);
          $("#xReglones").html(json.renglones);
          $("#xUnidades").html(json.unidades);
            $("#xTotalBs").html(formatter.format(json.total));
  ...
  function sendProccess(i) {
    ...
    window.location.href = "TdcnrpProcess?pedido=" + i + "&PorDesAct=" + PorDesAct;
  }
  ...
  function RefreshDescuento(i, j) {
    ...
    $.ajax({
      // The URL for the request
      url: "include/tdcnrp/descuento_tdcnrp_totales.php",
      // The data to send (will be converted to a query string)
      data: { 
              pedido: i, 
              descuentoG: j, 
  ...
  function RefreshMonedaTasa() {
    ...
    $.ajax({
      // The URL for the request
      url: "include/tdcnrp/moneda_tdcnrp_totales.php",
      // The data to send (will be converted to a query string)
      data: { 
              pedido: i, 
              moneda: moneda, 
  ...
  function check_item(i) { 
    $.ajax({
      url: "include/tdcnrp/check_linea_pedido_tdcnrp.php",
      data: { 
              item: i, 
            },
      type: "POST",
...
<script type="text/javascript">
  ...
  function getCodigos2() {
      ...
      if (inputCP3.length >= 0) {
          ...
          let url = "include/tdcnrp/buscar_articulos_tdcnrp.php"
          let formData = new FormData()
  ...
  function getCodigos3(i) { 
      ...
      if (inputCP > 0) {
          let url = "include/tdcnrp/listar_tdcnrp.php"
          let formData = new FormData()
  ...
  function guardar_nota() {
      ...
      if (inputCP > 0) {
          let url = "include/tdcnrp/guardar_nota.php"
          let formData = new FormData()
          formData.append("pedido", inputCP)
  ...
  function recepAll() { 
    var isChecked = document.getElementById("recepAll").checked;
    var pedido = <?= $pedido ?>;
    var username = "<?= CurrentUserName() ?>";
    if(isChecked) { 
        if(confirm("Seguro de Recepcionar Todos los Items?")) { 
            $.ajax({
            url: "include/tdcnrp/recepAll.php",
            data: { 
                    pedido: pedido, 
                    username: username, 
                    recepcionar: 'S', 
            ...
            .done(function( json ) {
                ...
                if(json.estatus == 1) { 
                    alert("Recepción Ejecutado Correctamente");
                    location.reload();
                } 
            })
            .fail(function( xhr, status, errorThrown ) {
        ...
        else document.getElementById("recepAll").checked = false;
    } 
    else {
        if(confirm("Seguro de Desrecepcionar Todos los Items?")) {
            $.ajax({
            url: "include/tdcnrp/recepAll.php",
            data: { 
                    pedido: pedido, 
            ...
            .done(function( json ) {
                ...
                if(json.estatus == 1) { 
                    alert("Desrecepción Ejecutada Correctamente");
                    location.reload();
                } 
            })
            .fail(function( xhr, status, errorThrown ) {
        ...
        else document.getElementById("recepAll").checked = true;
    }
  }


  const puedeEditarCodigoBarra = <?= ($puedeEditarCodigoBarra ? "true" : "false") ?>;

  function abrirModalCodigoBarra(articulo) {
      var username = "<?= CurrentUserName() ?>";
      $.ajax({
          url: "include/tdcnrp/actualizar_codigo_barra_articulo.php",
          type: "POST",
          dataType: "json",
          data: {
              accion: "get",
          ...
          success: function (response) {
              if (!response || response.success !== true) {
                  alert(response.error || "No se pudo consultar el artículo.");
                  return;
              }

              $("#modal_codigo_articulo_id").val(articulo);
              $("#modal_codigo").val(response.data.codigo || "");
              $("#modal_fabricante").val(response.data.fabricante || "");
              $("#modal_nombre_comercial").val(response.data.nombre_comercial || "");
              $("#modal_nombre_articulo").val(response.data.nombre_articulo || "");
              $("#modal_presentacion").val(response.data.presentacion || "");
              $("#modal_codigo_de_barra").val(response.data.codigo_de_barra || "");

              const modalEl = document.getElementById("modalCodigoBarraArticulo");

              if (typeof bootstrap !== "undefined" && bootstrap.Modal) {
                  bootstrap.Modal.getOrCreateInstance(modalEl).show();
              } else if (typeof jQuery !== "undefined" && jQuery.fn.modal) {
                  $("#modalCodigoBarraArticulo").modal("show");
              } else {
          ...
          error: function (xhr) {
              console.log(xhr.responseText);
              alert("Error consultando código de barra.");
          }
      });
  }

  function guardarCodigoBarraArticulo() {
      const articulo = $("#modal_codigo_articulo_id").val();
      const codigoBarra = $("#modal_codigo_de_barra").val();
      ...
      $.ajax({
          ...
          if (resp && resp.success) {
              bootstrap.Modal.getOrCreateInstance(
                  document.getElementById("modalCodigoBarraArticulo")
              ).hide();

              getCodigos3($("#pedido").val());
          } else {
              alert(resp.error || "No se pudo actualizar el código de barra.");
          }
      }).fail(function(xhr) {
          console.log(xhr.responseText);
          alert("Error actualizando código de barra.");
      });
  }
...
<style>
#modalCodigoBarraArticulo input[readonly]{
...
<div class="modal fade" id="modalCodigoBarraArticulo" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header bg-primary text-white py-2">
        <h5 class="modal-title">Código de Barra del Artículo</h5>
        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
      </div>

      <div class="modal-body">

          <input type="hidden" id="modal_codigo_articulo_id">
          ...
          <div class="row mb-2">
              ...
              <div class="col-md-8">
                  <input type="text" id="modal_fabricante"
                         class="form-control form-control-sm bg-light text-muted"
                         readonly>
              </div>
          </div>
          ...
          <div class="row mb-2">
              ...
              <div class="col-md-8">
                  <input type="text" id="modal_nombre_comercial"
                         class="form-control form-control-sm bg-light text-muted"
                         readonly>
              </div>
          </div>
          ...
          <div class="row mb-2">
              ...
              <div class="col-md-8">
                  <input type="text" id="modal_nombre_articulo"
                         class="form-control form-control-sm bg-light text-muted"
                         readonly>
              </div>
          </div>
          ...
          <div class="row mb-3">
              ...
              <div class="col-md-8">
                  <input type="text" id="modal_presentacion"
                         class="form-control form-control-sm bg-light text-muted"
                         readonly>
              </div>
          </div>
      ...
      <div class="modal-footer">
        ...
        <button type="button" class="btn btn-sm btn-success" onclick="guardarCodigoBarraArticulo();">Guardar</button>
      </div>
    </div>
  </div>
</div>
