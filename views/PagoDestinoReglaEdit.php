...
<?= $Page->Pager->render() ?>
<?php } ?>
<form name="fpago_destino_reglaedit" id="fpago_destino_reglaedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" autocomplete="off">
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { pago_destino_regla: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var fpago_destino_reglaedit;
...
<?php if ($Page->id->Visible) { // id ?>
    <div id="r_id"<?= $Page->id->rowAttributes() ?>>
        <label id="elh_pago_destino_regla_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->id->cellAttributes() ?>>
<span id="el_pago_destino_regla_id">
<span<?= $Page->id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id->getDisplayValue($Page->id->EditValue))) ?>"></span>
<input type="hidden" data-table="pago_destino_regla" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<span id="el_pago_destino_regla_compania">
    <select
        ...
        data-select2-id="fpago_destino_reglaedit_x_compania"
        <?php } ?>
...
<?php if (!$Page->compania->IsNativeSelect) { ?>
<script>
loadjs.ready("fpago_destino_reglaedit", function() {
    var options = { name: "x_compania", selectId: "fpago_destino_reglaedit_x_compania" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fpago_destino_reglaedit.lists.compania?.lookupOptions.length) {
        options.data = { id: "x_compania", form: "fpago_destino_reglaedit" };
    } else {
        options.ajax = { id: "x_compania", form: "fpago_destino_reglaedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_pago_destino_regla_metodo">
    <select
        ...
        data-select2-id="fpago_destino_reglaedit_x_metodo"
        <?php } ?>
        data-table="pago_destino_regla"
        data-field="x_metodo"
...
<script>
loadjs.ready("fpago_destino_reglaedit", function() {
    var options = { name: "x_metodo", selectId: "fpago_destino_reglaedit_x_metodo" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fpago_destino_reglaedit.lists.metodo?.lookupOptions.length) {
        options.data = { id: "x_metodo", form: "fpago_destino_reglaedit" };
    } else {
        options.ajax = { id: "x_metodo", form: "fpago_destino_reglaedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<span id="el_pago_destino_regla_cuenta_destino_id">
    <select
        ...
        data-select2-id="fpago_destino_reglaedit_x_cuenta_destino_id"
        <?php } ?>
        data-table="pago_destino_regla"
...
<script>
loadjs.ready("fpago_destino_reglaedit", function() {
    var options = { name: "x_cuenta_destino_id", selectId: "fpago_destino_reglaedit_x_cuenta_destino_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fpago_destino_reglaedit.lists.cuenta_destino_id?.lookupOptions.length) {
        options.data = { id: "x_cuenta_destino_id", form: "fpago_destino_reglaedit" };
    } else {
        options.ajax = { id: "x_cuenta_destino_id", form: "fpago_destino_reglaedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<span id="el_pago_destino_regla_prioridad">
    <select
        ...
        data-select2-id="fpago_destino_reglaedit_x_prioridad"
        <?php } ?>
        data-table="pago_destino_regla"
...
<script>
loadjs.ready("fpago_destino_reglaedit", function() {
    var options = { name: "x_prioridad", selectId: "fpago_destino_reglaedit_x_prioridad" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fpago_destino_reglaedit.lists.prioridad?.lookupOptions.length) {
        options.data = { id: "x_prioridad", form: "fpago_destino_reglaedit" };
    } else {
        options.ajax = { id: "x_prioridad", form: "fpago_destino_reglaedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fpago_destino_reglaedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fpago_destino_reglaedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
