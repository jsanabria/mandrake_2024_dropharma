...
$sql = "SELECT 
			...
			ROUND(((c.precio-(c.precio*(a.descuento/100)))*$tasa), 2) AS precio1, 
			...
			ROUND(((c.precio-(c.precio*(a.descuento/100)))*$tasa), 2) AS psugerido, 
			0.00 AS pgris, 
			0 AS nuevo, 
			/*
			(SELECT 
			...
			*/
			ROUND(((c.precio-(c.precio*(a.descuento/100)))*$tasa), 2) AS precio2, 
			ROUND(((c.precio-(c.precio*(a.descuento/100)))*$tasa), 2) AS precio3, 
			0.00 AS precio4, 
			0.00 AS precio5, 
			0.00 AS precio6, 
			1 AS undmin, 
			...
			0.00 AS dct, 
			'N/A' AS codac3, 
			IF(IFNULL(a.indexado, 'N') = 'N', '', 'PRODUCTO INDEXADO') AS indexado 
		FROM 
			articulo AS a 
