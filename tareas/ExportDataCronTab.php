...
$path = "/home2/dropharm/dropharmadm/profit/";
...
$sql = "SELECT 
	...
	total AS TOTAL_NETO, IFNULL(documento, 'FC') AS tipo_movimiento, IF(estatus = 'ANULADO', 1, 0) AS ANULADO, IFNULL(descuento, 0) AS DESCUENTO, IF(IFNULL(descuento, 0) = 0, 0, ABS(IFNULL(monto_sin_descuento, 0)-IFNULL(monto_total, 0))) AS MONTO_DESC  
FROM salidas 
...
$f = fopen($filename, 'w'); 
while($row = mysqli_fetch_array($rs)) {
	$lineData = array($row['NUMERO_RENGLON'], $row['NUMERO_FACTURA'], $row['ARTICULO'], $row['CANTIDAD'], $row['COSTO_UNITARIO'], $row['COSTO_TOTAL'], $row['PORCENTAJE_DEL_IMPUESTO'], $row['MONTO_IMPUESTO'], $row['MONTO_UNITARIO'], $row['MONTO_TOTAL'], $row['doc_afectado']); 
    _fputcsv($f, $lineData);
}
fseek($f, 0); 
