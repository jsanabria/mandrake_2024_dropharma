...
class PDF extends FPDF
{
	...
	function EndReport($id_invoice)
	{
		...
		$this->MultiCell(130, 3, strtoupper(utf8_decode($nota . " " . $asociado)), 0, 'L');
...
while($row = mysqli_fetch_array($rs))
{
	...
	if(strlen($row["articulo"]) < 63) 
		$pdf->Cell(85, 3, ($row["alicuota"]==0.00?"(E) ":"") . trim(utf8_decode($row["articulo"])), 0, 0, 'L');
	else 
		$pdf->Cell(85, 3, ($row["alicuota"]==0.00?"(E) ":"") . substr(trim(utf8_decode($row["articulo"])), 0, 63), 0, 0, 'L');
