...
class PDF extends FPDF
{
	...
	function EndReport($id_invoice)
	{
		...
		// ---------------------------------------------------------------
		// MÉTODOS DE PAGO (mismo detalle que muestra comprobante_pago.php)
		// Se imprime debajo de "Usuario:" y antes de la nota de IGTF/Tasa BCV.
		// Solo se imprime la tabla (METODO / REFERENCIA / MONTO DIVISA / MONTO BS.);
		// no se repiten aquí los totales que ya calcula comprobante_pago.php.
		// ---------------------------------------------------------------
		$sql = "SELECT
					a.metodo_pago,
		...
		while ($row_pago = mysqli_fetch_array($rs_pagos)) {
			if (!$hay_pagos) {
				$this->SetFont('Courier', 'B', 6);
				$this->Cell(25, 3, "METODO", 1, 0, 'C');
				$this->Cell(30, 3, "REFERENCIA", 1, 0, 'C');
				$this->Cell(22, 3, "MONTO DIVISA", 1, 0, 'C');
				$this->Cell(23, 3, "MONTO BS.", 1, 1, 'C');
				$hay_pagos = true;
			}
			...
			$this->SetFont('Courier', '', 6);
			$this->Cell(25, 3, substr($metodo_txt, 0, 16), 1, 0, 'L');
			$this->Cell(30, 3, substr($row_pago['referencia'] ?? '', 0, 20), 1, 0, 'L');
			$this->Cell(22, 3, $row_pago['moneda'] != 'Bs.' ? $row_pago['moneda'] . " " . number_format($row_pago['monto_moneda'], 2, ",", ".") : '-', 1, 0, 'R');
			$this->Cell(23, 3, number_format($row_pago['monto_bs'], 2, ",", "."), 1, 1, 'R');
		}
		...
		// Si el documento ya fue entregado (salidas.entregado = 'S') y está facturado
		// en Bolívares (salidas.moneda = 'Bs.'), se ocultan por completo las líneas de
		// I.G.T.F. y TOTAL GENERAL (no aplica IGTF en ese escenario).
		$ocultar_igtf_total_general = ($GLOBALS["entregado"] == "S" && $GLOBALS["moneda"] == "Bs.");
