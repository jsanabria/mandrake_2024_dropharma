...
class PDF extends FPDF
{
	...
	function Header()
	{
		...
		$this->MultiCell(160, 5, utf8_decode($direccion_cliente), '0', 'L');
	...
	function EndReport($id_invoice)
	{
		...
		$sql = "SELECT 
					...
					a.monto_usd, a.tasa_dia, a.descuento, a.monto_sin_descuento, a.unidades, a.consignacion  
...
//////////////
		if($consignacion == "S") {
			$igtf = $row["total"]*(3/100);
			$totaligtf = $row["total"]+($row["total"]*(3/100));
			$this->Ln(5);
			$this->SetFont('Arial','B',9);
			// $this->Cell(30, 4, "Unidades: $unidades", 0, 0, 'R');
			...
			$this->Cell(48, 4, "I.G.T.F. 3%:", 0, 0, 'R');
			$this->SetFont('Arial','',9);
			$this->Cell(35, 4, number_format($igtf, 2, ",", "."), 0, 0, 'R');
			...
			// $this->Cell(90, 4, strtoupper($nota), 0, 0, 'R');
			$this->Cell(90, 4, "", 0, 0, 'R');
			$this->Cell(48, 4, "TOTAL GENERAL $moneda:", 0, 0, 'R');
			$this->SetFont('Arial','',9);
			$this->Cell(35, 4, number_format($totaligtf, 2, ",", "."), 0, 0, 'R');
		}
//////////////
