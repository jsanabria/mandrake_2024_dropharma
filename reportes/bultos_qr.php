...
class PDF extends FPDF
{
	...
	function Header()
	{
		...
		$sql = "SELECT id FROM salidas WHERE id_documento_padre = $id AND estatus <> 'ANULADO';"; 
		$rs = mysqli_query($link, $sql);
		...
		$sql = "SELECT a.nombre  
				...
				WHERE 
					u.username = '$asesor';";
		$rs = mysqli_query($link, $sql);
		if($row = mysqli_fetch_array($rs)) $asesor = $row["nombre"];
		else $asesor = "";
		...
		//$contenido2 = "http://oficina.dropharmadm.com/view_facturas_a_entregaredit.php?id=$xId&id_padre=$xIdPadre"; 
		//$contenido2 = "http://oficina.dropharmadm.com/buscar_factura_a_entregar.php?id=$xId&id_padre=$xIdPadre";
		$contenido2 = "https://dropharmadm.com/dropharma/include/buscar_factura_a_entregar.php?id=$xId&id_padre=$xIdPadre&data=" . $_COOKIE["strcon"];
		...
		$this->Cell(10, 5);
		$this->MultiCell(125, 5, mb_convert_encoding($direccion, "ISO-8859-1"), '0', 'L');
		...
		$this->Cell(10, 5);
		$this->Cell(0,10,'Bulto: '.$this->PageNo().'/{nb}' . ' ESCANEE AQUI =>',0,0,'L');
		$this->Ln();
