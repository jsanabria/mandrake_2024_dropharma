...
while($row = mysqli_fetch_array($rs))
{
	...
	if(strlen($row["articulo"]) < 28) 
		$pdf->Cell(47, 3, trim($row["articulo"]), 0, 0, 'L');
	else 
		$pdf->Cell(47, 3, substr(trim($row["articulo"]), 0, 28), 0, 0, 'L');

	$pdf->Cell(16, 3, $row["lote"], 0, 0, 'R');
	$pdf->Cell(10, 3, ($row["vencimiento"]=="01/01/1990" ? "" : $row["vencimiento"]), 0, 0, 'R');
