...
$sin_pagos = isset($_REQUEST["sin_pagos"]) ? intval($_REQUEST["sin_pagos"]) : 0;
...
function construir_html_pagos($link, $id_compra, $pagos_json, $tipo_pago_actual) {
    ...
    $total_bs_factura_base = $total_bs; // Total original sin sumar líneas IGTF
    ...
    <div class="card shadow-sm border-0 mb-3 bg-light">
        <div class="card-body p-3 text-center">
            <div class="row g-0">
                ...
                <input type="hidden" id="total_factura_bs_base" value="<?= $total_bs_factura_base ?>">
...
// -----------------------------------------------------------------------
// Normaliza las líneas automáticas de IGTF. La base gravable de cada pago
// en divisas nunca puede superar el saldo principal de la factura que
// existía antes de aplicar ese pago. El excedente es vuelto y no genera IGTF.
//
// Retorna:
//   lista      => pagos con sus líneas IGTF corregidas
//   base_igtf  => base total del IGTF expresada en Bs.
//   monto_igtf => IGTF total expresado en Bs.
// -----------------------------------------------------------------------
function normalizar_igtf_pagos($lista, $total_factura_bs_base, $tasa_dia, $igtf_pct) {
    if (!is_array($lista)) $lista = array();

    $saldo_principal = max(0, floatval($total_factura_bs_base));
    $bases_por_gid = array();
    $pago_por_gid = array();
    $base_total = 0;

    // Primero determinamos cuánto de cada pago en divisas realmente cubre
    // capital de la factura. Todo lo que exceda ese capital es vuelto.
    foreach ($lista as $i => $p) {
        ...
        if ($gid !== "") $pago_por_gid[$gid] = $p;

        if ($mon !== "Bs." && $igtf_pct > 0 && $saldo_principal > 0) {
            $base = min($monto_bs, $saldo_principal);

            // Conservar la base fiscal real en el pago que originó el IGTF.
            // No debe cambiar aunque después se recorte el pago por vuelto.
            $lista[$i]["base_igtf_bs"] = round($base, 2);

            if ($gid !== "") $bases_por_gid[$gid] = $base;
            $base_total += $base;
        }

        // Cualquier método de pago cubre primero el principal pendiente.
        $saldo_principal = max(0, $saldo_principal - $monto_bs);
    }
    ...
    foreach ($lista as $i => $p) {
        ...
        $gid = (string)($p["gid"] ?? "");
        $base = ($gid !== "") ? floatval($bases_por_gid[$gid] ?? 0) : 0;
        $nuevo_igtf = ($base > 0 && $igtf_pct > 0) ? round(($base * $igtf_pct) / 100, 2) : 0;
        $lista[$i]["monto"] = $nuevo_igtf;
        $lista[$i]["moneda"] = "Bs.";
        if ($gid !== "") $vistos[$gid] = true;
    }

    // Salvaguarda del lado servidor: si el navegador no envió la línea IGTF,
    // la creamos usando los datos del pago que la originó.
    foreach ($bases_por_gid as $gid => $base) {
        if ($base <= 0 || isset($vistos[$gid])) continue;
        $origen = $pago_por_gid[$gid] ?? array();
        $nuevo_igtf = round(($base * $igtf_pct) / 100, 2);
        if ($nuevo_igtf <= 0) continue;
        ...
        $lista[] = array(
            ...
            "moneda" => "Bs.",
            "es_igtf" => 1,
            "destino_id" => intval($origen["destino_id"] ?? 0),
            "destino_nom" => trim($origen["destino_nom"] ?? "")
        );
    }
    ...
    foreach ($lista as $p) {
        if (trim($p["tipo"] ?? "") === "IG") {
            $monto_igtf += floatval($p["monto"] ?? 0);
        }
    }

    return array(
        "lista" => $lista,
        "base_igtf" => round($base_total, 2),
...
function finalizar_pago_factura($link, $id_compra, $username, $pagos_json, $confirmar_exceso = "0") {
    ...
    $cliente_id = intval($factura["cliente"] ?? 0);
    $tasa_dia   = floatval($factura["tasa_dia"] ?? 1);
    $total_bs_factura_base = floatval($factura["total_bs"] ?? 0);

    $igtf_pct_normalizar = floatval(scalar_pago($link, "SELECT alicuota AS IGTF FROM alicuota WHERE codigo = 'IGT' AND activo = 'S'") ?: 0);
    if ($igtf_pct_normalizar > 0 && $igtf_pct_normalizar < 1) {
        $igtf_pct_normalizar = $igtf_pct_normalizar * 100;
    }

    $norm_igtf = normalizar_igtf_pagos($lista, $total_bs_factura_base, $tasa_dia, $igtf_pct_normalizar);
    $lista = $norm_igtf["lista"];
    $base_igtf_normalizada = floatval($norm_igtf["base_igtf"] ?? 0);

    $total_bs = $total_bs_factura_base;
    ...
    if ($vuelto_bs > 0.02) {
        ...
        for ($i = count($lista) - 1; $i >= 0 && $remaining > 0.02; $i--) {
            ...
            if ($item_bs <= $remaining + 0.0001) {
                ...
                // La divisa puede quedar redondeada a dos decimales, pero
                // monto_bs debe conservar exactamente factura + IGTF.
        ...
        foreach ($lista as $i => $p) {
            ...
            if (in_array($gid, $gids_removidos, true)) {
                ...
                // El IGTF ya fue calculado sobre la base fiscal correcta
                // de la factura. No debe recalcularse sobre el pago recortado.
                $lista[$i]["moneda"] = "Bs.";
            }
        }
        ...
        // Volver a normalizar después del recorte. Esto es clave porque el
        // pago registrado puede incluir el importe del propio IGTF, pero la
        // base gravable continúa limitada al principal de la factura.
        $norm_igtf = normalizar_igtf_pagos($lista, $total_bs_factura_base, $tasa_dia, $igtf_pct_normalizar);
        $lista = $norm_igtf["lista"];
        $base_igtf_normalizada = floatval($norm_igtf["base_igtf"] ?? 0);

        // Recalcular pagado_bs y total_bs con el IGTF ya normalizado.
        $total_bs = $total_bs_factura_base;
        foreach ($lista as $pp) {
            if (trim($pp["tipo"] ?? "") === "IG") {
                $total_bs += floatval($pp["monto"] ?? 0);
...
// -----------------------------------------------------------------------
function render_pagina_pago($link, $id, $username, $auto_return, $generar_ne, $permite_sin_pagos = false) {
?>
<!DOCTYPE html>
<html lang="es">
<head>
...
<div class="container-fluid py-3" style="max-width: 900px;">
    ...
    <div class="mt-4 text-center">
        ...
        <?php if ($permite_sin_pagos) { ?>
        <button type="button" id="btn-sin-pagos" class="btn btn-warning btn-lg px-5 shadow-sm fw-bold" onclick="return emitirSinPagos(event)">
            EMITIR SIN PAGOS
        </button>
        <?php } ?>
    </div>
...
function agregarPago(e) {
    ...
    if (monSel !== "Bs." && igtfPct > 0) {
        ...
        const totalFacturaBaseBs = parseFloat($("#total_factura_bs_base").val() || "0");

        // La base del IGTF sólo puede ser el principal pendiente antes de
        // este pago. El excedente del efectivo recibido es vuelto y no debe
        // generar IGTF.
        let principalPagadoAntesBs = 0;
        lista.forEach(function (x) {
            if (((x.gid || "").toString()) === gid) return; // excluir el pago recién agregado
            if (((x.tipo || "").trim()) === "IG") return;
            const mx = parseFloat(x.monto || 0);
            const monx = ((x.moneda || "Bs.").trim());
            if (!(mx > 0)) return;
            principalPagadoAntesBs += (monx === "Bs.") ? mx : (mx * tasaDiaDoc);
        });

        const principalPendienteAntesBs = Math.max(0, totalFacturaBaseBs - principalPagadoAntesBs);
        const baseIgtfBs = Math.min(montoPagoBs, principalPendienteAntesBs);
        const montoIgtfBs = (baseIgtfBs * igtfPct) / 100;

        lista = lista.filter(x => !((x.gid || "") === gid && (x.tipo || "") === "IG"));

        if (montoIgtfBs > 0) lista.push({
...
function emitirSinPagos(e) {
    e = e || window.event;
    if (e) { e.preventDefault(); e.stopPropagation(); }

    if (!confirm("¿Confirma que desea emitir este documento SIN registrar pagos?\n\nSe imprimirá como pago de contado en efectivo (Bs.) por el monto total.")) {
        return false;
    }
...
// -----------------------------------------------------------------------
// Determinamos el tipo de documento (FC/NC/ND) ANTES del gate de pagos,
// para saber si aplica el botón "Emitir sin pagos" (solo NC y ND).
$cab_tipo_gate = row1_pago($link, "SELECT documento, tipo_documento FROM salidas WHERE id = $id LIMIT 1");
$tipo_doc_gate = map_tipo_fiscal($cab_tipo_gate["documento"] ?? "", $cab_tipo_gate["tipo_documento"] ?? "");
// $permite_sin_pagos = in_array($tipo_doc_gate, array("NC", "ND"));
$permite_sin_pagos = in_array($tipo_doc_gate, array("NC"));

if ($test_fiscal != 1 && $id > 0 && !factura_ya_pagada($link, $id)) {
    // "Emitir sin pagos" solo se respeta para NC/ND. Si llega sin_pagos=1
    // en una FC (o cualquier otro tipo), se ignora y se exige el pago como
    // siempre — evita que alguien arme la URL a mano para saltarse el
    // cobro de una factura de venta.
    if ($sin_pagos == 1 && $permite_sin_pagos) {
        // No se renderiza la pantalla de pagos: se sigue de largo hacia el
        // motor fiscal. generar_dat_fiscal() ya trae la red de seguridad
        // (PAGOS=1, PAGO1=EF|EFECTIVO|<total>|Bs.) cuando no encuentra
        // filas en cobros_cliente_detalle para este documento.
    } else {
        render_pagina_pago($link, $id, $username, $auto_return, $generar_ne, $permite_sin_pagos);
        exit;
    }
}
