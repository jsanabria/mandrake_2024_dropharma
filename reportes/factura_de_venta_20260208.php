...
if ($igtf_status == "S") {
    // Lógica para cuando aplica IGTF: se calcula sobre el monto total en Bs y se muestra el equivalente en USD
    // Usamos el total calculado (xTotal) más el monto_igtf que viene de la tabla salidas
    $total_con_igtf_bs = $xTotal + $monto_igtf;
    $total_indexado_usd = $total_con_igtf_bs / $tasa_dia;
    
    $this->SetFont('Courier', 'B', 7); 
    // Mostramos la alícuota dinámica (ej. 3%), la base en Bs y el total final en USD
    if($moneda == "USD") {
	    $texto_igtf = "I.G.T.F. ".number_format($alicuota_dinamica, 0)."% s/Base: ".number_format($total_con_igtf_bs, 2, ",", ".")." USD: " . number_format($monto_base_igtf, 2, ",", ".");
    } 
    else {
    ...
    if($moneda == "USD") {
	    $this->Cell(65, 4, "I.G.T.F. ".number_format($alicuota_dinamica, 0)."%: USD " . number_format($monto_total_referencia, 2, ",", "."), 0, 0, 'L');
    } 
    else {
	    ...
	    $this->Cell(91, 4, mb_convert_encoding("Tasa de cambio Publicada por el B.C.V. según la fecha de emisión de esta factura.", "UTF-8"), 0, 0, 'L');
	    $this->SetFont('Courier','B',8);
	    ...
	    $this->SetFont('Courier','B',7);
	    $this->Cell(5, 4);
	    $this->Cell(110, 4, mb_convert_encoding("IGTF Sujeto a Pago Recibido (Efectivo $) según Art 1 GO 42339 17/03/2022.", "UTF-8"), 0, 0, 'R');
	    $this->SetFont('Courier','B',8);
...
//
	    ...
	    $this->Cell(10, 4);
	    $this->SetFont('Courier','B',8);
	    $this->Cell(180, 4, mb_convert_encoding("Esta factura será indexada a la tasa de cambio expresada por el B.C.V. al momento de recibir el pago.", "UTF-8"), 0, 0, 'L');
...
if ($moneda == 'USD') {
    ...
    $val_precio_usd = $x_precio_full; // Ya está en USD, no se divide
} else {
    // Lógica original para Bs.
...
if ($moneda == 'USD') {
    ...
    $val_total_bs = ($GLOBALS["moneda_default"] == "USD" ? $precio_linea_total * $tasa_dia : $precio_linea_total);
    $val_total_usd = ($GLOBALS["moneda_default"] == "USD" ? $precio_linea_total : $precio_linea_total / $tasa_dia);
}
