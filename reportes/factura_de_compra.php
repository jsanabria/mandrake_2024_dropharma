...
class PDF extends FPDF
{
	...
	function EndReport($id)
	{
		...
		$sql = "SELECT
					SUM(cantidad_articulo) AS cantidad, 
					SUM(costo) AS precio, 
					SUM(IF(IFNULL(alicuota,0)=0, costo, 0)) AS exento, 
					SUM(IF(IFNULL(alicuota,0)=0, 0, costo)) AS gravado,
					SUM(IF(IFNULL(alicuota,0)=0, 0, costo) * (IFNULL(alicuota,0)/100)) AS iva, 
					SUM(IF(IFNULL(alicuota,0)=0, costo, 0)) + SUM(IF(IFNULL(alicuota,0)=0, 0, costo)) + (SUM(IF(IFNULL(alicuota,0)=0, 0, costo) * (IFNULL(alicuota,0)/100))) AS total 
				FROM entradas_salidas
				WHERE tipo_documento = 'TDCFCC' AND 
					id_documento = '$id'"; 
		$rs = mysqli_query($link, $sql);
		$row = mysqli_fetch_array($rs);
		...
		$this->SetFont('Courier','B',10);
		$this->Cell(59,4, "SUB-TOTAL GRAVADO:", 0, 0, 'R');
		$this->SetFont('Courier','',10);
		...
		$this->Ln(5);
		$this->SetFont('Courier','B',10);
		$this->Cell(50, 4, "Unidades: " . $row["cantidad"], 0, 0, 'R');
		$this->Cell(51, 4, strtoupper($nota), 0, 0, 'R');
		$this->Cell(60, 4, "TOTAL $moneda:", 0, 0, 'R');
		$this->SetFont('Courier','',10);
...
while($row = mysqli_fetch_array($rs))
{
	...
	if(strlen($row["articulo"]) < 50) 
		$pdf->Cell(90, 4, $row["articulo"], 0, 0, 'L');
	else 
		$pdf->Cell(90, 4, substr($row["articulo"], 0, 50), 0, 0, 'L');
	$pdf->Cell(15, 4, $row["unidad_medida"] . " " . number_format($row["cantidad"], 0, ".", ","), 0, 0, 'R');
	...
	if(strlen($row["articulo"]) >= 50) {
		$pdf->Ln();
		$pdf->Cell(30, 4);
		$pdf->MultiCell(90, 4, substr($row["articulo"], 50, strlen($row["articulo"])), 0, 'L');
	}
	else $pdf->Ln();
