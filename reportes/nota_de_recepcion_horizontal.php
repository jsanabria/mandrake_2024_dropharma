...
class PDF extends FPDF
{
	...
	function Header()
	{
		...
		$this->Cell(20, 5, substr(($GLOBALS["fecha"]),6,4),0,0,'C');
		$this->Ln();
		...
		$this->MultiCell(160, 4, "$direccion_proveedor. $ciudad_proveedor", '0', 'L');
...
$sql = "SELECT 
			...
			WHERE 
				bb.id = a.id_documento AND aa.articulo = a.articulo) AS codigo_proveedor,  
			...
			a.costo_unidad, 
			a.costo, a.lote, date_format(a.fecha_vencimiento, '%d/%m/%Y') AS fecha_vencimiento, b.codigo,  
...
while($row = mysqli_fetch_array($rs))
{
    ...
    // 1. Guardamos la posición Y actual antes de pintar el código
    $x_actual = $pdf->GetX();
    $y_actual = $pdf->GetY();
    
    // Usamos el código deseado (puedes cambiarlo a "codigo_proveedor" si corresponde)
    $codigo_articulo = trim($row["codigo"]); 
    
    // 2. Pintamos el código con MultiCell para que salte de línea si pasa de 20mm
    $pdf->MultiCell(20, 4, $codigo_articulo, 0, 'L');
    
    // 3. Guardamos la posición Y después del código (por si creció hacia abajo)
    $y_despues_codigo = $pdf->GetY();
    
    // 4. Forzamos al cursor a volver al lado del código para seguir pintando la fila
    ...
    $pdf->Cell(20, 4, $almacen, 0, 0, 'L');
    $pdf->Cell(20, 4, number_format($row["costo_unidad"], 2, ".", ","), 0, 0, 'R');
    $pdf->Cell(25, 4, number_format($row["costo"], 2, ".", ","), 0, 0, 'R');
    $pdf->Cell(15, 4, $row["cantidad"], 0, 0, 'R');
    ...
    if(strlen($row["articulo"]) >= 50) {
        ...
        if ($y_despues_codigo > $y_actual + 4) {
            $pdf->SetY($y_despues_codigo); // Vamos al final del código largo
        } else {
            $pdf->Ln(); // Salto de línea normal
        }
    }
