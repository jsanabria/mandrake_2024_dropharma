...
$sql = "SELECT 
				...
				ROUND((SELECT
					SUM(IF(IFNULL(alicuota,0)=0, (precio - (precio * (IFNULL(a.descuento, 0)/100))) - ((precio - (precio * (IFNULL(a.descuento, 0)/100))) * (IFNULL(a.descuento2, 0)/100)), 0)) AS exento_2 
				FROM entradas_salidas
				WHERE tipo_documento = a.tipo_documento AND 
					id_documento = a.id), 2) AS no_gravadas, 
				ROUND((SELECT
					SUM(IF(IFNULL(alicuota,0)=0, 0, (precio - (precio * (IFNULL(a.descuento, 0)/100))) - ((precio - (precio * (IFNULL(a.descuento, 0)/100))) * (IFNULL(a.descuento2, 0)/100)))) AS gravado_2 
				FROM entradas_salidas
				WHERE tipo_documento = a.tipo_documento AND 
