...
function monto_bs_usd($monto, $moneda, $tasa_dia) {
    ...
    if ($moneda == "Bs." || strtoupper($moneda) == "BS" || strtoupper($moneda) == "VES") {
        return ["bs" => $monto, "usd" => $monto / $tasa_dia];
    }
    return ["bs" => $monto * $tasa_dia, "usd" => $monto];
}
...
class PDF extends FPDF
{
    ...
    function EndReport($id_invoice)
    {
        ...
        // Subtotales originales, sin descuentos.
        ...
        // Descuentos en cascada sobre la moneda del documento.
        ...
        if ($igtf == "S") {
            // El IGTF siempre se guarda en Bs., independientemente de salidas.moneda.
            $monto_igtf_bs = $monto_igtf;
            $monto_igtf_usd = ($tasa_dia > 0) ? ($monto_igtf_bs / $tasa_dia) : 0;
            $monto_base_igtf_bs = $monto_base_igtf;
            $totaligtf_bs = $total_bs_recalculado + $monto_igtf_bs;
            $totaligtf_usd = ($tasa_dia > 0) ? ($totaligtf_bs / $tasa_dia) : 0;

            $this->Ln(3);
            $this->SetFont('Arial','B',7);
            $this->Cell(30, 3, "", 0, 0, 'R');
            $this->Cell(90, 3, "", 0, 0, 'R');
            $this->Cell(43, 3, "I.G.T.F. " . number_format($igtf_porc, 0, "", "") . "% SOBRE " . number_format($monto_base_igtf_bs, 2, ",", ".") . " Bs.:", 0, 0, 'R');
            $this->SetFont('Arial','',7);
...
while ($row = mysqli_fetch_array($rs)) {
    ...
    $pdf->Cell(20, 3, number_format($precio_conv["bs"], 2, ",", "."), 0, 0, 'R');
    $pdf->Cell(15, 3, number_format($precio_conv["usd"], 2, ",", "."), 0, 0, 'R');
