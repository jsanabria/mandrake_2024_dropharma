...
class PDF extends FPDF
{
	...
	function Header()
	{
		...
		if(trim($logo) != "") {
			$this->Image("../carpetacarga/$logo", 10, 10, 70);
		}
	...
	function EndReport($id_invoice)
	{
		...
		*/
		...
		if(floatval($row["iva"]) > 0.00) {
				if($GLOBALS["con_precio"] == "S") {
					/*
					$this->Cell(175, 3, "SUB-TOTAL EXENTO:", 0, 0, 'R');
					$this->SetFont('Arial','',8);
					$this->Cell(25, 3, number_format($row["exento"], 2, ",", "."), 0, 0, 'R');
					//$this->Cell(25, 3, number_format($row["exento"]/$tasa_dia, 2, ",", "."), 0, 0, 'R');
					$this->Ln(4);
					*/
					$this->SetFont('Arial','',8);
					...
					//$this->SetFont('Arial','',8);
					//$this->Cell(175,3, "IVA:", 0, 0, 'R');
					//$this->SetFont('Arial','',8);

					//}
					//$this->Cell(25, 3, number_format($row["iva"], 2, ",", "."), 0, 0, 'R');
					//$this->Cell(25, 3, number_format($row["iva"]/$tasa_dia, 2, ",", "."), 0, 0, 'R');
				}
				$this->Ln(4);
				$this->SetFont('Arial','',8);
				$this->Cell(90, 3);
				...
				$this->SetFont('Arial','',8);
				if($GLOBALS["con_precio"] == "S") {
					//$this->Cell(35, 3, "TOTAL $moneda:", 0, 0, 'R');
					//$this->SetFont('Arial','',8);
					//$this->Cell(25, 3, number_format($row["total"], 2, ",", "."), 0, 0, 'R');
...
$pdf->SetFont('Arial','',8);
		...
		while($row = mysqli_fetch_array($rs))
		{
			...
			if($GLOBALS["con_precio"] == "S") {
				$pdf->Cell(10, 4);
				$pdf->Cell(20, 4, substr($row["codigo"], 0, 10), 0, 0, 'L');				$desart = trim($row["articulo"]) . ". " . $row["codigo"]; 
				...
				$pdf->Cell(10, 4);
				$pdf->Cell(40, 4, substr($row["codigo"], 0, 20), 0, 0, 'L');				$desart = trim($row["articulo"]) . ". " . $row["codigo"]; 
				$pdf->Cell(135, 4, substr($desart, 0, 70), 0, 0, 'L');
				$pdf->Cell(15, 4, round($row["cantidad"], 0), 0, 0, 'C');
				if(strlen($row["articulo"]) >= 90) {
					$pdf->Ln();
					$pdf->Cell(50, 4);
