...
/*$sql = "SELECT * FROM entradas_salidas where id = '$id';"; die($sql);
$rs = mysqli_query($link, $sql);
if(!$row = mysqli_fetch_array($rs)) die("La Factura no tiene Detalle");*/

$sql = "SELECT 
			id, date_format(fecha, '%d/%m/%Y') as fecha, proveedor, nro_documento, tipo_documento, estatus, doc_afectado  
...
class PDF extends FPDF
{
	...
	function Header()
	{
		...
		$this->Cell(200, 6, "ESTATUS: " . $GLOBALS["estatus"] . " / " . "No. Doc.: " . $GLOBALS["nro_documento"] . " / " . "Doc. Relacionado: " . $GLOBALS["doc_afectado"],0,0,'R');
		...
		$this->Cell(30, 6, "LAB", 1, 0, 'C');
		$this->Cell(120, 6, "ARTICULO", 1, 0, 'L');
		$this->Cell(20, 6, "CANTIDAD", 1, 0, 'R');
		$this->Ln(6);
	}
	...
	function EndReport($items, $unidades)
	{
		//$this->AddPage();
		//require("../include/connect.php");
		$this->Ln();
		$this->Cell(200, 6, "TOTAL ITEMS: "  . $items . " | TOTAL UNIDADES: " . $unidades, 0, 0, 'R');
		//require("../include/desconnect.php");
	}
}
...
$sql = "SELECT 
			...
			WHERE 
				bb.id = a.id_documento AND aa.articulo = a.articulo) AS codigo2, b.codigo,  
...
while($row = mysqli_fetch_array($rs))
{
	$pdf->SetFont('Arial', '', 8);
	$pdf->Cell(10, 5);
	$pdf->Cell(20, 5, $row["codigo"], 0, 0, 'L');
	$pdf->Cell(30, 4, substr($row["lab"], 0, 12), 0, 0, 'L');
	if(strlen($row["articulo"]) < 60) 
		$pdf->Cell(120, 5, trim($row["articulo"]), 0, 0, 'L');
	else 
		$pdf->Cell(120, 5, trim(substr($row["articulo"], 0, 60)), 0, 0, 'L');
	$pdf->Cell(20, 5, $row["cantidad"], 0, 0, 'R');

	if(strlen($row["articulo"]) >= 60) {
		$pdf->Ln();
		$pdf->Cell(60, 5);
		$pdf->MultiCell(130, 5, trim(substr($row["articulo"], 60, strlen($row["articulo"]))), 0, 'L');
	}
	else $pdf->Ln();
