...
class PDF extends FPDF
{
	...
	function EndReport($id_invoice)
	{
		...
		if($descuento > 0) $this->Ln(225-$this->GetY());
		else $this->Ln(230-$this->GetY());
...
*/
		...
		$this->Ln();
		$this->Cell(10, 4);
		$this->Cell(100, 4, "Ley de IVA  art 51 y art 52.", 0, 0, 'L');
...
$sql = "SELECT 
			...
			IFNULL(a.alicuota, 0) AS alicuota, 
			a.precio_unidad, 
			a.precio, 
...
while($row = mysqli_fetch_array($rs))
{
	...
	$pdf->Cell(13, 3, number_format(($GLOBALS["moneda_default"]=="USD" ? $row["precio_ful"] : ($row["precio_ful"]/$tasa_dia)), 2, ",", "."), 0, 0, 'R');
	$pdf->Cell(8, 3, floatval($row["descuento"])>0 ? number_format($row["descuento"], 0, ",", ".") . "%" : "", 0, 0, 'R');
	$pdf->SetFillColor(255, 0, 0);
	$pdf->Cell(25, 3, number_format(($GLOBALS["moneda_default"]=="USD" ? $row["precio"]*$tasa_dia : $row["precio"]), 2, ",", "."), 0, 0, 'R', ($row["precio"] == 0 ? true: false));
	$pdf->Cell(17, 3, number_format(($GLOBALS["moneda_default"]=="USD" ? $row["precio"] : $row["precio"]/$tasa_dia), 2, ",", "."), 0, 0, 'R', ($row["precio"] == 0 ? true: false));
