...
$sql = "SELECT 
			...
			date_format(DATE_ADD(fecha,INTERVAL IFNULL(dias_credito, 0) DAY), '%d/%m/%y') AS fec_venc, doc_afectado, descuento 
...
class PDF extends FPDF
{
	...
	function Header()
	{
		...
		$this->SetFont('Courier','',8);
		$this->Cell(110, 3, mb_convert_encoding(substr($razon_social, 0, 55), "UTF-8", mb_detect_encoding($razon_social)),'0','0','L');
		$this->SetFont('Courier','B',8);
		...
		*/
		...
		$this->SetFont('Courier','B',8);
		$this->Cell(18,4,'Dias Cred:','0','0','L');
		$this->SetFont('Courier','',8);
	...
	function EndReport($id_invoice)
	{
		...
		$sql = "SELECT
					SUM(precio) AS precio, 
					SUM(IF(IFNULL(alicuota,0)=0, (precio_unidad*cantidad_articulo) - ((precio_unidad*cantidad_articulo) * ($descuento/100)), 0)) AS exento, 
					SUM(IF(IFNULL(alicuota,0)=0, 0, (precio_unidad*cantidad_articulo) - ((precio_unidad*cantidad_articulo) * ($descuento/100)))) AS gravado,
					SUM(IF(IFNULL(alicuota,0)=0, (precio_unidad*cantidad_articulo) - ((precio_unidad*cantidad_articulo) * ($descuento/100)), 0)) AS exento_2, 
					SUM(IF(IFNULL(alicuota,0)=0, 0, (precio_unidad*cantidad_articulo) - ((precio_unidad*cantidad_articulo) * ($descuento/100)))) AS gravado_2, 
					SUM(IF(IFNULL(alicuota,0)=0, 0, (precio_unidad*cantidad_articulo) - ((precio_unidad*cantidad_articulo) * ($descuento/100))) * (IFNULL(alicuota,0)/100)) AS iva, 
					SUM(IF(IFNULL(alicuota,0)=0, (precio_unidad*cantidad_articulo) - ((precio_unidad*cantidad_articulo) * ($descuento/100)), 0)) + SUM(IF(IFNULL(alicuota,0)=0, 0, (precio_unidad*cantidad_articulo) - ((precio_unidad*cantidad_articulo) * ($descuento/100)))) + (SUM(IF(IFNULL(alicuota,0)=0, 0, (precio_unidad*cantidad_articulo) - ((precio_unidad*cantidad_articulo) * ($descuento/100))) * (IFNULL(alicuota,0)/100))) AS total, 
					SUM(ABS(cantidad_movimiento)) AS cantidad  
				FROM entradas_salidas
				WHERE tipo_documento = 'TDCFCV' AND 
					id_documento = '$id_invoice'"; 
		...
		$this->SetFont('Courier','',8);
		$this->Cell(40, 4, number_format($GLOBALS["moneda_default"]=="USD" ? (($row["exento"]+$row["gravado"])*$tasa_dia) : ($row["exento"]+$row["gravado"]), 2, ",", "."), 0, 0, 'R');
		$this->Cell(19, 4, number_format($GLOBALS["moneda_default"]=="USD" ? ($row["exento"]+$row["gravado"]) : (($row["exento"]+$row["gravado"])/$tasa_dia), 2, ",", "."), 0, 0, 'R');
		$this->Ln(4);
		...
		if($descuento2 > 0) { 
			...
			$this->SetFont('Courier','',8);
			$this->Cell(40, 4, number_format($GLOBALS["moneda_default"]=="USD" ? ($row["exento"]-($row["exento"]*($descuento/100)))*$tasa_dia : $row["exento"]-($row["exento"]*($descuento/100)), 2, ",", "."), 0, 0, 'R');
			$this->Cell(19, 4, number_format($GLOBALS["moneda_default"]=="USD" ? $row["exento"]-($row["exento"]*($descuento/100)) : ($row["exento"]-($row["exento"]*($descuento/100)))/$tasa_dia, 2, ",", "."), 0, 0, 'R');
			$this->Ln(4);
			...
			$this->SetFont('Courier','',8);
			$this->Cell(40, 4, number_format($GLOBALS["moneda_default"]=="USD" ? ($row["gravado"]-($row["gravado"]*($descuento/100)))*$tasa_dia : $row["gravado"]-($row["gravado"]*($descuento/100)), 2, ",", "."), 0, 0, 'R');
			$this->Cell(19, 4, number_format($GLOBALS["moneda_default"]=="USD" ? $row["gravado"]-($row["gravado"]*($descuento/100)) : ($row["gravado"]-($row["gravado"]*($descuento/100)))/$tasa_dia, 2, ",", "."), 0, 0, 'R');
			$this->Ln(4);
		} 
		else {
			$this->SetFont('Courier','B',8);
			$this->Cell(149, 4, $asociado . " " . "TOTAL EXENTO:", 0, 0, 'R');
			$this->SetFont('Courier','',8);
			$this->Cell(40, 4, number_format($GLOBALS["moneda_default"]=="USD" ? $row["exento"]*$tasa_dia : $row["exento"], 2, ",", "."), 0, 0, 'R');
			$this->Cell(19, 4, number_format($GLOBALS["moneda_default"]=="USD" ? $row["exento"] : $row["exento"]/$tasa_dia, 2, ",", "."), 0, 0, 'R');
			$this->Ln(4);
			...
			$this->Cell(48,4, "TOTAL BASE IMPONIBLE:", 0, 0, 'R');
			$this->SetFont('Courier','',8);
			$this->Cell(40, 4, number_format($GLOBALS["moneda_default"]=="USD" ? $row["gravado"]*$tasa_dia : $row["gravado"], 2, ",", "."), 0, 0, 'R');
			$this->Cell(19, 4, number_format($GLOBALS["moneda_default"]=="USD" ? $row["gravado"] : $row["gravado"]/$tasa_dia, 2, ",", "."), 0, 0, 'R');
			$this->Ln(4);
		}
		//
		...
		*/
		$xIVA = ($row["gravado"]-($row["gravado"]*($descuento2/100)))*($xalicuota/100);
		...
		*/
		$xTotal = ($row["exento"]-($row["exento"]*($descuento2/100))) + ($row["gravado"]-($row["gravado"]*($descuento2/100))) + $xIVA;
		/*
		$this->Cell(40, 4, number_format($GLOBALS["moneda_default"]=="USD" ? $row["total"]*$tasa_dia : $row["total"], 2, ",", "."), 0, 0, 'R');
		$this->Cell(19, 4, number_format($GLOBALS["moneda_default"]=="USD" ? $row["total"] : $row["total"]/$tasa_dia, 2, ",", "."), 0, 0, 'R');
		*/
...
while($row = mysqli_fetch_array($rs))
{
	...
	$pdf->Cell(16, 3, ($row["vencimiento"]=="01/01/1990" ? "" : $row["vencimiento"]), 0, 0, 'R');
	...
	$pdf->Cell(22, 3, $printE . number_format(($GLOBALS["moneda_default"]=="USD" ? $precio_full*$tasa_dia : $precio_full) , 2, ",", "."), 0, 0, 'R');
	// 
	$pdf->Cell(13, 3, number_format(($GLOBALS["moneda_default"]=="USD" ? $precio_full : $precio_full/$tasa_dia), 2, ",", "."), 0, 0, 'R');
	...
	$pdf->Cell(25, 3, number_format(($GLOBALS["moneda_default"]=="USD" ? $precio * $tasa_dia : $precio), 2, ",", "."), 0, 0, 'R', ($row["precio"] == 0 ? true: false));
	$pdf->Cell(17, 3, number_format(($GLOBALS["moneda_default"]=="USD" ? $precio  : $precio /$tasa_dia), 2, ",", "."), 0, 0, 'R', ($row["precio"] == 0 ? true: false));
