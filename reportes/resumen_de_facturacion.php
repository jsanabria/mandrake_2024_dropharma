...
class PDF extends FPDF
{
	...
	function EndReport($Total, $granTotal, $items, $xfecha, $yfecha, $granTotalNC, $cantidadNC)
	{
		//$this->AddPage();
		//require("../connect.php");
		...
		$sql = "SELECT 
					...
					SUM(if(a.documento='NC', (-1)*a.total, a.total)) AS total, 
					COUNT(a.monto_total) AS cantidad 
				FROM 
					salidas AS a 
				WHERE 
		...
		while($row = mysqli_fetch_array($rs)) {
			$this->Ln();
			$this->Cell(170, 5, "ESTATUS: "  . $row["estatus"] . " (" . $row["cantidad"] . ")", 0, 0, 'R');
			$this->Cell(30, 5, number_format($row["total"], 2, ".", ","), 0, 0, 'R');
		}
		$this->Ln();
		$this->Cell(170, 5, "NOTAS DE CREDITO  ($cantidadNC)", 0, 0, 'R');
		$this->Cell(30, 5, number_format((-1)*$granTotalNC, 2, ".", ","), 0, 0, 'R');
...
$sql = "SELECT 
			...
			a.iva, if(a.documento='NC', (-1)*a.total, a.total) total, a.estatus, a.moneda, a.documento 
		FROM 
			salidas AS a 
			LEFT OUTER JOIN cliente AS b ON b.id = a.cliente 
			LEFT OUTER JOIN usuario AS c ON c.username = a.username 
		WHERE 
			a.tipo_documento = 'TDCFCV' AND $xcliente $xasesor 
			a.fecha BETWEEN '$xfecha 00:00:00' AND '$yfecha 23:59:59' AND a.estatus <> 'ANULADO' ORDER BY a.id ASC;"; 
...
while($row = mysqli_fetch_array($rs))
{
	...
	$granTotal += $row["total"];
	if($row["documento"] == "NC") {
		$granTotalNC += $row["total"];
		$cantidadNC++;
	} 
}

$pdf->EndReport($Total, $granTotal, $items, $xfecha, $yfecha, $granTotalNC, $cantidadNC);
