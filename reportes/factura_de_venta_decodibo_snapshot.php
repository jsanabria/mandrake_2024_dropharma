...
class PDF extends FPDF
{
    ...
    function EndReport($id_invoice)
    {
        ...
        if ($igtf == "S") {
            ...
            // --- AJUSTE: CÁLCULOS DE REFERENCIA EN Bs EN UNA SOLA LÍNEA ---
            $base_igtf_bs = ($moneda == "Bs.") ? $monto_total : $monto_total * $tasa_dia;
            $monto_impuesto_igtf_bs = $base_igtf_bs * ($igtf_porc / 100);
            $total_general_bs = $base_igtf_bs + $monto_impuesto_igtf_bs;

            $this->Ln(3);
            $this->SetFont('Arial', 'B', 7);
            
            // 1. Línea descriptiva con la Base y la Alícuota + el Monto del impuesto a la derecha
            $texto_linea_unica = "I.G.T.F. (" . number_format($igtf_porc, 2, ",", ".") . "%) SOBRE Bs. " . number_format($base_igtf_bs, 2, ",", ".") . ":";
            $this->Cell(168, 3, $texto_linea_unica, 0, 0, 'R');
            $this->SetFont('Arial', '', 7);
            $this->Cell(32, 3, number_format($monto_impuesto_igtf_bs, 2, ",", "."), 0, 0, 'R');

            // 2. Línea del TOTAL GENERAL sumando el IGTF referencial
            $this->Ln(3);
            $this->SetFont('Arial', 'B', 7);
            $this->Cell(168, 3, "TOTAL GENERAL Bs:", 0, 0, 'R');
            $this->SetFont('Arial', '', 7);
            $this->Cell(32, 3, number_format($total_general_bs, 2, ",", "."), 0, 0, 'R');
        }

        // --- NOTA FIJA DE LA GACETA + TASA DEL DIA EN LA MISMA LÍNEA ---
        $this->Ln(5);
        $this->SetFont('Arial', 'B', 7);
        $nota_gaceta = "IGTF Sujeto a Pago Recibido (Efectivo $) segun Art 1 GO 42339 17/03/2022.";
        $tasa_bcv_texto = "Tasa BCV del Dia: " . number_format($tasa_dia, 2, ",", ".");
        
        $this->Cell(5, 3);
        $this->Cell(135, 3, enc_pdf($nota_gaceta), 0, 0, 'L');
        $this->Cell(60, 3, enc_pdf($tasa_bcv_texto), 0, 0, 'R');
