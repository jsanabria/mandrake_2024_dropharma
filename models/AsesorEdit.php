 ...
 * Page class
 */
class AsesorEdit extends Asesor
{
    use MessagesTrait;
    ...
    // CSS class/style
    public $CurrentPageName = "AsesorEdit";
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "AsesorList") {
                            ...
                            $returnUrl = "AsesorList"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
    ...
    protected function validateForm()
    {
        ...
        if (in_array("asesor_cliente", $detailTblVar) && $detailPage->DetailEdit) {
            $detailPage->run();
            $validateForm = $validateForm && $detailPage->validateGridForm();
        }
        $detailPage = Container("AsesorFabricanteGrid");
        if (in_array("asesor_fabricante", $detailTblVar) && $detailPage->DetailEdit) {
            $detailPage->run();
            $validateForm = $validateForm && $detailPage->validateGridForm();
        }
    ...
    protected function editRow()
    {
        ...
        if ($updateRow) {
            ...
            $detailTblVar = explode(",", $this->getCurrentDetailTable());
            $detailPage = Container("AsesorClienteGrid");
            if (in_array("asesor_cliente", $detailTblVar) && $detailPage->DetailEdit && $editRow) {
                $Security->loadCurrentUserLevel($this->ProjectID . "asesor_cliente"); // Load user level of detail table
                $editRow = $detailPage->gridUpdate();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
            }
            $detailPage = Container("AsesorFabricanteGrid");
            if (in_array("asesor_fabricante", $detailTblVar) && $detailPage->DetailEdit && $editRow) {
