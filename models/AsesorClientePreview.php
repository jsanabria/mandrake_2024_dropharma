 ...
 * Page class
 */
class AsesorClientePreview extends AsesorCliente
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "AsesorClientePreview";
    ...
    // CSS class/style
    public $CurrentPageName = "AsesorClientePreview";
    ...
    protected function masterKeyUrl()
    {
        ...
        if ($masterTblVar == "asesor") {
            $url = "" . Config("TABLE_SHOW_MASTER") . "=asesor&" . GetForeignKeyUrl("fk_id", $this->asesor->QueryStringValue) . "";
        }
        return $url;
    }
    ...
    protected function setupForeignKeys($keys)
    {
        $masterTblVar = Get("t", "");
        if ($masterTblVar == "asesor") {
            $this->asesor->setQueryStringValue($keys[0] ?? "");
        }
    }
    ...
    public function listOptionsLoad() {
    	// Example:
    	$opt = &$this->ListOptions->Add("editar");
    	$opt->Header = "";
    	$opt->OnLeft = TRUE; // Link on left
    	$opt->MoveTo(5); // Move to first column
    	$opt = &$this->ListOptions->Add("ciudad");
    	$opt->Header = "Ciudad";
    	$opt->OnLeft = TRUE; // Link on left
    	$opt->MoveTo(5); // Move to first column
    }
    ...
    public function listOptionsRendered() {
    	// Example:
    	$cliente = $this->cliente->CurrentValue;
    	$url = '<a target="_blank" href="ClienteEdit?id=' . $cliente . '">Editar</a>';
    	$this->ListOptions->Items["editar"]->Body = $url;
    	$sql = "SELECT 
    				b.campo_descripcion AS ciudad  
    			FROM 
    				cliente AS a 
    				LEFT OUTER JOIN tabla AS b ON b.campo_codigo = a.ciudad 
    			WHERE 
    				a.id = '$cliente' AND b.tabla = 'CIUDAD';";
    	$this->ListOptions->Items["ciudad"]->Body = ExecuteScalar($sql);
    }
}
