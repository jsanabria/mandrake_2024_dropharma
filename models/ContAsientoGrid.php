 ...
 * Page class
 */
class ContAsientoGrid extends ContAsiento
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ContAsientoGrid";
    ...
    public $FormName = "fcont_asientogrid";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "ContAsientoGrid";
    ...
    public function emptyRow()
    {
        global $CurrentForm;
        if (
            $CurrentForm->hasValue("x_cuenta") &&
            $CurrentForm->hasValue("o_cuenta") &&
            $this->cuenta->CurrentValue != $this->cuenta->DefaultValue &&
            !($this->cuenta->IsForeignKey && $this->getCurrentMasterTable() != "" && $this->cuenta->CurrentValue == $this->cuenta->getSessionValue())
        ) {
            return false;
        }
        if (
            $CurrentForm->hasValue("x_nota") &&
            $CurrentForm->hasValue("o_nota") &&
            $this->nota->CurrentValue != $this->nota->DefaultValue &&
            !($this->nota->IsForeignKey && $this->getCurrentMasterTable() != "" && $this->nota->CurrentValue == $this->nota->getSessionValue())
        ) {
            return false;
        }
        if (
        ...
        if (
            $CurrentForm->hasValue("x_debe") &&
            $CurrentForm->hasValue("o_debe") &&
            $this->debe->CurrentValue != $this->debe->DefaultValue &&
            !($this->debe->IsForeignKey && $this->getCurrentMasterTable() != "" && $this->debe->CurrentValue == $this->debe->getSessionValue())
        ) {
            return false;
        }
        if (
            $CurrentForm->hasValue("x_haber") &&
            $CurrentForm->hasValue("o_haber") &&
            $this->haber->CurrentValue != $this->haber->DefaultValue &&
            !($this->haber->IsForeignKey && $this->getCurrentMasterTable() != "" && $this->haber->CurrentValue == $this->haber->getSessionValue())
        ) {
            return false;
        }
        return true;
    ...
    protected function loadFormValues()
    {
        ...
        if ($CurrentForm->hasValue("o_cuenta")) {
        ...
        if ($CurrentForm->hasValue("o_nota")) {
            $this->nota->setOldValue($CurrentForm->getValue("o_nota"));
        }
        ...
        if ($CurrentForm->hasValue("o_debe")) {
            $this->debe->setOldValue($CurrentForm->getValue("o_debe"));
        }
        ...
        if ($CurrentForm->hasValue("o_haber")) {
            $this->haber->setOldValue($CurrentForm->getValue("o_haber"));
        }
    ...
    protected function editRow()
    {
        ...
        // Check referential integrity for master table 'cont_comprobante'
        $detailKeys = [];
        $keyValue = $rsnew['comprobante'] ?? $rsold['comprobante'];
        $detailKeys['comprobante'] = $keyValue;
    ...
    protected function getEditRow($rsold)
    {
        ...
        // debe
        $this->debe->setDbValueDef($rsnew, $this->debe->CurrentValue, $this->debe->ReadOnly);

        // haber
        $this->haber->setDbValueDef($rsnew, $this->haber->CurrentValue, $this->haber->ReadOnly);
        return $rsnew;
    }
    ...
    protected function restoreEditFormFromRow($row)
    {
        ...
        if (isset($row['debe'])) { // debe
            $this->debe->CurrentValue = $row['debe'];
        }
        if (isset($row['haber'])) { // haber
            $this->haber->CurrentValue = $row['haber'];
        }
    }
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($this->getCurrentMasterTable() == "cont_comprobante") {
            $this->comprobante->Visible = true; // Need to insert foreign key
            $this->comprobante->CurrentValue = $this->comprobante->getSessionValue();
        }
