 ...
 * Page class
 */
class CompaniaCuentaPreview extends CompaniaCuenta
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "CompaniaCuentaPreview";
    ...
    // CSS class/style
    public $CurrentPageName = "CompaniaCuentaPreview";
    ...
    protected function masterKeyUrl()
    {
        ...
        if ($masterTblVar == "compania") {
            $url = "" . Config("TABLE_SHOW_MASTER") . "=compania&" . GetForeignKeyUrl("fk_id", $this->compania->QueryStringValue) . "";
        }
        return $url;
    }
    ...
    protected function setupForeignKeys($keys)
    {
        $masterTblVar = Get("t", "");
        if ($masterTblVar == "compania") {
            $this->compania->setQueryStringValue($keys[0] ?? "");
        }
    }
