...
/**
 * Table class for proveedor_articulo
 */
class ProveedorArticulo extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        // Update Table
        $this->UpdateTable = "proveedor_articulo";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        $this->articulo->Lookup = new Lookup($this->articulo, 'articulo', false, 'id', ["principio_activo","presentacion","nombre_comercial",""], '', '', ["x_fabricante"], [], ["fabricante"], ["x_fabricante"], [], [], false, '', '', "CONCAT(COALESCE(`principio_activo`, ''),'" . ValueSeparator(1, $this->articulo) . "',COALESCE(`presentacion`,''),'" . ValueSeparator(2, $this->articulo) . "',COALESCE(`nombre_comercial`,''))");
        ...
        // codigo_proveedor
        $this->codigo_proveedor = new DbField(
            $this, // Table
            'x_codigo_proveedor', // Variable name
            'codigo_proveedor', // Name
            '`codigo_proveedor`', // Expression
            '`codigo_proveedor`', // Basic search expression
            200, // Type
            30, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`codigo_proveedor`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->codigo_proveedor->InputTextType = "text";
        $this->codigo_proveedor->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY", "IS NULL", "IS NOT NULL"];
    ...
    public function getMasterFilterFromSession()
    {
        ...
        if ($this->getCurrentMasterTable() == "proveedor") {
            $masterTable = Container("proveedor");
            if ($this->proveedor->getSessionValue() != "") {
                $masterFilter .= "" . GetKeyFilter($masterTable->id, $this->proveedor->getSessionValue(), $masterTable->id->DataType, $masterTable->Dbid);
            } else {
                return "";
            }
        }
    ...
    public function getDetailFilterFromSession()
    {
        ...
        if ($this->getCurrentMasterTable() == "proveedor") {
            $masterTable = Container("proveedor");
            if ($this->proveedor->getSessionValue() != "") {
                $detailFilter .= "" . GetKeyFilter($this->proveedor, $this->proveedor->getSessionValue(), $masterTable->id->DataType, $this->Dbid);
            } else {
                return "";
            }
        }
    ...
    public function getMasterFilter($masterTable, $keys)
    {
        ...
        switch ($masterTable->TableVar) {
            case "proveedor":
                ...
                if ($validKeys) {
                    return GetKeyFilter($masterTable->id, $keys["proveedor"], $this->proveedor->DataType, $this->Dbid);
                }
                break;
        }
    ...
    public function getDetailFilter($masterTable)
    {
        switch ($masterTable->TableVar) {
            case "proveedor":
                return GetKeyFilter($this->proveedor, $masterTable->id->DbValue, $masterTable->id->DataType, $masterTable->Dbid);
        }
        return "";
    }
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "proveedor_articulo";
    }
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->codigo_proveedor->DbValue = $row['codigo_proveedor'];
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("ProveedorArticuloList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "ProveedorArticuloView" => $Language->phrase("View"),
            "ProveedorArticuloEdit" => $Language->phrase("Edit"),
            "ProveedorArticuloAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "ProveedorArticuloView",
            Config("API_ADD_ACTION") => "ProveedorArticuloAdd",
            Config("API_EDIT_ACTION") => "ProveedorArticuloEdit",
            Config("API_DELETE_ACTION") => "ProveedorArticuloDelete",
            Config("API_LIST_ACTION") => "ProveedorArticuloList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("ProveedorArticuloView", $parm);
        } else {
            $url = $this->keyUrl("ProveedorArticuloView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = "ProveedorArticuloAdd?" . $parm;
        } else {
            $url = "ProveedorArticuloAdd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ProveedorArticuloEdit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ProveedorArticuloList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ProveedorArticuloAdd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ProveedorArticuloList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("ProveedorArticuloDelete", $parm);
        }
    }
    ...
    {
        if ($this->getCurrentMasterTable() == "proveedor" && !ContainsString($url, Config("TABLE_SHOW_MASTER") . "=")) {
            $url .= (ContainsString($url, "?") ? "&" : "?") . Config("TABLE_SHOW_MASTER") . "=" . $this->getCurrentMasterTable();
            $url .= "&" . GetForeignKeyUrl("fk_id", $this->proveedor->getSessionValue()); // Use Session Value
        }
        return $url;
    }
    ...
    {
        global $Response;
        $listPage = "ProveedorArticuloList";
    ...
    public function renderEditRow()
    {
        ...
        $this->codigo_proveedor->EditValue = $this->codigo_proveedor->CurrentValue;
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->codigo_proveedor);
                } else {
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->codigo_proveedor);
                    } else {
    ...
    {
        WriteAuditLog(CurrentUserIdentifier(), $typ, 'proveedor_articulo');
    }
    ...
    public function writeAuditTrailOnAdd(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "A", 'proveedor_articulo', $fldname, $key, "", $newvalue);
            }
        }
    }
        ...
        foreach (array_keys($rsnew) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && array_key_exists($fldname, $rsold) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                if ($modified) {
                    ...
                    WriteAuditLog($usr, "U", 'proveedor_articulo', $fldname, $key, $oldvalue, $newvalue);
                }
            }
        }
    }
    ...
    public function writeAuditTrailOnDelete(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "D", 'proveedor_articulo', $fldname, $key, $oldvalue);
            }
        }
    }
