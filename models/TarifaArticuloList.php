 ...
 * Page class
 */
class TarifaArticuloList extends TarifaArticulo
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "TarifaArticuloList";
    ...
    public $FormName = "ftarifa_articulolist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "TarifaArticuloList";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'tarifa_articulo';
        $this->TableName = 'tarifa_articulo';
        ...
        // Table object (tarifa_articulo)
        if (!isset($GLOBALS["tarifa_articulo"]) || $GLOBALS["tarifa_articulo"]::class == PROJECT_NAMESPACE . "tarifa_articulo") {
            $GLOBALS["tarifa_articulo"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "TarifaArticuloAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "TarifaArticuloDelete";
        $this->MultiUpdateUrl = "TarifaArticuloUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'tarifa_articulo');
        }
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                    ...
                    $result["view"] = SameString($pageName, "TarifaArticuloView"); // If View page, no primary button
                } else { // List page
    ...
    public function run()
    {
        ...
        if ($this->IsModal) {
            $this->FormName = "ftarifa_articulogrid";
        }
        ...
        if (IsPost() && ($this->isGridUpdate() || $this->isMultiUpdate() || $this->isGridOverwrite()) && (Session(SESSION_INLINE_MODE) == "gridedit" || Session(SESSION_INLINE_MODE) == "multiedit")) {
            ...
            if ($gridUpdate) {
                ...
                if ($this->IsModal && !$this->UseAjaxActions) {
                    $this->terminate("TarifaArticuloList");
                    return;
                }
            } else {
        ...
        // Load master record
        if ($this->CurrentMode != "add" && $this->DbMasterFilter != "" && $this->getCurrentMasterTable() == "tarifa") {
            $masterTbl = Container("tarifa");
            $rsmaster = $masterTbl->loadRs($this->DbMasterFilter)->fetchAssociative();
            $this->MasterRecordExists = $rsmaster !== false;
            if (!$this->MasterRecordExists) {
                $this->setFailureMessage($Language->phrase("NoRecord")); // Set no record found
                $this->terminate("TarifaList"); // Return to master page
                return;
            } else {
    ...
    public function emptyRow()
    {
        global $CurrentForm;
        if (
            $CurrentForm->hasValue("x_tarifa") &&
            $CurrentForm->hasValue("o_tarifa") &&
            $this->tarifa->CurrentValue != $this->tarifa->DefaultValue &&
            !($this->tarifa->IsForeignKey && $this->getCurrentMasterTable() != "" && $this->tarifa->CurrentValue == $this->tarifa->getSessionValue())
        ) {
            return false;
        }
        if (
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server") {
            $savedFilterList = Profile()->getSearchFilters("ftarifa_articulosrch");
        }
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("ftarifa_articulosrch", $filters);
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"tarifa_articulo\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $Language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($Security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"tarifa_articulo\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-btn=\"SaveBtn\">" . $Language->phrase("EditLink") . "</a>";
                } else {
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"ftarifa_articulolist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"ftarifa_articulolist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
            $item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"tarifa_articulo\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("AddLink") . "</a>";
        } else {
        ...
        $item->Body = "<button type=\"button\" class=\"ew-action ew-multi-delete\" title=\"" .
            ...
            HtmlTitle($Language->phrase("DeleteSelectedLink")) . "\" form=\"ftarifa_articulolist\"" .
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"ftarifa_articulosrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"ftarifa_articulosrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
        ...
        if (!$this->isGridAdd() && !$this->isGridEdit() && !$this->isMultiEdit()) { // Not grid add/grid edit/multi edit mode
            ...
            foreach ($this->ListActions as $listAction) {
                if ($listAction->Select == ACTION_MULTIPLE) {
                    ...
                    $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="ftarifa_articulolist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
            ...
            if ($this->isGridAdd()) {
                ...
                if (!($this->ModalGridAdd && !IsMobile())) {
                    ...
                    $item->Body = "<button class=\"ew-action ew-grid-insert\" title=\"" . HtmlTitle($Language->phrase("GridInsertLink")) . "\" data-caption=\"" . HtmlTitle($Language->phrase("GridInsertLink")) . "\" form=\"ftarifa_articulolist\" formaction=\"" . GetUrl($this->pageName()) . "\">" . $Language->phrase("GridInsertLink") . "</button>";
                    // Add grid cancel
            ...
            if ($this->isGridEdit()) {
                ...
                if (!($this->ModalGridEdit && !IsMobile())) {
                    ...
                    $option->UseDropDownButton = false;
                        ...
                        $item->Body = "<button class=\"ew-action ew-grid-save\" title=\"" . HtmlTitle($Language->phrase("GridSaveLink")) . "\" data-caption=\"" . HtmlTitle($Language->phrase("GridSaveLink")) . "\" form=\"ftarifa_articulolist\" formaction=\"" . GetUrl($this->pageName()) . "\">" . $Language->phrase("GridSaveLink") . "</button>";
                        $item = &$option->add("gridcancel");
                        $cancelurl = $this->addMasterUrl($pageUrl . "action=cancel");
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_tarifa_articulo", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_tarifa_articulo",
    ...
    protected function loadFormValues()
    {
        ...
        if ($CurrentForm->hasValue("o_tarifa")) {
            $this->tarifa->setOldValue($CurrentForm->getValue("o_tarifa"));
        }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            if ($this->tarifa->getSessionValue() != "") {
                $this->tarifa->CurrentValue = GetForeignKeyValue($this->tarifa->getSessionValue());
            ...
            // tarifa
            $this->tarifa->setupEditAttributes();
            $curVal = trim(strval($this->tarifa->AdvancedSearch->SearchValue));
            if ($curVal != "") {
                $this->tarifa->AdvancedSearch->ViewValue = $this->tarifa->lookupCacheOption($curVal);
            } else {
                $this->tarifa->AdvancedSearch->ViewValue = $this->tarifa->Lookup !== null && is_array($this->tarifa->lookupOptions()) && count($this->tarifa->lookupOptions()) > 0 ? $curVal : null;
            }
            if ($this->tarifa->AdvancedSearch->ViewValue !== null) { // Load from cache
                $this->tarifa->EditValue = array_values($this->tarifa->lookupOptions());
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = SearchFilter($this->tarifa->Lookup->getTable()->Fields["id"]->searchExpression(), "=", $this->tarifa->AdvancedSearch->SearchValue, $this->tarifa->Lookup->getTable()->Fields["id"]->searchDataType(), "");
                }
                $lookupFilter = $this->tarifa->getSelectFilter($this); // PHP
                $sqlWrk = $this->tarifa->Lookup->getSql(true, $filterWrk, $lookupFilter, $this, false, true);
                $conn = Conn();
            ...
            // articulo
            $curVal = trim(strval($this->articulo->AdvancedSearch->SearchValue));
            if ($curVal != "") {
                $this->articulo->AdvancedSearch->ViewValue = $this->articulo->lookupCacheOption($curVal);
            } else {
                $this->articulo->AdvancedSearch->ViewValue = $this->articulo->Lookup !== null && is_array($this->articulo->lookupOptions()) && count($this->articulo->lookupOptions()) > 0 ? $curVal : null;
            }
            if ($this->articulo->AdvancedSearch->ViewValue !== null) { // Load from cache
                $this->articulo->EditValue = array_values($this->articulo->lookupOptions());
                if ($this->articulo->AdvancedSearch->ViewValue == "") {
                    $this->articulo->AdvancedSearch->ViewValue = $Language->phrase("PleaseSelect");
                }
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = SearchFilter($this->articulo->Lookup->getTable()->Fields["id"]->searchExpression(), "=", $this->articulo->AdvancedSearch->SearchValue, $this->articulo->Lookup->getTable()->Fields["id"]->searchDataType(), "");
                }
                $sqlWrk = $this->articulo->Lookup->getSql(true, $filterWrk, '', $this, false, true);
                $conn = Conn();
                $config = $conn->getConfiguration();
                ...
                if ($ari > 0) { // Lookup values found
                    $arwrk = $this->articulo->Lookup->renderViewRow($rswrk[0]);
                    $this->articulo->AdvancedSearch->ViewValue = $this->articulo->displayValue($arwrk);
                } else {
                    $this->articulo->AdvancedSearch->ViewValue = $Language->phrase("PleaseSelect");
                }
                $arwrk = $rswrk;
    ...
    public function getRowHash($row)
    {
        ...
        $hash = "";
        $hash .= GetFieldHash($row['tarifa']); // tarifa
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"ftarifa_articulosrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    {
        return $this->tarifa->Visible || $this->fabricante->Visible || $this->articulo->Visible;
    }
    ...
    */
    public function exportData($doc)
    {
        ...
        if (Config("EXPORT_MASTER_RECORD") && $this->DbMasterFilter != "" && $this->getCurrentMasterTable() == "tarifa") {
            $tarifa = new TarifaList();
            $rsmaster = $tarifa->loadRs($this->DbMasterFilter); // Load master record
            if ($rsmaster) {
                $exportStyle = $doc->Style;
                $doc->setStyle("v"); // Change to vertical
                if (!$this->isExport("csv") || Config("EXPORT_MASTER_RECORD_FOR_CSV")) {
                    $doc->setTable($tarifa);
                    $tarifa->exportDocument($doc, $rsmaster);
                    $doc->exportEmptyRow();
                    $doc->setTable($this);
                }
    ...
    protected function setupMasterParms()
    {
        ...
        if (($master = Get(Config("TABLE_SHOW_MASTER"), Get(Config("TABLE_MASTER")))) !== null) {
            ...
            if ($masterTblVar == "tarifa") {
                $validMaster = true;
                $masterTbl = Container("tarifa");
                if (($parm = Get("fk_id", Get("tarifa"))) !== null) {
                    $masterTbl->id->setQueryStringValue($parm);
                    $this->tarifa->QueryStringValue = $masterTbl->id->QueryStringValue; // DO NOT change, master/detail key data type can be different
                    $this->tarifa->setSessionValue($this->tarifa->QueryStringValue);
                    $foreignKeys["tarifa"] = $this->tarifa->QueryStringValue;
            ...
            if ($masterTblVar == "tarifa") {
                $validMaster = true;
                $masterTbl = Container("tarifa");
                if (($parm = Post("fk_id", Post("tarifa"))) !== null) {
                    $masterTbl->id->setFormValue($parm);
                    $this->tarifa->FormValue = $masterTbl->id->FormValue;
                    $this->tarifa->setSessionValue($this->tarifa->FormValue);
                    $foreignKeys["tarifa"] = $this->tarifa->FormValue;
        ...
        if ($validMaster) {
            ...
            if ($masterTblVar != "tarifa") {
                if (!array_key_exists("tarifa", $foreignKeys)) { // Not current foreign key
                    $this->tarifa->setSessionValue("");
                }
            }
        }
