 ...
 * Page class
 */
class ClienteList extends Cliente
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ClienteList";
    ...
    public $FormName = "fclientelist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "ClienteList";
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "ClienteAdd?" . Config("TABLE_SHOW_DETAIL") . "=";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "ClienteDelete";
        $this->MultiUpdateUrl = "ClienteUpdate";
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server") {
            $savedFilterList = Profile()->getSearchFilters("fclientesrch");
        }
        ...
        $filterList = Concat($filterList, $this->sucursal->AdvancedSearch->toJson(), ","); // Field sucursal
        ...
        $filterList = Concat($filterList, $this->email1->AdvancedSearch->toJson(), ","); // Field email1
        $filterList = Concat($filterList, $this->email2->AdvancedSearch->toJson(), ","); // Field email2
        ...
        $filterList = Concat($filterList, $this->tipo_cliente->AdvancedSearch->toJson(), ","); // Field tipo_cliente
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fclientesrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field sucursal
        $this->sucursal->AdvancedSearch->SearchValue = @$filter["x_sucursal"];
        $this->sucursal->AdvancedSearch->SearchOperator = @$filter["z_sucursal"];
        $this->sucursal->AdvancedSearch->SearchCondition = @$filter["v_sucursal"];
        $this->sucursal->AdvancedSearch->SearchValue2 = @$filter["y_sucursal"];
        $this->sucursal->AdvancedSearch->SearchOperator2 = @$filter["w_sucursal"];
        $this->sucursal->AdvancedSearch->save();
        ...
        // Field tipo_cliente
        $this->tipo_cliente->AdvancedSearch->SearchValue = @$filter["x_tipo_cliente"];
        $this->tipo_cliente->AdvancedSearch->SearchOperator = @$filter["z_tipo_cliente"];
        $this->tipo_cliente->AdvancedSearch->SearchCondition = @$filter["v_tipo_cliente"];
        $this->tipo_cliente->AdvancedSearch->SearchValue2 = @$filter["y_tipo_cliente"];
        $this->tipo_cliente->AdvancedSearch->SearchOperator2 = @$filter["w_tipo_cliente"];
        $this->tipo_cliente->AdvancedSearch->save();

        // Field tarifa
        $this->tarifa->AdvancedSearch->SearchValue = @$filter["x_tarifa"];
        $this->tarifa->AdvancedSearch->SearchOperator = @$filter["z_tarifa"];
        $this->tarifa->AdvancedSearch->SearchCondition = @$filter["v_tarifa"];
        $this->tarifa->AdvancedSearch->SearchValue2 = @$filter["y_tarifa"];
        $this->tarifa->AdvancedSearch->SearchOperator2 = @$filter["w_tarifa"];
        $this->tarifa->AdvancedSearch->save();
        ...
        $this->limite_credito->AdvancedSearch->SearchValue = @$filter["x_limite_credito"];
        $this->limite_credito->AdvancedSearch->SearchOperator = @$filter["z_limite_credito"];
        $this->limite_credito->AdvancedSearch->SearchCondition = @$filter["v_limite_credito"];
        $this->limite_credito->AdvancedSearch->SearchValue2 = @$filter["y_limite_credito"];
        $this->limite_credito->AdvancedSearch->SearchOperator2 = @$filter["w_limite_credito"];
        $this->limite_credito->AdvancedSearch->save();
        ...
        $this->cuenta->AdvancedSearch->SearchCondition = @$filter["v_cuenta"];
        $this->cuenta->AdvancedSearch->SearchValue2 = @$filter["y_cuenta"];
    ...
    public function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->email1, $default, false); // email1
        $this->buildSearchSql($where, $this->email2, $default, false); // email2
        $this->buildSearchSql($where, $this->codigo_ims, $default, false); // codigo_ims
        $this->buildSearchSql($where, $this->web, $default, false); // web
        $this->buildSearchSql($where, $this->tipo_cliente, $default, false); // tipo_cliente
        $this->buildSearchSql($where, $this->tarifa, $default, false); // tarifa
        ...
        $this->buildSearchSql($where, $this->foto2, $default, false); // foto2
        ...
        if (!$default && $this->Command == "search") {
            ...
            $this->sucursal->AdvancedSearch->save(); // sucursal
            ...
            $this->tipo_cliente->AdvancedSearch->save(); // tipo_cliente
    ...
    public function showFilterList()
    {
        ...
        // Field sucursal
        $filter = $this->queryBuilderWhere("sucursal");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->sucursal, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->sucursal->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
        ...
        // Field zona
        $filter = $this->queryBuilderWhere("zona");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->zona, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->zona->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }

        // Field tipo_cliente
        $filter = $this->queryBuilderWhere("tipo_cliente");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->tipo_cliente, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->tipo_cliente->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }

        // Field tarifa
        $filter = $this->queryBuilderWhere("tarifa");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->tarifa, false, false);
        }
        if ($filter != "") {
        ...
        if (!$filter) {
            $this->buildSearchSql($filter, $this->cuenta, false, false);
        }
        if ($filter != "") {
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->sucursal->AdvancedSearch->issetSession()) {
            return true;
        }
        ...
        if ($this->tipo_cliente->AdvancedSearch->issetSession()) {
            return true;
        }
        ...
        if ($this->limite_credito->AdvancedSearch->issetSession()) {
            return true;
        }
        ...
        if ($this->foto1->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    protected function resetAdvancedSearchParms()
    {
        ...
        $this->sucursal->AdvancedSearch->unsetSession();
        ...
        $this->tipo_cliente->AdvancedSearch->unsetSession();
    ...
    protected function restoreSearchParms()
    {
        ...
        $this->sucursal->AdvancedSearch->load();
        ...
        $this->tipo_cliente->AdvancedSearch->load();
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->sucursal); // sucursal
    ...
    public function renderListOptions()
    {
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"cliente\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $Language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($Security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"cliente\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-btn=\"SaveBtn\">" . $Language->phrase("EditLink") . "</a>";
                } else {
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fclientelist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fclientelist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
        ...
        if ($Security->allowList(CurrentProjectID() . 'adjunto')) {
            ...
            $body = "<a class=\"btn btn-default ew-row-link ew-detail" . ($this->ListOptions->UseDropDownButton ? " dropdown-toggle" : "") . "\" data-action=\"list\" href=\"" . HtmlEncode("AdjuntoList?" . Config("TABLE_SHOW_MASTER") . "=cliente&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("AdjuntoGrid");
            if ($detailPage->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'cliente')) {
            ...
            if ($detailPage->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'cliente')) {
    ...
    protected function renderListOptionsExt()
    {
        ...
        if ($this->DetailPages?->getItem("adjunto")?->Visible && $Security->allowList(CurrentProjectID() . 'adjunto')) {
            ...
            $url = "AdjuntoPreview?t=cliente&f=" . Encrypt($detailFilter . "|" . implode("|", array_values($masterKeys)));
            ...
            if ($Security->allowList(CurrentProjectID() . 'cliente')) {
                ...
                $detaillnk = GetUrl("AdjuntoList?" . Config("TABLE_SHOW_MASTER") . "=cliente");
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
            $item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"cliente\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("AddLink") . "</a>";
        } else {
        ...
        $item->Visible = ($detailPage->DetailAdd && $Security->allowAdd(CurrentProjectID() . 'cliente') && $Security->canAdd());
        if ($item->Visible) {
            if ($detailTableLink != "") {
                $detailTableLink .= ",";
            }
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "sucursal");
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fclientesrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fclientesrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions as $listAction) {
            if ($listAction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fclientelist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_cliente", "data-rowtype" => RowType::ADD]);
    ...
    protected function loadSearchValues()
    {
        ...
        // sucursal
        if ($this->sucursal->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->sucursal->AdvancedSearch->SearchValue != "" || $this->sucursal->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        // zona
        if ($this->zona->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->zona->AdvancedSearch->SearchValue != "" || $this->zona->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        // email1
        if ($this->email1->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->email1->AdvancedSearch->SearchValue != "" || $this->email1->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // email2
        if ($this->email2->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->email2->AdvancedSearch->SearchValue != "" || $this->email2->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        // tipo_cliente
        if ($this->tipo_cliente->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->tipo_cliente->AdvancedSearch->SearchValue != "" || $this->tipo_cliente->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // tarifa
        if ($this->tarifa->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->tarifa->AdvancedSearch->SearchValue != "" || $this->tarifa->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        // limite_credito
        if ($this->limite_credito->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->limite_credito->AdvancedSearch->SearchValue != "" || $this->limite_credito->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // condicion
        if ($this->condicion->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->condicion->AdvancedSearch->SearchValue != "" || $this->condicion->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        if ($this->cuenta->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->cuenta->AdvancedSearch->SearchValue != "" || $this->cuenta->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        // foto1
        if ($this->foto1->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->foto1->AdvancedSearch->SearchValue != "" || $this->foto1->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // foto2
        if ($this->foto2->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->foto2->AdvancedSearch->SearchValue != "" || $this->foto2->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            if (!$this->sucursal->Raw) {
                $this->sucursal->AdvancedSearch->SearchValue = HtmlDecode($this->sucursal->AdvancedSearch->SearchValue);
            }
            $this->sucursal->EditValue = HtmlEncode($this->sucursal->AdvancedSearch->SearchValue);
            $this->sucursal->PlaceHolder = RemoveHtml($this->sucursal->caption());

            // contacto
            $this->contacto->setupEditAttributes();
            if (!$this->contacto->Raw) {
                $this->contacto->AdvancedSearch->SearchValue = HtmlDecode($this->contacto->AdvancedSearch->SearchValue);
            }
            $this->contacto->EditValue = HtmlEncode($this->contacto->AdvancedSearch->SearchValue);
            ...
            if (!$this->zona->Raw) {
                $this->zona->AdvancedSearch->SearchValue = HtmlDecode($this->zona->AdvancedSearch->SearchValue);
            }
            $this->zona->EditValue = HtmlEncode($this->zona->AdvancedSearch->SearchValue);
            $this->zona->PlaceHolder = RemoveHtml($this->zona->caption());

            // tipo_cliente
            $this->tipo_cliente->setupEditAttributes();
            $curVal = trim(strval($this->tipo_cliente->AdvancedSearch->SearchValue));
            if ($curVal != "") {
                $this->tipo_cliente->AdvancedSearch->ViewValue = $this->tipo_cliente->lookupCacheOption($curVal);
            } else {
                $this->tipo_cliente->AdvancedSearch->ViewValue = $this->tipo_cliente->Lookup !== null && is_array($this->tipo_cliente->lookupOptions()) && count($this->tipo_cliente->lookupOptions()) > 0 ? $curVal : null;
            }
            if ($this->tipo_cliente->AdvancedSearch->ViewValue !== null) { // Load from cache
                $this->tipo_cliente->EditValue = array_values($this->tipo_cliente->lookupOptions());
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = SearchFilter($this->tipo_cliente->Lookup->getTable()->Fields["campo_codigo"]->searchExpression(), "=", $this->tipo_cliente->AdvancedSearch->SearchValue, $this->tipo_cliente->Lookup->getTable()->Fields["campo_codigo"]->searchDataType(), "");
                }
                $lookupFilter = $this->tipo_cliente->getSelectFilter($this); // PHP
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmail", true) . '" data-caption="' . $Language->phrase("ExportToEmail", true) . '" form="fclientelist" data-ew-action="email" data-custom="false" data-hdr="' . $Language->phrase("ExportToEmail", true) . '" data-exported-selected="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fclientesrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    */
    ...
    public function pageDataRendering(&$header) {
    	...
    	$header .= '<button
    					...
    					OnClick="js:window.location.href = \'include/clientes_exportar_excel.php\'">
    					Exportar Clientes/Asesor a Excel
    				</button>';
    }
    ...
    public function listOptionsRendered() {
        ...
        if(isset($this->id->CurrentValue)) {
            if(trim($this->id->CurrentValue) != "") {
                ...
                $this->ListOptions->Items["new"]->Body = $asesor;
            }
        }
    }
