 ...
 * Page class
 */
class ContComprobanteView extends ContComprobante
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ContComprobanteView";
    ...
    // CSS class/style
    public $CurrentPageName = "ContComprobanteView";
    ...
    public function setVisibility()
    {
        ...
        $this->contabiliza->setVisibility();
        $this->fecha_contabiliza->setVisibility();
    }
    ...
    public function run()
    {
        ...
        if (($keyValue = Get("id") ?? Route("id")) !== null) {
            ...
            $returnUrl = "ContComprobanteList"; // Return to list
        }
            ...
            case "show": // Get a record to display
                if (!$this->IsModal && !IsApi()) { // Normal view page
                    ...
                    if ($this->TotalRecords <= 0) { // No record found
                        ...
                        $this->terminate("ContComprobanteList"); // Return to list page
                        return;
                    ...
                    if (!$matchRecord) {
                        ...
                        $returnUrl = "ContComprobanteList"; // No matching record, return to list
                    } else {
    ...
    protected function setupOtherOptions()
    {
        ...
        */
        ...
        // "detail_cont_asiento"
        $item = &$option->add("detail_cont_asiento");
        $body = $Language->phrase("ViewPageDetailLink") . $Language->tablePhrase("cont_asiento", "TblCaption");
        $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode(GetUrl("ContAsientoList?" . Config("TABLE_SHOW_MASTER") . "=cont_comprobante&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "")) . "\">" . $body . "</a>";
        $links = "";
        ...
        if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'cont_comprobante')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailViewLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=cont_asiento"))) . "\">" . $Language->phrase("MasterDetailViewLink", null) . "</a></li>";
            if ($detailViewTblVar != "") {
                $detailViewTblVar .= ",";
            }
        ...
        if ($detailPageObj->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'cont_comprobante')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-edit\" data-action=\"edit\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailEditLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=cont_asiento"))) . "\">" . $Language->phrase("MasterDetailEditLink", null) . "</a></li>";
            if ($detailEditTblVar != "") {
                $detailEditTblVar .= ",";
            }
    ...
    protected function setupDetailParms()
    {
        ...
        if ($detailTblVar != "") {
            ...
            if (in_array("cont_asiento", $detailTblVar)) {
                $detailPageObj = Container("ContAsientoGrid");
                if ($detailPageObj->DetailView) {
                    ...
                    $detailPageObj->comprobante->IsDetailKey = true;
                    $detailPageObj->comprobante->CurrentValue = $this->id->CurrentValue;
                    $detailPageObj->comprobante->setSessionValue($detailPageObj->comprobante->CurrentValue);
                }
            }
        }
    }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("ContComprobanteList"), "", $this->TableVar, true);
        $pageId = "view";
    ...
    public function pageDataRendering(&$header) {
    	// Example:
    	//$header = "your header";
    	if(CuadraComprobante($this->id->CurrentValue)) {
    		$sql = "SELECT contabilizacion 
    			FROM cont_comprobante 
    			WHERE id = '" . $this->id->CurrentValue . "';";
    		...
    		if($contabilizacion == "") {
    			...
    			$descont = '';
    			if(VerificaFuncion('015')) {
    				$descont = '<button class="btn btn-warning" id="btnDescont">Descontabilizar</button>';
    			}
    			$header = '<div class="alert alert-success" role="alert">
    						Este Comprobante est&aacute; Contabilizado. ' . $descont . '
    					</div>';
    		}
    	}
    	else {
