 ...
 * Page class
 */
class ContLotesPagosView extends ContLotesPagos
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ContLotesPagosView";
    ...
    // CSS class/style
    public $CurrentPageName = "ContLotesPagosView";
    ...
    public function run()
    {
        ...
        if (($keyValue = Get("id") ?? Route("id")) !== null) {
            ...
            $returnUrl = "ContLotesPagosList"; // Return to list
        }
            ...
            case "show": // Get a record to display
                if (!$this->IsModal && !IsApi()) { // Normal view page
                    ...
                    if ($this->TotalRecords <= 0) { // No record found
                        ...
                        $this->terminate("ContLotesPagosList"); // Return to list page
                        return;
                    ...
                    if (!$matchRecord) {
                        ...
                        $returnUrl = "ContLotesPagosList"; // No matching record, return to list
                    } else {
    ...
    protected function setupOtherOptions()
    {
        ...
        */
        ...
        $item = &$option->add("detail_cont_lotes_pagos_detalle");
        $body = $Language->phrase("ViewPageDetailLink") . $Language->tablePhrase("cont_lotes_pagos_detalle", "TblCaption");
        $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode(GetUrl("ContLotesPagosDetalleList?" . Config("TABLE_SHOW_MASTER") . "=cont_lotes_pagos&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "")) . "\">" . $body . "</a>";
        $links = "";
        ...
        if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'cont_lotes_pagos')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailViewLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=cont_lotes_pagos_detalle"))) . "\">" . $Language->phrase("MasterDetailViewLink", null) . "</a></li>";
            if ($detailViewTblVar != "") {
                $detailViewTblVar .= ",";
            }
    ...
    protected function setupDetailParms()
    {
        ...
        if ($detailTblVar != "") {
            ...
            if (in_array("cont_lotes_pagos_detalle", $detailTblVar)) {
                $detailPageObj = Container("ContLotesPagosDetalleGrid");
                if ($detailPageObj->DetailView) {
                    ...
                    $detailPageObj->cont_lotes_pago->IsDetailKey = true;
                    $detailPageObj->cont_lotes_pago->CurrentValue = $this->id->CurrentValue;
                    $detailPageObj->cont_lotes_pago->setSessionValue($detailPageObj->cont_lotes_pago->CurrentValue);
                }
            }
        }
    }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("ContLotesPagosList"), "", $this->TableVar, true);
        $pageId = "view";
    ...
    public function pageRedirecting(&$url)
    {
        ...
        $url = "ContLotesPagosDetalleList?showmaster=cont_lotes_pagos&fk_id=$id";
    }
