 ...
 * Page class
 */
class PresupustoDetalleList extends PresupustoDetalle
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PresupustoDetalleList";
    ...
    public $FormName = "fpresupusto_detallelist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "PresupustoDetalleList";
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "PresupustoDetalleAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "PresupustoDetalleDelete";
        $this->MultiUpdateUrl = "PresupustoDetalleUpdate";
    ...
    public function run()
    {
        ...
        if ($this->IsModal) {
            $this->FormName = "fpresupusto_detallegrid";
        }
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server") {
            $savedFilterList = Profile()->getSearchFilters("fpresupusto_detallesrch");
        }
        $filterList = Concat($filterList, $this->id->AdvancedSearch->toJson(), ","); // Field id
        $filterList = Concat($filterList, $this->presupuesto->AdvancedSearch->toJson(), ","); // Field presupuesto
        $filterList = Concat($filterList, $this->grupo1->AdvancedSearch->toJson(), ","); // Field grupo1
        $filterList = Concat($filterList, $this->grupo2->AdvancedSearch->toJson(), ","); // Field grupo2
        ...
        $filterList = Concat($filterList, $this->linea->AdvancedSearch->toJson(), ","); // Field linea
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fpresupusto_detallesrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field grupo1
        $this->grupo1->AdvancedSearch->SearchValue = @$filter["x_grupo1"];
        $this->grupo1->AdvancedSearch->SearchOperator = @$filter["z_grupo1"];
        $this->grupo1->AdvancedSearch->SearchCondition = @$filter["v_grupo1"];
        $this->grupo1->AdvancedSearch->SearchValue2 = @$filter["y_grupo1"];
        $this->grupo1->AdvancedSearch->SearchOperator2 = @$filter["w_grupo1"];
        $this->grupo1->AdvancedSearch->save();

        // Field grupo2
        $this->grupo2->AdvancedSearch->SearchValue = @$filter["x_grupo2"];
        $this->grupo2->AdvancedSearch->SearchOperator = @$filter["z_grupo2"];
        $this->grupo2->AdvancedSearch->SearchCondition = @$filter["v_grupo2"];
        $this->grupo2->AdvancedSearch->SearchValue2 = @$filter["y_grupo2"];
        $this->grupo2->AdvancedSearch->SearchOperator2 = @$filter["w_grupo2"];
        $this->grupo2->AdvancedSearch->save();
        ...
        // Field linea
        $this->linea->AdvancedSearch->SearchValue = @$filter["x_linea"];
        $this->linea->AdvancedSearch->SearchOperator = @$filter["z_linea"];
        $this->linea->AdvancedSearch->SearchCondition = @$filter["v_linea"];
        $this->linea->AdvancedSearch->SearchValue2 = @$filter["y_linea"];
        $this->linea->AdvancedSearch->SearchOperator2 = @$filter["w_linea"];
        $this->linea->AdvancedSearch->save();
    ...
    public function basicSearchWhere($default = false)
    {
        ...
        // Fields to search
        $searchFlds = [];
        $searchFlds[] = &$this->grupo1;
        $searchFlds[] = &$this->grupo2;
        $searchFlds[] = &$this->articulo;
        $searchFlds[] = &$this->linea;
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->grupo1); // grupo1
            $this->updateSort($this->grupo2); // grupo2
            $this->updateSort($this->numero); // numero
            $this->updateSort($this->articulo); // articulo
            $this->updateSort($this->linea); // linea
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"presupusto_detalle\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $Language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($Security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"presupusto_detalle\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-btn=\"SaveBtn\">" . $Language->phrase("EditLink") . "</a>";
                } else {
            ...
            if ($Security->canAdd()) {
                if ($this->ModalAdd && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-copy\" title=\"" . $copycaption . "\" data-table=\"presupusto_detalle\" data-caption=\"" . $copycaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->CopyUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("CopyLink") . "</a>";
                } else {
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fpresupusto_detallelist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fpresupusto_detallelist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
            $item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"presupusto_detalle\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("AddLink") . "</a>";
        } else {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "presupuesto");
            $this->createColumnOption($option, "grupo1");
            $this->createColumnOption($option, "grupo2");
            $this->createColumnOption($option, "numero");
            $this->createColumnOption($option, "articulo");
            $this->createColumnOption($option, "linea");
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fpresupusto_detallesrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fpresupusto_detallesrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions as $listAction) {
            if ($listAction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fpresupusto_detallelist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_presupusto_detalle", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_presupusto_detalle",
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmail", true) . '" data-caption="' . $Language->phrase("ExportToEmail", true) . '" form="fpresupusto_detallelist" data-ew-action="email" data-custom="false" data-hdr="' . $Language->phrase("ExportToEmail", true) . '" data-exported-selected="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fpresupusto_detallesrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
