 ...
 * Page class
 */
class TempConsignacionDelete extends TempConsignacion
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "TempConsignacionDelete";
    ...
    // CSS class/style
    public $CurrentPageName = "TempConsignacionDelete";
    ...
    public function run()
    {
        ...
        if ($filter == "") {
            $this->terminate("TempConsignacionList"); // Prevent SQL injection, return to list
            return;
        }
        ...
        if ($this->isShow()) { // Load records for display
            ...
            if ($this->TotalRecords <= 0) { // No record found, exit
                $this->Recordset?->free();
                $this->terminate("TempConsignacionList"); // Return to list
                return;
            }
        }
