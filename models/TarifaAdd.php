 ...
 * Page class
 */
class TarifaAdd extends Tarifa
{
    use MessagesTrait;
    ...
    // CSS class/style
    public $CurrentPageName = "TarifaAdd";
        ...
        switch ($this->CurrentAction) {
            ...
            case "insert": // Add new record
                ...
                if ($this->addRow($rsold)) { // Add successful
                    ...
                    } elseif (GetPageName($returnUrl) == "TarifaView") {
    ...
    {
        $this->patron->DefaultValue = $this->patron->getDefault(); // PHP
        $this->patron->OldValue = $this->patron->DefaultValue;
        $this->activo->DefaultValue = $this->activo->getDefault(); // PHP
        $this->activo->OldValue = $this->activo->DefaultValue;
        $this->porcentaje->DefaultValue = $this->porcentaje->getDefault(); // PHP
        $this->porcentaje->OldValue = $this->porcentaje->DefaultValue;
    }
    ...
    protected function validateForm()
    {
        ...
        if (in_array("tarifa_descuento_utilidad", $detailTblVar) && $detailPage->DetailAdd) {
            $detailPage->run();
            $validateForm = $validateForm && $detailPage->validateGridForm();
        }
        $detailPage = Container("TarifaArticuloGrid");
        if (in_array("tarifa_articulo", $detailTblVar) && $detailPage->DetailAdd) {
            $detailPage->run();
            $validateForm = $validateForm && $detailPage->validateGridForm();
        }
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($addRow) {
            $detailTblVar = explode(",", $this->getCurrentDetailTable());
            $detailPage = Container("TarifaDescuentoUtilidadGrid");
            if (in_array("tarifa_descuento_utilidad", $detailTblVar) && $detailPage->DetailAdd && $addRow) {
                $detailPage->tarifa->setSessionValue($this->id->CurrentValue); // Set master key
                ...
                if (!$addRow) {
                $detailPage->tarifa->setSessionValue(""); // Clear master key if insert failed
                }
            }
            $detailPage = Container("TarifaArticuloGrid");
            if (in_array("tarifa_articulo", $detailTblVar) && $detailPage->DetailAdd && $addRow) {
                $detailPage->tarifa->setSessionValue($this->id->CurrentValue); // Set master key
    ...
    protected function getAddRow()
    {
        ...
        // patron
        $this->patron->setDbValueDef($rsnew, $this->patron->CurrentValue, strval($this->patron->CurrentValue) == "");

        // activo
        $this->activo->setDbValueDef($rsnew, $this->activo->CurrentValue, strval($this->activo->CurrentValue) == "");

        // porcentaje
        $this->porcentaje->setDbValueDef($rsnew, $this->porcentaje->CurrentValue, strval($this->porcentaje->CurrentValue) == "");
        return $rsnew;
    }
    ...
    protected function restoreAddFormFromRow($row)
    {
        ...
        if (isset($row['patron'])) { // patron
            $this->patron->setFormValue($row['patron']);
        }
