...
class UsuarioAdd extends Usuario
{
    ...
    protected function validateForm()
    {
        ...
        if (in_array("usuario_master2", $detailTblVar) && $detailPage->DetailAdd) {
            $detailPage->run();
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($addRow) {
            $detailTblVar = explode(",", $this->getCurrentDetailTable());
            $detailPage = Container("UsuarioMaster2Grid");
            if (in_array("usuario_master2", $detailTblVar) && $detailPage->DetailAdd && $addRow) {
                $detailPage->_username->setSessionValue($this->_username->CurrentValue); // Set master key
                $Security->loadCurrentUserLevel($this->ProjectID . "usuario_master"); // Load user level of detail table
                $addRow = $detailPage->gridInsert();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
                if (!$addRow) {
                $detailPage->_username->setSessionValue(""); // Clear master key if insert failed
                }
            }
        }
    ...
    protected function getAddRow()
    {
        ...
        // asesor
        $this->asesor->setDbValueDef($rsnew, $this->asesor->CurrentValue, false);
