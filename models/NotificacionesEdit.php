...
class NotificacionesEdit extends Notificaciones
{
    ...
    // CSS class/style
    public $CurrentPageName = "NotificacionesEdit";
                ...
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if ($this->TotalRecords <= 0) { // No record found
                        ...
                        if ($this->Nnotificaciones->CurrentValue != null) {
                            while ($this->fetch()) {
                                if (SameString($this->Nnotificaciones->CurrentValue, $this->CurrentRow['Nnotificaciones'])) {
    ...
    protected function getEditRow($rsold)
    {
        ...
        // asunto
        $this->asunto->setDbValueDef($rsnew, $this->asunto->CurrentValue, $this->asunto->ReadOnly);

        // notificacion
        $this->notificacion->setDbValueDef($rsnew, $this->notificacion->CurrentValue, $this->notificacion->ReadOnly);

        // notificados
        $this->notificados->setDbValueDef($rsnew, $this->notificados->CurrentValue, $this->notificados->ReadOnly);

        // notificados_efectivos
        $this->notificados_efectivos->setDbValueDef($rsnew, $this->notificados_efectivos->CurrentValue, $this->notificados_efectivos->ReadOnly);
        return $rsnew;
    }
    ...
    protected function restoreEditFormFromRow($row)
    {
        if (isset($row['asunto'])) { // asunto
            $this->asunto->CurrentValue = $row['asunto'];
        }
        if (isset($row['notificacion'])) { // notificacion
            $this->notificacion->CurrentValue = $row['notificacion'];
        }
        if (isset($row['notificados'])) { // notificados
            $this->notificados->CurrentValue = $row['notificados'];
        }
        if (isset($row['notificados_efectivos'])) { // notificados_efectivos
            $this->notificados_efectivos->CurrentValue = $row['notificados_efectivos'];
        }
    }
