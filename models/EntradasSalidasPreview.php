 ...
 * Page class
 */
class EntradasSalidasPreview extends EntradasSalidas
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "EntradasSalidasPreview";
    ...
    // CSS class/style
    public $CurrentPageName = "EntradasSalidasPreview";
    ...
    protected function masterKeyUrl()
    {
        ...
        if ($masterTblVar == "entradas") {
            $url = "" . Config("TABLE_SHOW_MASTER") . "=entradas&" . GetForeignKeyUrl("fk_tipo_documento", $this->tipo_documento->QueryStringValue) . "&" . GetForeignKeyUrl("fk_id", $this->id_documento->QueryStringValue) . "";
        }
        if ($masterTblVar == "salidas") {
            $url = "" . Config("TABLE_SHOW_MASTER") . "=salidas&" . GetForeignKeyUrl("fk_tipo_documento", $this->tipo_documento->QueryStringValue) . "&" . GetForeignKeyUrl("fk_id", $this->id_documento->QueryStringValue) . "";
        }
        return $url;
    }
    ...
    public function listOptionsRendered() {
    	// Example:
        $sql = "SELECT 
                    IFNULL(b.foto, '') AS foto  
                FROM 
                    entradas_salidas AS a 
                    JOIN articulo AS b ON b.id = a.articulo 
                WHERE 
                    a.id = '" . $this->id->ViewValue . "'";
        if($row = ExecuteRow($sql)) {
            $foto = $row["foto"];
            if(trim($foto) != "") {
                if(strpos($foto, ".pdf") > 0)
                    $this->ListOptions->Items["foto"]->Body = '<a href="carpetacarga/' . $foto . '" target="_blank">' . $foto . '</a>';
                else
                    $this->ListOptions->Items["foto"]->Body = '<a href="carpetacarga/' . $foto . '" target="_blank"><img src="carpetacarga/' . $foto . '" class="img-thumbnail" alt="' . $this->articulo->ViewValue . '" width="150"></a>';
