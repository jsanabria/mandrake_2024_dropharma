 ...
 * Page class
 */
class AlmacenistaEdit extends Almacenista
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "AlmacenistaEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "AlmacenistaEdit";
    ...
    public function run()
    {
            ...
            if ($this->isShow()) {
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if ($this->TotalRecords <= 0) { // No record found
                        ...
                        $this->terminate("AlmacenistaList"); // Return to list page
                        return;
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("AlmacenistaList"); // No matching record, return to list
                        return;
                    }
                } // End modal checking
                break;
            case "update": // Update
                ...
                if (GetPageName($returnUrl) == "AlmacenistaList") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "AlmacenistaList") {
                            ...
                            $returnUrl = "AlmacenistaList"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            ...
            if (!$this->clave->Raw && Config("REMOVE_XSS") && CheckPassword($this->clave->FormValue)) {
                $this->clave->addErrorMessage($Language->phrase("InvalidPasswordChars"));
            }
    ...
    protected function editRow()
    {
        ...
        // Check field with unique index (nombre)
        if ($this->nombre->CurrentValue != "") {
            $filterChk = "(`nombre` = '" . AdjustSql($this->nombre->CurrentValue, $this->Dbid) . "')";
            ...
            if ($rsChk->fetch()) {
                $idxErrMsg = str_replace("%f", $this->nombre->caption(), $Language->phrase("DupIndex"));
                $idxErrMsg = str_replace("%v", $this->nombre->CurrentValue, $idxErrMsg);
    ...
    protected function getEditRow($rsold)
    {
        ...
        // clave
        if (!IsMaskedPassword($this->clave->CurrentValue)) {
            $this->clave->setDbValueDef($rsnew, $this->clave->CurrentValue, $this->clave->ReadOnly);
        }
    ...
    protected function restoreEditFormFromRow($row)
    {
        if (isset($row['clave'])) { // clave
            $this->clave->CurrentValue = $row['clave'];
        }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("AlmacenistaList"), "", $this->TableVar, true);
        $pageId = "edit";
