...
class UserlevelsAdd extends Userlevels
{
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'tipo_acceso' first before field var 'x_tipo_acceso'
        $val = $CurrentForm->hasValue("tipo_acceso") ? $CurrentForm->getValue("tipo_acceso") : $CurrentForm->getValue("x_tipo_acceso");
        if (!$this->tipo_acceso->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->tipo_acceso->Visible = false; // Disable update for API request
            } else {
                $this->tipo_acceso->setFormValue($val);
            }
        }
    }
    ...
    public function restoreFormValues()
    {
        ...
        $this->tipo_acceso->CurrentValue = $this->tipo_acceso->FormValue;
    }
    ...
    public function renderRow()
    {
        ...
        // tipo_acceso
        $this->tipo_acceso->RowCssClass = "row";
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            ...
            if ($this->tipo_acceso->Visible && $this->tipo_acceso->Required) {
                if (!$this->tipo_acceso->IsDetailKey && EmptyValue($this->tipo_acceso->FormValue)) {
                    $this->tipo_acceso->addErrorMessage(str_replace("%s", $this->tipo_acceso->caption(), $this->tipo_acceso->RequiredErrorMessage));
                }
            }
        ...
        if (in_array("grupo_funciones", $detailTblVar) && $detailPage->DetailAdd) {
            $detailPage->run();
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($addRow) {
            $detailTblVar = explode(",", $this->getCurrentDetailTable());
            $detailPage = Container("GrupoFuncionesGrid");
            if (in_array("grupo_funciones", $detailTblVar) && $detailPage->DetailAdd && $addRow) {
                $detailPage->grupo->setSessionValue($this->userlevelid->CurrentValue); // Set master key
                $Security->loadCurrentUserLevel($this->ProjectID . "grupo_funciones"); // Load user level of detail table
                $addRow = $detailPage->gridInsert();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
                if (!$addRow) {
                $detailPage->grupo->setSessionValue(""); // Clear master key if insert failed
                }
            }
        }
    ...
    protected function getAddRow()
    {
        ...
        // tipo_acceso
        $this->tipo_acceso->setDbValueDef($rsnew, $this->tipo_acceso->CurrentValue, false);
        return $rsnew;
    }
    ...
    protected function restoreAddFormFromRow($row)
    {
        ...
        if (isset($row['tipo_acceso'])) { // tipo_acceso
            $this->tipo_acceso->setFormValue($row['tipo_acceso']);
        }
    }
    ...
    protected function setupDetailParms()
    {
        ...
        if ($detailTblVar != "") {
            ...
            if (in_array("grupo_funciones", $detailTblVar)) {
