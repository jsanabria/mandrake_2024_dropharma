 ...
 * Page class
 */
class EntradasSalidasList extends EntradasSalidas
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "EntradasSalidasList";
    ...
    public $FormName = "fentradas_salidaslist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "EntradasSalidasList";
    ...
    public function __construct()
    {
        ...
        $this->MultiDeleteUrl = "EntradasSalidasDelete";
        $this->MultiUpdateUrl = "EntradasSalidasUpdate";
        ...
        if (IsPost() && ($this->isGridUpdate() || $this->isMultiUpdate() || $this->isGridOverwrite()) && (Session(SESSION_INLINE_MODE) == "gridedit" || Session(SESSION_INLINE_MODE) == "multiedit")) {
            ...
            if ($gridUpdate) {
                ...
                if ($this->IsModal && !$this->UseAjaxActions) {
                    $this->terminate("EntradasSalidasList");
                    return;
                }
            } else {
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server") {
            $savedFilterList = Profile()->getSearchFilters("fentradas_salidassrch");
        }
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fentradas_salidassrch", $filters);
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fentradas_salidaslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fentradas_salidaslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<button type=\"button\" class=\"ew-action ew-multi-delete\" title=\"" .
            ...
            HtmlTitle($Language->phrase("DeleteSelectedLink")) . "\" form=\"fentradas_salidaslist\"" .
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fentradas_salidassrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fentradas_salidassrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
        ...
        if (!$this->isGridAdd() && !$this->isGridEdit() && !$this->isMultiEdit()) { // Not grid add/grid edit/multi edit mode
            ...
            foreach ($this->ListActions as $listAction) {
                if ($listAction->Select == ACTION_MULTIPLE) {
                    ...
                    $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fentradas_salidaslist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
            ...
            if ($this->isGridAdd()) {
                ...
                if (!($this->ModalGridAdd && !IsMobile())) {
                    ...
                    $item->Body = "<button class=\"ew-action ew-grid-insert\" title=\"" . HtmlTitle($Language->phrase("GridInsertLink")) . "\" data-caption=\"" . HtmlTitle($Language->phrase("GridInsertLink")) . "\" form=\"fentradas_salidaslist\" formaction=\"" . GetUrl($this->pageName()) . "\">" . $Language->phrase("GridInsertLink") . "</button>";
                    // Add grid cancel
            ...
            if ($this->isGridEdit()) {
                ...
                if (!($this->ModalGridEdit && !IsMobile())) {
                    ...
                    $option->UseDropDownButton = false;
                        ...
                        $item->Body = "<button class=\"ew-action ew-grid-save\" title=\"" . HtmlTitle($Language->phrase("GridSaveLink")) . "\" data-caption=\"" . HtmlTitle($Language->phrase("GridSaveLink")) . "\" form=\"fentradas_salidaslist\" formaction=\"" . GetUrl($this->pageName()) . "\">" . $Language->phrase("GridSaveLink") . "</button>";
                        $item = &$option->add("gridcancel");
                        $cancelurl = $this->addMasterUrl($pageUrl . "action=cancel");
    ...
    public function getRowHash($row)
    {
        ...
        $hash .= GetFieldHash($row['lote']); // lote
        $hash .= GetFieldHash($row['fecha_vencimiento']); // fecha_vencimiento
        $hash .= GetFieldHash($row['almacen']); // almacen
        $hash .= GetFieldHash($row['id_compra']); // id_compra
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmail", true) . '" data-caption="' . $Language->phrase("ExportToEmail", true) . '" form="fentradas_salidaslist" data-ew-action="email" data-custom="false" data-hdr="' . $Language->phrase("ExportToEmail", true) . '" data-exported-selected="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fentradas_salidassrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    */
    public function exportData($doc)
    {
        ...
        if (Config("EXPORT_MASTER_RECORD") && $this->DbMasterFilter != "" && $this->getCurrentMasterTable() == "entradas") {
            $entradas = new EntradasList();
            $rsmaster = $entradas->loadRs($this->DbMasterFilter); // Load master record
            if ($rsmaster) {
                $exportStyle = $doc->Style;
                $doc->setStyle("v"); // Change to vertical
                if (!$this->isExport("csv") || Config("EXPORT_MASTER_RECORD_FOR_CSV")) {
                    $doc->setTable($entradas);
                    $entradas->exportDocument($doc, $rsmaster);
                    $doc->exportEmptyRow();
                    $doc->setTable($this);
                }
        ...
        // Export master record
        if (Config("EXPORT_MASTER_RECORD") && $this->DbMasterFilter != "" && $this->getCurrentMasterTable() == "salidas") {
            $salidas = new SalidasList();
            $rsmaster = $salidas->loadRs($this->DbMasterFilter); // Load master record
            if ($rsmaster) {
                $exportStyle = $doc->Style;
                $doc->setStyle("v"); // Change to vertical
                if (!$this->isExport("csv") || Config("EXPORT_MASTER_RECORD_FOR_CSV")) {
                    $doc->setTable($salidas);
                    $salidas->exportDocument($doc, $rsmaster);
                    $doc->exportEmptyRow();
                    $doc->setTable($this);
                }
                $doc->setStyle($exportStyle); // Restore
    ...
    public function listOptionsLoad() {
    	// Example:
    	if($this->tipo_documento->CurrentValue == "TDCASA") {
    		$opt = &$this->ListOptions->Add("new");
    		$opt->Header = "Datos Lote";
    ...
    public function listOptionsRendered() {
    	// Example:
    	if($this->tipo_documento->CurrentValue == "TDCASA") {
    		$sql = "SELECT CONCAT('Lote: ', IFNULL(lote, ''), ' ' , 'Venc: ', IFNULL(date_format(fecha_vencimiento, '%d/%m/%Y'), '')) AS lote FROM entradas_salidas WHERE id = '" . $this->id_compra->CurrentValue . "'";
    		$this->ListOptions->Items["new"]->Body = ExecuteScalar($sql);
    	}
