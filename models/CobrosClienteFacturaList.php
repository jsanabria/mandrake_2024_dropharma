 ...
 * Page class
 */
class CobrosClienteFacturaList extends CobrosClienteFactura
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "CobrosClienteFacturaList";
    ...
    public $FormName = "fcobros_cliente_facturalist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "CobrosClienteFacturaList";
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "CobrosClienteFacturaAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "CobrosClienteFacturaDelete";
        $this->MultiUpdateUrl = "CobrosClienteFacturaUpdate";
    ...
    public function run()
    {
        ...
        if ($this->IsModal) {
            $this->FormName = "fcobros_cliente_facturagrid";
        }
        ...
        // Load master record
        if ($this->CurrentMode != "add" && $this->DbMasterFilter != "" && $this->getCurrentMasterTable() == "cobros_cliente") {
            $masterTbl = Container("cobros_cliente");
            $rsmaster = $masterTbl->loadRs($this->DbMasterFilter)->fetchAssociative();
            $this->MasterRecordExists = $rsmaster !== false;
            if (!$this->MasterRecordExists) {
                $this->setFailureMessage($Language->phrase("NoRecord")); // Set no record found
                $this->terminate("CobrosClienteList"); // Return to master page
                return;
            } else {
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server") {
            $savedFilterList = Profile()->getSearchFilters("fcobros_cliente_facturasrch");
        }
        ...
        $filterList = Concat($filterList, $this->retivamonto->AdvancedSearch->toJson(), ","); // Field retivamonto
        $filterList = Concat($filterList, $this->retiva->AdvancedSearch->toJson(), ","); // Field retiva
        $filterList = Concat($filterList, $this->retislrmonto->AdvancedSearch->toJson(), ","); // Field retislrmonto
        $filterList = Concat($filterList, $this->retislr->AdvancedSearch->toJson(), ","); // Field retislr
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fcobros_cliente_facturasrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field retivamonto
        $this->retivamonto->AdvancedSearch->SearchValue = @$filter["x_retivamonto"];
        $this->retivamonto->AdvancedSearch->SearchOperator = @$filter["z_retivamonto"];
        $this->retivamonto->AdvancedSearch->SearchCondition = @$filter["v_retivamonto"];
        $this->retivamonto->AdvancedSearch->SearchValue2 = @$filter["y_retivamonto"];
        $this->retivamonto->AdvancedSearch->SearchOperator2 = @$filter["w_retivamonto"];
        $this->retivamonto->AdvancedSearch->save();

        // Field retiva
        $this->retiva->AdvancedSearch->SearchValue = @$filter["x_retiva"];
        $this->retiva->AdvancedSearch->SearchOperator = @$filter["z_retiva"];
        $this->retiva->AdvancedSearch->SearchCondition = @$filter["v_retiva"];
        $this->retiva->AdvancedSearch->SearchValue2 = @$filter["y_retiva"];
        $this->retiva->AdvancedSearch->SearchOperator2 = @$filter["w_retiva"];
        $this->retiva->AdvancedSearch->save();

        // Field retislrmonto
        $this->retislrmonto->AdvancedSearch->SearchValue = @$filter["x_retislrmonto"];
        $this->retislrmonto->AdvancedSearch->SearchOperator = @$filter["z_retislrmonto"];
        $this->retislrmonto->AdvancedSearch->SearchCondition = @$filter["v_retislrmonto"];
        $this->retislrmonto->AdvancedSearch->SearchValue2 = @$filter["y_retislrmonto"];
        $this->retislrmonto->AdvancedSearch->SearchOperator2 = @$filter["w_retislrmonto"];
        $this->retislrmonto->AdvancedSearch->save();

        // Field retislr
        $this->retislr->AdvancedSearch->SearchValue = @$filter["x_retislr"];
        $this->retislr->AdvancedSearch->SearchOperator = @$filter["z_retislr"];
        $this->retislr->AdvancedSearch->SearchCondition = @$filter["v_retislr"];
        $this->retislr->AdvancedSearch->SearchValue2 = @$filter["y_retislr"];
        $this->retislr->AdvancedSearch->SearchOperator2 = @$filter["w_retislr"];
        $this->retislr->AdvancedSearch->save();
    ...
    public function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->cobros_cliente, $default, false); // cobros_cliente
        ...
        $this->buildSearchSql($where, $this->retivamonto, $default, false); // retivamonto
        $this->buildSearchSql($where, $this->retiva, $default, false); // retiva
        $this->buildSearchSql($where, $this->retislrmonto, $default, false); // retislrmonto
        $this->buildSearchSql($where, $this->retislr, $default, false); // retislr
        ...
        if (!$default && $this->Command == "search") {
            ...
            $this->cobros_cliente->AdvancedSearch->save(); // cobros_cliente
            ...
            $this->retivamonto->AdvancedSearch->save(); // retivamonto
            $this->retiva->AdvancedSearch->save(); // retiva
            $this->retislrmonto->AdvancedSearch->save(); // retislrmonto
            $this->retislr->AdvancedSearch->save(); // retislr
    ...
    public function showFilterList()
    {
        ...
        // Field abono
        $filter = $this->queryBuilderWhere("abono");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->abono, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->abono->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
        ...
        // Field retivamonto
        $filter = $this->queryBuilderWhere("retivamonto");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->retivamonto, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->retivamonto->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }

        // Field retiva
        $filter = $this->queryBuilderWhere("retiva");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->retiva, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->retiva->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }

        // Field retislrmonto
        $filter = $this->queryBuilderWhere("retislrmonto");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->retislrmonto, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->retislrmonto->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }

        // Field retislr
        $filter = $this->queryBuilderWhere("retislr");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->retislr, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->retislr->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
    ...
    public function basicSearchWhere($default = false)
    {
        ...
        $searchFlds[] = &$this->retiva;
        $searchFlds[] = &$this->retislr;
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->cobros_cliente->AdvancedSearch->issetSession()) {
            return true;
        }
        ...
        if ($this->retivamonto->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->retiva->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->retislrmonto->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->retislr->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    protected function resetAdvancedSearchParms()
    {
        ...
        $this->cobros_cliente->AdvancedSearch->unsetSession();
        ...
        $this->retivamonto->AdvancedSearch->unsetSession();
        $this->retiva->AdvancedSearch->unsetSession();
        $this->retislrmonto->AdvancedSearch->unsetSession();
        $this->retislr->AdvancedSearch->unsetSession();
    ...
    protected function restoreSearchParms()
    {
        ...
        $this->cobros_cliente->AdvancedSearch->load();
        ...
        $this->retivamonto->AdvancedSearch->load();
        $this->retiva->AdvancedSearch->load();
        $this->retislrmonto->AdvancedSearch->load();
        $this->retislr->AdvancedSearch->load();
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->retivamonto); // retivamonto
            $this->updateSort($this->retiva); // retiva
            $this->updateSort($this->retislrmonto); // retislrmonto
            $this->updateSort($this->retislr); // retislr
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"cobros_cliente_factura\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $Language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($Security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"cobros_cliente_factura\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-btn=\"SaveBtn\">" . $Language->phrase("EditLink") . "</a>";
                } else {
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fcobros_cliente_facturalist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fcobros_cliente_facturalist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "abono");
            $this->createColumnOption($option, "monto");
            $this->createColumnOption($option, "retivamonto");
            $this->createColumnOption($option, "retiva");
            $this->createColumnOption($option, "retislrmonto");
            $this->createColumnOption($option, "retislr");
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fcobros_cliente_facturasrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fcobros_cliente_facturasrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions as $listAction) {
            if ($listAction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fcobros_cliente_facturalist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_cobros_cliente_factura", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_cobros_cliente_factura",
    ...
    protected function loadSearchValues()
    {
        ...
        // cobros_cliente
        if ($this->cobros_cliente->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->cobros_cliente->AdvancedSearch->SearchValue != "" || $this->cobros_cliente->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        // abono
        if ($this->abono->AdvancedSearch->get()) {
            $hasValue = true;
        ...
        // retivamonto
        if ($this->retivamonto->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->retivamonto->AdvancedSearch->SearchValue != "" || $this->retivamonto->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // retiva
        if ($this->retiva->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->retiva->AdvancedSearch->SearchValue != "" || $this->retiva->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // retislrmonto
        if ($this->retislrmonto->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->retislrmonto->AdvancedSearch->SearchValue != "" || $this->retislrmonto->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // retislr
        if ($this->retislr->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->retislr->AdvancedSearch->SearchValue != "" || $this->retislr->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            // abono
            $this->abono->EditValue = $this->abono->options(false);
            ...
            $this->retivamonto->EditValue = $this->retivamonto->AdvancedSearch->SearchValue;
            $this->retivamonto->PlaceHolder = RemoveHtml($this->retivamonto->caption());

            // retiva
            $this->retiva->setupEditAttributes();
            if (!$this->retiva->Raw) {
                $this->retiva->AdvancedSearch->SearchValue = HtmlDecode($this->retiva->AdvancedSearch->SearchValue);
            }
            $this->retiva->EditValue = HtmlEncode($this->retiva->AdvancedSearch->SearchValue);
            $this->retiva->PlaceHolder = RemoveHtml($this->retiva->caption());

            // retislrmonto
            $this->retislrmonto->setupEditAttributes();
            $this->retislrmonto->EditValue = $this->retislrmonto->AdvancedSearch->SearchValue;
            $this->retislrmonto->PlaceHolder = RemoveHtml($this->retislrmonto->caption());

            // retislr
            $this->retislr->setupEditAttributes();
            if (!$this->retislr->Raw) {
                $this->retislr->AdvancedSearch->SearchValue = HtmlDecode($this->retislr->AdvancedSearch->SearchValue);
            }
            $this->retislr->EditValue = HtmlEncode($this->retislr->AdvancedSearch->SearchValue);
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            if ($custom) {
                return "<button type=\"button\" class=\"btn btn-default ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcel", true)) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcel", true)) . "\" form=\"fcobros_cliente_facturalist\" data-url=\"$exportUrl\" data-ew-action=\"export\" data-export=\"excel\" data-custom=\"true\" data-export-selected=\"false\">" . $Language->phrase("ExportToExcel") . "</button>";
            } else {
                return "<a href=\"$exportUrl\" class=\"btn btn-default ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcel", true)) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcel", true)) . "\">" . $Language->phrase("ExportToExcel") . "</a>";
            }
        } elseif (SameText($type, "word")) {
            if ($custom) {
                return "<button type=\"button\" class=\"btn btn-default ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWord", true)) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWord", true)) . "\" form=\"fcobros_cliente_facturalist\" data-url=\"$exportUrl\" data-ew-action=\"export\" data-export=\"word\" data-custom=\"true\" data-export-selected=\"false\">" . $Language->phrase("ExportToWord") . "</button>";
            } else {
                return "<a href=\"$exportUrl\" class=\"btn btn-default ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWord", true)) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWord", true)) . "\">" . $Language->phrase("ExportToWord") . "</a>";
            }
        } elseif (SameText($type, "pdf")) {
            if ($custom) {
                return "<button type=\"button\" class=\"btn btn-default ew-export-link ew-pdf\" title=\"" . HtmlEncode($Language->phrase("ExportToPdf", true)) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToPdf", true)) . "\" form=\"fcobros_cliente_facturalist\" data-url=\"$exportUrl\" data-ew-action=\"export\" data-export=\"pdf\" data-custom=\"true\" data-export-selected=\"false\">" . $Language->phrase("ExportToPdf") . "</button>";
            } else {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmail", true) . '" data-caption="' . $Language->phrase("ExportToEmail", true) . '" form="fcobros_cliente_facturalist" data-ew-action="email" data-custom="false" data-hdr="' . $Language->phrase("ExportToEmail", true) . '" data-exported-selected="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fcobros_cliente_facturasrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    */
    public function exportData($doc)
    {
        ...
        if (Config("EXPORT_MASTER_RECORD") && $this->DbMasterFilter != "" && $this->getCurrentMasterTable() == "cobros_cliente") {
            $cobros_cliente = new CobrosClienteList();
            $rsmaster = $cobros_cliente->loadRs($this->DbMasterFilter); // Load master record
            if ($rsmaster) {
                $exportStyle = $doc->Style;
                $doc->setStyle("v"); // Change to vertical
                if (!$this->isExport("csv") || Config("EXPORT_MASTER_RECORD_FOR_CSV")) {
                    $doc->setTable($cobros_cliente);
                    $cobros_cliente->exportDocument($doc, $rsmaster);
                    $doc->exportEmptyRow();
                    $doc->setTable($this);
                }
