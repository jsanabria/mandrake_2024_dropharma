...
/**
 * Table class for view_factura_asesor
 */
class ViewFacturaAsesor extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        // Update Table
        $this->UpdateTable = "view_factura_asesor";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        $this->asesor->addMethod("getSelectFilter", fn() => "IFNULL(asesor, 0) > 0");
        ...
        $this->asesor->Lookup = new Lookup($this->asesor, 'usuario', false, 'username', ["nombre","","",""], '', '', [], [], [], [], [], [], false, '`nombre`', '', "`nombre`");
        ...
        // asesor_nombre
        $this->asesor_nombre = new DbField(
            $this, // Table
            'x_asesor_nombre', // Variable name
            'asesor_nombre', // Name
            '`asesor_nombre`', // Expression
            '`asesor_nombre`', // Basic search expression
            200, // Type
            40, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`asesor_nombre`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->asesor_nombre->InputTextType = "text";
        $this->asesor_nombre->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY", "IS NULL", "IS NOT NULL"];
        $this->Fields['asesor_nombre'] = &$this->asesor_nombre;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "view_factura_asesor";
    }
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->asesor_nombre->DbValue = $row['asesor_nombre'];
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("ViewFacturaAsesorList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "ViewFacturaAsesorView" => $Language->phrase("View"),
            "ViewFacturaAsesorEdit" => $Language->phrase("Edit"),
            "ViewFacturaAsesorAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "ViewFacturaAsesorView",
            Config("API_ADD_ACTION") => "ViewFacturaAsesorAdd",
            Config("API_EDIT_ACTION") => "ViewFacturaAsesorEdit",
            Config("API_DELETE_ACTION") => "ViewFacturaAsesorDelete",
            Config("API_LIST_ACTION") => "ViewFacturaAsesorList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("ViewFacturaAsesorView", $parm);
        } else {
            $url = $this->keyUrl("ViewFacturaAsesorView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = "ViewFacturaAsesorAdd?" . $parm;
        } else {
            $url = "ViewFacturaAsesorAdd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ViewFacturaAsesorEdit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ViewFacturaAsesorList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ViewFacturaAsesorAdd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ViewFacturaAsesorList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("ViewFacturaAsesorDelete", $parm);
        }
    }
    ...
    {
        global $Response;
        $listPage = "ViewFacturaAsesorList";
    ...
    public function renderListRow()
    {
        ...
        // asesor_nombre
        $this->asesor_nombre->HrefValue = "";
        $this->asesor_nombre->TooltipValue = "";
    ...
    public function renderEditRow()
    {
        ...
        // asesor_nombre
        $this->asesor_nombre->setupEditAttributes();
        if (!$this->asesor_nombre->Raw) {
            $this->asesor_nombre->CurrentValue = HtmlDecode($this->asesor_nombre->CurrentValue);
        }
        $this->asesor_nombre->EditValue = $this->asesor_nombre->CurrentValue;
        $this->asesor_nombre->PlaceHolder = RemoveHtml($this->asesor_nombre->caption());
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->asesor);
                    $doc->exportCaption($this->total);
                    $doc->exportCaption($this->asesor_nombre);
                    $doc->exportCaption($this->documento);
                }
                $doc->endExportRow();
            }
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->asesor);
                        $doc->exportField($this->total);
                        $doc->exportField($this->asesor_nombre);
                        $doc->exportField($this->documento);
                    }
    ...
    {
        WriteAuditLog(CurrentUserIdentifier(), $typ, 'view_factura_asesor');
    }
    ...
    public function writeAuditTrailOnAdd(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "A", 'view_factura_asesor', $fldname, $key, "", $newvalue);
            }
        }
    }
        ...
        foreach (array_keys($rsnew) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && array_key_exists($fldname, $rsold) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                if ($modified) {
                    ...
                    WriteAuditLog($usr, "U", 'view_factura_asesor', $fldname, $key, $oldvalue, $newvalue);
                }
            }
        }
    }
    ...
    public function writeAuditTrailOnDelete(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "D", 'view_factura_asesor', $fldname, $key, $oldvalue);
            }
        }
    }
