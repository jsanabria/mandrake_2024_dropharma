 ...
 * Page class
 */
class NotaSalidaEdit extends NotaSalida
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "NotaSalidaEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "NotaSalidaEdit";
    ...
    public function run()
    {
            ...
            if ($this->isShow()) {
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if ($this->TotalRecords <= 0) { // No record found
                        ...
                        $this->terminate("NotaSalidaList"); // Return to list page
                        return;
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("NotaSalidaList"); // No matching record, return to list
                        return;
                    }
                } // End modal checking
                break;
            case "update": // Update
                ...
                if (GetPageName($returnUrl) == "NotaSalidaList") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "NotaSalidaList") {
                            ...
                            $returnUrl = "NotaSalidaList"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'salida' first before field var 'x_salida'
        $val = $CurrentForm->hasValue("salida") ? $CurrentForm->getValue("salida") : $CurrentForm->getValue("x_salida");
        if (!$this->salida->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->salida->Visible = false; // Disable update for API request
            } else {
                $this->salida->setFormValue($val, true, $validate);
            }
        }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            // salida
            $this->salida->setupEditAttributes();
            $this->salida->EditValue = $this->salida->CurrentValue;
            $this->salida->PlaceHolder = RemoveHtml($this->salida->caption());
            if (strval($this->salida->EditValue) != "" && is_numeric($this->salida->EditValue)) {
                $this->salida->EditValue = FormatNumber($this->salida->EditValue, null);
            }
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            ...
            if ($this->salida->Visible && $this->salida->Required) {
                if (!$this->salida->IsDetailKey && EmptyValue($this->salida->FormValue)) {
                    $this->salida->addErrorMessage(str_replace("%s", $this->salida->caption(), $this->salida->RequiredErrorMessage));
                }
            }
            if (!CheckInteger($this->salida->FormValue)) {
                $this->salida->addErrorMessage($this->salida->getErrorMessage(false));
            }
    ...
    protected function getEditRow($rsold)
    {
        ...
        // salida
        $this->salida->setDbValueDef($rsnew, $this->salida->CurrentValue, $this->salida->ReadOnly);
    ...
    {
        if (isset($row['salida'])) { // salida
            $this->salida->CurrentValue = $row['salida'];
        }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("NotaSalidaList"), "", $this->TableVar, true);
        $pageId = "edit";
