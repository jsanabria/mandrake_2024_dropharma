...
class ProveedorAdd extends Proveedor
{
    ...
    // CSS class/style
    public $CurrentPageName = "ProveedorAdd";
    ...
    protected function validateForm()
    {
        ...
        if (in_array("proveedor_articulo", $detailTblVar) && $detailPage->DetailAdd) {
            $detailPage->run();
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($addRow) {
            $detailTblVar = explode(",", $this->getCurrentDetailTable());
            $detailPage = Container("ProveedorArticuloGrid");
            if (in_array("proveedor_articulo", $detailTblVar) && $detailPage->DetailAdd && $addRow) {
                $detailPage->proveedor->setSessionValue($this->id->CurrentValue); // Set master key
    ...
    protected function getAddRow()
    {
        ...
        // cuenta_gasto
        $this->cuenta_gasto->setDbValueDef($rsnew, $this->cuenta_gasto->CurrentValue, false);

        // tipo_iva
        $this->tipo_iva->setDbValueDef($rsnew, $this->tipo_iva->CurrentValue, false);

        // tipo_islr
        $this->tipo_islr->setDbValueDef($rsnew, $this->tipo_islr->CurrentValue, false);

        // sustraendo
        $this->sustraendo->setDbValueDef($rsnew, $this->sustraendo->CurrentValue, false);

        // tipo_impmun
        $this->tipo_impmun->setDbValueDef($rsnew, $this->tipo_impmun->CurrentValue, false);
