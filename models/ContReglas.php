...
/**
 * Table class for cont_reglas
 */
class ContReglas extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        // regla
        $this->regla = new DbField(
            $this, // Table
            'x_regla', // Variable name
            'regla', // Name
            '`regla`', // Expression
            '`regla`', // Basic search expression
            19, // Type
            10, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`regla`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->regla->InputTextType = "text";
        $this->regla->Raw = true;
        $this->regla->IsForeignKey = true; // Foreign key field
        $this->regla->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->regla->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN", "IS NULL", "IS NOT NULL"];
        $this->Fields['regla'] = &$this->regla;
        ...
        $this->cargo->Lookup = new Lookup($this->cargo, 'cont_reglas', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
        $this->cargo->OptionCount = 2;
    ...
    public function getMasterFilterFromSession()
    {
        ...
        if ($this->getCurrentMasterTable() == "cont_reglas_hr") {
            ...
            if ($this->regla->getSessionValue() != "") {
                $masterFilter .= "" . GetKeyFilter($masterTable->id, $this->regla->getSessionValue(), $masterTable->id->DataType, $masterTable->Dbid);
            } else {
                return "";
            }
        }
    ...
    public function getDetailFilterFromSession()
    {
        ...
        if ($this->getCurrentMasterTable() == "cont_reglas_hr") {
            $masterTable = Container("cont_reglas_hr");
            if ($this->regla->getSessionValue() != "") {
                $detailFilter .= "" . GetKeyFilter($this->regla, $this->regla->getSessionValue(), $masterTable->id->DataType, $this->Dbid);
            } else {
                return "";
            }
        }
    ...
    public function getMasterFilter($masterTable, $keys)
    {
        ...
        switch ($masterTable->TableVar) {
            case "cont_reglas_hr":
                ...
                if ($validKeys) {
                    return GetKeyFilter($masterTable->id, $keys["regla"], $this->regla->DataType, $this->Dbid);
                }
                break;
        }
    ...
    public function getDetailFilter($masterTable)
    {
        switch ($masterTable->TableVar) {
            case "cont_reglas_hr":
                return GetKeyFilter($this->regla, $masterTable->id->DbValue, $masterTable->id->DataType, $masterTable->Dbid);
        }
        return "";
    }
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "cont_reglas";
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("ContReglasList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "ContReglasView" => $Language->phrase("View"),
            "ContReglasEdit" => $Language->phrase("Edit"),
            "ContReglasAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "ContReglasView",
            Config("API_ADD_ACTION") => "ContReglasAdd",
            Config("API_EDIT_ACTION") => "ContReglasEdit",
            Config("API_DELETE_ACTION") => "ContReglasDelete",
            Config("API_LIST_ACTION") => "ContReglasList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("ContReglasView", $parm);
        } else {
            $url = $this->keyUrl("ContReglasView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ContReglasEdit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ContReglasList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ContReglasAdd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ContReglasList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("ContReglasDelete", $parm);
        }
    }
    ...
    {
        if ($this->getCurrentMasterTable() == "cont_reglas_hr" && !ContainsString($url, Config("TABLE_SHOW_MASTER") . "=")) {
            $url .= (ContainsString($url, "?") ? "&" : "?") . Config("TABLE_SHOW_MASTER") . "=" . $this->getCurrentMasterTable();
            $url .= "&" . GetForeignKeyUrl("fk_id", $this->regla->getSessionValue()); // Use Session Value
        }
        return $url;
    }
    ...
    public function renderListRow()
    {
        ...
        // regla
        $this->regla->HrefValue = "";
        $this->regla->TooltipValue = "";
    ...
    public function renderEditRow()
    {
        ...
        // regla
        $this->regla->setupEditAttributes();
        if ($this->regla->getSessionValue() != "") {
            $this->regla->CurrentValue = GetForeignKeyValue($this->regla->getSessionValue());
            $this->regla->ViewValue = $this->regla->CurrentValue;
        } else {
            $this->regla->EditValue = $this->regla->CurrentValue;
            $this->regla->PlaceHolder = RemoveHtml($this->regla->caption());
            if (strval($this->regla->EditValue) != "" && is_numeric($this->regla->EditValue)) {
                $this->regla->EditValue = $this->regla->EditValue;
            }
        }
    ...
    {
        WriteAuditLog(CurrentUserIdentifier(), $typ, 'cont_reglas');
    }
    ...
    public function writeAuditTrailOnAdd(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "A", 'cont_reglas', $fldname, $key, "", $newvalue);
            }
        }
    }
        ...
        foreach (array_keys($rsnew) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && array_key_exists($fldname, $rsold) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                if ($modified) {
                    ...
                    WriteAuditLog($usr, "U", 'cont_reglas', $fldname, $key, $oldvalue, $newvalue);
                }
            }
        }
    }
    ...
    public function writeAuditTrailOnDelete(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "D", 'cont_reglas', $fldname, $key, $oldvalue);
            }
        }
    }
