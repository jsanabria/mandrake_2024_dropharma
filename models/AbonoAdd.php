...
class AbonoAdd extends Abono
{
    ...
    // CSS class/style
    public $CurrentPageName = "AbonoAdd";
                ...
                if ($this->addRow($rsold)) { // Add successful
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "AbonoList") {
                            ...
                            $returnUrl = "AbonoList"; // Return list page content
                        }
                    }
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->pago->DefaultValue = $this->pago->getDefault(); // PHP
        $this->pago->OldValue = $this->pago->DefaultValue;
    }
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->pago->IsDetailKey) {
            if (IsApi() && $val === null) {
                ...
                $this->pago->setFormValue($val, true, $validate);
            }
        }
    ...
    public function renderRow()
    {
        ...
        // nro_recibo
        $this->nro_recibo->RowCssClass = "row";
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            if ($curVal != "") {
                ...
                $this->metodo_pago->ViewValue = $this->metodo_pago->Lookup !== null && is_array($this->metodo_pago->lookupOptions()) && count($this->metodo_pago->lookupOptions()) > 0 ? $curVal : null;
            }
            if ($this->metodo_pago->ViewValue !== null) { // Load from cache
                $this->metodo_pago->EditValue = array_values($this->metodo_pago->lookupOptions());
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = SearchFilter($this->metodo_pago->Lookup->getTable()->Fields["valor1"]->searchExpression(), "=", $this->metodo_pago->CurrentValue, $this->metodo_pago->Lookup->getTable()->Fields["valor1"]->searchDataType(), "");
                }
                $lookupFilter = $this->metodo_pago->getSelectFilter($this); // PHP
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            ...
            if ($this->pago->Visible && $this->pago->Required) {
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($addRow) {
            $detailTblVar = explode(",", $this->getCurrentDetailTable());
            $detailPage = Container("RecargaGrid");
            if (in_array("recarga", $detailTblVar) && $detailPage->DetailAdd && $addRow) {
    ...
    protected function getAddRow()
    {
        ...
        // pago
        $this->pago->setDbValueDef($rsnew, $this->pago->CurrentValue, strval($this->pago->CurrentValue) == "");
