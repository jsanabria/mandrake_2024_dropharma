 ...
 * Page class
 */
class CobrosClienteList extends CobrosCliente
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "CobrosClienteList";
    ...
    public $FormName = "fcobros_clientelist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "CobrosClienteList";
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "CobrosClienteAdd?" . Config("TABLE_SHOW_DETAIL") . "=";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "CobrosClienteDelete";
        $this->MultiUpdateUrl = "CobrosClienteUpdate";
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                    ...
                    $result["view"] = SameString($pageName, "CobrosClienteView"); // If View page, no primary button
                } else { // List page
    ...
    public function run()
    {
        ...
        if ($this->IsModal) {
            $this->FormName = "fcobros_clientegrid";
        }
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fcobros_clientesrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field banco_origen
        $this->banco_origen->AdvancedSearch->SearchValue = @$filter["x_banco_origen"];
        $this->banco_origen->AdvancedSearch->SearchOperator = @$filter["z_banco_origen"];
        $this->banco_origen->AdvancedSearch->SearchCondition = @$filter["v_banco_origen"];
        $this->banco_origen->AdvancedSearch->SearchValue2 = @$filter["y_banco_origen"];
        ...
        $this->monto_recibido->AdvancedSearch->SearchCondition = @$filter["v_monto_recibido"];
        $this->monto_recibido->AdvancedSearch->SearchValue2 = @$filter["y_monto_recibido"];
        $this->monto_recibido->AdvancedSearch->SearchOperator2 = @$filter["w_monto_recibido"];
        $this->monto_recibido->AdvancedSearch->save();
    ...
    protected function setupListOptions()
    {
        ...
        // "detail_cobros_cliente_factura"
        $item = &$this->ListOptions->add("detail_cobros_cliente_factura");
        $item->CssClass = "text-nowrap";
        $item->Visible = $Security->allowList(CurrentProjectID() . 'cobros_cliente_factura');
        $item->OnLeft = true;
        ...
        // Set up detail pages
        $pages = new SubPages();
        $pages->add("cobros_cliente_factura");
    ...
    public function renderListOptions()
    {
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"cobros_cliente\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $Language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($Security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"cobros_cliente\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-btn=\"SaveBtn\">" . $Language->phrase("EditLink") . "</a>";
                } else {
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fcobros_clientelist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fcobros_clientelist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
        ...
        // "detail_cobros_cliente_factura"
        $opt = $this->ListOptions["detail_cobros_cliente_factura"];
        if ($Security->allowList(CurrentProjectID() . 'cobros_cliente_factura')) {
            $body = $Language->phrase("DetailLink") . $Language->tablePhrase("cobros_cliente_factura", "TblCaption");
            $body = "<a class=\"btn btn-default ew-row-link ew-detail" . ($this->ListOptions->UseDropDownButton ? " dropdown-toggle" : "") . "\" data-action=\"list\" href=\"" . HtmlEncode("CobrosClienteFacturaList?" . Config("TABLE_SHOW_MASTER") . "=cobros_cliente&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("CobrosClienteFacturaGrid");
            if ($detailPage->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'cobros_cliente')) {
                $caption = $Language->phrase("MasterDetailViewLink", null);
                $url = $this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=cobros_cliente_factura");
                $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode($url) . "\">" . $caption . "</a></li>";
                if ($detailViewTblVar != "") {
                    $detailViewTblVar .= ",";
                }
                $detailViewTblVar .= "cobros_cliente_factura";
            }
            if ($detailPage->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'cobros_cliente')) {
                $caption = $Language->phrase("MasterDetailEditLink", null);
                $url = $this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=cobros_cliente_factura");
                ...
                $detailEditTblVar .= "cobros_cliente_factura";
            }
            if ($links != "") {
    ...
    protected function renderListOptionsExt()
    {
        ...
        // Column "detail_cobros_cliente_factura"
        if ($this->DetailPages?->getItem("cobros_cliente_factura")?->Visible && $Security->allowList(CurrentProjectID() . 'cobros_cliente_factura')) {
            $link = "";
            $option = $this->ListOptions["detail_cobros_cliente_factura"];
            $detailTbl = Container("cobros_cliente_factura");
            $detailFilter = $detailTbl->getDetailFilter($this);
            $detailTbl->setCurrentMasterTable($this->TableVar);
            $detailFilter = $detailTbl->applyUserIDFilters($detailFilter);
            $url = "CobrosClienteFacturaPreview?t=cobros_cliente&f=" . Encrypt($detailFilter . "|" . implode("|", array_values($masterKeys)));
            $btngrp = "<div data-table=\"cobros_cliente_factura\" data-url=\"" . $url . "\" class=\"ew-detail-btn-group btn-group btn-group-sm d-none\">";
            if ($Security->allowList(CurrentProjectID() . 'cobros_cliente')) {
                $label = $Language->tablePhrase("cobros_cliente_factura", "TblCaption");
                $link = "<button class=\"nav-link\" data-bs-toggle=\"tab\" data-table=\"cobros_cliente_factura\" data-url=\"" . $url . "\" type=\"button\" role=\"tab\" aria-selected=\"false\">" . $label . "</button>";
                $detaillnk = GetUrl("CobrosClienteFacturaList?" . Config("TABLE_SHOW_MASTER") . "=cobros_cliente");
                foreach ($masterKeys as $key => $value) {
                    $detaillnk .= "&" . GetForeignKeyUrl("fk_$key", $value);
                }
                $title = $Language->tablePhrase("cobros_cliente_factura", "TblCaption");
            ...
            $detailPageObj = Container("CobrosClienteFacturaGrid");
            if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'cobros_cliente')) {
                $caption = $Language->phrase("MasterDetailViewLink");
                $viewurl = GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=cobros_cliente_factura"));
            ...
            if ($detailPageObj->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'cobros_cliente')) {
                $caption = $Language->phrase("MasterDetailEditLink");
                $editurl = GetUrl($this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=cobros_cliente_factura"));
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
            $item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"cobros_cliente\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("AddLink") . "</a>";
        } else {
        ...
        $detailTableLink = "";
        $item = &$option->add("detailadd_cobros_cliente_factura");
        $url = $this->getAddUrl(Config("TABLE_SHOW_DETAIL") . "=cobros_cliente_factura");
        $detailPage = Container("CobrosClienteFacturaGrid");
        $caption = $Language->phrase("Add") . "&nbsp;" . $this->tableCaption() . "/" . $detailPage->tableCaption();
        $item->Body = "<a class=\"ew-detail-add-group ew-detail-add\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode(GetUrl($url)) . "\">" . $caption . "</a>";
        $item->Visible = ($detailPage->DetailAdd && $Security->allowAdd(CurrentProjectID() . 'cobros_cliente') && $Security->canAdd());
        if ($item->Visible) {
            if ($detailTableLink != "") {
                $detailTableLink .= ",";
            }
            $detailTableLink .= "cobros_cliente_factura";
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fcobros_clientesrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fcobros_clientesrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions as $listAction) {
            if ($listAction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fcobros_clientelist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_cobros_cliente", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_cobros_cliente",
    ...
    protected function loadSearchValues()
    {
        ...
        // banco_origen
        if ($this->banco_origen->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->banco_origen->AdvancedSearch->SearchValue != "" || $this->banco_origen->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        if ($this->monto_recibido->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->monto_recibido->AdvancedSearch->SearchValue != "" || $this->monto_recibido->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fcobros_clientesrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
