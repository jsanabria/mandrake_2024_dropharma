 ...
 * Page class
 */
class PresupuestoEdit extends Presupuesto
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PresupuestoEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "PresupuestoEdit";
    ...
    public function run()
    {
            ...
            if ($this->isShow()) {
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if ($this->TotalRecords <= 0) { // No record found
                        ...
                        $this->terminate("PresupuestoList"); // Return to list page
                        return;
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("PresupuestoList"); // No matching record, return to list
                        return;
                    }
                } // End modal checking
                break;
            case "update": // Update
                ...
                if (GetPageName($returnUrl) == "PresupuestoList") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "PresupuestoList") {
                            ...
                            $returnUrl = "PresupuestoList"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'cliente_potencial' first before field var 'x_cliente_potencial'
        $val = $CurrentForm->hasValue("cliente_potencial") ? $CurrentForm->getValue("cliente_potencial") : $CurrentForm->getValue("x_cliente_potencial");
        if (!$this->cliente_potencial->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->cliente_potencial->Visible = false; // Disable update for API request
            } else {
                $this->cliente_potencial->setFormValue($val);
            }
        }
        ...
        // Check field name 'proyecto' first before field var 'x_proyecto'
        $val = $CurrentForm->hasValue("proyecto") ? $CurrentForm->getValue("proyecto") : $CurrentForm->getValue("x_proyecto");
        if (!$this->proyecto->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->proyecto->Visible = false; // Disable update for API request
            } else {
                $this->proyecto->setFormValue($val);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->fecha->CurrentValue = UnFormatDateTime($this->fecha->CurrentValue, $this->fecha->formatPattern());
        $this->cliente_potencial->CurrentValue = $this->cliente_potencial->FormValue;
    ...
    public function renderRow()
    {
        ...
        // cliente_potencial
        $this->cliente_potencial->RowCssClass = "row";
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            // cliente_potencial
            $this->cliente_potencial->setupEditAttributes();
            if (!$this->cliente_potencial->Raw) {
                $this->cliente_potencial->CurrentValue = HtmlDecode($this->cliente_potencial->CurrentValue);
            }
            $this->cliente_potencial->EditValue = HtmlEncode($this->cliente_potencial->CurrentValue);
            $this->cliente_potencial->PlaceHolder = RemoveHtml($this->cliente_potencial->caption());
            ...
            // proyecto
            $this->proyecto->setupEditAttributes();
            if (!$this->proyecto->Raw) {
                $this->proyecto->CurrentValue = HtmlDecode($this->proyecto->CurrentValue);
            }
            $this->proyecto->EditValue = HtmlEncode($this->proyecto->CurrentValue);
            $this->proyecto->PlaceHolder = RemoveHtml($this->proyecto->caption());

            // estatus
            $this->estatus->EditValue = $this->estatus->options(false);
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            ...
            if ($this->cliente_potencial->Visible && $this->cliente_potencial->Required) {
                if (!$this->cliente_potencial->IsDetailKey && EmptyValue($this->cliente_potencial->FormValue)) {
                    $this->cliente_potencial->addErrorMessage(str_replace("%s", $this->cliente_potencial->caption(), $this->cliente_potencial->RequiredErrorMessage));
                }
            }
            ...
            if ($this->proyecto->Visible && $this->proyecto->Required) {
                if (!$this->proyecto->IsDetailKey && EmptyValue($this->proyecto->FormValue)) {
                    $this->proyecto->addErrorMessage(str_replace("%s", $this->proyecto->caption(), $this->proyecto->RequiredErrorMessage));
                }
            }
            if ($this->estatus->Visible && $this->estatus->Required) {
                if ($this->estatus->FormValue == "") {
    ...
    protected function getEditRow($rsold)
    {
        ...
        // cliente_potencial
        $this->cliente_potencial->setDbValueDef($rsnew, $this->cliente_potencial->CurrentValue, $this->cliente_potencial->ReadOnly);

        // rif
        $this->rif->setDbValueDef($rsnew, $this->rif->CurrentValue, $this->rif->ReadOnly);

        // cliente
        $this->cliente->setDbValueDef($rsnew, $this->cliente->CurrentValue, $this->cliente->ReadOnly);

        // proyecto
        $this->proyecto->setDbValueDef($rsnew, $this->proyecto->CurrentValue, $this->proyecto->ReadOnly);
    ...
    protected function restoreEditFormFromRow($row)
    {
        ...
        if (isset($row['cliente_potencial'])) { // cliente_potencial
            $this->cliente_potencial->CurrentValue = $row['cliente_potencial'];
        }
        if (isset($row['rif'])) { // rif
            $this->rif->CurrentValue = $row['rif'];
        }
        if (isset($row['cliente'])) { // cliente
            $this->cliente->CurrentValue = $row['cliente'];
        }
        if (isset($row['proyecto'])) { // proyecto
            $this->proyecto->CurrentValue = $row['proyecto'];
        }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("PresupuestoList"), "", $this->TableVar, true);
        $pageId = "edit";
