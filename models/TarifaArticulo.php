...
/**
 * Table class for tarifa_articulo
 */
class TarifaArticulo extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        // Update Table
        $this->UpdateTable = "tarifa_articulo";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        $this->articulo->Lookup = new Lookup($this->articulo, 'articulo', false, 'id', ["principio_activo","presentacion","nombre_comercial","codigo"], '', '', ["x_fabricante"], [], ["fabricante"], ["x_fabricante"], [], [], false, '`principio_activo`', '', "CONCAT(COALESCE(`principio_activo`, ''),'" . ValueSeparator(1, $this->articulo) . "',COALESCE(`presentacion`,''),'" . ValueSeparator(2, $this->articulo) . "',COALESCE(`nombre_comercial`,''),'" . ValueSeparator(3, $this->articulo) . "',COALESCE(`codigo`,''))");
        ...
        $this->precio->Required = true; // Required field
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "tarifa_articulo";
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("TarifaArticuloList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "TarifaArticuloView" => $Language->phrase("View"),
            "TarifaArticuloEdit" => $Language->phrase("Edit"),
            "TarifaArticuloAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "TarifaArticuloView",
            Config("API_ADD_ACTION") => "TarifaArticuloAdd",
            Config("API_EDIT_ACTION") => "TarifaArticuloEdit",
            Config("API_DELETE_ACTION") => "TarifaArticuloDelete",
            Config("API_LIST_ACTION") => "TarifaArticuloList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("TarifaArticuloView", $parm);
        } else {
            $url = $this->keyUrl("TarifaArticuloView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = "TarifaArticuloAdd?" . $parm;
        } else {
            $url = "TarifaArticuloAdd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("TarifaArticuloEdit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("TarifaArticuloList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("TarifaArticuloAdd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("TarifaArticuloList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("TarifaArticuloDelete", $parm);
        }
    }
    ...
    {
        WriteAuditLog(CurrentUserIdentifier(), $typ, 'tarifa_articulo');
    }
    ...
    public function writeAuditTrailOnAdd(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "A", 'tarifa_articulo', $fldname, $key, "", $newvalue);
            }
        }
    }
        ...
        foreach (array_keys($rsnew) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && array_key_exists($fldname, $rsold) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                if ($modified) {
                    ...
                    WriteAuditLog($usr, "U", 'tarifa_articulo', $fldname, $key, $oldvalue, $newvalue);
                }
            }
        }
    }
    ...
    public function writeAuditTrailOnDelete(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "D", 'tarifa_articulo', $fldname, $key, $oldvalue);
            }
        }
    }
    ...
    public function recordsetSelecting(&$filter) {
    	// Enter your code here
    	// AddFilter($filter, "(SELECT a.activo FROM articulo AS a WHERE a.id = articulo) = 'S'"); 
    }
    ...
    public function rowInserting($rsold, &$rsnew) {
    	...
    	if($rsnew["tarifa"] == "") {
    		$this->CancelMessage = "Debe agregar el art&iacute;culo por el men&uacute; Sistema->Tarifa; !Verifique!";
    		return FALSE;
    	}
    	return TRUE;
    }
    ...
    public function rowRendered() {
    	...
    	$sql = "SELECT cantidad_en_mano FROM articulo WHERE id = '" . $this->articulo->CurrentValue . "'";
    	$cantidad_en_mano = intval(ExecuteScalar($sql));
    	if ($cantidad_en_mano <= 0) // List page only
