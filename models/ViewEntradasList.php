 ...
 * Page class
 */
class ViewEntradasList extends ViewEntradas
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ViewEntradasList";
    ...
    public $FormName = "fview_entradaslist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "ViewEntradasList";
    ...
    public function setVisibility()
    {
        ...
        $this->nombre_documento->setVisibility();
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'view_entradas';
        $this->TableName = 'view_entradas';
        ...
        // Table object (view_entradas)
        if (!isset($GLOBALS["view_entradas"]) || $GLOBALS["view_entradas"]::class == PROJECT_NAMESPACE . "view_entradas") {
            $GLOBALS["view_entradas"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "ViewEntradasAdd?" . Config("TABLE_SHOW_DETAIL") . "=";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "ViewEntradasDelete";
        $this->MultiUpdateUrl = "ViewEntradasUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'view_entradas');
        }
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                    ...
                    $result["view"] = SameString($pageName, "ViewEntradasView"); // If View page, no primary button
                } else { // List page
    ...
    public function run()
    {
        ...
        if ($this->IsModal) {
            $this->FormName = "fview_entradasgrid";
        }
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->nombre_proveedor->AdvancedSearch->toJson(), ","); // Field nombre_proveedor
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fview_entradassrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        $this->nombre_documento->AdvancedSearch->SearchValue = @$filter["x_nombre_documento"];
        $this->nombre_documento->AdvancedSearch->SearchOperator = @$filter["z_nombre_documento"];
        $this->nombre_documento->AdvancedSearch->SearchCondition = @$filter["v_nombre_documento"];
        $this->nombre_documento->AdvancedSearch->SearchValue2 = @$filter["y_nombre_documento"];
        $this->nombre_documento->AdvancedSearch->SearchOperator2 = @$filter["w_nombre_documento"];
        $this->nombre_documento->AdvancedSearch->save();
        ...
        $this->nombre_proveedor->AdvancedSearch->SearchValue = @$filter["x_nombre_proveedor"];
        $this->nombre_proveedor->AdvancedSearch->SearchOperator = @$filter["z_nombre_proveedor"];
        $this->nombre_proveedor->AdvancedSearch->SearchCondition = @$filter["v_nombre_proveedor"];
        $this->nombre_proveedor->AdvancedSearch->SearchValue2 = @$filter["y_nombre_proveedor"];
        $this->nombre_proveedor->AdvancedSearch->SearchOperator2 = @$filter["w_nombre_proveedor"];
        $this->nombre_proveedor->AdvancedSearch->save();
    ...
    public function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->nombre_documento, $default, false); // nombre_documento
        $this->buildSearchSql($where, $this->proveedor, $default, false); // proveedor
        $this->buildSearchSql($where, $this->nombre_proveedor, $default, false); // nombre_proveedor
        ...
        if (!$default && $this->Command == "search") {
            ...
            $this->nombre_documento->AdvancedSearch->save(); // nombre_documento
            $this->proveedor->AdvancedSearch->save(); // proveedor
            $this->nombre_proveedor->AdvancedSearch->save(); // nombre_proveedor
    ...
    public function showFilterList()
    {
        ...
        // Field nombre_documento
        $filter = $this->queryBuilderWhere("nombre_documento");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->nombre_documento, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->nombre_documento->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
    ...
    public function basicSearchWhere($default = false)
    {
        ...
        $searchFlds[] = &$this->nombre_documento;
        $searchFlds[] = &$this->nombre_proveedor;
        $searchFlds[] = &$this->nro_documento;
        $searchFlds[] = &$this->fecha;
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->nombre_documento->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->proveedor->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->nombre_proveedor->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    protected function resetAdvancedSearchParms()
    {
        ...
        $this->nombre_documento->AdvancedSearch->unsetSession();
        $this->proveedor->AdvancedSearch->unsetSession();
        $this->nombre_proveedor->AdvancedSearch->unsetSession();
    ...
    protected function restoreSearchParms()
    {
        ...
        $this->nombre_documento->AdvancedSearch->load();
        $this->proveedor->AdvancedSearch->load();
        $this->nombre_proveedor->AdvancedSearch->load();
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->nombre_documento); // nombre_documento
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->nombre_documento->setSort("");
                $this->proveedor->setSort("");
                $this->nombre_proveedor->setSort("");
    ...
    protected function setupListOptions()
    {
        ...
        // "detail_view_entradas_salidas"
        $item = &$this->ListOptions->add("detail_view_entradas_salidas");
        $item->CssClass = "text-nowrap";
        $item->Visible = $Security->allowList(CurrentProjectID() . 'view_entradas_salidas');
        $item->OnLeft = true;
        ...
        // Set up detail pages
        $pages = new SubPages();
        $pages->add("view_entradas_salidas");
    ...
    public function renderListOptions()
    {
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fview_entradaslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fview_entradaslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
        ...
        // "detail_view_entradas_salidas"
        $opt = $this->ListOptions["detail_view_entradas_salidas"];
        if ($Security->allowList(CurrentProjectID() . 'view_entradas_salidas')) {
            $body = $Language->phrase("DetailLink") . $Language->tablePhrase("view_entradas_salidas", "TblCaption");
            $body = "<a class=\"btn btn-default ew-row-link ew-detail" . ($this->ListOptions->UseDropDownButton ? " dropdown-toggle" : "") . "\" data-action=\"list\" href=\"" . HtmlEncode("ViewEntradasSalidasList?" . Config("TABLE_SHOW_MASTER") . "=view_entradas&" . GetForeignKeyUrl("fk_tipo_documento", $this->tipo_documento->CurrentValue) . "&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("ViewEntradasSalidasGrid");
            if ($links != "") {
    ...
    protected function renderListOptionsExt()
    {
        ...
        // Column "detail_view_entradas_salidas"
        if ($this->DetailPages?->getItem("view_entradas_salidas")?->Visible && $Security->allowList(CurrentProjectID() . 'view_entradas_salidas')) {
            $link = "";
            $option = $this->ListOptions["detail_view_entradas_salidas"];
            $detailTbl = Container("view_entradas_salidas");
            $detailFilter = $detailTbl->getDetailFilter($this);
            $detailTbl->setCurrentMasterTable($this->TableVar);
            $detailFilter = $detailTbl->applyUserIDFilters($detailFilter);
            $url = "ViewEntradasSalidasPreview?t=view_entradas&f=" . Encrypt($detailFilter . "|" . implode("|", array_values($masterKeys)));
            $btngrp = "<div data-table=\"view_entradas_salidas\" data-url=\"" . $url . "\" class=\"ew-detail-btn-group d-none\">";
            if ($Security->allowList(CurrentProjectID() . 'view_entradas')) {
                $label = $Language->tablePhrase("view_entradas_salidas", "TblCaption");
                $link = "<button class=\"nav-link\" data-bs-toggle=\"tab\" data-table=\"view_entradas_salidas\" data-url=\"" . $url . "\" type=\"button\" role=\"tab\" aria-selected=\"false\">" . $label . "</button>";
                $detaillnk = GetUrl("ViewEntradasSalidasList?" . Config("TABLE_SHOW_MASTER") . "=view_entradas");
                foreach ($masterKeys as $key => $value) {
                    $detaillnk .= "&" . GetForeignKeyUrl("fk_$key", $value);
                }
                $title = $Language->tablePhrase("view_entradas_salidas", "TblCaption");
            ...
            $detailPageObj = Container("ViewEntradasSalidasGrid");
            if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'view_entradas')) {
                $caption = $Language->phrase("MasterDetailViewLink");
                $viewurl = GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=view_entradas_salidas"));
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "nombre_documento");
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fview_entradassrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fview_entradassrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions as $listAction) {
            if ($listAction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fview_entradaslist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_view_entradas", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_view_entradas",
    ...
    protected function loadSearchValues()
    {
        ...
        // nombre_documento
        if ($this->nombre_documento->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->nombre_documento->AdvancedSearch->SearchValue != "" || $this->nombre_documento->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        // nombre_proveedor
        if ($this->nombre_proveedor->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->nombre_proveedor->AdvancedSearch->SearchValue != "" || $this->nombre_proveedor->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
    ...
    protected function newRow()
    {
        ...
        $row['nombre_documento'] = $this->nombre_documento->DefaultValue;
        $row['proveedor'] = $this->proveedor->DefaultValue;
        $row['nombre_proveedor'] = $this->nombre_proveedor->DefaultValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            if (!$this->nombre_documento->Raw) {
                $this->nombre_documento->AdvancedSearch->SearchValue = HtmlDecode($this->nombre_documento->AdvancedSearch->SearchValue);
            }
            $this->nombre_documento->EditValue = HtmlEncode($this->nombre_documento->AdvancedSearch->SearchValue);
            $this->nombre_documento->PlaceHolder = RemoveHtml($this->nombre_documento->caption());

            // proveedor
            $this->proveedor->setupEditAttributes();
            $this->proveedor->EditValue = $this->proveedor->AdvancedSearch->SearchValue;
            $curVal = strval($this->proveedor->AdvancedSearch->SearchValue);
            if ($curVal != "") {
                ...
                if ($this->proveedor->EditValue === null) { // Lookup from database
                    ...
                    if ($ari > 0) { // Lookup values found
                        ...
                        $this->proveedor->EditValue = HtmlEncode($this->proveedor->AdvancedSearch->SearchValue);
                    }
                }
            } else {
            ...
            if (!$this->nota->Raw) {
                $this->nota->AdvancedSearch->SearchValue = HtmlDecode($this->nota->AdvancedSearch->SearchValue);
            }
    ...
    protected function validateSearch()
    {
        ...
        if (!CheckInteger($this->proveedor->AdvancedSearch->SearchValue)) {
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmail", true) . '" data-caption="' . $Language->phrase("ExportToEmail", true) . '" form="fview_entradaslist" data-ew-action="email" data-custom="false" data-hdr="' . $Language->phrase("ExportToEmail", true) . '" data-exported-selected="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fview_entradassrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
