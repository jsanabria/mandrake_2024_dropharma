...
class UsuarioList extends Usuario
{
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "UsuarioAdd?" . Config("TABLE_SHOW_DETAIL") . "=";
    ...
    protected function restoreFilterList()
    {
        ...
        // Field userlevelid2
        $this->userlevelid2->AdvancedSearch->SearchValue = @$filter["x_userlevelid2"];
        $this->userlevelid2->AdvancedSearch->SearchOperator = @$filter["z_userlevelid2"];
        $this->userlevelid2->AdvancedSearch->SearchCondition = @$filter["v_userlevelid2"];
    ...
    protected function setupListOptions()
    {
        ...
        // "detail_usuario_master2"
        $item = &$this->ListOptions->add("detail_usuario_master2");
        $item->CssClass = "text-nowrap";
        $item->Visible = $Security->allowList(CurrentProjectID() . 'usuario_master');
        $item->OnLeft = true;
    ...
    public function renderListOptions()
    {
        ...
        // "detail_usuario_master2"
        $opt = $this->ListOptions["detail_usuario_master2"];
        if ($Security->allowList(CurrentProjectID() . 'usuario_master')) {
            $body = $Language->phrase("DetailLink") . $Language->tablePhrase("usuario_master2", "TblCaption");
            $body = "<a class=\"btn btn-default ew-row-link ew-detail" . ($this->ListOptions->UseDropDownButton ? " dropdown-toggle" : "") . "\" data-action=\"list\" href=\"" . HtmlEncode("UsuarioMaster2List?" . Config("TABLE_SHOW_MASTER") . "=usuario&" . GetForeignKeyUrl("fk__username", $this->_username->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("UsuarioMaster2Grid");
            if ($detailPage->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'usuario')) {
                $caption = $Language->phrase("MasterDetailViewLink", null);
                $url = $this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=usuario_master2");
                $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode($url) . "\">" . $caption . "</a></li>";
                if ($detailViewTblVar != "") {
                    $detailViewTblVar .= ",";
                }
                $detailViewTblVar .= "usuario_master2";
            }
            if ($detailPage->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'usuario')) {
                $caption = $Language->phrase("MasterDetailEditLink", null);
                $url = $this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=usuario_master2");
                ...
                $detailEditTblVar .= "usuario_master2";
            }
            if ($links != "") {
    ...
    protected function renderListOptionsExt()
    {
        ...
        $detailFilters = [];
        $masterKeys = []; // Reset
        $masterKeys["_username"] = strval($this->_username->DbValue);

        // Column "detail_usuario_master2"
        if ($this->DetailPages?->getItem("usuario_master2")?->Visible && $Security->allowList(CurrentProjectID() . 'usuario_master')) {
            $link = "";
            $option = $this->ListOptions["detail_usuario_master2"];
            $detailTbl = Container("usuario_master2");
            $detailFilter = $detailTbl->getDetailFilter($this);
            $detailTbl->setCurrentMasterTable($this->TableVar);
            $detailFilter = $detailTbl->applyUserIDFilters($detailFilter);
            $url = "UsuarioMaster2Preview?t=usuario&f=" . Encrypt($detailFilter . "|" . implode("|", array_values($masterKeys)));
            $btngrp = "<div data-table=\"usuario_master2\" data-url=\"" . $url . "\" class=\"ew-detail-btn-group btn-group btn-group-sm d-none\">";
            if ($Security->allowList(CurrentProjectID() . 'usuario')) {
                $label = $Language->tablePhrase("usuario_master2", "TblCaption");
                $link = "<button class=\"nav-link\" data-bs-toggle=\"tab\" data-table=\"usuario_master2\" data-url=\"" . $url . "\" type=\"button\" role=\"tab\" aria-selected=\"false\">" . $label . "</button>";
                $detaillnk = GetUrl("UsuarioMaster2List?" . Config("TABLE_SHOW_MASTER") . "=usuario");
                foreach ($masterKeys as $key => $value) {
                    $detaillnk .= "&" . GetForeignKeyUrl("fk_$key", $value);
                }
                $title = $Language->tablePhrase("usuario_master2", "TblCaption");
            ...
            if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'usuario')) {
                $caption = $Language->phrase("MasterDetailViewLink");
                $viewurl = GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=usuario_master2"));
            ...
            if ($detailPageObj->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'usuario')) {
                $caption = $Language->phrase("MasterDetailEditLink");
                $editurl = GetUrl($this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=usuario_master2"));
    ...
    protected function setupOtherOptions()
    {
        ...
        $detailTableLink = "";
        $item = &$option->add("detailadd_usuario_master2");
        $url = $this->getAddUrl(Config("TABLE_SHOW_DETAIL") . "=usuario_master2");
        $detailPage = Container("UsuarioMaster2Grid");
        $caption = $Language->phrase("Add") . "&nbsp;" . $this->tableCaption() . "/" . $detailPage->tableCaption();
        $item->Body = "<a class=\"ew-detail-add-group ew-detail-add\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode(GetUrl($url)) . "\">" . $caption . "</a>";
        $item->Visible = ($detailPage->DetailAdd && $Security->allowAdd(CurrentProjectID() . 'usuario') && $Security->canAdd());
        if ($item->Visible) {
            if ($detailTableLink != "") {
                $detailTableLink .= ",";
            }
            $detailTableLink .= "usuario_master2";
        }
    ...
    protected function loadSearchValues()
    {
        ...
        // userlevelid2
        if ($this->userlevelid2->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->userlevelid2->AdvancedSearch->SearchValue != "" || $this->userlevelid2->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            if ($this->foto->UseColorbox) {
                ...
                $this->foto->LinkAttrs["data-rel"] = "usuario_x" . $this->RowCount . "_foto";
            ...
            if (!$Security->canAdmin()) { // System admin
                ...
                $curVal = trim(strval($this->userlevelid->AdvancedSearch->SearchValue));
                if ($curVal != "") {
                    $this->userlevelid->AdvancedSearch->ViewValue = $this->userlevelid->lookupCacheOption($curVal);
                } else {
                    $this->userlevelid->AdvancedSearch->ViewValue = $this->userlevelid->Lookup !== null && is_array($this->userlevelid->lookupOptions()) && count($this->userlevelid->lookupOptions()) > 0 ? $curVal : null;
                }
                if ($this->userlevelid->AdvancedSearch->ViewValue !== null) { // Load from cache
                    $this->userlevelid->EditValue = array_values($this->userlevelid->lookupOptions());
                } else { // Lookup from database
                    if ($curVal == "") {
                        $filterWrk = "0=1";
                    } else {
                        $filterWrk = SearchFilter($this->userlevelid->Lookup->getTable()->Fields["userlevelid"]->searchExpression(), "=", $this->userlevelid->AdvancedSearch->SearchValue, $this->userlevelid->Lookup->getTable()->Fields["userlevelid"]->searchDataType(), "");
                    }
            ...
            if ($this->asesor->AdvancedSearch->ViewValue !== null) { // Load from cache
                ...
                if ($curVal == "") {
                    ...
                    $filterWrk = SearchFilter($this->asesor->Lookup->getTable()->Fields["id"]->searchExpression(), "=", $this->asesor->AdvancedSearch->SearchValue, $this->asesor->Lookup->getTable()->Fields["id"]->searchDataType(), "");
                }
