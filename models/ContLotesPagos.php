...
class ContLotesPagos extends DbTable
{
    ...
    public function __construct()
    {
        ...
        $this->procesado->setSelectMultiple(false); // Select one
        ...
        $this->procesado->Lookup = new Lookup($this->procesado, 'cont_lotes_pagos', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
    ...
    public function delete(&$rs, $where = "", $curfilter = false)
    {
        ...
        $dtlrows = Container("cont_lotes_pagos_detalle")->loadRs("`cont_lotes_pago` = " . QuotedValue($rs['id'], DataType::NUMBER, "DB"))->fetchAllAssociative();
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "ContLotesPagosView" => $Language->phrase("View"),
            "ContLotesPagosEdit" => $Language->phrase("Edit"),
            "ContLotesPagosAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "ContLotesPagosView",
            Config("API_ADD_ACTION") => "ContLotesPagosAdd",
            Config("API_EDIT_ACTION") => "ContLotesPagosEdit",
            Config("API_DELETE_ACTION") => "ContLotesPagosDelete",
            Config("API_LIST_ACTION") => "ContLotesPagosList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("ContLotesPagosView", $parm);
        } else {
            $url = $this->keyUrl("ContLotesPagosView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("ContLotesPagosEdit", $parm);
        } else {
            $url = $this->keyUrl("ContLotesPagosEdit", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ContLotesPagosList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("ContLotesPagosAdd", $parm);
        } else {
            $url = $this->keyUrl("ContLotesPagosAdd", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ContLotesPagosList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("ContLotesPagosDelete", $parm);
        }
    }
    ...
    public function renderEditRow()
    {
        ...
        // procesado
        $this->procesado->setupEditAttributes();
    ...
    {
        WriteAuditLog(CurrentUserIdentifier(), $typ, 'cont_lotes_pagos');
    }
    ...
    public function writeAuditTrailOnAdd(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "A", 'cont_lotes_pagos', $fldname, $key, "", $newvalue);
            }
        }
    }
        ...
        foreach (array_keys($rsnew) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && array_key_exists($fldname, $rsold) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                if ($modified) {
                    ...
                    WriteAuditLog($usr, "U", 'cont_lotes_pagos', $fldname, $key, $oldvalue, $newvalue);
                }
            }
        }
    }
    ...
    public function writeAuditTrailOnDelete(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "D", 'cont_lotes_pagos', $fldname, $key, $oldvalue);
            }
        }
    }
    ...
    public function rowInserted($rsold, $rsnew)
    {
    	$sql = "INSERT INTO cont_lotes_pagos_detalle
    				(Id, cont_lotes_pago, id_documento, proveedor, tipo_documento, tipodoc, nro_documento, monto_a_pagar, monto_pagdo, saldo)				
    			SELECT 
    					NULL AS id, " . $rsnew["id"] . " AS lote, 
    					aa.id_documento, aa.proveedor, aa.tipo_documento,
    					aa.tipodoc, aa.nro_documento, 
    					aa.monto_pagar, aa.monto_pagado, (aa.monto_pagar-ABS(aa.monto_pagado)) AS saldo 
    			FROM	
    				(			
    				SELECT 
    ...
    public function rowUpdated($rsold, $rsnew)
    {
    	if($rsnew["procesado"] == "S") {
            $sql = "SELECT 
            			Id, id_documento, tipo_documento, nro_documento  
            		FROM 
            			cont_lotes_pagos_detalle 
            		WHERE 
            			cont_lotes_pago = 5;"; 
            $arr = ExecuteRows($sql100);
            foreach ($arr as $key => $value) {
            	$Cmpb = new CrearComprobante(3, $value["tipo_documento"], $rsnew["fecha"], $value["nro_documento"], CurrentUserName());
            	if($Cmpb->NoExite == false) {
            		$Cmpb->Comprobante($value["id_documento"]);
            	}
            	$sql = "UPDATE cont_lotes_pagos_detalle
            			SET
            				comprobante = " . $Cmpb->NroComprobante . "
            			WHERE Id = " . $value["Id"] . ";";
            	Execute($sql);
            	unset($Cmpb);
            }
