 ...
 * Page class
 */
class ViewFacturasAEntregarList extends ViewFacturasAEntregar
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ViewFacturasAEntregarList";
    ...
    public $FormName = "fview_facturas_a_entregarlist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "ViewFacturasAEntregarList";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'view_facturas_a_entregar';
        $this->TableName = 'view_facturas_a_entregar';
        ...
        if (!isset($GLOBALS["view_facturas_a_entregar"]) || $GLOBALS["view_facturas_a_entregar"]::class == PROJECT_NAMESPACE . "view_facturas_a_entregar") {
        ...
        // Initialize URLs
        $this->AddUrl = "ViewFacturasAEntregarAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "ViewFacturasAEntregarDelete";
        $this->MultiUpdateUrl = "ViewFacturasAEntregarUpdate";
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                    ...
                    $result["view"] = SameString($pageName, "ViewFacturasAEntregarView"); // If View page, no primary button
                } else { // List page
    ...
    public function run()
    {
        ...
        if ($this->IsModal) {
            $this->FormName = "fview_facturas_a_entregargrid";
        }
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server") {
            $savedFilterList = Profile()->getSearchFilters("fview_facturas_a_entregarsrch");
        }
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fview_facturas_a_entregarsrch", $filters);
    ...
    protected function setupSortOrder()
    {
        ...
        if ($this->Command != "json") {
            $defaultSort = $this->codcli->Expression . " ASC" . ", " . $this->fecha->Expression . " DESC"; // Set up default sort
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"view_facturas_a_entregar\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-btn=\"SaveBtn\">" . $Language->phrase("EditLink") . "</a>";
                } else {
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fview_facturas_a_entregarlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fview_facturas_a_entregarlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fview_facturas_a_entregarsrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fview_facturas_a_entregarsrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions as $listAction) {
            if ($listAction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fview_facturas_a_entregarlist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_view_facturas_a_entregar", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_view_facturas_a_entregar",
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fview_facturas_a_entregarsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    */
    ...
    public function pageDataRendering(&$header) {
    	// Example:
    	$header = '<h3>Marque como entregado aquellas facturas relacionadas a pedidos de ventas que han sido entregados a sus clientes.</h3>';
    }
    ...
    public function listOptionsRendered() {
    	// Example: 
    	$sql = "SELECT id_documento_padre FROM salidas WHERE id = " . intval($this->id->CurrentValue) . ";";
    	$id_documento_padre = intval(ExecuteScalar($sql));
    	$sql = "SELECT
    				a.id, a.tipo_documento, 
    				a.nro_documento, a.id_documento_padre 
    			FROM 
    				salidas AS a 
    	...
    	if($row = ExecuteRow($sql)) {
    		...
    		$id = intval($row["id"]);
    		if($tipo_documento != "TDCFCV" AND $id_documento_padre != 0) {
    			$sql = "SELECT 
    					a.id, a.tipo_documento, 
    					a.nro_documento, a.id_documento_padre 
    				FROM 
    		...
    		$this->ListOptions->Items["new"]->Body = '<a href="reportes/pedido_de_venta.php?id=' . $id . '&tipo=' . $tipo_documento . '" target="_blank">' . $nro_documento . '</a>';
    	}
    }
