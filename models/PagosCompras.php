...
/**
 * Table class for pagos_compras
 */
class PagosCompras extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        // Update Table
        $this->UpdateTable = "pagos_compras";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        $this->tipo_pago->Lookup = new Lookup($this->tipo_pago, 'parametro', false, 'valor1', ["valor2","","",""], '', '', [], [], [], [], [], [], false, '`valor2`', '', "`valor2`");
        $this->tipo_pago->SearchOperators = ["=", "<>", "IS NULL", "IS NOT NULL"];
        $this->Fields['tipo_pago'] = &$this->tipo_pago;

        // anexos
        $this->anexos = new DbField(
            $this, // Table
            'x_anexos', // Variable name
            'anexos', // Name
            '`anexos`', // Expression
        ...
        $this->anexos->InputTextType = "text";
        $this->anexos->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY", "IS NULL", "IS NOT NULL"];
        $this->Fields['anexos'] = &$this->anexos;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "pagos_compras";
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("PagosComprasList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "PagosComprasView" => $Language->phrase("View"),
            "PagosComprasEdit" => $Language->phrase("Edit"),
            "PagosComprasAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "PagosComprasView",
            Config("API_ADD_ACTION") => "PagosComprasAdd",
            Config("API_EDIT_ACTION") => "PagosComprasEdit",
            Config("API_DELETE_ACTION") => "PagosComprasDelete",
            Config("API_LIST_ACTION") => "PagosComprasList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("PagosComprasView", $parm);
        } else {
            $url = $this->keyUrl("PagosComprasView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("PagosComprasList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("PagosComprasList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("PagosComprasDelete", $parm);
        }
    }
    ...
    public function renderEditRow()
    {
        ...
        $this->anexos->EditValue = $this->anexos->CurrentValue;
    ...
    public function rowInserting($rsold, &$rsnew)
    {
    	...
    	if(isset($_POST["xCantidad"])) {
    		...
    		$this->CancelMessage = "No ha seleccionado facturas a pagar.";
    		return FALSE;
    	}
    	return TRUE;
    }
    ...
    public function rowInserted($rsold, &$rsnew)
    {
    	...
    	$arr = explode(",-,", $_POST["pagos"]);
      	foreach ($arr as $key => $value) {
      		if(trim($value) != "") {
      			...
      			if(count($arr2) > 2) {
    		    	...
    		    	if($rowb = ExecuteRow("SELECT banco FROM banco_tipo_pago WHERE tipo_pago = '" . $arr2[1] . "' AND moneda = '" . $arr2[5] . "'"))
    		    		$banco = $rowb["banco"];
    		  		$sql = "INSERT INTO pagos_compras_detalle (
    		  					id,
    		  					pagos_compras,
    		  					metodo_pago,
    		  					referencia,
      	 ...
      	 $sql = "UPDATE compara SET pagado = 'S' 
