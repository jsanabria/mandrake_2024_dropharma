 ...
 * Page class
 */
class ViewFacturaAsesorList extends ViewFacturaAsesor
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ViewFacturaAsesorList";
    ...
    public $FormName = "fview_factura_asesorlist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "ViewFacturaAsesorList";
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "ViewFacturaAsesorAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "ViewFacturaAsesorDelete";
        $this->MultiUpdateUrl = "ViewFacturaAsesorUpdate";
    ...
    public function run()
    {
        ...
        if ($this->IsModal) {
            $this->FormName = "fview_factura_asesorgrid";
        }
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server") {
            $savedFilterList = Profile()->getSearchFilters("fview_factura_asesorsrch");
        }
        ...
        $filterList = Concat($filterList, $this->asesor_nombre->AdvancedSearch->toJson(), ","); // Field asesor_nombre
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fview_factura_asesorsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field asesor_nombre
        $this->asesor_nombre->AdvancedSearch->SearchValue = @$filter["x_asesor_nombre"];
        $this->asesor_nombre->AdvancedSearch->SearchOperator = @$filter["z_asesor_nombre"];
        $this->asesor_nombre->AdvancedSearch->SearchCondition = @$filter["v_asesor_nombre"];
        $this->asesor_nombre->AdvancedSearch->SearchValue2 = @$filter["y_asesor_nombre"];
        $this->asesor_nombre->AdvancedSearch->SearchOperator2 = @$filter["w_asesor_nombre"];
        $this->asesor_nombre->AdvancedSearch->save();
    ...
    public function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->asesor_nombre, $default, false); // asesor_nombre
        ...
        if (!$default && $this->Command == "search") {
            ...
            $this->asesor_nombre->AdvancedSearch->save(); // asesor_nombre
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->asesor_nombre->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    protected function resetAdvancedSearchParms()
    {
        ...
        $this->asesor_nombre->AdvancedSearch->unsetSession();
    ...
    protected function restoreSearchParms()
    {
        ...
        $this->asesor_nombre->AdvancedSearch->load();
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"view_factura_asesor\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-btn=\"SaveBtn\">" . $Language->phrase("EditLink") . "</a>";
                } else {
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fview_factura_asesorlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fview_factura_asesorlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fview_factura_asesorsrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fview_factura_asesorsrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions as $listAction) {
            if ($listAction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fview_factura_asesorlist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_view_factura_asesor", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_view_factura_asesor",
    ...
    protected function loadSearchValues()
    {
        ...
        // asesor_nombre
        if ($this->asesor_nombre->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->asesor_nombre->AdvancedSearch->SearchValue != "" || $this->asesor_nombre->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fview_factura_asesorsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
