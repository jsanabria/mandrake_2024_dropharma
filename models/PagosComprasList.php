 ...
 * Page class
 */
class PagosComprasList extends PagosCompras
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PagosComprasList";
    ...
    public $FormName = "fpagos_compraslist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "PagosComprasList";
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "PagosComprasAdd?" . Config("TABLE_SHOW_DETAIL") . "=";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "PagosComprasDelete";
        $this->MultiUpdateUrl = "PagosComprasUpdate";
    ...
    public function run()
    {
        ...
        if ($this->IsModal) {
            $this->FormName = "fpagos_comprasgrid";
        }
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server") {
            $savedFilterList = Profile()->getSearchFilters("fpagos_comprassrch");
        }
        ...
        $filterList = Concat($filterList, $this->anexos->AdvancedSearch->toJson(), ","); // Field anexos
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fpagos_comprassrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field anexos
        $this->anexos->AdvancedSearch->SearchValue = @$filter["x_anexos"];
        $this->anexos->AdvancedSearch->SearchOperator = @$filter["z_anexos"];
        $this->anexos->AdvancedSearch->SearchCondition = @$filter["v_anexos"];
        $this->anexos->AdvancedSearch->SearchValue2 = @$filter["y_anexos"];
        $this->anexos->AdvancedSearch->SearchOperator2 = @$filter["w_anexos"];
        $this->anexos->AdvancedSearch->save();
    ...
    public function basicSearchWhere($default = false)
    {
        ...
        $searchFlds[] = &$this->comprobante;
        $searchFlds[] = &$this->tipo_pago;
        $searchFlds[] = &$this->anexos;
    ...
    protected function setupListOptions()
    {
        ...
        $item = &$this->ListOptions->add("detail_pagos_compras_detalle");
        ...
        // Set up detail pages
        $pages = new SubPages();
        $pages->add("pagos_compras_detalle");
    ...
    public function renderListOptions()
    {
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fpagos_compraslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fpagos_compraslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
        ...
        // "detail_pagos_compras_detalle"
        $opt = $this->ListOptions["detail_pagos_compras_detalle"];
        if ($Security->allowList(CurrentProjectID() . 'pagos_compras_detalle')) {
            $body = $Language->phrase("DetailLink") . $Language->tablePhrase("pagos_compras_detalle", "TblCaption");
            $body = "<a class=\"btn btn-default ew-row-link ew-detail" . ($this->ListOptions->UseDropDownButton ? " dropdown-toggle" : "") . "\" data-action=\"list\" href=\"" . HtmlEncode("PagosComprasDetalleList?" . Config("TABLE_SHOW_MASTER") . "=pagos_compras&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("PagosComprasDetalleGrid");
            if ($links != "") {
    ...
    protected function renderListOptionsExt()
    {
        ...
        // Column "detail_pagos_compras_detalle"
        if ($this->DetailPages?->getItem("pagos_compras_detalle")?->Visible && $Security->allowList(CurrentProjectID() . 'pagos_compras_detalle')) {
            $link = "";
            $option = $this->ListOptions["detail_pagos_compras_detalle"];
            $detailTbl = Container("pagos_compras_detalle");
            $detailFilter = $detailTbl->getDetailFilter($this);
            $detailTbl->setCurrentMasterTable($this->TableVar);
            $detailFilter = $detailTbl->applyUserIDFilters($detailFilter);
            $url = "PagosComprasDetallePreview?t=pagos_compras&f=" . Encrypt($detailFilter . "|" . implode("|", array_values($masterKeys)));
            $btngrp = "<div data-table=\"pagos_compras_detalle\" data-url=\"" . $url . "\" class=\"ew-detail-btn-group d-none\">";
            if ($Security->allowList(CurrentProjectID() . 'pagos_compras')) {
                $label = $Language->tablePhrase("pagos_compras_detalle", "TblCaption");
                $link = "<button class=\"nav-link\" data-bs-toggle=\"tab\" data-table=\"pagos_compras_detalle\" data-url=\"" . $url . "\" type=\"button\" role=\"tab\" aria-selected=\"false\">" . $label . "</button>";
                $detaillnk = GetUrl("PagosComprasDetalleList?" . Config("TABLE_SHOW_MASTER") . "=pagos_compras");
                foreach ($masterKeys as $key => $value) {
                    $detaillnk .= "&" . GetForeignKeyUrl("fk_$key", $value);
                }
                $title = $Language->tablePhrase("pagos_compras_detalle", "TblCaption");
            ...
            if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'pagos_compras')) {
                ...
                $viewurl = GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=pagos_compras_detalle"));
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fpagos_comprassrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fpagos_comprassrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions as $listAction) {
            if ($listAction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fpagos_compraslist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_pagos_compras", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_pagos_compras",
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmail", true) . '" data-caption="' . $Language->phrase("ExportToEmail", true) . '" form="fpagos_compraslist" data-ew-action="email" data-custom="false" data-hdr="' . $Language->phrase("ExportToEmail", true) . '" data-exported-selected="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fpagos_comprassrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    */
    ...
    {
        // Example:
        $opt = &$this->ListOptions->add("documento");
        $opt->Header = "Documento";
    ...
    {
        // Verificamos si la opción "documento" existe en la lista
        if (isset($this->ListOptions["documento"])) {
            $tipoDoc = $this->tipo_documento->CurrentValue;
            $idReg = $this->id_documento->CurrentValue;
            $infoDocumento = "";
            if (!empty($idReg)) {
                // Evaluamos el tipo de documento para decidir la consulta
                if ($tipoDoc == 'TDCFCC') {
                    $sql = "SELECT documento, nro_documento FROM entradas WHERE id = " . AdjustSql($idReg, $this->Dbid) . " AND tipo_documento = 'TDCFCC'";
                    $row = ExecuteRow($sql);
                    if ($row) {
                        $infoDocumento = "TDCFCC - " . $row["documento"] . " Nro: " . $row["nro_documento"];
                    }
                } elseif ($tipoDoc == 'GASTOS' || $tipoDoc == 'GASTO') {
                    $sql = "SELECT tipo_documento AS documento, documento AS nro_documento FROM compra WHERE id = " . AdjustSql($idReg, $this->Dbid);
                    $row = ExecuteRow($sql);
                    if ($row) {
                        $infoDocumento = "GASTOS - " . $row["documento"] . " - " . $row["nro_documento"];
                    }
                } else {
                    // De lo contrario (cualquier otro caso)
                    $sql = "SELECT tipo_documento AS documento, documento AS nro_documento FROM compra WHERE id = " . AdjustSql($idReg, $this->Dbid);
                    $row = ExecuteRow($sql);
                    if ($row) {
                        $infoDocumento = ($row["documento"] ? $row["documento"] : "Compra") . " - " . $row["nro_documento"];
                    }
                }
            }

            // Asignamos el resultado informativo al Body de la opción
            $this->ListOptions["documento"]->Body = !empty($infoDocumento) ? $infoDocumento : "N/D";
        }
    }
