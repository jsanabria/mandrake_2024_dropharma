...
class Medicamentos extends DbTable
{
    ...
    public function __construct()
    {
        ...
        // laboratorio
        $this->laboratorio = new DbField(
            $this, // Table
            'x_laboratorio', // Variable name
            'laboratorio', // Name
            '`laboratorio`', // Expression
            '`laboratorio`', // Basic search expression
            200, // Type
            100, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`laboratorio`', // Virtual expression
            false, // Is virtual
        ...
        $this->laboratorio->InputTextType = "text";
        $this->laboratorio->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY", "IS NULL", "IS NOT NULL"];
        $this->Fields['laboratorio'] = &$this->laboratorio;
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->laboratorio->DbValue = $row['laboratorio'];
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("MedicamentosList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "MedicamentosView" => $Language->phrase("View"),
            "MedicamentosEdit" => $Language->phrase("Edit"),
            "MedicamentosAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "MedicamentosView",
            Config("API_ADD_ACTION") => "MedicamentosAdd",
            Config("API_EDIT_ACTION") => "MedicamentosEdit",
            Config("API_DELETE_ACTION") => "MedicamentosDelete",
            Config("API_LIST_ACTION") => "MedicamentosList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("MedicamentosView", $parm);
        } else {
            $url = $this->keyUrl("MedicamentosView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("MedicamentosEdit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("MedicamentosList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("MedicamentosAdd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("MedicamentosList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("MedicamentosDelete", $parm);
        }
    }
    ...
    public function loadListRowValues(&$rs)
    {
        ...
        $this->laboratorio->setDbValue($row['laboratorio']);
    ...
    public function renderListRow()
    {
        ...
        // laboratorio
        $this->laboratorio->ViewValue = $this->laboratorio->CurrentValue;
        ...
        // laboratorio
        $this->laboratorio->HrefValue = "";
        $this->laboratorio->TooltipValue = "";
    ...
    public function renderEditRow()
    {
        ...
        // laboratorio
        $this->laboratorio->setupEditAttributes();
        if (!$this->laboratorio->Raw) {
            $this->laboratorio->CurrentValue = HtmlDecode($this->laboratorio->CurrentValue);
        }
        $this->laboratorio->EditValue = $this->laboratorio->CurrentValue;
        $this->laboratorio->PlaceHolder = RemoveHtml($this->laboratorio->caption());
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    $doc->exportCaption($this->laboratorio);
        ...
        while (($row = $result->fetch()) && $recCnt < $stopRec) {
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        $doc->exportField($this->laboratorio);
    ...
    {
        WriteAuditLog(CurrentUserIdentifier(), $typ, 'medicamentos');
    }
    ...
    public function writeAuditTrailOnAdd(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "A", 'medicamentos', $fldname, $key, "", $newvalue);
            }
        }
    }
        ...
        foreach (array_keys($rsnew) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && array_key_exists($fldname, $rsold) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                if ($modified) {
                    ...
                    WriteAuditLog($usr, "U", 'medicamentos', $fldname, $key, $oldvalue, $newvalue);
                }
            }
        }
    }
    ...
    public function writeAuditTrailOnDelete(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "D", 'medicamentos', $fldname, $key, $oldvalue);
            }
        }
    }
