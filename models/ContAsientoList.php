 ...
 * Page class
 */
class ContAsientoList extends ContAsiento
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ContAsientoList";
    ...
    public $FormName = "fcont_asientolist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "ContAsientoList";
    ...
    public function __construct()
    {
        ...
        if (!isset($GLOBALS["cont_asiento"]) || $GLOBALS["cont_asiento"]::class == PROJECT_NAMESPACE . "cont_asiento") {
        ...
        // Initialize URLs
        $this->AddUrl = "ContAsientoAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "ContAsientoDelete";
        $this->MultiUpdateUrl = "ContAsientoUpdate";
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                    ...
                    $result["view"] = SameString($pageName, "ContAsientoView"); // If View page, no primary button
                } else { // List page
    ...
    public $DisplayRecords = 400;
    public $StartRecord;
    public $StopRecord;
    public $TotalRecords = 0;
    public $RecordRange = 10;
    public $PageSizes = "10,20,50,400,-1"; // Page sizes (comma separated)
    ...
    public function run()
    {
        ...
        if ($this->IsModal) {
            $this->FormName = "fcont_asientogrid";
        }
        ...
        if ($this->Command != "json" && $this->getRecordsPerPage() != "") {
            ...
            $this->DisplayRecords = 400; // Load default
        ...
        // Load master record
        if ($this->CurrentMode != "add" && $this->DbMasterFilter != "" && $this->getCurrentMasterTable() == "cont_comprobante") {
            $masterTbl = Container("cont_comprobante");
            $rsmaster = $masterTbl->loadRs($this->DbMasterFilter)->fetchAssociative();
            $this->MasterRecordExists = $rsmaster !== false;
            if (!$this->MasterRecordExists) {
                $this->setFailureMessage($Language->phrase("NoRecord")); // Set no record found
                $this->terminate("ContComprobanteList"); // Return to master page
                return;
            } else {
        ...
        if ($wrk != "") {
            if (is_numeric($wrk)) {
                ...
                if (SameText($wrk, "all")) { // Display all records
                    ...
                    $this->DisplayRecords = 400; // Non-numeric, load default
                }
            }
    ...
    {
        $this->debe->FormValue = ""; // Clear form value
        $this->haber->FormValue = ""; // Clear form value
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->debe->AdvancedSearch->toJson(), ","); // Field debe
        $filterList = Concat($filterList, $this->haber->AdvancedSearch->toJson(), ","); // Field haber
        $filterList = Concat($filterList, $this->id_referencia->AdvancedSearch->toJson(), ","); // Field id_referencia
        if ($this->BasicSearch->Keyword != "") {
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fcont_asientosrch", $filters);
    ...
    protected function setupSortOrder()
    {
        ...
        if ($this->Command != "json") {
            $defaultSort = $this->id_referencia->Expression . " ASC" . ", " . $this->debe->Expression . " DESC" . ", " . $this->cuenta->Expression . " ASC"; // Set up default sort
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->debe); // debe
            $this->updateSort($this->haber); // haber
    ...
    public function renderListOptions()
    {
        ...
        if ($this->isInlineAddRow() || $this->isInlineCopyRow()) { // Inline Add/Copy
            ...
            if ($this->UseAjaxActions) {
                ...
                $opt->Body = <<<INLINEADD
                    <div{$divClass}>
                        <button class="ew-grid-link ew-inline-insert" title="{$insertTitle}" data-caption="{$insertTitle}" form="fcont_asientolist" formaction="{$inlineInsertUrl}">{$insertCaption}</button>
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"cont_asiento\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-btn=\"SaveBtn\">" . $Language->phrase("EditLink") . "</a>";
                } else {
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fcont_asientolist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fcont_asientolist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
            $item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"cont_asiento\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("AddLink") . "</a>";
        } else {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "debe");
            $this->createColumnOption($option, "haber");
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fcont_asientosrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fcont_asientosrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions as $listAction) {
            if ($listAction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fcont_asientolist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_cont_asiento", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_cont_asiento",
    ...
    {
        $this->id_referencia->DefaultValue = $this->id_referencia->getDefault(); // PHP
        $this->id_referencia->OldValue = $this->id_referencia->DefaultValue;
    }
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->debe->IsDetailKey) {
            if (IsApi() && $val === null) {
                ...
                $this->debe->setFormValue($val, true, $validate);
            }
        }
        ...
        if (!$this->haber->IsDetailKey) {
            if (IsApi() && $val === null) {
                ...
                $this->haber->setFormValue($val, true, $validate);
            }
        }
    ...
    protected function newRow()
    {
        ...
        $row['debe'] = $this->debe->DefaultValue;
        $row['haber'] = $this->haber->DefaultValue;
        $row['id_referencia'] = $this->id_referencia->DefaultValue;
        return $row;
    }
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            ...
            if ($this->debe->Visible && $this->debe->Required) {
            ...
            if ($this->haber->Visible && $this->haber->Required) {
    ...
    protected function addRow($rsold = null)
    {
        ...
        $detailKeys = [];
        $detailKeys["comprobante"] = $this->comprobante->getSessionValue();
    ...
    protected function getAddRow()
    {
        ...
        // cuenta
        $this->cuenta->setDbValueDef($rsnew, $this->cuenta->CurrentValue, false);
        ...
        // debe
        $this->debe->setDbValueDef($rsnew, $this->debe->CurrentValue, false);

        // haber
        $this->haber->setDbValueDef($rsnew, $this->haber->CurrentValue, false);

        // comprobante
        if ($this->comprobante->getSessionValue() != "") {
            $rsnew['comprobante'] = $this->comprobante->getSessionValue();
        }
        return $rsnew;
    }
    ...
    protected function restoreAddFormFromRow($row)
    {
        ...
        if (isset($row['debe'])) { // debe
            $this->debe->setFormValue($row['debe']);
        }
        if (isset($row['haber'])) { // haber
            $this->haber->setFormValue($row['haber']);
        }
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmail", true) . '" data-caption="' . $Language->phrase("ExportToEmail", true) . '" form="fcont_asientolist" data-ew-action="email" data-custom="false" data-hdr="' . $Language->phrase("ExportToEmail", true) . '" data-exported-selected="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fcont_asientosrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    */
    public function exportData($doc)
    {
        ...
        if (Config("EXPORT_MASTER_RECORD") && $this->DbMasterFilter != "" && $this->getCurrentMasterTable() == "cont_comprobante") {
            $cont_comprobante = new ContComprobanteList();
            $rsmaster = $cont_comprobante->loadRs($this->DbMasterFilter); // Load master record
            if ($rsmaster) {
                $exportStyle = $doc->Style;
                $doc->setStyle("v"); // Change to vertical
                if (!$this->isExport("csv") || Config("EXPORT_MASTER_RECORD_FOR_CSV")) {
                    $doc->setTable($cont_comprobante);
                    $cont_comprobante->exportDocument($doc, $rsmaster);
                    $doc->exportEmptyRow();
                    $doc->setTable($this);
                }
    ...
    protected function setupMasterParms()
    {
        ...
        if (($master = Get(Config("TABLE_SHOW_MASTER"), Get(Config("TABLE_MASTER")))) !== null) {
            ...
            if ($masterTblVar == "cont_comprobante") {
                ...
                if (($parm = Get("fk_id", Get("comprobante"))) !== null) {
                    ...
                    $this->comprobante->QueryStringValue = $masterTbl->id->QueryStringValue; // DO NOT change, master/detail key data type can be different
                    $this->comprobante->setSessionValue($this->comprobante->QueryStringValue);
                    $foreignKeys["comprobante"] = $this->comprobante->QueryStringValue;
            ...
            if ($masterTblVar == "cont_comprobante") {
                ...
                if (($parm = Post("fk_id", Post("comprobante"))) !== null) {
                    ...
                    $this->comprobante->FormValue = $masterTbl->id->FormValue;
                    $this->comprobante->setSessionValue($this->comprobante->FormValue);
                    $foreignKeys["comprobante"] = $this->comprobante->FormValue;
        ...
        if ($validMaster) {
            ...
            if ($masterTblVar != "cont_comprobante") {
                if (!array_key_exists("comprobante", $foreignKeys)) { // Not current foreign key
                    $this->comprobante->setSessionValue("");
                }
            }
        }
    ...
    public function pageDataRendering(&$header) {
    	...
    	if(CurrentPageID() == "list") {
    		...
    		elseif(isset($_REQUEST["id"])) 
    			...
    			$comprobante = $this->comprobante->CurrentValue;
    		if(CuadraComprobante($comprobante)) {
    			$sql = "SELECT contabilizacion 
    				FROM cont_comprobante 
    				WHERE id = '$comprobante';";
    			$contabilizacion = ExecuteScalar($sql);
    			if($contabilizacion == "") {
    				if(VerificaFuncion('012')) {
    					$header = '<div id="result" class="container">
    						<buttom class="btn btn-success" id="cmbContab" name="cmbContab">Contabilizar</buttom>
    						</div>';
    				}
    			}
    			else {
    				$header = '<div class="alert alert-success" role="alert">
    							Este Comprobante est&aacute; Contabilizado
    						</div>';
    			}
    		}
    		else {
    			$header = '<div class="alert alert-danger" role="alert">
    						Comprobante no cuadrado o no tiene asientos registrados.
    					</div>';
    		}
    	}
    }
    ...
    public function pageDataRendered(&$footer) {
    	...
    	//$comprobante = intval((isset($_REQUEST["id"]) ? $_REQUEST["id"] : (isset($_REQUEST["id"]) ? $_REQUEST["fk_id"] : CurrentTable()->id->CurrentValue)));
    	if(isset($_REQUEST["fk_id"]))
    		$comprobante = intval($_REQUEST["fk_id"]);
    	elseif(isset($_REQUEST["id"])) 
    		$comprobante = intval($_REQUEST["id"]);
    	...
    	$debe = $row["debe"];
    	$haber = $row["haber"];
    	$_class = $debe == $haber ? 'alert alert-success' : 'alert alert-danger';
    	$footer = '<div class="ewMessageDialog">';
    	$footer .= '<div class="' . $_class . '">
