...
/**
 * Table class for view_entradas_salidas
 */
class ViewEntradasSalidas extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        // Update Table
        $this->UpdateTable = "view_entradas_salidas";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        $this->articulo_unidad_medida->Lookup = new Lookup($this->articulo_unidad_medida, 'unidad_medida', false, 'codigo', ["descripcion","","",""], '', '', [], [], [], [], [], [], false, '', '', "`descripcion`");
        $this->articulo_unidad_medida->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY", "IS NULL", "IS NOT NULL"];
        ...
        $this->check_ne->Lookup = new Lookup($this->check_ne, 'view_entradas_salidas', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
        ...
        $this->newdata->Lookup = new Lookup($this->newdata, 'view_entradas_salidas', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
    ...
    public function getMasterFilterFromSession()
    {
        ...
        if ($this->getCurrentMasterTable() == "view_entradas") {
            $masterTable = Container("view_entradas");
        ...
        if ($this->getCurrentMasterTable() == "view_salidas") {
            $masterTable = Container("view_salidas");
            if ($this->tipo_documento->getSessionValue() != "") {
                $masterFilter .= "" . GetKeyFilter($masterTable->tipo_documento, $this->tipo_documento->getSessionValue(), $masterTable->tipo_documento->DataType, $masterTable->Dbid);
            } else {
                return "";
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "view_entradas_salidas";
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("ViewEntradasSalidasList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "ViewEntradasSalidasView" => $Language->phrase("View"),
            "ViewEntradasSalidasEdit" => $Language->phrase("Edit"),
            "ViewEntradasSalidasAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    {
        return "ViewEntradasSalidasList";
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "ViewEntradasSalidasView",
            Config("API_ADD_ACTION") => "ViewEntradasSalidasAdd",
            Config("API_EDIT_ACTION") => "ViewEntradasSalidasEdit",
            Config("API_DELETE_ACTION") => "ViewEntradasSalidasDelete",
            Config("API_LIST_ACTION") => "ViewEntradasSalidasList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("ViewEntradasSalidasView", $parm);
        } else {
            $url = $this->keyUrl("ViewEntradasSalidasView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = "ViewEntradasSalidasAdd?" . $parm;
        } else {
            $url = "ViewEntradasSalidasAdd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ViewEntradasSalidasEdit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ViewEntradasSalidasList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ViewEntradasSalidasAdd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ViewEntradasSalidasList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("ViewEntradasSalidasDelete", $parm);
        }
    }
    ...
    {
        if ($this->getCurrentMasterTable() == "view_entradas" && !ContainsString($url, Config("TABLE_SHOW_MASTER") . "=")) {
        ...
        if ($this->getCurrentMasterTable() == "view_salidas" && !ContainsString($url, Config("TABLE_SHOW_MASTER") . "=")) {
            $url .= (ContainsString($url, "?") ? "&" : "?") . Config("TABLE_SHOW_MASTER") . "=" . $this->getCurrentMasterTable();
            $url .= "&" . GetForeignKeyUrl("fk_tipo_documento", $this->tipo_documento->getSessionValue()); // Use Session Value
            $url .= "&" . GetForeignKeyUrl("fk_id", $this->id_documento->getSessionValue()); // Use Session Value
        }
    ...
    {
        global $Response;
        $listPage = "ViewEntradasSalidasList";
    ...
    public function renderEditRow()
    {
        ...
        $this->articulo_unidad_medida->EditValue = $this->articulo_unidad_medida->CurrentValue;
    ...
    public function rowRendered()
    {
    	...
    	if ($this->PageID == "list" || $this->PageID == "view") {
    		if (intval($this->cantidad_articulo->ViewValue) != intval($this->packer_cantidad->ViewValue)) { 
    			$color = "background-color: RED; color: #ffffff;";
    			$this->articulo->CellAttrs["style"] = $color;
    			$this->cantidad_articulo->CellAttrs["style"] = $color;
    			$this->packer_cantidad->CellAttrs["style"] = $color;
        	}
        }
    }
