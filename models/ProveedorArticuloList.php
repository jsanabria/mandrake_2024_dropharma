 ...
 * Page class
 */
class ProveedorArticuloList extends ProveedorArticulo
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ProveedorArticuloList";
    ...
    public $FormName = "fproveedor_articulolist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "ProveedorArticuloList";
    ...
    public function setVisibility()
    {
        ...
        $this->codigo_proveedor->setVisibility();
    }
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'proveedor_articulo';
        $this->TableName = 'proveedor_articulo';
        ...
        // Table object (proveedor_articulo)
        if (!isset($GLOBALS["proveedor_articulo"]) || $GLOBALS["proveedor_articulo"]::class == PROJECT_NAMESPACE . "proveedor_articulo") {
            $GLOBALS["proveedor_articulo"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "ProveedorArticuloAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "ProveedorArticuloDelete";
        $this->MultiUpdateUrl = "ProveedorArticuloUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'proveedor_articulo');
        }
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                    ...
                    $result["view"] = SameString($pageName, "ProveedorArticuloView"); // If View page, no primary button
                } else { // List page
    ...
    public function run()
    {
        ...
        if ($this->IsModal) {
            $this->FormName = "fproveedor_articulogrid";
        }
        ...
        if (IsPost() && ($this->isGridUpdate() || $this->isMultiUpdate() || $this->isGridOverwrite()) && (Session(SESSION_INLINE_MODE) == "gridedit" || Session(SESSION_INLINE_MODE) == "multiedit")) {
            ...
            if ($gridUpdate) {
                ...
                if ($this->IsModal && !$this->UseAjaxActions) {
                    $this->terminate("ProveedorArticuloList");
                    return;
                }
            } else {
        ...
        // Load master record
        if ($this->CurrentMode != "add" && $this->DbMasterFilter != "" && $this->getCurrentMasterTable() == "proveedor") {
            $masterTbl = Container("proveedor");
            $rsmaster = $masterTbl->loadRs($this->DbMasterFilter)->fetchAssociative();
            $this->MasterRecordExists = $rsmaster !== false;
            if (!$this->MasterRecordExists) {
                $this->setFailureMessage($Language->phrase("NoRecord")); // Set no record found
                $this->terminate("ProveedorList"); // Return to master page
                return;
            } else {
    ...
    public function emptyRow()
    {
        ...
        if (
            $CurrentForm->hasValue("x_codigo_proveedor") &&
            $CurrentForm->hasValue("o_codigo_proveedor") &&
            $this->codigo_proveedor->CurrentValue != $this->codigo_proveedor->DefaultValue &&
            !($this->codigo_proveedor->IsForeignKey && $this->getCurrentMasterTable() != "" && $this->codigo_proveedor->CurrentValue == $this->codigo_proveedor->getSessionValue())
        ) {
            return false;
        }
        return true;
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server") {
            $savedFilterList = Profile()->getSearchFilters("fproveedor_articulosrch");
        }
        ...
        $filterList = Concat($filterList, $this->codigo_proveedor->AdvancedSearch->toJson(), ","); // Field codigo_proveedor
        if ($this->BasicSearch->Keyword != "") {
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fproveedor_articulosrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        $this->codigo_proveedor->AdvancedSearch->SearchValue = @$filter["x_codigo_proveedor"];
        $this->codigo_proveedor->AdvancedSearch->SearchOperator = @$filter["z_codigo_proveedor"];
        $this->codigo_proveedor->AdvancedSearch->SearchCondition = @$filter["v_codigo_proveedor"];
        $this->codigo_proveedor->AdvancedSearch->SearchValue2 = @$filter["y_codigo_proveedor"];
        $this->codigo_proveedor->AdvancedSearch->SearchOperator2 = @$filter["w_codigo_proveedor"];
        $this->codigo_proveedor->AdvancedSearch->save();
    ...
    public function basicSearchWhere($default = false)
    {
        ...
        // Fields to search
        $searchFlds = [];
        $searchFlds[] = &$this->codigo_proveedor;
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->codigo_proveedor); // codigo_proveedor
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"proveedor_articulo\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $Language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($Security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"proveedor_articulo\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-btn=\"SaveBtn\">" . $Language->phrase("EditLink") . "</a>";
                } else {
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fproveedor_articulolist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fproveedor_articulolist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
            $item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"proveedor_articulo\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("AddLink") . "</a>";
        } else {
        ...
        $item->Body = "<button type=\"button\" class=\"ew-action ew-multi-delete\" title=\"" .
            ...
            HtmlTitle($Language->phrase("DeleteSelectedLink")) . "\" form=\"fproveedor_articulolist\"" .
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "codigo_proveedor");
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fproveedor_articulosrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fproveedor_articulosrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
        ...
        if (!$this->isGridAdd() && !$this->isGridEdit() && !$this->isMultiEdit()) { // Not grid add/grid edit/multi edit mode
            ...
            foreach ($this->ListActions as $listAction) {
                if ($listAction->Select == ACTION_MULTIPLE) {
                    ...
                    $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fproveedor_articulolist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
            ...
            if ($this->isGridAdd()) {
                ...
                if (!($this->ModalGridAdd && !IsMobile())) {
                    ...
                    $item->Body = "<button class=\"ew-action ew-grid-insert\" title=\"" . HtmlTitle($Language->phrase("GridInsertLink")) . "\" data-caption=\"" . HtmlTitle($Language->phrase("GridInsertLink")) . "\" form=\"fproveedor_articulolist\" formaction=\"" . GetUrl($this->pageName()) . "\">" . $Language->phrase("GridInsertLink") . "</button>";
                    // Add grid cancel
            ...
            if ($this->isGridEdit()) {
                ...
                if (!($this->ModalGridEdit && !IsMobile())) {
                    ...
                    $option->UseDropDownButton = false;
                        ...
                        $item->Body = "<button class=\"ew-action ew-grid-save\" title=\"" . HtmlTitle($Language->phrase("GridSaveLink")) . "\" data-caption=\"" . HtmlTitle($Language->phrase("GridSaveLink")) . "\" form=\"fproveedor_articulolist\" formaction=\"" . GetUrl($this->pageName()) . "\">" . $Language->phrase("GridSaveLink") . "</button>";
                        $item = &$option->add("gridcancel");
                        $cancelurl = $this->addMasterUrl($pageUrl . "action=cancel");
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_proveedor_articulo", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_proveedor_articulo",
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'codigo_proveedor' first before field var 'x_codigo_proveedor'
        $val = $CurrentForm->hasValue("codigo_proveedor") ? $CurrentForm->getValue("codigo_proveedor") : $CurrentForm->getValue("x_codigo_proveedor");
        if (!$this->codigo_proveedor->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->codigo_proveedor->Visible = false; // Disable update for API request
            } else {
                $this->codigo_proveedor->setFormValue($val);
            }
        }
        if ($CurrentForm->hasValue("o_codigo_proveedor")) {
            $this->codigo_proveedor->setOldValue($CurrentForm->getValue("o_codigo_proveedor"));
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->codigo_proveedor->CurrentValue = $this->codigo_proveedor->FormValue;
    }
    ...
    public function loadRowValues($row = null)
    {
        ...
        $this->codigo_proveedor->setDbValue($row['codigo_proveedor']);
    }
    ...
    protected function newRow()
    {
        ...
        $row['codigo_proveedor'] = $this->codigo_proveedor->DefaultValue;
        return $row;
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            // codigo_proveedor
            $this->codigo_proveedor->ViewValue = $this->codigo_proveedor->CurrentValue;
            ...
            // codigo_proveedor
            $this->codigo_proveedor->HrefValue = "";
            $this->codigo_proveedor->TooltipValue = "";
            ...
            // codigo_proveedor
            $this->codigo_proveedor->setupEditAttributes();
            if (!$this->codigo_proveedor->Raw) {
                $this->codigo_proveedor->CurrentValue = HtmlDecode($this->codigo_proveedor->CurrentValue);
            }
            $this->codigo_proveedor->EditValue = HtmlEncode($this->codigo_proveedor->CurrentValue);
            $this->codigo_proveedor->PlaceHolder = RemoveHtml($this->codigo_proveedor->caption());
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            ...
            if ($this->codigo_proveedor->Visible && $this->codigo_proveedor->Required) {
                if (!$this->codigo_proveedor->IsDetailKey && EmptyValue($this->codigo_proveedor->FormValue)) {
                    $this->codigo_proveedor->addErrorMessage(str_replace("%s", $this->codigo_proveedor->caption(), $this->codigo_proveedor->RequiredErrorMessage));
                }
            }
    ...
    protected function getEditRow($rsold)
    {
        ...
        // codigo_proveedor
        $this->codigo_proveedor->setDbValueDef($rsnew, $this->codigo_proveedor->CurrentValue, $this->codigo_proveedor->ReadOnly);
        return $rsnew;
    }
    ...
    protected function restoreEditFormFromRow($row)
    {
        ...
        if (isset($row['codigo_proveedor'])) { // codigo_proveedor
            $this->codigo_proveedor->CurrentValue = $row['codigo_proveedor'];
        }
    }
    ...
    public function getRowHash($row)
    {
        ...
        $hash .= GetFieldHash($row['codigo_proveedor']); // codigo_proveedor
        return md5($hash);
    }
    ...
    protected function getAddRow()
    {
        ...
        // codigo_proveedor
        $this->codigo_proveedor->setDbValueDef($rsnew, $this->codigo_proveedor->CurrentValue, false);
    ...
    protected function restoreAddFormFromRow($row)
    {
        ...
        if (isset($row['codigo_proveedor'])) { // codigo_proveedor
            $this->codigo_proveedor->setFormValue($row['codigo_proveedor']);
        }
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmail", true) . '" data-caption="' . $Language->phrase("ExportToEmail", true) . '" form="fproveedor_articulolist" data-ew-action="email" data-custom="false" data-hdr="' . $Language->phrase("ExportToEmail", true) . '" data-exported-selected="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fproveedor_articulosrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    */
    public function exportData($doc)
    {
        ...
        if (Config("EXPORT_MASTER_RECORD") && $this->DbMasterFilter != "" && $this->getCurrentMasterTable() == "proveedor") {
            $proveedor = new ProveedorList();
            $rsmaster = $proveedor->loadRs($this->DbMasterFilter); // Load master record
            if ($rsmaster) {
                $exportStyle = $doc->Style;
                $doc->setStyle("v"); // Change to vertical
                if (!$this->isExport("csv") || Config("EXPORT_MASTER_RECORD_FOR_CSV")) {
                    $doc->setTable($proveedor);
                    $proveedor->exportDocument($doc, $rsmaster);
                    $doc->exportEmptyRow();
                    $doc->setTable($this);
                }
        ...
        if (($master = Get(Config("TABLE_SHOW_MASTER"), Get(Config("TABLE_MASTER")))) !== null) {
            ...
            if ($masterTblVar == "proveedor") {
                ...
                if (($parm = Get("fk_id", Get("proveedor"))) !== null) {
                    ...
                    $this->proveedor->QueryStringValue = $masterTbl->id->QueryStringValue; // DO NOT change, master/detail key data type can be different
            ...
            if ($masterTblVar == "proveedor") {
                ...
                if (($parm = Post("fk_id", Post("proveedor"))) !== null) {
                    ...
                    $this->proveedor->FormValue = $masterTbl->id->FormValue;
