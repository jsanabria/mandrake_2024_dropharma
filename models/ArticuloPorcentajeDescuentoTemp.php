...
/**
 * Table class for articulo_porcentaje_descuento_temp
 */
class ArticuloPorcentajeDescuentoTemp extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        // Update Table
        $this->UpdateTable = "articulo_porcentaje_descuento_temp";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        $this->codigo->Lookup = new Lookup($this->codigo, 'articulo', false, 'codigo', ["codigo","nombre_comercial","principio_activo","presentacion"], '', '', [], [], [], [], [], [], false, '', '', "CONCAT(COALESCE(`codigo`, ''),'" . ValueSeparator(1, $this->codigo) . "',COALESCE(`nombre_comercial`,''),'" . ValueSeparator(2, $this->codigo) . "',COALESCE(`principio_activo`,''),'" . ValueSeparator(3, $this->codigo) . "',COALESCE(`presentacion`,''))");
        ...
        $this->nombre->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "articulo_porcentaje_descuento_temp";
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("ArticuloPorcentajeDescuentoTempList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "ArticuloPorcentajeDescuentoTempView" => $Language->phrase("View"),
            "ArticuloPorcentajeDescuentoTempEdit" => $Language->phrase("Edit"),
            "ArticuloPorcentajeDescuentoTempAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    {
        return "ArticuloPorcentajeDescuentoTempList";
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "ArticuloPorcentajeDescuentoTempView",
            Config("API_ADD_ACTION") => "ArticuloPorcentajeDescuentoTempAdd",
            Config("API_EDIT_ACTION") => "ArticuloPorcentajeDescuentoTempEdit",
            Config("API_DELETE_ACTION") => "ArticuloPorcentajeDescuentoTempDelete",
            Config("API_LIST_ACTION") => "ArticuloPorcentajeDescuentoTempList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("ArticuloPorcentajeDescuentoTempView", $parm);
        } else {
            $url = $this->keyUrl("ArticuloPorcentajeDescuentoTempView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = "ArticuloPorcentajeDescuentoTempAdd?" . $parm;
        } else {
            $url = "ArticuloPorcentajeDescuentoTempAdd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ArticuloPorcentajeDescuentoTempEdit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ArticuloPorcentajeDescuentoTempList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ArticuloPorcentajeDescuentoTempAdd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ArticuloPorcentajeDescuentoTempList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("ArticuloPorcentajeDescuentoTempDelete", $parm);
        }
    }
    ...
    {
        $json = "";
        $json .= "\"codigo\":" . VarToJson($this->codigo->CurrentValue, "string");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        global $Response;
        $listPage = "ArticuloPorcentajeDescuentoTempList";
    ...
    {
        WriteAuditLog(CurrentUserIdentifier(), $typ, 'articulo_porcentaje_descuento_temp');
    }
    ...
    public function writeAuditTrailOnAdd(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "A", 'articulo_porcentaje_descuento_temp', $fldname, $key, "", $newvalue);
            }
        }
    }
        ...
        foreach (array_keys($rsnew) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && array_key_exists($fldname, $rsold) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                if ($modified) {
                    ...
                    WriteAuditLog($usr, "U", 'articulo_porcentaje_descuento_temp', $fldname, $key, $oldvalue, $newvalue);
                }
            }
        }
    }
    ...
    public function writeAuditTrailOnDelete(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "D", 'articulo_porcentaje_descuento_temp', $fldname, $key, $oldvalue);
            }
        }
    }
