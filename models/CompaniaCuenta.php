...
class CompaniaCuenta extends DbTable
{
    ...
    public function __construct()
    {
        ...
        $this->tipo->Lookup = new Lookup($this->tipo, 'compania_cuenta', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
        ...
        // mostrar
        $this->mostrar = new DbField(
            $this, // Table
            'x_mostrar', // Variable name
            'mostrar', // Name
            '`mostrar`', // Expression
            '`mostrar`', // Basic search expression
            200, // Type
            1, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`mostrar`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->mostrar->addMethod("getDefault", fn() => "S");
        $this->mostrar->InputTextType = "text";
        $this->mostrar->Raw = true;
        $this->mostrar->setSelectMultiple(false); // Select one
        ...
        $this->mostrar->Lookup = new Lookup($this->mostrar, 'compania_cuenta', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
        $this->mostrar->OptionCount = 2;
        $this->mostrar->SearchOperators = ["=", "<>", "IS NULL", "IS NOT NULL"];
        ...
        $this->cuenta->addMethod("getSelectFilter", fn() => "codigo LIKE CONCAT((SELECT valor2 FROM parametro WHERE codigo = '018' and valor1 = 'Bancos'), '%')
OR 
	...
	codigo LIKE CONCAT((SELECT valor3 FROM parametro WHERE codigo = '018' and valor1 = 'Caja'), '%')");
        $this->cuenta->addMethod("getDefault", fn() => 0);
        ...
        $this->cuenta->Lookup = new Lookup($this->cuenta, 'view_plancta', false, 'id', ["codigo","descripcion","",""], '', '', [], [], [], [], [], [], false, '`descripcion`', '', "CONCAT(COALESCE(`codigo`, ''),'" . ValueSeparator(1, $this->cuenta) . "',COALESCE(`descripcion`,''))");
        ...
        $this->activo->Lookup = new Lookup($this->activo, 'compania_cuenta', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
    ...
    public function getMasterFilterFromSession()
    {
        ...
        if ($this->getCurrentMasterTable() == "compania") {
            ...
            if ($this->compania->getSessionValue() != "") {
                $masterFilter .= "" . GetKeyFilter($masterTable->id, $this->compania->getSessionValue(), $masterTable->id->DataType, $masterTable->Dbid);
            } else {
                return "";
            }
        }
    ...
    public function getDetailFilterFromSession()
    {
        ...
        if ($this->getCurrentMasterTable() == "compania") {
            $masterTable = Container("compania");
            if ($this->compania->getSessionValue() != "") {
                $detailFilter .= "" . GetKeyFilter($this->compania, $this->compania->getSessionValue(), $masterTable->id->DataType, $this->Dbid);
            } else {
                return "";
            }
        }
    ...
    public function getMasterFilter($masterTable, $keys)
    {
        ...
        switch ($masterTable->TableVar) {
            case "compania":
                ...
                if ($validKeys) {
                    return GetKeyFilter($masterTable->id, $keys["compania"], $this->compania->DataType, $this->Dbid);
                }
                break;
        }
    ...
    public function getDetailFilter($masterTable)
    {
        switch ($masterTable->TableVar) {
            case "compania":
                return GetKeyFilter($this->compania, $masterTable->id->DbValue, $masterTable->id->DataType, $masterTable->Dbid);
        }
        return "";
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "CompaniaCuentaView" => $Language->phrase("View"),
            "CompaniaCuentaEdit" => $Language->phrase("Edit"),
            "CompaniaCuentaAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "CompaniaCuentaView",
            Config("API_ADD_ACTION") => "CompaniaCuentaAdd",
            Config("API_EDIT_ACTION") => "CompaniaCuentaEdit",
            Config("API_DELETE_ACTION") => "CompaniaCuentaDelete",
            Config("API_LIST_ACTION") => "CompaniaCuentaList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("CompaniaCuentaView", $parm);
        } else {
            $url = $this->keyUrl("CompaniaCuentaView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("CompaniaCuentaEdit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("CompaniaCuentaList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("CompaniaCuentaAdd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("CompaniaCuentaList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("CompaniaCuentaDelete", $parm);
        }
    }
    ...
    public function addMasterUrl($url)
    {
        if ($this->getCurrentMasterTable() == "compania" && !ContainsString($url, Config("TABLE_SHOW_MASTER") . "=")) {
            ...
            $url .= "&" . GetForeignKeyUrl("fk_id", $this->compania->getSessionValue()); // Use Session Value
        }
        return $url;
    }
    ...
    {
        WriteAuditLog(CurrentUserIdentifier(), $typ, 'compania_cuenta');
    }
    ...
    public function writeAuditTrailOnAdd(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "A", 'compania_cuenta', $fldname, $key, "", $newvalue);
            }
        }
    }
        ...
        foreach (array_keys($rsnew) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && array_key_exists($fldname, $rsold) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                if ($modified) {
                    ...
                    WriteAuditLog($usr, "U", 'compania_cuenta', $fldname, $key, $oldvalue, $newvalue);
                }
            }
        }
    }
    ...
    public function writeAuditTrailOnDelete(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "D", 'compania_cuenta', $fldname, $key, $oldvalue);
            }
        }
    }
