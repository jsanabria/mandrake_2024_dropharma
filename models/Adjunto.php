...
/**
 * Table class for adjunto
 */
class Adjunto extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->articulo->IsForeignKey = true; // Foreign key field
        ...
        $this->nota->Nullable = false; // NOT NULL field
        $this->nota->Required = true; // Required field
        $this->nota->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY"];
    ...
    public function getMasterFilterFromSession()
    {
        // Master filter
        $masterFilter = "";
        if ($this->getCurrentMasterTable() == "articulo") {
            $masterTable = Container("articulo");
            if ($this->articulo->getSessionValue() != "") {
                $masterFilter .= "" . GetKeyFilter($masterTable->id, $this->articulo->getSessionValue(), $masterTable->id->DataType, $masterTable->Dbid);
            } else {
                return "";
            }
        }
        if ($this->getCurrentMasterTable() == "cliente") {
            ...
            if ($this->cliente->getSessionValue() != "") {
                $masterFilter .= "" . GetKeyFilter($masterTable->id, $this->cliente->getSessionValue(), $masterTable->id->DataType, $masterTable->Dbid);
            } else {
                return "";
            }
        }
    ...
    public function getDetailFilterFromSession()
    {
        ...
        if ($this->getCurrentMasterTable() == "articulo") {
            $masterTable = Container("articulo");
            if ($this->articulo->getSessionValue() != "") {
                $detailFilter .= "" . GetKeyFilter($this->articulo, $this->articulo->getSessionValue(), $masterTable->id->DataType, $this->Dbid);
            } else {
                return "";
            }
        }
        if ($this->getCurrentMasterTable() == "cliente") {
            $masterTable = Container("cliente");
            if ($this->cliente->getSessionValue() != "") {
                $detailFilter .= "" . GetKeyFilter($this->cliente, $this->cliente->getSessionValue(), $masterTable->id->DataType, $this->Dbid);
            } else {
                return "";
            }
        }
    ...
    public function getMasterFilter($masterTable, $keys)
    {
        ...
        switch ($masterTable->TableVar) {
            case "articulo":
                ...
                if ($validKeys) {
                    return GetKeyFilter($masterTable->id, $keys["articulo"], $this->articulo->DataType, $this->Dbid);
                }
                break;
            case "cliente":
                $key = $keys["cliente"] ?? "";
                ...
                if ($validKeys) {
                    return GetKeyFilter($masterTable->id, $keys["cliente"], $this->cliente->DataType, $this->Dbid);
                }
                break;
            case "proveedor":
                $key = $keys["proveedor"] ?? "";
    ...
    public function getDetailFilter($masterTable)
    {
        switch ($masterTable->TableVar) {
            case "articulo":
                return GetKeyFilter($this->articulo, $masterTable->id->DbValue, $masterTable->id->DataType, $masterTable->Dbid);
            case "cliente":
                return GetKeyFilter($this->cliente, $masterTable->id->DbValue, $masterTable->id->DataType, $masterTable->Dbid);
            case "proveedor":
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "adjunto";
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("AdjuntoList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "AdjuntoView" => $Language->phrase("View"),
            "AdjuntoEdit" => $Language->phrase("Edit"),
            "AdjuntoAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "AdjuntoView",
            Config("API_ADD_ACTION") => "AdjuntoAdd",
            Config("API_EDIT_ACTION") => "AdjuntoEdit",
            Config("API_DELETE_ACTION") => "AdjuntoDelete",
            Config("API_LIST_ACTION") => "AdjuntoList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("AdjuntoView", $parm);
        } else {
            $url = $this->keyUrl("AdjuntoView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("AdjuntoEdit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("AdjuntoList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("AdjuntoAdd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("AdjuntoList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("AdjuntoDelete", $parm);
        }
    }
    ...
    {
        if ($this->getCurrentMasterTable() == "articulo" && !ContainsString($url, Config("TABLE_SHOW_MASTER") . "=")) {
            $url .= (ContainsString($url, "?") ? "&" : "?") . Config("TABLE_SHOW_MASTER") . "=" . $this->getCurrentMasterTable();
            $url .= "&" . GetForeignKeyUrl("fk_id", $this->articulo->getSessionValue()); // Use Session Value
        }
        if ($this->getCurrentMasterTable() == "cliente" && !ContainsString($url, Config("TABLE_SHOW_MASTER") . "=")) {
            $url .= (ContainsString($url, "?") ? "&" : "?") . Config("TABLE_SHOW_MASTER") . "=" . $this->getCurrentMasterTable();
            $url .= "&" . GetForeignKeyUrl("fk_id", $this->cliente->getSessionValue()); // Use Session Value
        }
    ...
    public function renderEditRow()
    {
        ...
        if ($this->articulo->getSessionValue() != "") {
            $this->articulo->CurrentValue = GetForeignKeyValue($this->articulo->getSessionValue());
            $this->articulo->ViewValue = $this->articulo->CurrentValue;
        } else {
    ...
    {
        WriteAuditLog(CurrentUserIdentifier(), $typ, 'adjunto');
    }
    ...
    public function writeAuditTrailOnAdd(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "A", 'adjunto', $fldname, $key, "", $newvalue);
            }
        }
    }
        ...
        foreach (array_keys($rsnew) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && array_key_exists($fldname, $rsold) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                if ($modified) {
                    ...
                    WriteAuditLog($usr, "U", 'adjunto', $fldname, $key, $oldvalue, $newvalue);
                }
            }
        }
    }
    ...
    public function writeAuditTrailOnDelete(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "D", 'adjunto', $fldname, $key, $oldvalue);
            }
        }
    }
