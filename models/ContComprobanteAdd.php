 ...
 * Page class
 */
class ContComprobanteAdd extends ContComprobante
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ContComprobanteAdd";
    ...
    // CSS class/style
    public $CurrentPageName = "ContComprobanteAdd";
    ...
    public function setVisibility()
    {
        ...
        $this->contabilizacion->Visible = false;
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$rsold) { // Record not loaded
                    ...
                    $this->terminate("ContComprobanteList"); // No matching record, return to list
                    return;
                }
            ...
            case "insert": // Add new record
                ...
                if ($this->addRow($rsold)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "ContComprobanteList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "ContComprobanteView") {
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "ContComprobanteList") {
                            ...
                            $returnUrl = "ContComprobanteList"; // Return list page content
                        }
                    }
    ...
    public function renderRow()
    {
        ...
        // contabilizacion
        $this->contabilizacion->RowCssClass = "row";
        ...
        // contabiliza
        $this->contabiliza->RowCssClass = "row";

        // fecha_contabiliza
        $this->fecha_contabiliza->RowCssClass = "row";
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            if ($this->tipo->ViewValue !== null) { // Load from cache
                ...
                if ($curVal == "") {
                    ...
                    $filterWrk = SearchFilter($this->tipo->Lookup->getTable()->Fields["tipo_comprobante"]->searchExpression(), "=", $this->tipo->CurrentValue, $this->tipo->Lookup->getTable()->Fields["tipo_comprobante"]->searchDataType(), "");
                }
                $lookupFilter = $this->tipo->getSelectFilter($this); // PHP
    ...
    protected function validateForm()
    {
        ...
        if (in_array("cont_asiento", $detailTblVar) && $detailPage->DetailAdd) {
            $detailPage->run();
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($addRow) {
            $detailTblVar = explode(",", $this->getCurrentDetailTable());
            $detailPage = Container("ContAsientoGrid");
            if (in_array("cont_asiento", $detailTblVar) && $detailPage->DetailAdd && $addRow) {
                $detailPage->comprobante->setSessionValue($this->id->CurrentValue); // Set master key
                $Security->loadCurrentUserLevel($this->ProjectID . "cont_asiento"); // Load user level of detail table
                $addRow = $detailPage->gridInsert();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
                if (!$addRow) {
                $detailPage->comprobante->setSessionValue(""); // Clear master key if insert failed
                }
            }
        }
