 ...
 * Page class
 */
class ViewArticulosList extends ViewArticulos
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ViewArticulosList";
    ...
    public $FormName = "fview_articuloslist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "ViewArticulosList";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'view_articulos';
        $this->TableName = 'view_articulos';
        ...
        // Table object (view_articulos)
        if (!isset($GLOBALS["view_articulos"]) || $GLOBALS["view_articulos"]::class == PROJECT_NAMESPACE . "view_articulos") {
            $GLOBALS["view_articulos"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "ViewArticulosAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "ViewArticulosDelete";
        $this->MultiUpdateUrl = "ViewArticulosUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'view_articulos');
        }
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                    ...
                    $result["view"] = SameString($pageName, "ViewArticulosView"); // If View page, no primary button
                } else { // List page
    ...
    public function run()
    {
        ...
        if ($this->IsModal) {
            $this->FormName = "fview_articulosgrid";
        }
        ...
        if (IsPost() && ($this->isGridUpdate() || $this->isMultiUpdate() || $this->isGridOverwrite()) && (Session(SESSION_INLINE_MODE) == "gridedit" || Session(SESSION_INLINE_MODE) == "multiedit")) {
            ...
            if ($gridUpdate) {
                ...
                if ($this->IsModal && !$this->UseAjaxActions) {
                    $this->terminate("ViewArticulosList");
                    return;
                }
            } else {
    ...
    public function emptyRow()
    {
        global $CurrentForm;
        if (
            $CurrentForm->hasValue("x_codigo") &&
            $CurrentForm->hasValue("o_codigo") &&
            $this->codigo->CurrentValue != $this->codigo->DefaultValue &&
            !($this->codigo->IsForeignKey && $this->getCurrentMasterTable() != "" && $this->codigo->CurrentValue == $this->codigo->getSessionValue())
        ) {
            return false;
        }
        if (
        ...
        if (
            $CurrentForm->hasValue("x_nombre_comercial") &&
            $CurrentForm->hasValue("o_nombre_comercial") &&
            $this->nombre_comercial->CurrentValue != $this->nombre_comercial->DefaultValue &&
            !($this->nombre_comercial->IsForeignKey && $this->getCurrentMasterTable() != "" && $this->nombre_comercial->CurrentValue == $this->nombre_comercial->getSessionValue())
        ) {
            return false;
        }
        if (
            $CurrentForm->hasValue("x_principio_activo") &&
            $CurrentForm->hasValue("o_principio_activo") &&
            $this->principio_activo->CurrentValue != $this->principio_activo->DefaultValue &&
            !($this->principio_activo->IsForeignKey && $this->getCurrentMasterTable() != "" && $this->principio_activo->CurrentValue == $this->principio_activo->getSessionValue())
        ) {
            return false;
        }
        if (
            $CurrentForm->hasValue("x_presentacion") &&
            $CurrentForm->hasValue("o_presentacion") &&
            $this->presentacion->CurrentValue != $this->presentacion->DefaultValue &&
            !($this->presentacion->IsForeignKey && $this->getCurrentMasterTable() != "" && $this->presentacion->CurrentValue == $this->presentacion->getSessionValue())
        ) {
            return false;
        }
        if (
            $CurrentForm->hasValue("x_cantidad_en_mano") &&
            $CurrentForm->hasValue("o_cantidad_en_mano") &&
            $this->cantidad_en_mano->CurrentValue != $this->cantidad_en_mano->DefaultValue &&
            !($this->cantidad_en_mano->IsForeignKey && $this->getCurrentMasterTable() != "" && $this->cantidad_en_mano->CurrentValue == $this->cantidad_en_mano->getSessionValue())
        ) {
            return false;
        }
        if (
            $CurrentForm->hasValue("x_ultimo_costo") &&
            $CurrentForm->hasValue("o_ultimo_costo") &&
            $this->ultimo_costo->CurrentValue != $this->ultimo_costo->DefaultValue &&
            !($this->ultimo_costo->IsForeignKey && $this->getCurrentMasterTable() != "" && $this->ultimo_costo->CurrentValue == $this->ultimo_costo->getSessionValue())
        ) {
            return false;
        }
        return true;
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server") {
            $savedFilterList = Profile()->getSearchFilters("fview_articulossrch");
        }
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fview_articulossrch", $filters);
    ...
    public function showFilterList()
    {
        ...
        // Field ultimo_costo
        $filter = $this->queryBuilderWhere("ultimo_costo");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->ultimo_costo, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->ultimo_costo->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
        if ($this->BasicSearch->Keyword != "") {
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"view_articulos\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-btn=\"SaveBtn\">" . $Language->phrase("EditLink") . "</a>";
                } else {
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fview_articuloslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fview_articuloslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "ultimo_costo");
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fview_articulossrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fview_articulossrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
        ...
        if (!$this->isGridAdd() && !$this->isGridEdit() && !$this->isMultiEdit()) { // Not grid add/grid edit/multi edit mode
            ...
            foreach ($this->ListActions as $listAction) {
                if ($listAction->Select == ACTION_MULTIPLE) {
                    ...
                    $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fview_articuloslist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
            ...
            if ($this->isGridEdit()) {
                ...
                if (!($this->ModalGridEdit && !IsMobile())) {
                    ...
                    $option->UseDropDownButton = false;
                        ...
                        $item->Body = "<button class=\"ew-action ew-grid-save\" title=\"" . HtmlTitle($Language->phrase("GridSaveLink")) . "\" data-caption=\"" . HtmlTitle($Language->phrase("GridSaveLink")) . "\" form=\"fview_articuloslist\" formaction=\"" . GetUrl($this->pageName()) . "\">" . $Language->phrase("GridSaveLink") . "</button>";
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_view_articulos", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_view_articulos",
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'cantidad_en_mano' first before field var 'x_cantidad_en_mano'
        $val = $CurrentForm->hasValue("cantidad_en_mano") ? $CurrentForm->getValue("cantidad_en_mano") : $CurrentForm->getValue("x_cantidad_en_mano");
        if (!$this->cantidad_en_mano->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->cantidad_en_mano->Visible = false; // Disable update for API request
            } else {
                $this->cantidad_en_mano->setFormValue($val);
            }
        }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            $this->cantidad_en_mano->setupEditAttributes();
            $this->cantidad_en_mano->EditValue = $this->cantidad_en_mano->CurrentValue;
            $this->cantidad_en_mano->EditValue = FormatNumber($this->cantidad_en_mano->EditValue, $this->cantidad_en_mano->formatPattern());
            ...
            // ultimo_costo
            $this->ultimo_costo->setupEditAttributes();
            $this->ultimo_costo->EditValue = $this->ultimo_costo->AdvancedSearch->SearchValue;
            $this->ultimo_costo->PlaceHolder = RemoveHtml($this->ultimo_costo->caption());
        }
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            ...
            if ($this->cantidad_en_mano->Visible && $this->cantidad_en_mano->Required) {
                if (!$this->cantidad_en_mano->IsDetailKey && EmptyValue($this->cantidad_en_mano->FormValue)) {
                    $this->cantidad_en_mano->addErrorMessage(str_replace("%s", $this->cantidad_en_mano->caption(), $this->cantidad_en_mano->RequiredErrorMessage));
                }
            }
    ...
    protected function getAddRow()
    {
        ...
        // cantidad_en_mano
        $this->cantidad_en_mano->setDbValueDef($rsnew, $this->cantidad_en_mano->CurrentValue, strval($this->cantidad_en_mano->CurrentValue) == "");

        // ultimo_costo
        $this->ultimo_costo->setDbValueDef($rsnew, $this->ultimo_costo->CurrentValue, strval($this->ultimo_costo->CurrentValue) == "");
        return $rsnew;
    }
    ...
    protected function restoreAddFormFromRow($row)
    {
        ...
        if (isset($row['cantidad_en_mano'])) { // cantidad_en_mano
            $this->cantidad_en_mano->setFormValue($row['cantidad_en_mano']);
        }
        if (isset($row['ultimo_costo'])) { // ultimo_costo
            $this->ultimo_costo->setFormValue($row['ultimo_costo']);
        }
    }
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmail", true) . '" data-caption="' . $Language->phrase("ExportToEmail", true) . '" form="fview_articuloslist" data-ew-action="email" data-custom="false" data-hdr="' . $Language->phrase("ExportToEmail", true) . '" data-exported-selected="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fview_articulossrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
