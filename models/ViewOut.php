...
/**
 * Table class for view_out
 */
class ViewOut extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->check_ne->Lookup = new Lookup($this->check_ne, 'view_out', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
        ...
        $this->newdata->Lookup = new Lookup($this->newdata, 'view_out', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
    ...
    public function getMasterFilterFromSession()
    {
        ...
        if ($this->getCurrentMasterTable() == "view_out_tdcpdv") {
            $masterTable = Container("view_out_tdcpdv");
            if ($this->id_documento->getSessionValue() != "") {
                $masterFilter .= "" . GetKeyFilter($masterTable->id, $this->id_documento->getSessionValue(), $masterTable->id->DataType, $masterTable->Dbid);
            } else {
                return "";
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "view_out";
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("ViewOutList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "ViewOutView" => $Language->phrase("View"),
            "ViewOutEdit" => $Language->phrase("Edit"),
            "ViewOutAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "ViewOutView",
            Config("API_ADD_ACTION") => "ViewOutAdd",
            Config("API_EDIT_ACTION") => "ViewOutEdit",
            Config("API_DELETE_ACTION") => "ViewOutDelete",
            Config("API_LIST_ACTION") => "ViewOutList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("ViewOutView", $parm);
        } else {
            $url = $this->keyUrl("ViewOutView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ViewOutEdit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ViewOutList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ViewOutAdd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ViewOutList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("ViewOutDelete", $parm);
        }
    }
    ...
    public function addMasterUrl($url)
    {
        ...
        if ($this->getCurrentMasterTable() == "view_out_tdcpdv" && !ContainsString($url, Config("TABLE_SHOW_MASTER") . "=")) {
            $url .= (ContainsString($url, "?") ? "&" : "?") . Config("TABLE_SHOW_MASTER") . "=" . $this->getCurrentMasterTable();
            $url .= "&" . GetForeignKeyUrl("fk_id", $this->id_documento->getSessionValue()); // Use Session Value
            $url .= "&" . GetForeignKeyUrl("fk_tipo_documento", $this->tipo_documento->getSessionValue()); // Use Session Value
        }
    ...
    {
        WriteAuditLog(CurrentUserIdentifier(), $typ, 'view_out');
    }
    ...
    public function writeAuditTrailOnAdd(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "A", 'view_out', $fldname, $key, "", $newvalue);
            }
        }
    }
        ...
        foreach (array_keys($rsnew) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && array_key_exists($fldname, $rsold) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                if ($modified) {
                    ...
                    WriteAuditLog($usr, "U", 'view_out', $fldname, $key, $oldvalue, $newvalue);
                }
            }
        }
    }
    ...
    public function writeAuditTrailOnDelete(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "D", 'view_out', $fldname, $key, $oldvalue);
            }
        }
    }
