...
/**
 * Table class for tarifa_anterior
 */
class TarifaAnterior extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        // Update Table
        $this->UpdateTable = "tarifa_anterior";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        $this->tarifa->Lookup = new Lookup($this->tarifa, 'tarifa', false, 'id', ["nombre","","",""], '', '', [], [], [], [], [], [], false, '', '', "`nombre`");
        $this->tarifa->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->tarifa->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN", "IS NULL", "IS NOT NULL"];
        ...
        // precio_anterior
        $this->precio_anterior = new DbField(
            $this, // Table
            'x_precio_anterior', // Variable name
            'precio_anterior', // Name
            '`precio_anterior`', // Expression
            '`precio_anterior`', // Basic search expression
            131, // Type
            15, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`precio_anterior`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->precio_anterior->InputTextType = "text";
        $this->precio_anterior->Raw = true;
        $this->precio_anterior->DefaultErrorMessage = $Language->phrase("IncorrectFloat");
        $this->precio_anterior->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN", "IS NULL", "IS NOT NULL"];
        $this->Fields['precio_anterior'] = &$this->precio_anterior;

        // precio_nuevo
        $this->precio_nuevo = new DbField(
            $this, // Table
            'x_precio_nuevo', // Variable name
            'precio_nuevo', // Name
            '`precio_nuevo`', // Expression
            '`precio_nuevo`', // Basic search expression
            131, // Type
            15, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`precio_nuevo`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->precio_nuevo->InputTextType = "text";
        $this->precio_nuevo->Raw = true;
        $this->precio_nuevo->DefaultErrorMessage = $Language->phrase("IncorrectFloat");
        $this->precio_nuevo->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN", "IS NULL", "IS NOT NULL"];
        $this->Fields['precio_nuevo'] = &$this->precio_nuevo;
        ...
        $this->activo->Lookup = new Lookup($this->activo, 'tarifa_anterior', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "tarifa_anterior";
    }
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->precio_anterior->DbValue = $row['precio_anterior'];
        $this->precio_nuevo->DbValue = $row['precio_nuevo'];
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("TarifaAnteriorList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "TarifaAnteriorView" => $Language->phrase("View"),
            "TarifaAnteriorEdit" => $Language->phrase("Edit"),
            "TarifaAnteriorAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "TarifaAnteriorView",
            Config("API_ADD_ACTION") => "TarifaAnteriorAdd",
            Config("API_EDIT_ACTION") => "TarifaAnteriorEdit",
            Config("API_DELETE_ACTION") => "TarifaAnteriorDelete",
            Config("API_LIST_ACTION") => "TarifaAnteriorList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("TarifaAnteriorView", $parm);
        } else {
            $url = $this->keyUrl("TarifaAnteriorView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = "TarifaAnteriorAdd?" . $parm;
        } else {
            $url = "TarifaAnteriorAdd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("TarifaAnteriorEdit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("TarifaAnteriorList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("TarifaAnteriorAdd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("TarifaAnteriorList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("TarifaAnteriorDelete", $parm);
        }
    }
    ...
    public function renderEditRow()
    {
        ...
        $this->precio_anterior->EditValue = $this->precio_anterior->CurrentValue;
        $this->precio_anterior->PlaceHolder = RemoveHtml($this->precio_anterior->caption());
        if (strval($this->precio_anterior->EditValue) != "" && is_numeric($this->precio_anterior->EditValue)) {
            $this->precio_anterior->EditValue = FormatNumber($this->precio_anterior->EditValue, null);
        }
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->precio_anterior);
                    $doc->exportCaption($this->precio_nuevo);
                } else {
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->precio_anterior);
                        $doc->exportField($this->precio_nuevo);
                    } else {
    ...
    {
        WriteAuditLog(CurrentUserIdentifier(), $typ, 'tarifa_anterior');
    }
    ...
    public function writeAuditTrailOnAdd(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "A", 'tarifa_anterior', $fldname, $key, "", $newvalue);
            }
        }
    }
        ...
        foreach (array_keys($rsnew) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && array_key_exists($fldname, $rsold) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                if ($modified) {
                    ...
                    WriteAuditLog($usr, "U", 'tarifa_anterior', $fldname, $key, $oldvalue, $newvalue);
                }
            }
        }
    }
    ...
    public function writeAuditTrailOnDelete(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "D", 'tarifa_anterior', $fldname, $key, $oldvalue);
            }
        }
    }
