...
class ProveedorList extends Proveedor
{
    ...
    // CSS class/style
    public $CurrentPageName = "ProveedorList";
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "ProveedorAdd?" . Config("TABLE_SHOW_DETAIL") . "=";
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fproveedorsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field cuenta_auxiliar
        $this->cuenta_auxiliar->AdvancedSearch->SearchValue = @$filter["x_cuenta_auxiliar"];
        $this->cuenta_auxiliar->AdvancedSearch->SearchOperator = @$filter["z_cuenta_auxiliar"];
        $this->cuenta_auxiliar->AdvancedSearch->SearchCondition = @$filter["v_cuenta_auxiliar"];
        $this->cuenta_auxiliar->AdvancedSearch->SearchValue2 = @$filter["y_cuenta_auxiliar"];
        $this->cuenta_auxiliar->AdvancedSearch->SearchOperator2 = @$filter["w_cuenta_auxiliar"];
        $this->cuenta_auxiliar->AdvancedSearch->save();

        // Field cuenta_gasto
        $this->cuenta_gasto->AdvancedSearch->SearchValue = @$filter["x_cuenta_gasto"];
        $this->cuenta_gasto->AdvancedSearch->SearchOperator = @$filter["z_cuenta_gasto"];
        $this->cuenta_gasto->AdvancedSearch->SearchCondition = @$filter["v_cuenta_gasto"];
        $this->cuenta_gasto->AdvancedSearch->SearchValue2 = @$filter["y_cuenta_gasto"];
        ...
        $this->tipo_ret_iva->AdvancedSearch->SearchCondition = @$filter["v_tipo_ret_iva"];
        $this->tipo_ret_iva->AdvancedSearch->SearchValue2 = @$filter["y_tipo_ret_iva"];
        $this->tipo_ret_iva->AdvancedSearch->SearchOperator2 = @$filter["w_tipo_ret_iva"];
        $this->tipo_ret_iva->AdvancedSearch->save();

        // Field tipo_ret_islr_concepto
        $this->tipo_ret_islr_concepto->AdvancedSearch->SearchValue = @$filter["x_tipo_ret_islr_concepto"];
        $this->tipo_ret_islr_concepto->AdvancedSearch->SearchOperator = @$filter["z_tipo_ret_islr_concepto"];
        $this->tipo_ret_islr_concepto->AdvancedSearch->SearchCondition = @$filter["v_tipo_ret_islr_concepto"];
        $this->tipo_ret_islr_concepto->AdvancedSearch->SearchValue2 = @$filter["y_tipo_ret_islr_concepto"];
        $this->tipo_ret_islr_concepto->AdvancedSearch->SearchOperator2 = @$filter["w_tipo_ret_islr_concepto"];
        $this->tipo_ret_islr_concepto->AdvancedSearch->save();

        // Field tipo_ret_islr
        $this->tipo_ret_islr->AdvancedSearch->SearchValue = @$filter["x_tipo_ret_islr"];
        $this->tipo_ret_islr->AdvancedSearch->SearchOperator = @$filter["z_tipo_ret_islr"];
        $this->tipo_ret_islr->AdvancedSearch->SearchCondition = @$filter["v_tipo_ret_islr"];
        $this->tipo_ret_islr->AdvancedSearch->SearchValue2 = @$filter["y_tipo_ret_islr"];
        $this->tipo_ret_islr->AdvancedSearch->SearchOperator2 = @$filter["w_tipo_ret_islr"];
        $this->tipo_ret_islr->AdvancedSearch->save();

        // Field tipo_ret_mun
        $this->tipo_ret_mun->AdvancedSearch->SearchValue = @$filter["x_tipo_ret_mun"];
        $this->tipo_ret_mun->AdvancedSearch->SearchOperator = @$filter["z_tipo_ret_mun"];
        $this->tipo_ret_mun->AdvancedSearch->SearchCondition = @$filter["v_tipo_ret_mun"];
        ...
        $this->tipo_impmun->AdvancedSearch->SearchOperator = @$filter["z_tipo_impmun"];
        $this->tipo_impmun->AdvancedSearch->SearchCondition = @$filter["v_tipo_impmun"];
        $this->tipo_impmun->AdvancedSearch->SearchValue2 = @$filter["y_tipo_impmun"];
        $this->tipo_impmun->AdvancedSearch->SearchOperator2 = @$filter["w_tipo_impmun"];
        $this->tipo_impmun->AdvancedSearch->save();

        // Field cta_bco
        $this->cta_bco->AdvancedSearch->SearchValue = @$filter["x_cta_bco"];
        $this->cta_bco->AdvancedSearch->SearchOperator = @$filter["z_cta_bco"];
        $this->cta_bco->AdvancedSearch->SearchCondition = @$filter["v_cta_bco"];
        $this->cta_bco->AdvancedSearch->SearchValue2 = @$filter["y_cta_bco"];
        $this->cta_bco->AdvancedSearch->SearchOperator2 = @$filter["w_cta_bco"];
        $this->cta_bco->AdvancedSearch->save();
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->cuenta_auxiliar->AdvancedSearch->issetSession()) {
            return true;
        }
        ...
        if ($this->tipo_ret_islr_concepto->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->tipo_ret_islr->AdvancedSearch->issetSession()) {
            return true;
        }
        ...
        if ($this->cta_bco->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    protected function setupListOptions()
    {
        ...
        // "detail_proveedor_articulo"
        $item = &$this->ListOptions->add("detail_proveedor_articulo");
        $item->CssClass = "text-nowrap";
        $item->Visible = $Security->allowList(CurrentProjectID() . 'proveedor_articulo');
        $item->OnLeft = true;
    ...
    public function renderListOptions()
    {
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"proveedor\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $Language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($Security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"proveedor\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-btn=\"SaveBtn\">" . $Language->phrase("EditLink") . "</a>";
                } else {
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fproveedorlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fproveedorlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
        ...
        // "detail_proveedor_articulo"
        $opt = $this->ListOptions["detail_proveedor_articulo"];
        if ($Security->allowList(CurrentProjectID() . 'proveedor_articulo')) {
            $body = $Language->phrase("DetailLink") . $Language->tablePhrase("proveedor_articulo", "TblCaption");
            $body = "<a class=\"btn btn-default ew-row-link ew-detail" . ($this->ListOptions->UseDropDownButton ? " dropdown-toggle" : "") . "\" data-action=\"list\" href=\"" . HtmlEncode("ProveedorArticuloList?" . Config("TABLE_SHOW_MASTER") . "=proveedor&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("ProveedorArticuloGrid");
            if ($detailPage->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'proveedor')) {
                $caption = $Language->phrase("MasterDetailViewLink", null);
                $url = $this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=proveedor_articulo");
                $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode($url) . "\">" . $caption . "</a></li>";
                if ($detailViewTblVar != "") {
                    $detailViewTblVar .= ",";
                }
                $detailViewTblVar .= "proveedor_articulo";
            }
            if ($detailPage->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'proveedor')) {
                $caption = $Language->phrase("MasterDetailEditLink", null);
                $url = $this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=proveedor_articulo");
                ...
                $detailEditTblVar .= "proveedor_articulo";
            }
            if ($links != "") {
        ...
        if ($Security->allowList(CurrentProjectID() . 'adjunto')) {
            ...
            $body = "<a class=\"btn btn-default ew-row-link ew-detail" . ($this->ListOptions->UseDropDownButton ? " dropdown-toggle" : "") . "\" data-action=\"list\" href=\"" . HtmlEncode("AdjuntoList?" . Config("TABLE_SHOW_MASTER") . "=proveedor&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
    ...
    protected function renderListOptionsExt()
    {
        ...
        // Column "detail_proveedor_articulo"
        if ($this->DetailPages?->getItem("proveedor_articulo")?->Visible && $Security->allowList(CurrentProjectID() . 'proveedor_articulo')) {
            $link = "";
            $option = $this->ListOptions["detail_proveedor_articulo"];
            $detailTbl = Container("proveedor_articulo");
            $detailFilter = $detailTbl->getDetailFilter($this);
            $detailTbl->setCurrentMasterTable($this->TableVar);
            $detailFilter = $detailTbl->applyUserIDFilters($detailFilter);
            $url = "ProveedorArticuloPreview?t=proveedor&f=" . Encrypt($detailFilter . "|" . implode("|", array_values($masterKeys)));
            $btngrp = "<div data-table=\"proveedor_articulo\" data-url=\"" . $url . "\" class=\"ew-detail-btn-group btn-group btn-group-sm d-none\">";
            if ($Security->allowList(CurrentProjectID() . 'proveedor')) {
                $label = $Language->tablePhrase("proveedor_articulo", "TblCaption");
                $link = "<button class=\"nav-link\" data-bs-toggle=\"tab\" data-table=\"proveedor_articulo\" data-url=\"" . $url . "\" type=\"button\" role=\"tab\" aria-selected=\"false\">" . $label . "</button>";
                $detaillnk = GetUrl("ProveedorArticuloList?" . Config("TABLE_SHOW_MASTER") . "=proveedor");
                foreach ($masterKeys as $key => $value) {
                    $detaillnk .= "&" . GetForeignKeyUrl("fk_$key", $value);
                }
                $title = $Language->tablePhrase("proveedor_articulo", "TblCaption");
            ...
            if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'proveedor')) {
                $caption = $Language->phrase("MasterDetailViewLink");
                $viewurl = GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=proveedor_articulo"));
            ...
            if ($detailPageObj->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'proveedor')) {
                $caption = $Language->phrase("MasterDetailEditLink");
                $editurl = GetUrl($this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=proveedor_articulo"));
        ...
        if ($this->DetailPages?->getItem("adjunto")?->Visible && $Security->allowList(CurrentProjectID() . 'adjunto')) {
            ...
            $detailTbl->setCurrentMasterTable($this->TableVar);
            $detailFilter = $detailTbl->applyUserIDFilters($detailFilter);
            $url = "AdjuntoPreview?t=proveedor&f=" . Encrypt($detailFilter . "|" . implode("|", array_values($masterKeys)));
            ...
            if ($Security->allowList(CurrentProjectID() . 'proveedor')) {
                ...
                $detaillnk = GetUrl("AdjuntoList?" . Config("TABLE_SHOW_MASTER") . "=proveedor");
                foreach ($masterKeys as $key => $value) {
                    $detaillnk .= "&" . GetForeignKeyUrl("fk_$key", $value);
                }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
            $item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"proveedor\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("AddLink") . "</a>";
        } else {
        ...
        $detailTableLink = "";
        $item = &$option->add("detailadd_proveedor_articulo");
        $url = $this->getAddUrl(Config("TABLE_SHOW_DETAIL") . "=proveedor_articulo");
        $detailPage = Container("ProveedorArticuloGrid");
        $caption = $Language->phrase("Add") . "&nbsp;" . $this->tableCaption() . "/" . $detailPage->tableCaption();
        $item->Body = "<a class=\"ew-detail-add-group ew-detail-add\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode(GetUrl($url)) . "\">" . $caption . "</a>";
        $item->Visible = ($detailPage->DetailAdd && $Security->allowAdd(CurrentProjectID() . 'proveedor') && $Security->canAdd());
        if ($item->Visible) {
            if ($detailTableLink != "") {
                $detailTableLink .= ",";
            }
            $detailTableLink .= "proveedor_articulo";
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fproveedorsrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fproveedorsrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions as $listAction) {
            if ($listAction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fproveedorlist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_proveedor", "data-rowtype" => RowType::ADD]);
    ...
    protected function loadSearchValues()
    {
        ...
        // cuenta_auxiliar
        if ($this->cuenta_auxiliar->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->cuenta_auxiliar->AdvancedSearch->SearchValue != "" || $this->cuenta_auxiliar->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // cuenta_gasto
        if ($this->cuenta_gasto->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->cuenta_gasto->AdvancedSearch->SearchValue != "" || $this->cuenta_gasto->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        if ($this->tipo_ret_iva->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->tipo_ret_iva->AdvancedSearch->SearchValue != "" || $this->tipo_ret_iva->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // tipo_ret_islr_concepto
        if ($this->tipo_ret_islr_concepto->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->tipo_ret_islr_concepto->AdvancedSearch->SearchValue != "" || $this->tipo_ret_islr_concepto->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // tipo_ret_islr
        if ($this->tipo_ret_islr->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->tipo_ret_islr->AdvancedSearch->SearchValue != "" || $this->tipo_ret_islr->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        // tipo_impmun
        if ($this->tipo_impmun->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->tipo_impmun->AdvancedSearch->SearchValue != "" || $this->tipo_impmun->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // cta_bco
        if ($this->cta_bco->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->cta_bco->AdvancedSearch->SearchValue != "" || $this->cta_bco->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmail", true) . '" data-caption="' . $Language->phrase("ExportToEmail", true) . '" form="fproveedorlist" data-ew-action="email" data-custom="false" data-hdr="' . $Language->phrase("ExportToEmail", true) . '" data-exported-selected="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fproveedorsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
