 ...
 * Page class
 */
class ViewBultosList extends ViewBultos
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ViewBultosList";
    ...
    public $FormName = "fview_bultoslist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "ViewBultosList";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'view_bultos';
        $this->TableName = 'view_bultos';
        ...
        // Table object (view_bultos)
        if (!isset($GLOBALS["view_bultos"]) || $GLOBALS["view_bultos"]::class == PROJECT_NAMESPACE . "view_bultos") {
            $GLOBALS["view_bultos"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "ViewBultosAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "ViewBultosDelete";
        $this->MultiUpdateUrl = "ViewBultosUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'view_bultos');
        }
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                    ...
                    $result["view"] = SameString($pageName, "ViewBultosView"); // If View page, no primary button
                } else { // List page
    ...
    public function run()
    {
        ...
        if ($this->IsModal) {
            $this->FormName = "fview_bultosgrid";
        }
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fview_bultossrch", $filters);
    ...
    protected function setupSortOrder()
    {
        ...
        if ($this->Command != "json") {
            $defaultSort = $this->fecha_despacho->Expression . " DESC"; // Set up default sort
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->user_despacho); // user_despacho
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fview_bultoslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fview_bultoslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "user_despacho");
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fview_bultossrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fview_bultossrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions as $listAction) {
            if ($listAction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fview_bultoslist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_view_bultos", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_view_bultos",
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            $this->fecha_despacho->CssClass = "fw-bold fst-italic";
            ...
            $this->user_despacho->CssClass = "fw-bold fst-italic";
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmail", true) . '" data-caption="' . $Language->phrase("ExportToEmail", true) . '" form="fview_bultoslist" data-ew-action="email" data-custom="false" data-hdr="' . $Language->phrase("ExportToEmail", true) . '" data-exported-selected="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fview_bultossrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    */
    ...
    public function listOptionsRendered() {
    	...
    	if(trim($this->user_despacho->CurrentValue) == "") {
    		//$this->ListOptions->Items["new"]->Body = '<input type="checkbox" value="S" id="chkEntregar" name="chkEntregar" onclick="js:entregar(' . $this->id->CurrentValue . ')">';
    		//$this->ListOptions->Items["new"]->Body = '<a class="btn btn-primary" onclick="js:entregar(' . $this->id->CurrentValue . ')">Entregar</a>';
    		$this->ListOptions->Items["new"]->Body = '<a class="btn btn-primary" href="include/marcar_bulto.php?id=' . $this->id->CurrentValue . '&username=' . CurrentUserName() . '">Entregar</a>';
    	}
    	else {
    		//$this->ListOptions->Items["new"]->Body = '<i class="fa fa-check"></i>';
    		$this->ListOptions->Items["new"]->Body = '<a class="btn btn-success"><i class="fa fa-check"></i></a>';		
    	}
    }
