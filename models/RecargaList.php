...
class RecargaList extends Recarga
{
    ...
    // CSS class/style
    public $CurrentPageName = "RecargaList";
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("frecargasrch", $filters);
    ...
    public function showFilterList()
    {
        ...
        // Field metodo_pago
        $filter = $this->queryBuilderWhere("metodo_pago");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->metodo_pago, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->metodo_pago->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
        ...
        // Field monto_moneda
        $filter = $this->queryBuilderWhere("monto_moneda");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->monto_moneda, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->monto_moneda->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
        ...
        // Field saldo
        $filter = $this->queryBuilderWhere("saldo");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->saldo, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->saldo->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"frecargalist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"frecargalist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"frecargasrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"frecargasrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions as $listAction) {
            if ($listAction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="frecargalist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
    ...
    protected function loadSearchValues()
    {
        ...
        // nro_recibo
        if ($this->nro_recibo->AdvancedSearch->get()) {
            $hasValue = true;
        ...
        // metodo_pago
        if ($this->metodo_pago->AdvancedSearch->get()) {
            $hasValue = true;
        ...
        // reverso
        if ($this->reverso->AdvancedSearch->get()) {
            $hasValue = true;
        ...
        // monto_moneda
        if ($this->monto_moneda->AdvancedSearch->get()) {
            $hasValue = true;
        ...
        // tasa_moneda
        if ($this->tasa_moneda->AdvancedSearch->get()) {
            $hasValue = true;
        ...
        // saldo
        if ($this->saldo->AdvancedSearch->get()) {
            $hasValue = true;
        ...
        if ($this->cobro_cliente_reverso->AdvancedSearch->get()) {
            $hasValue = true;
        ...
        // nota_recepcion
        if ($this->nota_recepcion->AdvancedSearch->get()) {
            $hasValue = true;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            if (!$this->metodo_pago->Raw) {
                $this->metodo_pago->AdvancedSearch->SearchValue = HtmlDecode($this->metodo_pago->AdvancedSearch->SearchValue);
            }
            $this->metodo_pago->EditValue = HtmlEncode($this->metodo_pago->AdvancedSearch->SearchValue);
            $this->metodo_pago->PlaceHolder = RemoveHtml($this->metodo_pago->caption());

            // banco
            $this->banco->setupEditAttributes();
            $this->banco->EditValue = $this->banco->AdvancedSearch->SearchValue;
            ...
            $this->monto_moneda->EditValue = $this->monto_moneda->AdvancedSearch->SearchValue;
            ...
            $this->saldo->EditValue = $this->saldo->AdvancedSearch->SearchValue;
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmail", true) . '" data-caption="' . $Language->phrase("ExportToEmail", true) . '" form="frecargalist" data-ew-action="email" data-custom="false" data-hdr="' . $Language->phrase("ExportToEmail", true) . '" data-exported-selected="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"frecargasrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    */
    public function exportData($doc)
    {
        ...
        if (Config("EXPORT_MASTER_RECORD") && $this->DbMasterFilter != "" && $this->getCurrentMasterTable() == "abono") {
            $abono = new AbonoList();
            ...
            if ($rsmaster) {
                ...
                if (!$this->isExport("csv") || Config("EXPORT_MASTER_RECORD_FOR_CSV")) {
                    $doc->setTable($abono);
                    $abono->exportDocument($doc, $rsmaster);
                    $doc->exportEmptyRow();
                    $doc->setTable($this);
                }
