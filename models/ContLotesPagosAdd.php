 ...
 * Page class
 */
class ContLotesPagosAdd extends ContLotesPagos
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ContLotesPagosAdd";
    ...
    // CSS class/style
    public $CurrentPageName = "ContLotesPagosAdd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$rsold) { // Record not loaded
                    ...
                    $this->terminate("ContLotesPagosList"); // No matching record, return to list
                    return;
                }
            ...
            case "insert": // Add new record
                ...
                if ($this->addRow($rsold)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "ContLotesPagosList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "ContLotesPagosView") {
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "ContLotesPagosList") {
                            ...
                            $returnUrl = "ContLotesPagosList"; // Return list page content
                        }
                    }
    ...
    protected function validateForm()
    {
        ...
        if (in_array("cont_lotes_pagos_detalle", $detailTblVar) && $detailPage->DetailAdd) {
            $detailPage->run();
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($addRow) {
            $detailTblVar = explode(",", $this->getCurrentDetailTable());
            $detailPage = Container("ContLotesPagosDetalleGrid");
            if (in_array("cont_lotes_pagos_detalle", $detailTblVar) && $detailPage->DetailAdd && $addRow) {
                $detailPage->cont_lotes_pago->setSessionValue($this->id->CurrentValue); // Set master key
                $Security->loadCurrentUserLevel($this->ProjectID . "cont_lotes_pagos_detalle"); // Load user level of detail table
                $addRow = $detailPage->gridInsert();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
                if (!$addRow) {
                $detailPage->cont_lotes_pago->setSessionValue(""); // Clear master key if insert failed
                }
            }
        }
