 ...
 * Page class
 */
class PagosProveedorView extends PagosProveedor
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PagosProveedorView";
    ...
    // CSS class/style
    public $CurrentPageName = "PagosProveedorView";
    ...
    public function run()
    {
        ...
        if (($keyValue = Get("id") ?? Route("id")) !== null) {
            ...
            $returnUrl = "PagosProveedorList"; // Return to list
        }
            ...
            case "show": // Get a record to display
                if (!$this->IsModal && !IsApi()) { // Normal view page
                    ...
                    if ($this->TotalRecords <= 0) { // No record found
                        ...
                        $this->terminate("PagosProveedorList"); // Return to list page
                        return;
                    ...
                    if (!$matchRecord) {
                        ...
                        $returnUrl = "PagosProveedorList"; // No matching record, return to list
                    } else {
    ...
    protected function setupOtherOptions()
    {
        ...
        */
        ...
        $item = &$option->add("detail_pagos_proveedor_factura");
        $body = $Language->phrase("ViewPageDetailLink") . $Language->tablePhrase("pagos_proveedor_factura", "TblCaption");
        $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode(GetUrl("PagosProveedorFacturaList?" . Config("TABLE_SHOW_MASTER") . "=pagos_proveedor&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "")) . "\">" . $body . "</a>";
        $links = "";
        ...
        if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'pagos_proveedor')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailViewLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=pagos_proveedor_factura"))) . "\">" . $Language->phrase("MasterDetailViewLink", null) . "</a></li>";
            if ($detailViewTblVar != "") {
                $detailViewTblVar .= ",";
            }
        ...
        if ($detailPageObj->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'pagos_proveedor')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-edit\" data-action=\"edit\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailEditLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=pagos_proveedor_factura"))) . "\">" . $Language->phrase("MasterDetailEditLink", null) . "</a></li>";
            if ($detailEditTblVar != "") {
                $detailEditTblVar .= ",";
            }
    ...
    public function pageDataRendered(&$footer) {
    	// Example:
    	$sql = "SELECT COUNT(id_documento) AS cantidad FROM pagos_proveedor_factura WHERE pagos_proveedor = " . $this->id->CurrentValue . ";";
    	$cantidad = ExecuteScalar($sql);
    	$footer = '<div class="alert alert-success" role="success">
    					Facturas asocialdas al pago ' . $cantidad . '
    				</div>';
    }
