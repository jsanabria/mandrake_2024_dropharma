 ...
 * Page class
 */
class CompaniaList extends Compania
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "CompaniaList";
    ...
    public $FormName = "fcompanialist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "CompaniaList";
    ...
    public function setVisibility()
    {
        ...
        $this->agente_retencion->setVisibility();
    ...
    public function __construct()
    {
        ...
        // Table object (compania)
        if (!isset($GLOBALS["compania"]) || $GLOBALS["compania"]::class == PROJECT_NAMESPACE . "compania") {
            $GLOBALS["compania"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "CompaniaAdd?" . Config("TABLE_SHOW_DETAIL") . "=";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "CompaniaDelete";
        $this->MultiUpdateUrl = "CompaniaUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'compania');
        }
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                    ...
                    $result["view"] = SameString($pageName, "CompaniaView"); // If View page, no primary button
                } else { // List page
    ...
    public function run()
    {
        ...
        $this->setupLookupOptions($this->agente_retencion);
        $this->setupLookupOptions($this->activo);

        // Update form name to avoid conflict
        if ($this->IsModal) {
            $this->FormName = "fcompaniagrid";
        }
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server") {
            $savedFilterList = Profile()->getSearchFilters("fcompaniasrch");
        }
        ...
        $filterList = Concat($filterList, $this->agente_retencion->AdvancedSearch->toJson(), ","); // Field agente_retencion
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fcompaniasrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field agente_retencion
        $this->agente_retencion->AdvancedSearch->SearchValue = @$filter["x_agente_retencion"];
        $this->agente_retencion->AdvancedSearch->SearchOperator = @$filter["z_agente_retencion"];
        $this->agente_retencion->AdvancedSearch->SearchCondition = @$filter["v_agente_retencion"];
        $this->agente_retencion->AdvancedSearch->SearchValue2 = @$filter["y_agente_retencion"];
        $this->agente_retencion->AdvancedSearch->SearchOperator2 = @$filter["w_agente_retencion"];
        $this->agente_retencion->AdvancedSearch->save();
    ...
    public function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->agente_retencion, $default, false); // agente_retencion
        ...
        if (!$default && $this->Command == "search") {
            ...
            $this->agente_retencion->AdvancedSearch->save(); // agente_retencion
    ...
    public function showFilterList()
    {
        ...
        // Field email1
        $filter = $this->queryBuilderWhere("email1");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->email1, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->email1->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }

        // Field agente_retencion
        $filter = $this->queryBuilderWhere("agente_retencion");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->agente_retencion, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->agente_retencion->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }

        // Field logo
        $filter = $this->queryBuilderWhere("logo");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->logo, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->logo->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
        if ($this->BasicSearch->Keyword != "") {
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->agente_retencion->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    protected function resetAdvancedSearchParms()
    {
        ...
        $this->agente_retencion->AdvancedSearch->unsetSession();
    ...
    protected function restoreSearchParms()
    {
        ...
        $this->agente_retencion->AdvancedSearch->load();
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->agente_retencion); // agente_retencion
    ...
    protected function setupListOptions()
    {
        ...
        // "detail_compania_cuenta"
        $item = &$this->ListOptions->add("detail_compania_cuenta");
        $item->CssClass = "text-nowrap";
        $item->Visible = $Security->allowList(CurrentProjectID() . 'compania_cuenta');
        $item->OnLeft = true;
    ...
    public function renderListOptions()
    {
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"compania\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $Language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($Security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"compania\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-btn=\"SaveBtn\">" . $Language->phrase("EditLink") . "</a>";
                } else {
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fcompanialist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fcompanialist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
        ...
        // "detail_compania_cuenta"
        $opt = $this->ListOptions["detail_compania_cuenta"];
        if ($Security->allowList(CurrentProjectID() . 'compania_cuenta')) {
            $body = $Language->phrase("DetailLink") . $Language->tablePhrase("compania_cuenta", "TblCaption");
            $body = "<a class=\"btn btn-default ew-row-link ew-detail" . ($this->ListOptions->UseDropDownButton ? " dropdown-toggle" : "") . "\" data-action=\"list\" href=\"" . HtmlEncode("CompaniaCuentaList?" . Config("TABLE_SHOW_MASTER") . "=compania&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("CompaniaCuentaGrid");
            if ($detailPage->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'compania')) {
                $caption = $Language->phrase("MasterDetailViewLink", null);
                $url = $this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=compania_cuenta");
                $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode($url) . "\">" . $caption . "</a></li>";
                if ($detailViewTblVar != "") {
                    $detailViewTblVar .= ",";
                }
                $detailViewTblVar .= "compania_cuenta";
            }
            if ($detailPage->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'compania')) {
                $caption = $Language->phrase("MasterDetailEditLink", null);
                $url = $this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=compania_cuenta");
                ...
                $detailEditTblVar .= "compania_cuenta";
            }
            if ($links != "") {
    ...
    protected function renderListOptionsExt()
    {
        ...
        // Column "detail_compania_cuenta"
        if ($this->DetailPages?->getItem("compania_cuenta")?->Visible && $Security->allowList(CurrentProjectID() . 'compania_cuenta')) {
            $link = "";
            $option = $this->ListOptions["detail_compania_cuenta"];
            $detailTbl = Container("compania_cuenta");
            $detailFilter = $detailTbl->getDetailFilter($this);
            $detailTbl->setCurrentMasterTable($this->TableVar);
            $detailFilter = $detailTbl->applyUserIDFilters($detailFilter);
            $url = "CompaniaCuentaPreview?t=compania&f=" . Encrypt($detailFilter . "|" . implode("|", array_values($masterKeys)));
            $btngrp = "<div data-table=\"compania_cuenta\" data-url=\"" . $url . "\" class=\"ew-detail-btn-group btn-group btn-group-sm d-none\">";
            if ($Security->allowList(CurrentProjectID() . 'compania')) {
                $label = $Language->tablePhrase("compania_cuenta", "TblCaption");
                $link = "<button class=\"nav-link\" data-bs-toggle=\"tab\" data-table=\"compania_cuenta\" data-url=\"" . $url . "\" type=\"button\" role=\"tab\" aria-selected=\"false\">" . $label . "</button>";
                $detaillnk = GetUrl("CompaniaCuentaList?" . Config("TABLE_SHOW_MASTER") . "=compania");
                foreach ($masterKeys as $key => $value) {
                    $detaillnk .= "&" . GetForeignKeyUrl("fk_$key", $value);
                }
                $title = $Language->tablePhrase("compania_cuenta", "TblCaption");
            ...
            $detailPageObj = Container("CompaniaCuentaGrid");
            if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'compania')) {
                $caption = $Language->phrase("MasterDetailViewLink");
                $viewurl = GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=compania_cuenta"));
            ...
            if ($detailPageObj->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'compania')) {
                $caption = $Language->phrase("MasterDetailEditLink");
                $editurl = GetUrl($this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=compania_cuenta"));
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "email1");
            $this->createColumnOption($option, "agente_retencion");
            $this->createColumnOption($option, "logo");
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fcompaniasrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fcompaniasrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions as $listAction) {
            if ($listAction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fcompanialist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_compania", "data-rowtype" => RowType::ADD]);
    ...
    protected function loadSearchValues()
    {
        ...
        // agente_retencion
        if ($this->agente_retencion->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->agente_retencion->AdvancedSearch->SearchValue != "" || $this->agente_retencion->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
    ...
    protected function newRow()
    {
        ...
        $row['agente_retencion'] = $this->agente_retencion->DefaultValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            if ($this->logo->UseColorbox) {
                ...
                $this->logo->LinkAttrs["data-rel"] = "compania_x" . $this->RowCount . "_logo";
            ...
            if (!$this->email1->Raw) {
                $this->email1->AdvancedSearch->SearchValue = HtmlDecode($this->email1->AdvancedSearch->SearchValue);
            }
            $this->email1->EditValue = HtmlEncode($this->email1->AdvancedSearch->SearchValue);
            $this->email1->PlaceHolder = RemoveHtml($this->email1->caption());

            // agente_retencion
            $this->agente_retencion->setupEditAttributes();
            ...
            if (!$this->logo->Raw) {
                $this->logo->AdvancedSearch->SearchValue = HtmlDecode($this->logo->AdvancedSearch->SearchValue);
            }
            $this->logo->EditValue = HtmlEncode($this->logo->AdvancedSearch->SearchValue);
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmail", true) . '" data-caption="' . $Language->phrase("ExportToEmail", true) . '" form="fcompanialist" data-ew-action="email" data-custom="false" data-hdr="' . $Language->phrase("ExportToEmail", true) . '" data-exported-selected="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fcompaniasrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
