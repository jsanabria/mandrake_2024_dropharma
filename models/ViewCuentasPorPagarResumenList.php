 ...
 * Page class
 */
class ViewCuentasPorPagarResumenList extends ViewCuentasPorPagarResumen
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ViewCuentasPorPagarResumenList";
    ...
    public $FormName = "fview_cuentas_por_pagar_resumenlist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "ViewCuentasPorPagarResumenList";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'view_cuentas_por_pagar_resumen';
        $this->TableName = 'view_cuentas_por_pagar_resumen';
        ...
        // Table object (view_cuentas_por_pagar_resumen)
        if (!isset($GLOBALS["view_cuentas_por_pagar_resumen"]) || $GLOBALS["view_cuentas_por_pagar_resumen"]::class == PROJECT_NAMESPACE . "view_cuentas_por_pagar_resumen") {
            $GLOBALS["view_cuentas_por_pagar_resumen"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "ViewCuentasPorPagarResumenAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "ViewCuentasPorPagarResumenDelete";
        $this->MultiUpdateUrl = "ViewCuentasPorPagarResumenUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'view_cuentas_por_pagar_resumen');
        }
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                    ...
                    $result["view"] = SameString($pageName, "ViewCuentasPorPagarResumenView"); // If View page, no primary button
                } else { // List page
    ...
    public function run()
    {
        ...
        if ($this->IsModal) {
            $this->FormName = "fview_cuentas_por_pagar_resumengrid";
        }
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server") {
            $savedFilterList = Profile()->getSearchFilters("fview_cuentas_por_pagar_resumensrch");
        }
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fview_cuentas_por_pagar_resumensrch", $filters);
    ...
    public function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->proveedor_rif, $default, false); // proveedor_rif
        $this->buildSearchSql($where, $this->proveedor_nombre, $default, false); // proveedor_nombre
        ...
        $this->buildSearchSql($where, $this->total_pagado_bs, $default, false); // total_pagado_bs
        $this->buildSearchSql($where, $this->total_pagado_usd, $default, false); // total_pagado_usd
        ...
        if (!$default && $this->Command == "search") {
            ...
            $this->proveedor_rif->AdvancedSearch->save(); // proveedor_rif
            $this->proveedor_nombre->AdvancedSearch->save(); // proveedor_nombre
            ...
            $this->total_pagado_bs->AdvancedSearch->save(); // total_pagado_bs
            $this->total_pagado_usd->AdvancedSearch->save(); // total_pagado_usd
    ...
    public function showFilterList()
    {
        ...
        // Field proveedor_rif
        $filter = $this->queryBuilderWhere("proveedor_rif");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->proveedor_rif, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->proveedor_rif->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }

        // Field proveedor_nombre
        $filter = $this->queryBuilderWhere("proveedor_nombre");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->proveedor_nombre, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->proveedor_nombre->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
        ...
        // Field total_pagado_bs
        $filter = $this->queryBuilderWhere("total_pagado_bs");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->total_pagado_bs, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->total_pagado_bs->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }

        // Field total_pagado_usd
        $filter = $this->queryBuilderWhere("total_pagado_usd");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->total_pagado_usd, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->total_pagado_usd->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->proveedor_rif->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->proveedor_nombre->AdvancedSearch->issetSession()) {
            return true;
        }
        ...
        if ($this->total_pagado_bs->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->total_pagado_usd->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    protected function resetAdvancedSearchParms()
    {
        ...
        $this->proveedor_rif->AdvancedSearch->unsetSession();
        $this->proveedor_nombre->AdvancedSearch->unsetSession();
        ...
        $this->total_pagado_bs->AdvancedSearch->unsetSession();
        $this->total_pagado_usd->AdvancedSearch->unsetSession();
    ...
    protected function restoreSearchParms()
    {
        ...
        $this->proveedor_rif->AdvancedSearch->load();
        $this->proveedor_nombre->AdvancedSearch->load();
        ...
        $this->total_pagado_bs->AdvancedSearch->load();
        $this->total_pagado_usd->AdvancedSearch->load();
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->proveedor_rif); // proveedor_rif
            $this->updateSort($this->proveedor_nombre); // proveedor_nombre
            ...
            $this->updateSort($this->total_pagado_usd); // total_pagado_usd
    ...
    protected function setupListOptions()
    {
        ...
        // "detail_view_cuentas_por_pagar"
        $item = &$this->ListOptions->add("detail_view_cuentas_por_pagar");
        $item->CssClass = "text-nowrap";
        $item->Visible = $Security->allowList(CurrentProjectID() . 'view_cuentas_por_pagar');
        $item->OnLeft = true;
        ...
        // Set up detail pages
        $pages = new SubPages();
        $pages->add("view_cuentas_por_pagar");
    ...
    public function renderListOptions()
    {
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fview_cuentas_por_pagar_resumenlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fview_cuentas_por_pagar_resumenlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
        ...
        // "detail_view_cuentas_por_pagar"
        $opt = $this->ListOptions["detail_view_cuentas_por_pagar"];
        if ($Security->allowList(CurrentProjectID() . 'view_cuentas_por_pagar')) {
            $body = $Language->phrase("DetailLink") . $Language->tablePhrase("view_cuentas_por_pagar", "TblCaption");
            $body = "<a class=\"btn btn-default ew-row-link ew-detail" . ($this->ListOptions->UseDropDownButton ? " dropdown-toggle" : "") . "\" data-action=\"list\" href=\"" . HtmlEncode("ViewCuentasPorPagarList?" . Config("TABLE_SHOW_MASTER") . "=view_cuentas_por_pagar_resumen&" . GetForeignKeyUrl("fk_proveedor", $this->proveedor->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("ViewCuentasPorPagarGrid");
            if ($links != "") {
    ...
    protected function renderListOptionsExt()
    {
        ...
        $detailFilters = [];
        $masterKeys = []; // Reset
        $masterKeys["proveedor"] = strval($this->proveedor->DbValue);

        // Column "detail_view_cuentas_por_pagar"
        if ($this->DetailPages?->getItem("view_cuentas_por_pagar")?->Visible && $Security->allowList(CurrentProjectID() . 'view_cuentas_por_pagar')) {
            $link = "";
            $option = $this->ListOptions["detail_view_cuentas_por_pagar"];
            $detailTbl = Container("view_cuentas_por_pagar");
            $detailFilter = $detailTbl->getDetailFilter($this);
            $detailTbl->setCurrentMasterTable($this->TableVar);
            $detailFilter = $detailTbl->applyUserIDFilters($detailFilter);
            $url = "ViewCuentasPorPagarPreview?t=view_cuentas_por_pagar_resumen&f=" . Encrypt($detailFilter . "|" . implode("|", array_values($masterKeys)));
            $btngrp = "<div data-table=\"view_cuentas_por_pagar\" data-url=\"" . $url . "\" class=\"ew-detail-btn-group btn-group btn-group-sm d-none\">";
            if ($Security->allowList(CurrentProjectID() . 'view_cuentas_por_pagar_resumen')) {
                $label = $Language->tablePhrase("view_cuentas_por_pagar", "TblCaption");
                $link = "<button class=\"nav-link\" data-bs-toggle=\"tab\" data-table=\"view_cuentas_por_pagar\" data-url=\"" . $url . "\" type=\"button\" role=\"tab\" aria-selected=\"false\">" . $label . "</button>";
                $detaillnk = GetUrl("ViewCuentasPorPagarList?" . Config("TABLE_SHOW_MASTER") . "=view_cuentas_por_pagar_resumen");
                foreach ($masterKeys as $key => $value) {
                    $detaillnk .= "&" . GetForeignKeyUrl("fk_$key", $value);
                }
                $title = $Language->tablePhrase("view_cuentas_por_pagar", "TblCaption");
            ...
            $detailPageObj = Container("ViewCuentasPorPagarGrid");
            if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'view_cuentas_por_pagar_resumen')) {
                $caption = $Language->phrase("MasterDetailViewLink");
                $viewurl = GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=view_cuentas_por_pagar"));
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "proveedor_rif");
            $this->createColumnOption($option, "proveedor_nombre");
            ...
            $this->createColumnOption($option, "total_pagado_usd");
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fview_cuentas_por_pagar_resumensrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fview_cuentas_por_pagar_resumensrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions as $listAction) {
            if ($listAction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fview_cuentas_por_pagar_resumenlist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_view_cuentas_por_pagar_resumen", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_view_cuentas_por_pagar_resumen",
    ...
    protected function loadSearchValues()
    {
        ...
        // proveedor_rif
        if ($this->proveedor_rif->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->proveedor_rif->AdvancedSearch->SearchValue != "" || $this->proveedor_rif->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // proveedor_nombre
        if ($this->proveedor_nombre->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->proveedor_nombre->AdvancedSearch->SearchValue != "" || $this->proveedor_nombre->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        // total_pagado_bs
        if ($this->total_pagado_bs->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->total_pagado_bs->AdvancedSearch->SearchValue != "" || $this->total_pagado_bs->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // total_pagado_usd
        if ($this->total_pagado_usd->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->total_pagado_usd->AdvancedSearch->SearchValue != "" || $this->total_pagado_usd->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            // proveedor_rif
            $this->proveedor_rif->setupEditAttributes();
            if (!$this->proveedor_rif->Raw) {
                $this->proveedor_rif->AdvancedSearch->SearchValue = HtmlDecode($this->proveedor_rif->AdvancedSearch->SearchValue);
            }
            $this->proveedor_rif->EditValue = HtmlEncode($this->proveedor_rif->AdvancedSearch->SearchValue);
            $this->proveedor_rif->PlaceHolder = RemoveHtml($this->proveedor_rif->caption());

            // proveedor_nombre
            $this->proveedor_nombre->setupEditAttributes();
            if (!$this->proveedor_nombre->Raw) {
                $this->proveedor_nombre->AdvancedSearch->SearchValue = HtmlDecode($this->proveedor_nombre->AdvancedSearch->SearchValue);
            }
            $this->proveedor_nombre->EditValue = HtmlEncode($this->proveedor_nombre->AdvancedSearch->SearchValue);
            $this->proveedor_nombre->PlaceHolder = RemoveHtml($this->proveedor_nombre->caption());
            ...
            $this->total_pagado_bs->EditValue = $this->total_pagado_bs->AdvancedSearch->SearchValue;
            $this->total_pagado_bs->PlaceHolder = RemoveHtml($this->total_pagado_bs->caption());

            // total_pagado_usd
            $this->total_pagado_usd->setupEditAttributes();
            $this->total_pagado_usd->EditValue = $this->total_pagado_usd->AdvancedSearch->SearchValue;
            $this->total_pagado_usd->PlaceHolder = RemoveHtml($this->total_pagado_usd->caption());
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            if ($custom) {
                return "<button type=\"button\" class=\"btn btn-default ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcel", true)) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcel", true)) . "\" form=\"fview_cuentas_por_pagar_resumenlist\" data-url=\"$exportUrl\" data-ew-action=\"export\" data-export=\"excel\" data-custom=\"true\" data-export-selected=\"false\">" . $Language->phrase("ExportToExcel") . "</button>";
            } else {
                return "<a href=\"$exportUrl\" class=\"btn btn-default ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcel", true)) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcel", true)) . "\">" . $Language->phrase("ExportToExcel") . "</a>";
            }
        } elseif (SameText($type, "word")) {
            if ($custom) {
                return "<button type=\"button\" class=\"btn btn-default ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWord", true)) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWord", true)) . "\" form=\"fview_cuentas_por_pagar_resumenlist\" data-url=\"$exportUrl\" data-ew-action=\"export\" data-export=\"word\" data-custom=\"true\" data-export-selected=\"false\">" . $Language->phrase("ExportToWord") . "</button>";
            } else {
                return "<a href=\"$exportUrl\" class=\"btn btn-default ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWord", true)) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWord", true)) . "\">" . $Language->phrase("ExportToWord") . "</a>";
            }
        } elseif (SameText($type, "pdf")) {
            if ($custom) {
                return "<button type=\"button\" class=\"btn btn-default ew-export-link ew-pdf\" title=\"" . HtmlEncode($Language->phrase("ExportToPdf", true)) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToPdf", true)) . "\" form=\"fview_cuentas_por_pagar_resumenlist\" data-url=\"$exportUrl\" data-ew-action=\"export\" data-export=\"pdf\" data-custom=\"true\" data-export-selected=\"false\">" . $Language->phrase("ExportToPdf") . "</button>";
            } else {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmail", true) . '" data-caption="' . $Language->phrase("ExportToEmail", true) . '" form="fview_cuentas_por_pagar_resumenlist" data-ew-action="email" data-custom="false" data-hdr="' . $Language->phrase("ExportToEmail", true) . '" data-exported-selected="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    public function pageDataRendering(&$header)
    {
        $sql = "
            SELECT
                ...
                SUM(
                    CASE
                        WHEN estado_cuenta = 'PAGO PARCIAL'
                        THEN 1 ELSE 0
                    END
                ) AS parciales,
                SUM(
                    CASE
                        WHEN antiguedad <> 'NO VENCIDO'
                             AND estado_cuenta NOT IN ('PAGADO', 'ANULADO')
                        THEN 1 ELSE 0
                    END
                ) AS vencidos,
        ...
        ";
        $row = ExecuteRow($sql);
        $proveedores = intval($row["proveedores"] ?? 0);
