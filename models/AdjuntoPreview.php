 ...
 * Page class
 */
class AdjuntoPreview extends Adjunto
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "AdjuntoPreview";
    ...
    // CSS class/style
    public $CurrentPageName = "AdjuntoPreview";
    ...
    protected function masterKeyUrl()
    {
        ...
        if ($masterTblVar == "articulo") {
            $url = "" . Config("TABLE_SHOW_MASTER") . "=articulo&" . GetForeignKeyUrl("fk_id", $this->articulo->QueryStringValue) . "";
        }
        if ($masterTblVar == "cliente") {
            $url = "" . Config("TABLE_SHOW_MASTER") . "=cliente&" . GetForeignKeyUrl("fk_id", $this->cliente->QueryStringValue) . "";
        }
        if ($masterTblVar == "proveedor") {
            $url = "" . Config("TABLE_SHOW_MASTER") . "=proveedor&" . GetForeignKeyUrl("fk_id", $this->proveedor->QueryStringValue) . "";
        }
        return $url;
    }
    ...
    protected function setupForeignKeys($keys)
    {
        $masterTblVar = Get("t", "");
        if ($masterTblVar == "articulo") {
            $this->articulo->setQueryStringValue($keys[0] ?? "");
        }
        if ($masterTblVar == "cliente") {
            $this->cliente->setQueryStringValue($keys[0] ?? "");
        }
        if ($masterTblVar == "proveedor") {
    ...
    public function listOptionsRendered() {
    	// Example:
    	if(strpos($this->foto->ViewValue, ".pdf") > 0)
    		$this->ListOptions->Items["foto"]->Body = '<a href="carpetacarga/' . $this->foto->ViewValue . '" target="_blank">' . $this->foto->ViewValue . '</a>';
    	else
    		$this->ListOptions->Items["foto"]->Body = '<a href="carpetacarga/' . $this->foto->ViewValue . '" target="_blank"><img src="carpetacarga/' . $this->foto->ViewValue . '" class="img-thumbnail" alt="' . $this->nota->ViewValue . '" width="150"></a>';
    }
}
