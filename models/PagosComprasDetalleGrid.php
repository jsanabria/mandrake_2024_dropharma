 ...
 * Page class
 */
class PagosComprasDetalleGrid extends PagosComprasDetalle
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PagosComprasDetalleGrid";
    ...
    public $FormName = "fpagos_compras_detallegrid";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "PagosComprasDetalleGrid";
    ...
    protected function clearInlineMode()
    {
        ...
        $this->tasa_moneda->FormValue = ""; // Clear form value
    ...
    public function emptyRow()
    {
        ...
        if (
            $CurrentForm->hasValue("x_tasa_moneda") &&
            $CurrentForm->hasValue("o_tasa_moneda") &&
            $this->tasa_moneda->CurrentValue != $this->tasa_moneda->DefaultValue &&
            !($this->tasa_moneda->IsForeignKey && $this->getCurrentMasterTable() != "" && $this->tasa_moneda->CurrentValue == $this->tasa_moneda->getSessionValue())
        ) {
            return false;
        }
        if (
        ...
        if (
            $CurrentForm->hasValue("x_tasa_usd") &&
            $CurrentForm->hasValue("o_tasa_usd") &&
            $this->tasa_usd->CurrentValue != $this->tasa_usd->DefaultValue &&
            !($this->tasa_usd->IsForeignKey && $this->getCurrentMasterTable() != "" && $this->tasa_usd->CurrentValue == $this->tasa_usd->getSessionValue())
        ) {
            return false;
        }
        if (
            $CurrentForm->hasValue("x_monto_usd") &&
            $CurrentForm->hasValue("o_monto_usd") &&
            $this->monto_usd->CurrentValue != $this->monto_usd->DefaultValue &&
            !($this->monto_usd->IsForeignKey && $this->getCurrentMasterTable() != "" && $this->monto_usd->CurrentValue == $this->monto_usd->getSessionValue())
        ) {
            return false;
        }
        if (
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'tasa_moneda' first before field var 'x_tasa_moneda'
        $val = $CurrentForm->hasValue("tasa_moneda") ? $CurrentForm->getValue("tasa_moneda") : $CurrentForm->getValue("x_tasa_moneda");
        if (!$this->tasa_moneda->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->tasa_moneda->Visible = false; // Disable update for API request
            } else {
                $this->tasa_moneda->setFormValue($val, true, $validate);
            }
        }
        if ($CurrentForm->hasValue("o_tasa_moneda")) {
            $this->tasa_moneda->setOldValue($CurrentForm->getValue("o_tasa_moneda"));
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->tasa_moneda->CurrentValue = $this->tasa_moneda->FormValue;
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            ...
            if ($this->tasa_moneda->Visible && $this->tasa_moneda->Required) {
                if (!$this->tasa_moneda->IsDetailKey && EmptyValue($this->tasa_moneda->FormValue)) {
                    $this->tasa_moneda->addErrorMessage(str_replace("%s", $this->tasa_moneda->caption(), $this->tasa_moneda->RequiredErrorMessage));
                }
            }
            if (!CheckNumber($this->tasa_moneda->FormValue)) {
                $this->tasa_moneda->addErrorMessage($this->tasa_moneda->getErrorMessage(false));
            }
    ...
    protected function getEditRow($rsold)
    {
        ...
        // tasa_moneda
        $this->tasa_moneda->setDbValueDef($rsnew, $this->tasa_moneda->CurrentValue, $this->tasa_moneda->ReadOnly);
    ...
    protected function restoreEditFormFromRow($row)
    {
        ...
        if (isset($row['tasa_moneda'])) { // tasa_moneda
            $this->tasa_moneda->CurrentValue = $row['tasa_moneda'];
        }
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($this->getCurrentMasterTable() == "pagos_compras") {
            $this->pagos_compras->Visible = true; // Need to insert foreign key
            $this->pagos_compras->CurrentValue = $this->pagos_compras->getSessionValue();
        }
    ...
    protected function getAddRow()
    {
        ...
        // tasa_moneda
        $this->tasa_moneda->setDbValueDef($rsnew, $this->tasa_moneda->CurrentValue, false);

        // monto_bs
        $this->monto_bs->setDbValueDef($rsnew, $this->monto_bs->CurrentValue, false);

        // tasa_usd
        $this->tasa_usd->setDbValueDef($rsnew, $this->tasa_usd->CurrentValue, false);
        ...
        if ($this->pagos_compras->getSessionValue() != "") {
            $rsnew['pagos_compras'] = $this->pagos_compras->getSessionValue();
        }
        return $rsnew;
    }
    ...
    protected function restoreAddFormFromRow($row)
    {
        ...
        if (isset($row['tasa_moneda'])) { // tasa_moneda
            $this->tasa_moneda->setFormValue($row['tasa_moneda']);
        }
        if (isset($row['monto_bs'])) { // monto_bs
            $this->monto_bs->setFormValue($row['monto_bs']);
        }
        if (isset($row['tasa_usd'])) { // tasa_usd
            $this->tasa_usd->setFormValue($row['tasa_usd']);
        }
        if (isset($row['monto_usd'])) { // monto_usd
        ...
        if (isset($row['pagos_compras'])) { // pagos_compras
            $this->pagos_compras->setFormValue($row['pagos_compras']);
        }
    }
