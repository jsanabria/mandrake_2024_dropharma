 ...
 * Page class
 */
class RecargaPreview extends Recarga
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "RecargaPreview";
    ...
    // CSS class/style
    public $CurrentPageName = "RecargaPreview";
    ...
    protected function masterKeyUrl()
    {
        ...
        if ($masterTblVar == "abono") {
            $url = "" . Config("TABLE_SHOW_MASTER") . "=abono&" . GetForeignKeyUrl("fk_id", $this->abono->QueryStringValue) . "";
        }
        return $url;
    }
    ...
    protected function setupForeignKeys($keys)
    {
        $masterTblVar = Get("t", "");
        if ($masterTblVar == "abono") {
            $this->abono->setQueryStringValue($keys[0] ?? "");
        }
    }
    ...
    {
        // Example:
        $opt = &$this->ListOptions->Add("reverso");
        $opt->Header = "";
    ...
    {
        // Example:
        $url = "ConfirmPage?page=REVERSO&id=" . $this->id->CurrentValue;
        if($this->fecha->CurrentValue == date("Y-m-d") and !($this->metodo_pago->CurrentValue == "RC" or $this->metodo_pago->CurrentValue == "DV" or $this->metodo_pago->CurrentValue == "ND" or $this->metodo_pago->CurrentValue == "NC")) {
        	if($this->reverso->CurrentValue == "N")
        		$this->ListOptions["reverso"]->Body = '<a class="btn btn-primary" target="_blank" href="' . $url . '"><i class="fas fa-clock"></i></a>';
        	else
        		$this->ListOptions["reverso"]->Body = '';
        }
        else
        	$this->ListOptions["reverso"]->Body = '';
    }
