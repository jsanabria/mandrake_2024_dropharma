 ...
 * Page class
 */
class PagosComprasDetalleAdd extends PagosComprasDetalle
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PagosComprasDetalleAdd";
    ...
    // CSS class/style
    public $CurrentPageName = "PagosComprasDetalleAdd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$rsold) { // Record not loaded
                    ...
                    $this->terminate("PagosComprasDetalleList"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($rsold)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "PagosComprasDetalleList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "PagosComprasDetalleView") {
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "PagosComprasDetalleList") {
                            ...
                            $returnUrl = "PagosComprasDetalleList"; // Return list page content
                        }
                    }
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'pagos_compras' first before field var 'x_pagos_compras'
        $val = $CurrentForm->hasValue("pagos_compras") ? $CurrentForm->getValue("pagos_compras") : $CurrentForm->getValue("x_pagos_compras");
        if (!$this->pagos_compras->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->pagos_compras->Visible = false; // Disable update for API request
            } else {
                $this->pagos_compras->setFormValue($val, true, $validate);
            }
        }
    ...
    {
        global $CurrentForm;
        $this->pagos_compras->CurrentValue = $this->pagos_compras->FormValue;
    ...
    public function renderRow()
    {
        ...
        // pagos_compras
        $this->pagos_compras->RowCssClass = "row";
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            if ($this->pagos_compras->Visible && $this->pagos_compras->Required) {
                if (!$this->pagos_compras->IsDetailKey && EmptyValue($this->pagos_compras->FormValue)) {
                    $this->pagos_compras->addErrorMessage(str_replace("%s", $this->pagos_compras->caption(), $this->pagos_compras->RequiredErrorMessage));
                }
            }
            if (!CheckInteger($this->pagos_compras->FormValue)) {
                $this->pagos_compras->addErrorMessage($this->pagos_compras->getErrorMessage(false));
            }
    ...
    protected function getAddRow()
    {
        ...
        // pagos_compras
        $this->pagos_compras->setDbValueDef($rsnew, $this->pagos_compras->CurrentValue, false);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("PagosComprasDetalleList"), "", $this->TableVar, true);
