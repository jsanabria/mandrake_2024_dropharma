...
/**
 * Table class for view_in_out
 */
class ViewInOut extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->check_ne->Lookup = new Lookup($this->check_ne, 'view_in_out', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
    ...
    public function getMasterFilterFromSession()
    {
        ...
        if ($this->getCurrentMasterTable() == "view_in_tdcpdc") {
            $masterTable = Container("view_in_tdcpdc");
        ...
        if ($this->getCurrentMasterTable() == "view_in_tdcnrp") {
            $masterTable = Container("view_in_tdcnrp");
            if ($this->id_documento->getSessionValue() != "") {
                $masterFilter .= "" . GetKeyFilter($masterTable->id, $this->id_documento->getSessionValue(), $masterTable->id->DataType, $masterTable->Dbid);
            } else {
                return "";
        ...
        if ($this->getCurrentMasterTable() == "view_in_tdcfcc") {
            $masterTable = Container("view_in_tdcfcc");
            if ($this->id_documento->getSessionValue() != "") {
                $masterFilter .= "" . GetKeyFilter($masterTable->id, $this->id_documento->getSessionValue(), $masterTable->id->DataType, $masterTable->Dbid);
            } else {
                return "";
        ...
        if ($this->getCurrentMasterTable() == "view_in_tdcaen") {
            $masterTable = Container("view_in_tdcaen");
            if ($this->id_documento->getSessionValue() != "") {
                $masterFilter .= "" . GetKeyFilter($masterTable->id, $this->id_documento->getSessionValue(), $masterTable->id->DataType, $masterTable->Dbid);
            } else {
                return "";
        ...
        if ($this->getCurrentMasterTable() == "view_out_tdcnet") {
            $masterTable = Container("view_out_tdcnet");
            if ($this->id_documento->getSessionValue() != "") {
                $masterFilter .= "" . GetKeyFilter($masterTable->id, $this->id_documento->getSessionValue(), $masterTable->id->DataType, $masterTable->Dbid);
            } else {
                return "";
        ...
        if ($this->getCurrentMasterTable() == "view_out_tdcfcv") {
            $masterTable = Container("view_out_tdcfcv");
            if ($this->id_documento->getSessionValue() != "") {
                $masterFilter .= "" . GetKeyFilter($masterTable->id, $this->id_documento->getSessionValue(), $masterTable->id->DataType, $masterTable->Dbid);
            } else {
                return "";
        ...
        if ($this->getCurrentMasterTable() == "view_out_tdcasa") {
            $masterTable = Container("view_out_tdcasa");
            if ($this->id_documento->getSessionValue() != "") {
                $masterFilter .= "" . GetKeyFilter($masterTable->id, $this->id_documento->getSessionValue(), $masterTable->id->DataType, $masterTable->Dbid);
            } else {
                return "";
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "view_in_out";
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("ViewInOutList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "ViewInOutView" => $Language->phrase("View"),
            "ViewInOutEdit" => $Language->phrase("Edit"),
            "ViewInOutAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "ViewInOutView",
            Config("API_ADD_ACTION") => "ViewInOutAdd",
            Config("API_EDIT_ACTION") => "ViewInOutEdit",
            Config("API_DELETE_ACTION") => "ViewInOutDelete",
            Config("API_LIST_ACTION") => "ViewInOutList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("ViewInOutView", $parm);
        } else {
            $url = $this->keyUrl("ViewInOutView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ViewInOutEdit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ViewInOutList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ViewInOutAdd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ViewInOutList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("ViewInOutDelete", $parm);
        }
    }
    ...
    {
        if ($this->getCurrentMasterTable() == "view_in_tdcpdc" && !ContainsString($url, Config("TABLE_SHOW_MASTER") . "=")) {
        ...
        if ($this->getCurrentMasterTable() == "view_in_tdcnrp" && !ContainsString($url, Config("TABLE_SHOW_MASTER") . "=")) {
            $url .= (ContainsString($url, "?") ? "&" : "?") . Config("TABLE_SHOW_MASTER") . "=" . $this->getCurrentMasterTable();
            $url .= "&" . GetForeignKeyUrl("fk_id", $this->id_documento->getSessionValue()); // Use Session Value
            $url .= "&" . GetForeignKeyUrl("fk_tipo_documento", $this->tipo_documento->getSessionValue()); // Use Session Value
        }
        if ($this->getCurrentMasterTable() == "view_in_tdcfcc" && !ContainsString($url, Config("TABLE_SHOW_MASTER") . "=")) {
            $url .= (ContainsString($url, "?") ? "&" : "?") . Config("TABLE_SHOW_MASTER") . "=" . $this->getCurrentMasterTable();
            $url .= "&" . GetForeignKeyUrl("fk_id", $this->id_documento->getSessionValue()); // Use Session Value
            $url .= "&" . GetForeignKeyUrl("fk_tipo_documento", $this->tipo_documento->getSessionValue()); // Use Session Value
        }
        if ($this->getCurrentMasterTable() == "view_in_tdcaen" && !ContainsString($url, Config("TABLE_SHOW_MASTER") . "=")) {
            $url .= (ContainsString($url, "?") ? "&" : "?") . Config("TABLE_SHOW_MASTER") . "=" . $this->getCurrentMasterTable();
            $url .= "&" . GetForeignKeyUrl("fk_id", $this->id_documento->getSessionValue()); // Use Session Value
            $url .= "&" . GetForeignKeyUrl("fk_tipo_documento", $this->tipo_documento->getSessionValue()); // Use Session Value
        }
        if ($this->getCurrentMasterTable() == "view_out_tdcnet" && !ContainsString($url, Config("TABLE_SHOW_MASTER") . "=")) {
            $url .= (ContainsString($url, "?") ? "&" : "?") . Config("TABLE_SHOW_MASTER") . "=" . $this->getCurrentMasterTable();
            $url .= "&" . GetForeignKeyUrl("fk_id", $this->id_documento->getSessionValue()); // Use Session Value
            $url .= "&" . GetForeignKeyUrl("fk_tipo_documento", $this->tipo_documento->getSessionValue()); // Use Session Value
        }
        if ($this->getCurrentMasterTable() == "view_out_tdcfcv" && !ContainsString($url, Config("TABLE_SHOW_MASTER") . "=")) {
            $url .= (ContainsString($url, "?") ? "&" : "?") . Config("TABLE_SHOW_MASTER") . "=" . $this->getCurrentMasterTable();
            $url .= "&" . GetForeignKeyUrl("fk_id", $this->id_documento->getSessionValue()); // Use Session Value
            $url .= "&" . GetForeignKeyUrl("fk_tipo_documento", $this->tipo_documento->getSessionValue()); // Use Session Value
        }
        if ($this->getCurrentMasterTable() == "view_out_tdcasa" && !ContainsString($url, Config("TABLE_SHOW_MASTER") . "=")) {
            $url .= (ContainsString($url, "?") ? "&" : "?") . Config("TABLE_SHOW_MASTER") . "=" . $this->getCurrentMasterTable();
            $url .= "&" . GetForeignKeyUrl("fk_id", $this->id_documento->getSessionValue()); // Use Session Value
            $url .= "&" . GetForeignKeyUrl("fk_tipo_documento", $this->tipo_documento->getSessionValue()); // Use Session Value
        }
