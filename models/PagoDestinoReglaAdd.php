 ...
 * Page class
 */
class PagoDestinoReglaAdd extends PagoDestinoRegla
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PagoDestinoReglaAdd";
    ...
    // CSS class/style
    public $CurrentPageName = "PagoDestinoReglaAdd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$rsold) { // Record not loaded
                    ...
                    $this->terminate("PagoDestinoReglaList"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($rsold)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "PagoDestinoReglaList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "PagoDestinoReglaView") {
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "PagoDestinoReglaList") {
                            ...
                            $returnUrl = "PagoDestinoReglaList"; // Return list page content
                        }
                    }
    ...
    {
        $this->prioridad->DefaultValue = $this->prioridad->getDefault(); // PHP
        $this->prioridad->OldValue = $this->prioridad->DefaultValue;
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'compania' first before field var 'x_compania'
        $val = $CurrentForm->hasValue("compania") ? $CurrentForm->getValue("compania") : $CurrentForm->getValue("x_compania");
        if (!$this->compania->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->compania->Visible = false; // Disable update for API request
            } else {
                $this->compania->setFormValue($val);
            }
        }
        ...
        // Check field name 'cuenta_destino_id' first before field var 'x_cuenta_destino_id'
        $val = $CurrentForm->hasValue("cuenta_destino_id") ? $CurrentForm->getValue("cuenta_destino_id") : $CurrentForm->getValue("x_cuenta_destino_id");
        if (!$this->cuenta_destino_id->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->cuenta_destino_id->Visible = false; // Disable update for API request
            } else {
                $this->cuenta_destino_id->setFormValue($val);
            }
        }
    ...
    {
        global $CurrentForm;
        $this->compania->CurrentValue = $this->compania->FormValue;
        $this->metodo->CurrentValue = $this->metodo->FormValue;
        $this->moneda->CurrentValue = $this->moneda->FormValue;
        $this->cuenta_destino_id->CurrentValue = $this->cuenta_destino_id->FormValue;
    ...
    public function renderRow()
    {
        ...
        // cuenta_destino_id
        $this->cuenta_destino_id->RowCssClass = "row";

        // prioridad
        $this->prioridad->RowCssClass = "row";
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            $curVal = trim(strval($this->compania->CurrentValue));
            if ($curVal != "") {
                $this->compania->ViewValue = $this->compania->lookupCacheOption($curVal);
            } else {
                $this->compania->ViewValue = $this->compania->Lookup !== null && is_array($this->compania->lookupOptions()) && count($this->compania->lookupOptions()) > 0 ? $curVal : null;
            }
            if ($this->compania->ViewValue !== null) { // Load from cache
                $this->compania->EditValue = array_values($this->compania->lookupOptions());
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = SearchFilter($this->compania->Lookup->getTable()->Fields["id"]->searchExpression(), "=", $this->compania->CurrentValue, $this->compania->Lookup->getTable()->Fields["id"]->searchDataType(), "");
                }
                $sqlWrk = $this->compania->Lookup->getSql(true, $filterWrk, '', $this, false, true);
                $conn = Conn();
                $config = $conn->getConfiguration();
                $config->setResultCache($this->Cache);
                $rswrk = $conn->executeCacheQuery($sqlWrk, [], [], $this->CacheProfile)->fetchAll();
                $ari = count($rswrk);
                $arwrk = $rswrk;
                $this->compania->EditValue = $arwrk;
            }
            $this->compania->PlaceHolder = RemoveHtml($this->compania->caption());

            // metodo
            $this->metodo->setupEditAttributes();
            $curVal = trim(strval($this->metodo->CurrentValue));
            if ($curVal != "") {
                $this->metodo->ViewValue = $this->metodo->lookupCacheOption($curVal);
            } else {
                $this->metodo->ViewValue = $this->metodo->Lookup !== null && is_array($this->metodo->lookupOptions()) && count($this->metodo->lookupOptions()) > 0 ? $curVal : null;
            }
            if ($this->metodo->ViewValue !== null) { // Load from cache
                $this->metodo->EditValue = array_values($this->metodo->lookupOptions());
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = SearchFilter($this->metodo->Lookup->getTable()->Fields["valor1"]->searchExpression(), "=", $this->metodo->CurrentValue, $this->metodo->Lookup->getTable()->Fields["valor1"]->searchDataType(), "");
                }
                $lookupFilter = $this->metodo->getSelectFilter($this); // PHP
                $sqlWrk = $this->metodo->Lookup->getSql(true, $filterWrk, $lookupFilter, $this, false, true);
                $conn = Conn();
                $config = $conn->getConfiguration();
                $config->setResultCache($this->Cache);
                $rswrk = $conn->executeCacheQuery($sqlWrk, [], [], $this->CacheProfile)->fetchAll();
            ...
            $curVal = trim(strval($this->cuenta_destino_id->CurrentValue));
            if ($curVal != "") {
                $this->cuenta_destino_id->ViewValue = $this->cuenta_destino_id->lookupCacheOption($curVal);
            } else {
                $this->cuenta_destino_id->ViewValue = $this->cuenta_destino_id->Lookup !== null && is_array($this->cuenta_destino_id->lookupOptions()) && count($this->cuenta_destino_id->lookupOptions()) > 0 ? $curVal : null;
            }
            if ($this->cuenta_destino_id->ViewValue !== null) { // Load from cache
                $this->cuenta_destino_id->EditValue = array_values($this->cuenta_destino_id->lookupOptions());
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = SearchFilter($this->cuenta_destino_id->Lookup->getTable()->Fields["id"]->searchExpression(), "=", $this->cuenta_destino_id->CurrentValue, $this->cuenta_destino_id->Lookup->getTable()->Fields["id"]->searchDataType(), "");
                }
                $sqlWrk = $this->cuenta_destino_id->Lookup->getSql(true, $filterWrk, '', $this, false, true);
                $conn = Conn();
                $config = $conn->getConfiguration();
                $config->setResultCache($this->Cache);
                $rswrk = $conn->executeCacheQuery($sqlWrk, [], [], $this->CacheProfile)->fetchAll();
                $ari = count($rswrk);
                $arwrk = $rswrk;
                $this->cuenta_destino_id->EditValue = $arwrk;
            }
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            if ($this->compania->Visible && $this->compania->Required) {
                if (!$this->compania->IsDetailKey && EmptyValue($this->compania->FormValue)) {
                    $this->compania->addErrorMessage(str_replace("%s", $this->compania->caption(), $this->compania->RequiredErrorMessage));
                }
            }
            if ($this->metodo->Visible && $this->metodo->Required) {
                if (!$this->metodo->IsDetailKey && EmptyValue($this->metodo->FormValue)) {
            ...
            if ($this->moneda->Visible && $this->moneda->Required) {
                if ($this->moneda->FormValue == "") {
                    $this->moneda->addErrorMessage(str_replace("%s", $this->moneda->caption(), $this->moneda->RequiredErrorMessage));
                }
            }
            if ($this->cuenta_destino_id->Visible && $this->cuenta_destino_id->Required) {
                if (!$this->cuenta_destino_id->IsDetailKey && EmptyValue($this->cuenta_destino_id->FormValue)) {
                    $this->cuenta_destino_id->addErrorMessage(str_replace("%s", $this->cuenta_destino_id->caption(), $this->cuenta_destino_id->RequiredErrorMessage));
                }
            }
            if ($this->prioridad->Visible && $this->prioridad->Required) {
                if (!$this->prioridad->IsDetailKey && EmptyValue($this->prioridad->FormValue)) {
                    $this->prioridad->addErrorMessage(str_replace("%s", $this->prioridad->caption(), $this->prioridad->RequiredErrorMessage));
                }
            }
    ...
    protected function getAddRow()
    {
        ...
        // compania
        $this->compania->setDbValueDef($rsnew, $this->compania->CurrentValue, false);

        // metodo
        $this->metodo->setDbValueDef($rsnew, $this->metodo->CurrentValue, false);

        // moneda
        $this->moneda->setDbValueDef($rsnew, $this->moneda->CurrentValue, false);

        // cuenta_destino_id
        $this->cuenta_destino_id->setDbValueDef($rsnew, $this->cuenta_destino_id->CurrentValue, false);
    ...
    protected function restoreAddFormFromRow($row)
    {
        ...
        if (isset($row['cuenta_destino_id'])) { // cuenta_destino_id
            $this->cuenta_destino_id->setFormValue($row['cuenta_destino_id']);
        }
        if (isset($row['prioridad'])) { // prioridad
            $this->prioridad->setFormValue($row['prioridad']);
        }
