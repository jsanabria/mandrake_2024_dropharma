 ...
 * Page class
 */
class ContAsientoPreview extends ContAsiento
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ContAsientoPreview";
    ...
    // CSS class/style
    public $CurrentPageName = "ContAsientoPreview";
    ...
    protected function masterKeyUrl()
    {
        ...
        if ($masterTblVar == "cont_comprobante") {
            $url = "" . Config("TABLE_SHOW_MASTER") . "=cont_comprobante&" . GetForeignKeyUrl("fk_id", $this->comprobante->QueryStringValue) . "";
        }
        return $url;
    }
    ...
    protected function setupForeignKeys($keys)
    {
        $masterTblVar = Get("t", "");
        if ($masterTblVar == "cont_comprobante") {
            $this->comprobante->setQueryStringValue($keys[0] ?? "");
        }
    }
