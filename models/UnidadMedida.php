...
/**
 * Table class for unidad_medida
 */
class UnidadMedida extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        // Update Table
        $this->UpdateTable = "unidad_medida";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        $this->tipo->Lookup = new Lookup($this->tipo, 'unidad_medida', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "unidad_medida";
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("UnidadMedidaList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "UnidadMedidaView" => $Language->phrase("View"),
            "UnidadMedidaEdit" => $Language->phrase("Edit"),
            "UnidadMedidaAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "UnidadMedidaView",
            Config("API_ADD_ACTION") => "UnidadMedidaAdd",
            Config("API_EDIT_ACTION") => "UnidadMedidaEdit",
            Config("API_DELETE_ACTION") => "UnidadMedidaDelete",
            Config("API_LIST_ACTION") => "UnidadMedidaList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("UnidadMedidaView", $parm);
        } else {
            $url = $this->keyUrl("UnidadMedidaView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("UnidadMedidaEdit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("UnidadMedidaList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("UnidadMedidaAdd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("UnidadMedidaList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("UnidadMedidaDelete", $parm);
        }
    }
    ...
    {
        WriteAuditLog(CurrentUserIdentifier(), $typ, 'unidad_medida');
    }
    ...
    public function writeAuditTrailOnAdd(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "A", 'unidad_medida', $fldname, $key, "", $newvalue);
            }
        }
    }
        ...
        foreach (array_keys($rsnew) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && array_key_exists($fldname, $rsold) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                if ($modified) {
                    ...
                    WriteAuditLog($usr, "U", 'unidad_medida', $fldname, $key, $oldvalue, $newvalue);
                }
            }
        }
    }
    ...
    public function writeAuditTrailOnDelete(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "D", 'unidad_medida', $fldname, $key, $oldvalue);
            }
        }
    }
    ...
    public function rowInserting($rsold, &$rsnew) {
    	...
    	$rsnew["codigo"] = strtoupper($rsnew["codigo"]);
    	$rsnew["descripcion"] = strtoupper($rsnew["descripcion"]);
    	return TRUE;
    }
