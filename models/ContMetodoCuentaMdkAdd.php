 ...
 * Page class
 */
class ContMetodoCuentaMdkAdd extends ContMetodoCuentaMdk
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ContMetodoCuentaMdkAdd";
    ...
    // CSS class/style
    public $CurrentPageName = "ContMetodoCuentaMdkAdd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$rsold) { // Record not loaded
                    ...
                    $this->terminate("ContMetodoCuentaMdkList"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($rsold)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "ContMetodoCuentaMdkList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "ContMetodoCuentaMdkView") {
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "ContMetodoCuentaMdkList") {
                            ...
                            $returnUrl = "ContMetodoCuentaMdkList"; // Return list page content
                        }
                    }
    ...
    {
        $this->tipo_destino->DefaultValue = $this->tipo_destino->getDefault(); // PHP
        $this->tipo_destino->OldValue = $this->tipo_destino->DefaultValue;
        $this->forzar_destino->DefaultValue = $this->forzar_destino->getDefault(); // PHP
        $this->forzar_destino->OldValue = $this->forzar_destino->DefaultValue;
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'tipo_destino' first before field var 'x_tipo_destino'
        $val = $CurrentForm->hasValue("tipo_destino") ? $CurrentForm->getValue("tipo_destino") : $CurrentForm->getValue("x_tipo_destino");
        if (!$this->tipo_destino->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->tipo_destino->Visible = false; // Disable update for API request
            } else {
                $this->tipo_destino->setFormValue($val);
            }
        }
        ...
        // Check field name 'forzar_destino' first before field var 'x_forzar_destino'
        $val = $CurrentForm->hasValue("forzar_destino") ? $CurrentForm->getValue("forzar_destino") : $CurrentForm->getValue("x_forzar_destino");
        if (!$this->forzar_destino->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->forzar_destino->Visible = false; // Disable update for API request
            } else {
                $this->forzar_destino->setFormValue($val);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->tipo_destino->CurrentValue = $this->tipo_destino->FormValue;
        $this->cuenta_id->CurrentValue = $this->cuenta_id->FormValue;
        $this->forzar_destino->CurrentValue = $this->forzar_destino->FormValue;
    ...
    public function renderRow()
    {
        ...
        // tipo_destino
        $this->tipo_destino->RowCssClass = "row";

        // cuenta_id
        $this->cuenta_id->RowCssClass = "row";

        // forzar_destino
        $this->forzar_destino->RowCssClass = "row";
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            // tipo_destino
            $this->tipo_destino->setupEditAttributes();
            $this->tipo_destino->EditValue = $this->tipo_destino->options(true);
            $this->tipo_destino->PlaceHolder = RemoveHtml($this->tipo_destino->caption());
            ...
            // forzar_destino
            $this->forzar_destino->EditValue = $this->forzar_destino->options(false);
            $this->forzar_destino->PlaceHolder = RemoveHtml($this->forzar_destino->caption());
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            ...
            if ($this->tipo_destino->Visible && $this->tipo_destino->Required) {
                if (!$this->tipo_destino->IsDetailKey && EmptyValue($this->tipo_destino->FormValue)) {
                    $this->tipo_destino->addErrorMessage(str_replace("%s", $this->tipo_destino->caption(), $this->tipo_destino->RequiredErrorMessage));
                }
            }
            ...
            if ($this->forzar_destino->Visible && $this->forzar_destino->Required) {
                if ($this->forzar_destino->FormValue == "") {
                    $this->forzar_destino->addErrorMessage(str_replace("%s", $this->forzar_destino->caption(), $this->forzar_destino->RequiredErrorMessage));
                }
            }
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($this->metodo_pago->CurrentValue != "") { // Check field with unique index
            $filter = "(`metodo_pago` = '" . AdjustSql($this->metodo_pago->CurrentValue, $this->Dbid) . "')";
            $rsChk = $this->loadRs($filter)->fetch();
            if ($rsChk !== false) {
                $idxErrMsg = str_replace("%f", $this->metodo_pago->caption(), $Language->phrase("DupIndex"));
                $idxErrMsg = str_replace("%v", $this->metodo_pago->CurrentValue, $idxErrMsg);
    ...
    protected function getAddRow()
    {
        ...
        // tipo_destino
        $this->tipo_destino->setDbValueDef($rsnew, $this->tipo_destino->CurrentValue, strval($this->tipo_destino->CurrentValue) == "");

        // cuenta_id
        $this->cuenta_id->setDbValueDef($rsnew, $this->cuenta_id->CurrentValue, false);

        // forzar_destino
        $this->forzar_destino->setDbValueDef($rsnew, $this->forzar_destino->CurrentValue, strval($this->forzar_destino->CurrentValue) == "");
    ...
    protected function restoreAddFormFromRow($row)
    {
        ...
        if (isset($row['tipo_destino'])) { // tipo_destino
            $this->tipo_destino->setFormValue($row['tipo_destino']);
        }
        if (isset($row['cuenta_id'])) { // cuenta_id
            $this->cuenta_id->setFormValue($row['cuenta_id']);
        }
        if (isset($row['forzar_destino'])) { // forzar_destino
            $this->forzar_destino->setFormValue($row['forzar_destino']);
        }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("ContMetodoCuentaMdkList"), "", $this->TableVar, true);
