 ...
 * Page class
 */
class CobrosClienteFacturaGrid extends CobrosClienteFactura
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "CobrosClienteFacturaGrid";
    ...
    public $FormName = "fcobros_cliente_facturagrid";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "CobrosClienteFacturaGrid";
    ...
    protected function clearInlineMode()
    {
        ...
        $this->retivamonto->FormValue = ""; // Clear form value
        $this->retislrmonto->FormValue = ""; // Clear form value
    ...
    public function emptyRow()
    {
        ...
        if (
            $CurrentForm->hasValue("x_abono") &&
            $CurrentForm->hasValue("o_abono") &&
            $this->abono->CurrentValue != $this->abono->DefaultValue &&
            !($this->abono->IsForeignKey && $this->getCurrentMasterTable() != "" && $this->abono->CurrentValue == $this->abono->getSessionValue())
        ) {
            return false;
        }
        if (
        ...
        if (
            $CurrentForm->hasValue("x_retivamonto") &&
            $CurrentForm->hasValue("o_retivamonto") &&
            $this->retivamonto->CurrentValue != $this->retivamonto->DefaultValue &&
            !($this->retivamonto->IsForeignKey && $this->getCurrentMasterTable() != "" && $this->retivamonto->CurrentValue == $this->retivamonto->getSessionValue())
        ) {
            return false;
        }
        if (
            $CurrentForm->hasValue("x_retiva") &&
            $CurrentForm->hasValue("o_retiva") &&
            $this->retiva->CurrentValue != $this->retiva->DefaultValue &&
            !($this->retiva->IsForeignKey && $this->getCurrentMasterTable() != "" && $this->retiva->CurrentValue == $this->retiva->getSessionValue())
        ) {
            return false;
        }
        if (
            $CurrentForm->hasValue("x_retislrmonto") &&
            $CurrentForm->hasValue("o_retislrmonto") &&
            $this->retislrmonto->CurrentValue != $this->retislrmonto->DefaultValue &&
            !($this->retislrmonto->IsForeignKey && $this->getCurrentMasterTable() != "" && $this->retislrmonto->CurrentValue == $this->retislrmonto->getSessionValue())
        ) {
            return false;
        }
        if (
            $CurrentForm->hasValue("x_retislr") &&
            $CurrentForm->hasValue("o_retislr") &&
            $this->retislr->CurrentValue != $this->retislr->DefaultValue &&
            !($this->retislr->IsForeignKey && $this->getCurrentMasterTable() != "" && $this->retislr->CurrentValue == $this->retislr->getSessionValue())
        ) {
            return false;
        }
        if (
    ...
    protected function loadFormValues()
    {
        ...
        if ($CurrentForm->hasValue("o_abono")) {
            $this->abono->setOldValue($CurrentForm->getValue("o_abono"));
        }
        ...
        if ($CurrentForm->hasValue("o_retivamonto")) {
            $this->retivamonto->setOldValue($CurrentForm->getValue("o_retivamonto"));
        }
        ...
        if ($CurrentForm->hasValue("o_retiva")) {
            $this->retiva->setOldValue($CurrentForm->getValue("o_retiva"));
        }
        ...
        if ($CurrentForm->hasValue("o_retislrmonto")) {
            $this->retislrmonto->setOldValue($CurrentForm->getValue("o_retislrmonto"));
        }
        ...
        if ($CurrentForm->hasValue("o_retislr")) {
            $this->retislr->setOldValue($CurrentForm->getValue("o_retislr"));
        }
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($this->getCurrentMasterTable() == "cobros_cliente") {
            $this->cobros_cliente->Visible = true; // Need to insert foreign key
            $this->cobros_cliente->CurrentValue = $this->cobros_cliente->getSessionValue();
        }
    ...
    protected function getAddRow()
    {
        ...
        // abono
        $this->abono->setDbValueDef($rsnew, $this->abono->CurrentValue, strval($this->abono->CurrentValue) == "");

        // monto
        $this->monto->setDbValueDef($rsnew, $this->monto->CurrentValue, false);

        // retivamonto
        $this->retivamonto->setDbValueDef($rsnew, $this->retivamonto->CurrentValue, false);

        // retiva
        $this->retiva->setDbValueDef($rsnew, $this->retiva->CurrentValue, false);

        // retislrmonto
        $this->retislrmonto->setDbValueDef($rsnew, $this->retislrmonto->CurrentValue, false);

        // retislr
        $this->retislr->setDbValueDef($rsnew, $this->retislr->CurrentValue, false);
        ...
        if ($this->cobros_cliente->getSessionValue() != "") {
            $rsnew['cobros_cliente'] = $this->cobros_cliente->getSessionValue();
        }
        return $rsnew;
    }
    ...
    protected function restoreAddFormFromRow($row)
    {
        ...
        if (isset($row['retivamonto'])) { // retivamonto
            $this->retivamonto->setFormValue($row['retivamonto']);
        }
        if (isset($row['retiva'])) { // retiva
            $this->retiva->setFormValue($row['retiva']);
        }
        if (isset($row['retislrmonto'])) { // retislrmonto
            $this->retislrmonto->setFormValue($row['retislrmonto']);
        }
        if (isset($row['retislr'])) { // retislr
            $this->retislr->setFormValue($row['retislr']);
        }
        if (isset($row['comprobante'])) { // comprobante
            $this->comprobante->setFormValue($row['comprobante']);
        }
        if (isset($row['cobros_cliente'])) { // cobros_cliente
            $this->cobros_cliente->setFormValue($row['cobros_cliente']);
        }
    }
