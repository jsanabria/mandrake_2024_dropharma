 ...
 * Page class
 */
class ViewOutTdcfcvList extends ViewOutTdcfcv
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ViewOutTdcfcvList";
    ...
    public $FormName = "fview_out_tdcfcvlist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "ViewOutTdcfcvList";
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "ViewOutTdcfcvAdd?" . Config("TABLE_SHOW_DETAIL") . "=";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "ViewOutTdcfcvDelete";
        $this->MultiUpdateUrl = "ViewOutTdcfcvUpdate";
    ...
    public function run()
    {
        ...
        if ($this->IsModal) {
            $this->FormName = "fview_out_tdcfcvgrid";
        }
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server") {
            $savedFilterList = Profile()->getSearchFilters("fview_out_tdcfcvsrch");
        }
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fview_out_tdcfcvsrch", $filters);
    ...
    public function renderListOptions()
    {
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"view_out_tdcfcv\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $Language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($Security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"view_out_tdcfcv\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-btn=\"SaveBtn\">" . $Language->phrase("EditLink") . "</a>";
                } else {
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fview_out_tdcfcvlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fview_out_tdcfcvlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
        ...
        if ($Security->allowList(CurrentProjectID() . 'view_out')) {
            ...
            $body = "<a class=\"btn btn-default ew-row-link ew-detail" . ($this->ListOptions->UseDropDownButton ? " dropdown-toggle" : "") . "\" data-action=\"list\" href=\"" . HtmlEncode("ViewOutList?" . Config("TABLE_SHOW_MASTER") . "=view_out_tdcfcv&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "&" . GetForeignKeyUrl("fk_tipo_documento", $this->tipo_documento->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("ViewOutGrid");
            if ($detailPage->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'view_out_tdcfcv')) {
            ...
            if ($detailPage->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'view_out_tdcfcv')) {
    ...
    protected function renderListOptionsExt()
    {
        ...
        if ($this->DetailPages?->getItem("view_out")?->Visible && $Security->allowList(CurrentProjectID() . 'view_out')) {
            ...
            $url = "ViewOutPreview?t=view_out_tdcfcv&f=" . Encrypt($detailFilter . "|" . implode("|", array_values($masterKeys)));
            $btngrp = "<div data-table=\"view_out\" data-url=\"" . $url . "\" class=\"ew-detail-btn-group btn-group btn-group-sm d-none\">";
            if ($Security->allowList(CurrentProjectID() . 'view_out_tdcfcv')) {
                ...
                $detaillnk = GetUrl("ViewOutList?" . Config("TABLE_SHOW_MASTER") . "=view_out_tdcfcv");
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fview_out_tdcfcvsrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fview_out_tdcfcvsrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions as $listAction) {
            if ($listAction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fview_out_tdcfcvlist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_view_out_tdcfcv", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_view_out_tdcfcv",
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fview_out_tdcfcvsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    */
    ...
    {
        // Verificar si la acción es "exportar"
        if (Get("action") == "export" && Get("tipo_documento") == "TDCFCV") {
            // Obtener las fechas del formulario usando JQuery,
            // que PHPMaker típicamente maneja a través de las variables de POST/GET
            // Sin embargo, para obtener los valores del filtro actual en la página de lista
            // necesitamos usar los IDs de los campos de filtro que PHPMaker usa:

            // Suponiendo que los filtros son los campos "fecha" y tienen los IDs x_fecha y y_fecha
            $fecha_desde = Post("x_fecha") ?? Get("x_fecha") ?? ""; // Asume que se usa el filtro de búsqueda/lista
            $fecha_hasta = Post("y_fecha") ?? Get("y_fecha") ?? "";

            // --- Validación de Fechas ---
            if (empty($fecha_desde) || empty($fecha_hasta)) {
                // Debes mostrar un mensaje de error dentro del contexto de PHPMaker.
                // Usamos la función de error de PHPMaker (SetFailureMessage) y redirigimos.
                // SetFailureMessage("¡Atención! Debe seleccionar un rango de fechas ('Desde' y 'Hasta') en el filtro para poder descargar el reporte.");
                $_SESSION[Config("SESSION_FAILURE_MESSAGE")] = "¡Atención! Debe seleccionar un rango de fechas ('Desde' y 'Hasta') en el filtro para poder descargar el reporte.";

                // Redirige para mostrar el mensaje de error en la página actual
                Redirect(CurrentPageUrl()); 
                return; // Detiene la ejecución para que la redirección surta efecto
            }

            // --- Preparación de la Consulta ---

            // PHPMaker necesita la fecha en formato YYYY-MM-DD para la base de datos
            // Aquí asumimos que $fecha_desde y $fecha_hasta ya están en formato compatible o los convertimos si es necesario.
            // Si la entrada es DD/MM/YYYY, usa DateFormat("Y-m-d", $fecha_desde) (función nativa de PHPMaker)
            $fecha_desde_db = DateFormat("Y-m-d", $fecha_desde);
            $fecha_hasta_db = DateFormat("Y-m-d", $fecha_hasta);
            $sql = "SELECT 
            ...
            FROM 
                salidas AS a 
            WHERE 
                a.tipo_documento = 'TDCFCV' AND a.estatus = 'PROCESADO' AND 
                a.fecha BETWEEN '" . AdjustSql($fecha_desde_db) . "' AND '" . AdjustSql($fecha_hasta_db) . "' 
            ORDER BY a.nro_control";

            // --- Ejecución y Exportación ---

            // Usar la función de base de datos de PHPMaker para obtener los registros
            $results = ExecuteRows($sql); 

            // PHPMaker usa GetDb() para obtener la conexión
            $db = GetDb();

            // 1. Cabeceras HTTP para la descarga del archivo CSV (compatible con Excel)
            header('Content-Type: text/csv; charset=utf-8');
            header('Content-Disposition: attachment; filename="reporte_ventas_TDCFCV.csv"');

            // 2. Abrir el "output"
            $output = fopen('php://output', 'w');

            // 3. Agregar el BOM de UTF-8 para Excel (Crucial para acentos)
            fputs($output, $bom = (chr(0xEF) . chr(0xBB) . chr(0xBF)));

            // 4. Escribir las Cabeceras (deben coincidir con el SELECT)
            $headers = [
                'CODIGO_CLIENTE', 'FECHAEMISION', 'F_VENCIM', 'NoFact', 'NoControl', 
                'MONTODOCUMENTO_BS', 'BIMP', 'IVA', 'MONTO_EN_$', 'FACTOR_DE_CAMBIO'
            ];
            // Usamos ';' como delimitador para compatibilidad con Excel en LATAM
            fputcsv($output, $headers, ';'); 

            // 5. Escribir los datos
            if ($results) {
                foreach ($results as $row) {
                    // fputcsv espera un array indexado numéricamente o asociativamente
                    fputcsv($output, $row, ';'); 
                }
            }
