...
/**
 * Table class for cont_lotes
 */
class ContLotes extends DbTable
{
    protected $SqlFrom = "";
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "cont_lotes";
    }
    ...
    public function getSqlWhere()
    {
        ...
        $this->DefaultFilter = "`comprobante` IS NOT NULL";
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("ContLotesList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "ContLotesView" => $Language->phrase("View"),
            "ContLotesEdit" => $Language->phrase("Edit"),
            "ContLotesAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "ContLotesView",
            Config("API_ADD_ACTION") => "ContLotesAdd",
            Config("API_EDIT_ACTION") => "ContLotesEdit",
            Config("API_DELETE_ACTION") => "ContLotesDelete",
            Config("API_LIST_ACTION") => "ContLotesList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("ContLotesView", $parm);
        } else {
            $url = $this->keyUrl("ContLotesView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ContLotesEdit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ContLotesList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ContLotesAdd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ContLotesList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("ContLotesDelete", $parm);
        }
    }
    ...
    {
        WriteAuditLog(CurrentUserIdentifier(), $typ, 'cont_lotes');
    }
    ...
    public function writeAuditTrailOnAdd(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "A", 'cont_lotes', $fldname, $key, "", $newvalue);
            }
        }
    }
        ...
        foreach (array_keys($rsnew) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && array_key_exists($fldname, $rsold) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                if ($modified) {
                    ...
                    WriteAuditLog($usr, "U", 'cont_lotes', $fldname, $key, $oldvalue, $newvalue);
                }
            }
        }
    }
    ...
    public function writeAuditTrailOnDelete(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "D", 'cont_lotes', $fldname, $key, $oldvalue);
            }
        }
    }
    ...
    public function rowInserted($rsold, $rsnew) {
    	$Cmpb = new CrearComprobante(4, 'TDCFCV', $rsnew["fecha"], $rsnew["nota"], CurrentUserName());
    	if($Cmpb->NoExite == false) { 
    		$sql100 = "UPDATE cont_lotes SET comprobante = " . $Cmpb->NroComprobante . " WHERE id = " . $rsnew["id"] . ";";
    		Execute($sql100);
            $sql100 = "SELECT id  FROM salidas 
                        WHERE
                            tipo_documento = 'TDCFCV'
                            AND estatus = 'PROCESADO'
                            AND fecha = '" . $rsnew["fecha"] . "' AND comprobante IS NULL ORDER BY id;"; 
