 ...
 * Page class
 */
class ContPlanctaList extends ContPlancta
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ContPlanctaList";
    ...
    public $FormName = "fcont_planctalist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "ContPlanctaList";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'cont_plancta';
        $this->TableName = 'cont_plancta';
        ...
        if (!isset($GLOBALS["cont_plancta"]) || $GLOBALS["cont_plancta"]::class == PROJECT_NAMESPACE . "cont_plancta") {
        ...
        // Initialize URLs
        $this->AddUrl = "ContPlanctaAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "ContPlanctaDelete";
        $this->MultiUpdateUrl = "ContPlanctaUpdate";
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                    ...
                    $result["view"] = SameString($pageName, "ContPlanctaView"); // If View page, no primary button
                } else { // List page
    ...
    public function run()
    {
        ...
        // Set up lookup cache
        $this->setupLookupOptions($this->clasificacion);
        $this->setupLookupOptions($this->naturaleza);
        $this->setupLookupOptions($this->tipo);
        $this->setupLookupOptions($this->moneda);
        $this->setupLookupOptions($this->activa);

        // Update form name to avoid conflict
        if ($this->IsModal) {
            $this->FormName = "fcont_planctagrid";
        }
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->subcuenta->AdvancedSearch->toJson(), ","); // Field subcuenta
        ...
        $filterList = Concat($filterList, $this->naturaleza->AdvancedSearch->toJson(), ","); // Field naturaleza
        ...
        $filterList = Concat($filterList, $this->activa->AdvancedSearch->toJson(), ","); // Field activa
        if ($this->BasicSearch->Keyword != "") {
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fcont_planctasrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field subcuenta
        $this->subcuenta->AdvancedSearch->SearchValue = @$filter["x_subcuenta"];
        $this->subcuenta->AdvancedSearch->SearchOperator = @$filter["z_subcuenta"];
        $this->subcuenta->AdvancedSearch->SearchCondition = @$filter["v_subcuenta"];
        $this->subcuenta->AdvancedSearch->SearchValue2 = @$filter["y_subcuenta"];
        $this->subcuenta->AdvancedSearch->SearchOperator2 = @$filter["w_subcuenta"];
        $this->subcuenta->AdvancedSearch->save();
        ...
        // Field naturaleza
        $this->naturaleza->AdvancedSearch->SearchValue = @$filter["x_naturaleza"];
        $this->naturaleza->AdvancedSearch->SearchOperator = @$filter["z_naturaleza"];
        $this->naturaleza->AdvancedSearch->SearchCondition = @$filter["v_naturaleza"];
        $this->naturaleza->AdvancedSearch->SearchValue2 = @$filter["y_naturaleza"];
        $this->naturaleza->AdvancedSearch->SearchOperator2 = @$filter["w_naturaleza"];
        $this->naturaleza->AdvancedSearch->save();
    ...
    public function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->clase, $default, false); // clase
        $this->buildSearchSql($where, $this->grupo, $default, false); // grupo
        $this->buildSearchSql($where, $this->cuenta, $default, false); // cuenta
        $this->buildSearchSql($where, $this->subcuenta, $default, false); // subcuenta
        $this->buildSearchSql($where, $this->descripcion, $default, false); // descripcion
        $this->buildSearchSql($where, $this->clasificacion, $default, false); // clasificacion
        $this->buildSearchSql($where, $this->naturaleza, $default, false); // naturaleza
        $this->buildSearchSql($where, $this->tipo, $default, false); // tipo
        $this->buildSearchSql($where, $this->moneda, $default, false); // moneda
        $this->buildSearchSql($where, $this->activa, $default, false); // activa
        ...
        if (!$default && $this->Command == "search") {
            ...
            $this->clase->AdvancedSearch->save(); // clase
            $this->grupo->AdvancedSearch->save(); // grupo
            $this->cuenta->AdvancedSearch->save(); // cuenta
            $this->subcuenta->AdvancedSearch->save(); // subcuenta
            $this->descripcion->AdvancedSearch->save(); // descripcion
            $this->clasificacion->AdvancedSearch->save(); // clasificacion
            $this->naturaleza->AdvancedSearch->save(); // naturaleza
    ...
    public function showFilterList()
    {
        ...
        // Field clase
        $filter = $this->queryBuilderWhere("clase");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->clase, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->clase->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
        ...
        // Field subcuenta
        $filter = $this->queryBuilderWhere("subcuenta");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->subcuenta, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->subcuenta->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
        ...
        // Field clasificacion
        $filter = $this->queryBuilderWhere("clasificacion");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->clasificacion, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->clasificacion->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
        ...
        // Field activa
        $filter = $this->queryBuilderWhere("activa");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->activa, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->activa->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
        if ($this->BasicSearch->Keyword != "") {
    ...
    public function basicSearchWhere($default = false)
    {
        ...
        // Fields to search
        $searchFlds = [];
        $searchFlds[] = &$this->clase;
        $searchFlds[] = &$this->grupo;
        $searchFlds[] = &$this->cuenta;
        $searchFlds[] = &$this->subcuenta;
        $searchFlds[] = &$this->descripcion;
        $searchFlds[] = &$this->clasificacion;
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->clase->AdvancedSearch->issetSession()) {
            return true;
        }
        ...
        if ($this->subcuenta->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->descripcion->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->clasificacion->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->naturaleza->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    protected function resetAdvancedSearchParms()
    {
        ...
        $this->clase->AdvancedSearch->unsetSession();
        $this->grupo->AdvancedSearch->unsetSession();
        $this->cuenta->AdvancedSearch->unsetSession();
        $this->subcuenta->AdvancedSearch->unsetSession();
        $this->descripcion->AdvancedSearch->unsetSession();
        $this->clasificacion->AdvancedSearch->unsetSession();
        $this->naturaleza->AdvancedSearch->unsetSession();
    ...
    protected function restoreSearchParms()
    {
        ...
        $this->clase->AdvancedSearch->load();
        $this->grupo->AdvancedSearch->load();
        $this->cuenta->AdvancedSearch->load();
        $this->subcuenta->AdvancedSearch->load();
        $this->descripcion->AdvancedSearch->load();
        $this->clasificacion->AdvancedSearch->load();
        $this->naturaleza->AdvancedSearch->load();
    ...
    protected function setupSortOrder()
    {
        ...
        if ($this->Command != "json") {
            $defaultSort = $this->clase->Expression . " ASC" . ", " . $this->grupo->Expression . " ASC" . ", " . $this->cuenta->Expression . " ASC" . ", " . $this->subcuenta->Expression . " ASC"; // Set up default sort
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->subcuenta); // subcuenta
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"cont_plancta\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $Language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($Security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"cont_plancta\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-btn=\"SaveBtn\">" . $Language->phrase("EditLink") . "</a>";
                } else {
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fcont_planctalist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fcont_planctalist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
            $item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"cont_plancta\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("AddLink") . "</a>";
        } else {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "clase");
            $this->createColumnOption($option, "grupo");
            $this->createColumnOption($option, "cuenta");
            $this->createColumnOption($option, "subcuenta");
            $this->createColumnOption($option, "descripcion");
            $this->createColumnOption($option, "clasificacion");
            $this->createColumnOption($option, "moneda");
            $this->createColumnOption($option, "activa");
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fcont_planctasrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fcont_planctasrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions as $listAction) {
            if ($listAction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fcont_planctalist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_cont_plancta", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_cont_plancta",
    ...
    protected function loadSearchValues()
    {
        ...
        // clase
        if ($this->clase->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->clase->AdvancedSearch->SearchValue != "" || $this->clase->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        // subcuenta
        if ($this->subcuenta->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->subcuenta->AdvancedSearch->SearchValue != "" || $this->subcuenta->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        // clasificacion
        if ($this->clasificacion->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->clasificacion->AdvancedSearch->SearchValue != "" || $this->clasificacion->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // naturaleza
        if ($this->naturaleza->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->naturaleza->AdvancedSearch->SearchValue != "" || $this->naturaleza->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        // activa
        if ($this->activa->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->activa->AdvancedSearch->SearchValue != "" || $this->activa->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        return $hasValue;
    ...
    protected function newRow()
    {
        ...
        $row['clase'] = $this->clase->DefaultValue;
        $row['grupo'] = $this->grupo->DefaultValue;
        $row['cuenta'] = $this->cuenta->DefaultValue;
        $row['subcuenta'] = $this->subcuenta->DefaultValue;
        $row['descripcion'] = $this->descripcion->DefaultValue;
        $row['clasificacion'] = $this->clasificacion->DefaultValue;
        $row['naturaleza'] = $this->naturaleza->DefaultValue;
        $row['tipo'] = $this->tipo->DefaultValue;
        $row['moneda'] = $this->moneda->DefaultValue;
        $row['activa'] = $this->activa->DefaultValue;
        return $row;
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            // clase
            $this->clase->setupEditAttributes();
            if (!$this->clase->Raw) {
                $this->clase->AdvancedSearch->SearchValue = HtmlDecode($this->clase->AdvancedSearch->SearchValue);
            }
            $this->clase->EditValue = HtmlEncode($this->clase->AdvancedSearch->SearchValue);
            $this->clase->PlaceHolder = RemoveHtml($this->clase->caption());

            // grupo
            $this->grupo->setupEditAttributes();
            if (!$this->grupo->Raw) {
                $this->grupo->AdvancedSearch->SearchValue = HtmlDecode($this->grupo->AdvancedSearch->SearchValue);
            }
            $this->grupo->EditValue = HtmlEncode($this->grupo->AdvancedSearch->SearchValue);
            $this->grupo->PlaceHolder = RemoveHtml($this->grupo->caption());

            // cuenta
            $this->cuenta->setupEditAttributes();
            if (!$this->cuenta->Raw) {
                $this->cuenta->AdvancedSearch->SearchValue = HtmlDecode($this->cuenta->AdvancedSearch->SearchValue);
            }
            $this->cuenta->EditValue = HtmlEncode($this->cuenta->AdvancedSearch->SearchValue);
            $this->cuenta->PlaceHolder = RemoveHtml($this->cuenta->caption());

            // subcuenta
            $this->subcuenta->setupEditAttributes();
            if (!$this->subcuenta->Raw) {
                $this->subcuenta->AdvancedSearch->SearchValue = HtmlDecode($this->subcuenta->AdvancedSearch->SearchValue);
            }
            $this->subcuenta->EditValue = HtmlEncode($this->subcuenta->AdvancedSearch->SearchValue);
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmail", true) . '" data-caption="' . $Language->phrase("ExportToEmail", true) . '" form="fcont_planctalist" data-ew-action="email" data-custom="false" data-hdr="' . $Language->phrase("ExportToEmail", true) . '" data-exported-selected="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fcont_planctasrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
