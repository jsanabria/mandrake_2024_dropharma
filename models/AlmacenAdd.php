 ...
 * Page class
 */
class AlmacenAdd extends Almacen
{
    use MessagesTrait;
    ...
    // CSS class/style
    public $CurrentPageName = "AlmacenAdd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$rsold) { // Record not loaded
                    ...
                    $this->terminate("AlmacenList"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($rsold)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "AlmacenList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "AlmacenView") {
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "AlmacenList") {
                            ...
                            $returnUrl = "AlmacenList"; // Return list page content
                        }
                    }
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'movimiento' first before field var 'x_movimiento'
        $val = $CurrentForm->hasValue("movimiento") ? $CurrentForm->getValue("movimiento") : $CurrentForm->getValue("x_movimiento");
        if (!$this->movimiento->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->movimiento->Visible = false; // Disable update for API request
            } else {
                $this->movimiento->setFormValue($val);
            }
        }
    ...
    public function renderRow()
    {
        ...
        // movimiento
        $this->movimiento->RowCssClass = "row";
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            ...
            if ($this->movimiento->Visible && $this->movimiento->Required) {
                if (!$this->movimiento->IsDetailKey && EmptyValue($this->movimiento->FormValue)) {
                    $this->movimiento->addErrorMessage(str_replace("%s", $this->movimiento->caption(), $this->movimiento->RequiredErrorMessage));
                }
            }
    ...
    protected function getAddRow()
    {
        ...
        // movimiento
        $this->movimiento->setDbValueDef($rsnew, $this->movimiento->CurrentValue, false);
        return $rsnew;
    }
    ...
    protected function restoreAddFormFromRow($row)
    {
        ...
        if (isset($row['movimiento'])) { // movimiento
            $this->movimiento->setFormValue($row['movimiento']);
        }
    }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("AlmacenList"), "", $this->TableVar, true);
