...
/**
 * Table class for almacenista
 */
class Almacenista extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->clave->SearchOperators = ["=", "<>", "IS NULL", "IS NOT NULL"];
        ...
        $this->tipo->Lookup = new Lookup($this->tipo, 'almacenista', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
        ...
        $this->activo->Lookup = new Lookup($this->activo, 'almacenista', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "almacenista";
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("AlmacenistaList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "AlmacenistaView" => $Language->phrase("View"),
            "AlmacenistaEdit" => $Language->phrase("Edit"),
            "AlmacenistaAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "AlmacenistaView",
            Config("API_ADD_ACTION") => "AlmacenistaAdd",
            Config("API_EDIT_ACTION") => "AlmacenistaEdit",
            Config("API_DELETE_ACTION") => "AlmacenistaDelete",
            Config("API_LIST_ACTION") => "AlmacenistaList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("AlmacenistaView", $parm);
        } else {
            $url = $this->keyUrl("AlmacenistaView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("AlmacenistaEdit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("AlmacenistaList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("AlmacenistaAdd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("AlmacenistaList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("AlmacenistaDelete", $parm);
        }
    }
    ...
    public function rowInserting($rsold, &$rsnew)
    {
        ...
        if(strlen(trim($rsnew["clave"])) < 6) {
    		$this->CancelMessage = "La clave no cumple con los niveles de seguridad; !Cambiela!";
    		return FALSE;
        }
        $sql = "SELECT clave FROM almacenista WHERE clave = '" . $rsnew["clave"] . "'";
    ...
    public function rowUpdating($rsold, &$rsnew)
    {
        // Enter your code here
        // To cancel, set return value to false
        if(isset($rsnew["clave"])) {
        if($rsold["clave"] != $rsnew["clave"]) {
        	if(strlen(trim($rsnew["clave"])) < 6) {
    			$this->CancelMessage = "La clave no cumple con los niveles de seguridad; !Cambiela!";
    			return FALSE;
    		}
