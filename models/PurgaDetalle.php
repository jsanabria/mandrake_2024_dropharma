...
/**
 * Table class for purga_detalle
 */
class PurgaDetalle extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        // Update Table
        $this->UpdateTable = "purga_detalle";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // purga
        $this->purga = new DbField(
            $this, // Table
            'x_purga', // Variable name
            'purga', // Name
            '`purga`', // Expression
            '`purga`', // Basic search expression
            19, // Type
            10, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`purga`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->purga->InputTextType = "text";
        $this->purga->Raw = true;
        $this->purga->IsForeignKey = true; // Foreign key field
        $this->purga->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->purga->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN", "IS NULL", "IS NOT NULL"];
        $this->Fields['purga'] = &$this->purga;
        ...
        $this->fabricante->Lookup = new Lookup($this->fabricante, 'fabricante', false, 'Id', ["nombre","","",""], '', '', [], ["x_articulo"], [], [], [], [], false, '`nombre`', '', "`nombre`");
        ...
        $this->articulo->Lookup = new Lookup($this->articulo, 'articulo', false, 'id', ["principio_activo","presentacion","nombre_comercial","codigo_de_barra"], '', '', ["x_fabricante"], [], ["fabricante"], ["x_fabricante"], [], [], false, '`principio_activo` ASC', '', "CONCAT(COALESCE(`principio_activo`, ''),'" . ValueSeparator(1, $this->articulo) . "',COALESCE(`presentacion`,''),'" . ValueSeparator(2, $this->articulo) . "',COALESCE(`nombre_comercial`,''),'" . ValueSeparator(3, $this->articulo) . "',COALESCE(`codigo_de_barra`,''))");
        ...
        // id_entradas_salidas
        $this->id_entradas_salidas = new DbField(
            $this, // Table
            'x_id_entradas_salidas', // Variable name
            'id_entradas_salidas', // Name
            '`id_entradas_salidas`', // Expression
            '`id_entradas_salidas`', // Basic search expression
            21, // Type
            20, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`id_entradas_salidas`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->id_entradas_salidas->InputTextType = "text";
        $this->id_entradas_salidas->Raw = true;
        $this->id_entradas_salidas->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->id_entradas_salidas->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN", "IS NULL", "IS NOT NULL"];
        $this->Fields['id_entradas_salidas'] = &$this->id_entradas_salidas;
        ...
        $this->almacen->Lookup = new Lookup($this->almacen, 'almacen', false, 'codigo', ["descripcion","","",""], '', '', [], [], [], [], [], [], false, '`descripcion` ASC', '', "`descripcion`");
        ...
        // procesado
        $this->procesado = new DbField(
            $this, // Table
            'x_procesado', // Variable name
            'procesado', // Name
            '`procesado`', // Expression
            '`procesado`', // Basic search expression
            200, // Type
            1, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`procesado`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->procesado->addMethod("getDefault", fn() => "N");
        $this->procesado->InputTextType = "text";
        $this->procesado->Raw = true;
        $this->procesado->Lookup = new Lookup($this->procesado, 'purga_detalle', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
        $this->procesado->OptionCount = 2;
        $this->procesado->SearchOperators = ["=", "<>", "IS NULL", "IS NOT NULL"];
    ...
    public function getMasterFilterFromSession()
    {
        // Master filter
        $masterFilter = "";
        if ($this->getCurrentMasterTable() == "purga") {
            $masterTable = Container("purga");
            if ($this->purga->getSessionValue() != "") {
                $masterFilter .= "" . GetKeyFilter($masterTable->id, $this->purga->getSessionValue(), $masterTable->id->DataType, $masterTable->Dbid);
            } else {
                return "";
            }
        }
    ...
    public function getDetailFilterFromSession()
    {
        ...
        if ($this->getCurrentMasterTable() == "purga") {
            $masterTable = Container("purga");
            if ($this->purga->getSessionValue() != "") {
                $detailFilter .= "" . GetKeyFilter($this->purga, $this->purga->getSessionValue(), $masterTable->id->DataType, $this->Dbid);
            } else {
                return "";
            }
        }
    ...
    public function getMasterFilter($masterTable, $keys)
    {
        ...
        switch ($masterTable->TableVar) {
            case "purga":
                ...
                if ($validKeys) {
                    return GetKeyFilter($masterTable->id, $keys["purga"], $this->purga->DataType, $this->Dbid);
                }
                break;
        }
    ...
    public function getDetailFilter($masterTable)
    {
        switch ($masterTable->TableVar) {
            case "purga":
                return GetKeyFilter($this->purga, $masterTable->id->DbValue, $masterTable->id->DataType, $masterTable->Dbid);
        }
        return "";
    }
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "purga_detalle";
    }
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->purga->DbValue = $row['purga'];
        $this->fabricante->DbValue = $row['fabricante'];
        $this->articulo->DbValue = $row['articulo'];
        $this->id_entradas_salidas->DbValue = $row['id_entradas_salidas'];
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("PurgaDetalleList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "PurgaDetalleView" => $Language->phrase("View"),
            "PurgaDetalleEdit" => $Language->phrase("Edit"),
            "PurgaDetalleAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "PurgaDetalleView",
            Config("API_ADD_ACTION") => "PurgaDetalleAdd",
            Config("API_EDIT_ACTION") => "PurgaDetalleEdit",
            Config("API_DELETE_ACTION") => "PurgaDetalleDelete",
            Config("API_LIST_ACTION") => "PurgaDetalleList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("PurgaDetalleView", $parm);
        } else {
            $url = $this->keyUrl("PurgaDetalleView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("PurgaDetalleEdit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("PurgaDetalleList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("PurgaDetalleAdd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("PurgaDetalleList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("PurgaDetalleDelete", $parm);
        }
    }
    ...
    {
        if ($this->getCurrentMasterTable() == "purga" && !ContainsString($url, Config("TABLE_SHOW_MASTER") . "=")) {
            $url .= (ContainsString($url, "?") ? "&" : "?") . Config("TABLE_SHOW_MASTER") . "=" . $this->getCurrentMasterTable();
            $url .= "&" . GetForeignKeyUrl("fk_id", $this->purga->getSessionValue()); // Use Session Value
        }
        return $url;
    }
    ...
    public function renderListRow()
    {
        ...
        // purga
        $this->purga->HrefValue = "";
        $this->purga->TooltipValue = "";
        ...
        // id_entradas_salidas
        $this->id_entradas_salidas->HrefValue = "";
        $this->id_entradas_salidas->TooltipValue = "";
    ...
    public function renderEditRow()
    {
        ...
        // purga
        $this->purga->setupEditAttributes();
        if ($this->purga->getSessionValue() != "") {
            $this->purga->CurrentValue = GetForeignKeyValue($this->purga->getSessionValue());
            $this->purga->ViewValue = $this->purga->CurrentValue;
            $this->purga->ViewValue = FormatNumber($this->purga->ViewValue, $this->purga->formatPattern());
        } else {
            $this->purga->EditValue = $this->purga->CurrentValue;
            $this->purga->PlaceHolder = RemoveHtml($this->purga->caption());
            if (strval($this->purga->EditValue) != "" && is_numeric($this->purga->EditValue)) {
                $this->purga->EditValue = FormatNumber($this->purga->EditValue, null);
            }
        }
        ...
        // id_entradas_salidas
        $this->id_entradas_salidas->setupEditAttributes();
        $this->id_entradas_salidas->EditValue = $this->id_entradas_salidas->CurrentValue;
        $this->id_entradas_salidas->PlaceHolder = RemoveHtml($this->id_entradas_salidas->caption());
        if (strval($this->id_entradas_salidas->EditValue) != "" && is_numeric($this->id_entradas_salidas->EditValue)) {
            $this->id_entradas_salidas->EditValue = FormatNumber($this->id_entradas_salidas->EditValue, null);
        }
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->purga);
                    $doc->exportCaption($this->fabricante);
                    $doc->exportCaption($this->articulo);
                    $doc->exportCaption($this->id_entradas_salidas);
                    $doc->exportCaption($this->almacen);
                    $doc->exportCaption($this->lote);
                    $doc->exportCaption($this->fecha);
                    $doc->exportCaption($this->cantidad_articulo);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->purga);
                        $doc->exportField($this->fabricante);
                        $doc->exportField($this->articulo);
                        $doc->exportField($this->id_entradas_salidas);
                        $doc->exportField($this->almacen);
                        $doc->exportField($this->lote);
                        $doc->exportField($this->fecha);
                        $doc->exportField($this->cantidad_articulo);
    ...
    public function rowInserting($rsold, &$rsnew)
    {
        ...
        $sql = "SELECT procesado FROM purga WHERE id = '" . $rsnew["purga"] . "';";
        $procesado = ExecuteScalar($sql);
        if($procesado == "S") {
    		$this->CancelMessage = "Conteo manual procesado; no se pueden agregar m&aacute;s art&iacute;los.";
        	return FALSE;
        }
        $sql = "SELECT articulo FROM purga_detalle WHERE purga = '" . $rsnew["purga"] . "' AND articulo = '" . $rsnew["articulo"] . "' AND lote = '" . $rsnew["lote"] . "';";
        if($row = ExecuteRow($sql)) {
        	$sql = "SELECT 
        				b.nombre AS fabricante, a.principio_activo AS articulo,
        				a.presentacion 
        			FROM 
        				articulo AS a 
        	...
        	$row2 = ExecuteRow($sql);
    		$this->CancelMessage = "El art&iacute;culo " . $row2["fabricante"] . "; " . $row2["articulo"] . "; " . $row2["presentacion"] . "; Lote " . $rsnew["lote"] . " ya est&aacute; agregado en el conteo.";
        	return FALSE;
        }
        return TRUE;
    }
    ...
    public function rowUpdating($rsold, &$rsnew)
    {
        // Enter your code here
        // To cancel, set return value to false
        $sql = "SELECT procesado FROM purga WHERE id = '" . $rsold["purga"] . "';";
        $procesado = ExecuteScalar($sql);
        if($procesado == "S") {
    		$this->CancelMessage = "Conteo manual procesado; no se puede modificar.";
        	return FALSE;
        }
        $sql = "SELECT articulo FROM purga_detalle WHERE purga = '" . $rsold["purga"] . "' AND articulo = '" . $rsold["articulo"] . "' AND lote = '" . $rsnew["lote"] . "';";
        if($row = ExecuteRow($sql)) {
        	$sql = "SELECT 
        				b.nombre AS fabricante, a.principio_activo AS articulo,
        				a.presentacion 
    ...
    public function rowDeleting(&$rs)
    {
        // Enter your code here
        // To cancel, set return value to False
        $sql = "SELECT procesado FROM purga WHERE id = '" . $rs["purga"] . "';";
        $procesado = ExecuteScalar($sql);
        if($procesado == "S") {
    		$this->CancelMessage = "Conteo manual procesado; no se puede eliminar.";
        	return FALSE;
        }
        return TRUE;
    }
    ...
    public function rowRendered()
    {
        ...
        if ($this->PageID == "list" || $this->PageID == "view") {
    		if ($this->procesado->CurrentValue == "S") { 
    			$color = "background-color: #51aa51; color: #ffffff;";
    			$this->id->CellAttrs["style"] = $color;
    			$this->fecha->CellAttrs["style"] = $color;
    			$this->procesado->CellAttrs["style"] = $color;
    			$this->lote->CellAttrs["style"] = $color;
    			$this->articulo->CellAttrs["style"] = $color;
    		}
    		$articulo = intval($this->articulo->CurrentValue);
    	}
    }
