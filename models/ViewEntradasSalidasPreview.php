 ...
 * Page class
 */
class ViewEntradasSalidasPreview extends ViewEntradasSalidas
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ViewEntradasSalidasPreview";
    ...
    // CSS class/style
    public $CurrentPageName = "ViewEntradasSalidasPreview";
    ...
    protected function masterKeyUrl()
    {
        ...
        if ($masterTblVar == "view_entradas") {
            $url = "" . Config("TABLE_SHOW_MASTER") . "=view_entradas&" . GetForeignKeyUrl("fk_tipo_documento", $this->tipo_documento->QueryStringValue) . "&" . GetForeignKeyUrl("fk_id", $this->id_documento->QueryStringValue) . "";
        }
        if ($masterTblVar == "view_salidas") {
            $url = "" . Config("TABLE_SHOW_MASTER") . "=view_salidas&" . GetForeignKeyUrl("fk_tipo_documento", $this->tipo_documento->QueryStringValue) . "&" . GetForeignKeyUrl("fk_id", $this->id_documento->QueryStringValue) . "";
        }
        return $url;
    }
    ...
    protected function setupForeignKeys($keys)
    {
        $masterTblVar = Get("t", "");
        if ($masterTblVar == "view_entradas") {
            $this->tipo_documento->setQueryStringValue($keys[0] ?? "");
            $this->id_documento->setQueryStringValue($keys[1] ?? "");
        }
        if ($masterTblVar == "view_salidas") {
            $this->tipo_documento->setQueryStringValue($keys[0] ?? "");
            $this->id_documento->setQueryStringValue($keys[1] ?? "");
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("ViewEntradasSalidasList"), "", $this->TableVar, true);
        $pageId = "preview";
    ...
    public function listOptionsRendered()
    {
        ...
        //$this->ListOptions["new"]->Body = "xxx";
    		if (intval($this->cantidad_articulo->ViewValue)!= intval($this->packer_cantidad->ViewValue) and intval($this->packer_cantidad->ViewValue) > 0) { 
        	...
        	elseif (intval($this->cantidad_articulo->ViewValue)== intval($this->packer_cantidad->ViewValue)) {
