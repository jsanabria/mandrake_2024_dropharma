 ...
 * Page class
 */
class CobrosClienteView extends CobrosCliente
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "CobrosClienteView";
    ...
    // CSS class/style
    public $CurrentPageName = "CobrosClienteView";
    ...
    public function run()
    {
        ...
        if (($keyValue = Get("id") ?? Route("id")) !== null) {
            ...
            $returnUrl = "CobrosClienteList"; // Return to list
        }
            ...
            case "show": // Get a record to display
                if (!$this->IsModal && !IsApi()) { // Normal view page
                    ...
                    if (!$matchRecord) {
                        ...
                        $returnUrl = "CobrosClienteList"; // No matching record, return to list
                    } else {
    ...
    protected function setupOtherOptions()
    {
        ...
        */
        ...
        $item = &$option->add("detail_cobros_cliente_factura");
        $body = $Language->phrase("ViewPageDetailLink") . $Language->tablePhrase("cobros_cliente_factura", "TblCaption");
        $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode(GetUrl("CobrosClienteFacturaList?" . Config("TABLE_SHOW_MASTER") . "=cobros_cliente&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "")) . "\">" . $body . "</a>";
        $links = "";
        ...
        if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'cobros_cliente')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailViewLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=cobros_cliente_factura"))) . "\">" . $Language->phrase("MasterDetailViewLink", null) . "</a></li>";
            if ($detailViewTblVar != "") {
                $detailViewTblVar .= ",";
            }
        ...
        if ($detailPageObj->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'cobros_cliente')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-edit\" data-action=\"edit\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailEditLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=cobros_cliente_factura"))) . "\">" . $Language->phrase("MasterDetailEditLink", null) . "</a></li>";
            if ($detailEditTblVar != "") {
                $detailEditTblVar .= ",";
            }
    ...
    public function pageDataRendered(&$footer) {
    	// Example:
    	$sql = "SELECT COUNT(id_documento) AS cantidad FROM cobros_cliente_factura WHERE cobros_cliente = " . $this->id->CurrentValue . ";";
    	$cantidad = ExecuteScalar($sql);
    	$footer = '<div class="alert alert-success" role="success">
    					Facturas asocialdas al cobro ' . $cantidad . '
    				</div>';
    }
