 ...
 * Page class
 */
class PurgaDetallePreview extends PurgaDetalle
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PurgaDetallePreview";
    ...
    // CSS class/style
    public $CurrentPageName = "PurgaDetallePreview";
    ...
    protected function masterKeyUrl()
    {
        ...
        if ($masterTblVar == "purga") {
            $url = "" . Config("TABLE_SHOW_MASTER") . "=purga&" . GetForeignKeyUrl("fk_id", $this->purga->QueryStringValue) . "";
        }
        return $url;
    }
    ...
    protected function setupForeignKeys($keys)
    {
        $masterTblVar = Get("t", "");
        if ($masterTblVar == "purga") {
            $this->purga->setQueryStringValue($keys[0] ?? "");
        }
    }
