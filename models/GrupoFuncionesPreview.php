 ...
 * Page class
 */
class GrupoFuncionesPreview extends GrupoFunciones
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "GrupoFuncionesPreview";
    ...
    // CSS class/style
    public $CurrentPageName = "GrupoFuncionesPreview";
    ...
    protected function masterKeyUrl()
    {
        ...
        if ($masterTblVar == "userlevels") {
            $url = "" . Config("TABLE_SHOW_MASTER") . "=userlevels&" . GetForeignKeyUrl("fk_userlevelid", $this->grupo->QueryStringValue) . "";
        }
        return $url;
    }
    ...
    protected function setupForeignKeys($keys)
    {
        $masterTblVar = Get("t", "");
        if ($masterTblVar == "userlevels") {
            $this->grupo->setQueryStringValue($keys[0] ?? "");
        }
    }
