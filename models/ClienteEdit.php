...
class ClienteEdit extends Cliente
{
    ...
    protected function editRow()
    {
        ...
        if ($this->foto1->Visible && !$this->foto1->Upload->KeepFile) {
            if (!EmptyValue($this->foto1->Upload->FileName)) {
                ...
                $this->foto1->setDbValueDef($rsnew, $this->foto1->Upload->FileName, $this->foto1->ReadOnly);
            }
        }
        if ($this->foto2->Visible && !$this->foto2->Upload->KeepFile) {
            if (!EmptyValue($this->foto2->Upload->FileName)) {
                FixUploadFileNames($this->foto2);
                $this->foto2->setDbValueDef($rsnew, $this->foto2->Upload->FileName, $this->foto2->ReadOnly);
            }
        }
    ...
    protected function getEditRow($rsold)
    {
        ...
        // sucursal
        $this->sucursal->setDbValueDef($rsnew, $this->sucursal->CurrentValue, $this->sucursal->ReadOnly);

        // contacto
        $this->contacto->setDbValueDef($rsnew, $this->contacto->CurrentValue, $this->contacto->ReadOnly);
        ...
        // web
        $this->web->setDbValueDef($rsnew, $this->web->CurrentValue, $this->web->ReadOnly);

        // tipo_cliente
        $this->tipo_cliente->setDbValueDef($rsnew, $this->tipo_cliente->CurrentValue, $this->tipo_cliente->ReadOnly);
        ...
        // limite_credito
        $this->limite_credito->setDbValueDef($rsnew, $this->limite_credito->CurrentValue, $this->limite_credito->ReadOnly);

        // condicion
        $this->condicion->setDbValueDef($rsnew, $this->condicion->CurrentValue, $this->condicion->ReadOnly);
        ...
        // foto1
        if ($this->foto1->Visible && !$this->foto1->ReadOnly && !$this->foto1->Upload->KeepFile) {
    ...
    protected function restoreEditFormFromRow($row)
    {
        ...
        if (isset($row['sucursal'])) { // sucursal
            $this->sucursal->CurrentValue = $row['sucursal'];
        }
        ...
        if (isset($row['zona'])) { // zona
            $this->zona->CurrentValue = $row['zona'];
        }
        ...
        if (isset($row['web'])) { // web
            $this->web->CurrentValue = $row['web'];
        }
        if (isset($row['tipo_cliente'])) { // tipo_cliente
            $this->tipo_cliente->CurrentValue = $row['tipo_cliente'];
        }
        ...
        if (isset($row['limite_credito'])) { // limite_credito
            $this->limite_credito->CurrentValue = $row['limite_credito'];
        }
        if (isset($row['condicion'])) { // condicion
            $this->condicion->CurrentValue = $row['condicion'];
        }
        ...
        if (isset($row['foto1'])) { // foto1
            $this->foto1->CurrentValue = $row['foto1'];
        }
        if (isset($row['foto2'])) { // foto2
            $this->foto2->CurrentValue = $row['foto2'];
        }
