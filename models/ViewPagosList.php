 ...
 * Page class
 */
class ViewPagosList extends ViewPagos
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ViewPagosList";
    ...
    public $FormName = "fview_pagoslist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "ViewPagosList";
    ...
    public function setVisibility()
    {
        ...
        $this->fecha_resgistro->setVisibility();
    }
    ...
    public function __construct()
    {
        ...
        // Table object (view_pagos)
        if (!isset($GLOBALS["view_pagos"]) || $GLOBALS["view_pagos"]::class == PROJECT_NAMESPACE . "view_pagos") {
            $GLOBALS["view_pagos"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "ViewPagosAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "ViewPagosDelete";
        $this->MultiUpdateUrl = "ViewPagosUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'view_pagos');
        }
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                    ...
                    $result["view"] = SameString($pageName, "ViewPagosView"); // If View page, no primary button
                } else { // List page
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->fecha_pago->AdvancedSearch->toJson(), ","); // Field fecha_pago
        ...
        $filterList = Concat($filterList, $this->banco_destino->AdvancedSearch->toJson(), ","); // Field banco_destino
        ...
        $filterList = Concat($filterList, $this->fecha_resgistro->AdvancedSearch->toJson(), ","); // Field fecha_resgistro
        if ($this->BasicSearch->Keyword != "") {
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fview_pagossrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field fecha_pago
        $this->fecha_pago->AdvancedSearch->SearchValue = @$filter["x_fecha_pago"];
        $this->fecha_pago->AdvancedSearch->SearchOperator = @$filter["z_fecha_pago"];
        $this->fecha_pago->AdvancedSearch->SearchCondition = @$filter["v_fecha_pago"];
        $this->fecha_pago->AdvancedSearch->SearchValue2 = @$filter["y_fecha_pago"];
        $this->fecha_pago->AdvancedSearch->SearchOperator2 = @$filter["w_fecha_pago"];
        $this->fecha_pago->AdvancedSearch->save();
        ...
        // Field banco_destino
        $this->banco_destino->AdvancedSearch->SearchValue = @$filter["x_banco_destino"];
        $this->banco_destino->AdvancedSearch->SearchOperator = @$filter["z_banco_destino"];
        $this->banco_destino->AdvancedSearch->SearchCondition = @$filter["v_banco_destino"];
        $this->banco_destino->AdvancedSearch->SearchValue2 = @$filter["y_banco_destino"];
        $this->banco_destino->AdvancedSearch->SearchOperator2 = @$filter["w_banco_destino"];
        $this->banco_destino->AdvancedSearch->save();
        ...
        // Field fecha_resgistro
        $this->fecha_resgistro->AdvancedSearch->SearchValue = @$filter["x_fecha_resgistro"];
        $this->fecha_resgistro->AdvancedSearch->SearchOperator = @$filter["z_fecha_resgistro"];
        $this->fecha_resgistro->AdvancedSearch->SearchCondition = @$filter["v_fecha_resgistro"];
        $this->fecha_resgistro->AdvancedSearch->SearchValue2 = @$filter["y_fecha_resgistro"];
        $this->fecha_resgistro->AdvancedSearch->SearchOperator2 = @$filter["w_fecha_resgistro"];
        $this->fecha_resgistro->AdvancedSearch->save();
    ...
    public function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->fecha_documento, $default, false); // fecha_documento
        ...
        $this->buildSearchSql($where, $this->fecha_pago, $default, false); // fecha_pago
        ...
        $this->buildSearchSql($where, $this->banco_destino, $default, false); // banco_destino
        ...
        $this->buildSearchSql($where, $this->fecha_resgistro, $default, false); // fecha_resgistro
        ...
        if (!$default && $this->Command == "search") {
            ...
            $this->fecha_documento->AdvancedSearch->save(); // fecha_documento
            ...
            $this->fecha_pago->AdvancedSearch->save(); // fecha_pago
            $this->referencia->AdvancedSearch->save(); // referencia
            $this->banco->AdvancedSearch->save(); // banco
            $this->banco_destino->AdvancedSearch->save(); // banco_destino
            ...
            $this->fecha_resgistro->AdvancedSearch->save(); // fecha_resgistro
    ...
    public function showFilterList()
    {
        ...
        // Field fecha_documento
        $filter = $this->queryBuilderWhere("fecha_documento");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->fecha_documento, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->fecha_documento->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }

        // Field base_imponible
        $filter = $this->queryBuilderWhere("base_imponible");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->base_imponible, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->base_imponible->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
        ...
        // Field fecha_pago
        $filter = $this->queryBuilderWhere("fecha_pago");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->fecha_pago, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->fecha_pago->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
        ...
        // Field banco_destino
        $filter = $this->queryBuilderWhere("banco_destino");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->banco_destino, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->banco_destino->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
        ...
        // Field tasa_usd
        $filter = $this->queryBuilderWhere("tasa_usd");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->tasa_usd, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->tasa_usd->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
        ...
        // Field fecha_resgistro
        $filter = $this->queryBuilderWhere("fecha_resgistro");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->fecha_resgistro, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->fecha_resgistro->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
        if ($this->BasicSearch->Keyword != "") {
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->fecha_documento->AdvancedSearch->issetSession()) {
            return true;
        }
        ...
        if ($this->fecha_pago->AdvancedSearch->issetSession()) {
            return true;
        }
        ...
        if ($this->banco_destino->AdvancedSearch->issetSession()) {
            return true;
        }
        ...
        if ($this->fecha_resgistro->AdvancedSearch->issetSession()) {
            return true;
        }
        return false;
    }
    ...
    protected function resetAdvancedSearchParms()
    {
        ...
        $this->fecha_documento->AdvancedSearch->unsetSession();
        ...
        $this->fecha_pago->AdvancedSearch->unsetSession();
        $this->referencia->AdvancedSearch->unsetSession();
        $this->banco->AdvancedSearch->unsetSession();
        $this->banco_destino->AdvancedSearch->unsetSession();
        ...
        $this->fecha_resgistro->AdvancedSearch->unsetSession();
    }
    ...
    protected function restoreSearchParms()
    {
        ...
        $this->fecha_resgistro->AdvancedSearch->load();
    }
    ...
    protected function setupSortOrder()
    {
        ...
        if ($this->Command != "json") {
            $defaultSort = $this->fecha_documento->Expression . " DESC" . ", " . $this->id->Expression . " DESC"; // Set up default sort
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->fecha_documento); // fecha_documento
            $this->updateSort($this->base_imponible); // base_imponible
            $this->updateSort($this->total); // total
            $this->updateSort($this->tipo_pago); // tipo_pago
            $this->updateSort($this->fecha_pago); // fecha_pago
            ...
            $this->updateSort($this->fecha_resgistro); // fecha_resgistro
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fview_pagoslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fview_pagoslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "fecha_documento");
            $this->createColumnOption($option, "base_imponible");
            $this->createColumnOption($option, "total");
            $this->createColumnOption($option, "tipo_pago");
            $this->createColumnOption($option, "fecha_pago");
            $this->createColumnOption($option, "referencia");
            $this->createColumnOption($option, "banco");
            $this->createColumnOption($option, "banco_destino");
            ...
            $this->createColumnOption($option, "fecha_resgistro");
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fview_pagossrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fview_pagossrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions as $listAction) {
            if ($listAction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fview_pagoslist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_view_pagos", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_view_pagos",
    ...
    protected function loadSearchValues()
    {
        ...
        // fecha_documento
        if ($this->fecha_documento->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->fecha_documento->AdvancedSearch->SearchValue != "" || $this->fecha_documento->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        // fecha_pago
        if ($this->fecha_pago->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->fecha_pago->AdvancedSearch->SearchValue != "" || $this->fecha_pago->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // referencia
        if ($this->referencia->AdvancedSearch->get()) {
            $hasValue = true;
        ...
        // banco_destino
        if ($this->banco_destino->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->banco_destino->AdvancedSearch->SearchValue != "" || $this->banco_destino->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        // tasa_usd
        if ($this->tasa_usd->AdvancedSearch->get()) {
            $hasValue = true;
        ...
        // fecha_resgistro
        if ($this->fecha_resgistro->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->fecha_resgistro->AdvancedSearch->SearchValue != "" || $this->fecha_resgistro->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        return $hasValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            $this->fecha_documento->EditValue = HtmlEncode(FormatDateTime(UnFormatDateTime($this->fecha_documento->AdvancedSearch->SearchValue, $this->fecha_documento->formatPattern()), $this->fecha_documento->formatPattern()));
            $this->fecha_documento->PlaceHolder = RemoveHtml($this->fecha_documento->caption());

            // base_imponible
            $this->base_imponible->setupEditAttributes();
            $this->base_imponible->EditValue = $this->base_imponible->AdvancedSearch->SearchValue;
            ...
            $this->fecha_pago->EditValue = HtmlEncode(FormatDateTime(UnFormatDateTime($this->fecha_pago->AdvancedSearch->SearchValue, $this->fecha_pago->formatPattern()), $this->fecha_pago->formatPattern()));
            $this->fecha_pago->PlaceHolder = RemoveHtml($this->fecha_pago->caption());
            $this->fecha_pago->setupEditAttributes();
            $this->fecha_pago->EditValue2 = HtmlEncode(FormatDateTime(UnFormatDateTime($this->fecha_pago->AdvancedSearch->SearchValue2, $this->fecha_pago->formatPattern()), $this->fecha_pago->formatPattern()));
            $this->fecha_pago->PlaceHolder = RemoveHtml($this->fecha_pago->caption());
            ...
            $curVal = trim(strval($this->banco_destino->AdvancedSearch->SearchValue));
            if ($curVal != "") {
                $this->banco_destino->AdvancedSearch->ViewValue = $this->banco_destino->lookupCacheOption($curVal);
            } else {
                $this->banco_destino->AdvancedSearch->ViewValue = $this->banco_destino->Lookup !== null && is_array($this->banco_destino->lookupOptions()) && count($this->banco_destino->lookupOptions()) > 0 ? $curVal : null;
            }
            if ($this->banco_destino->AdvancedSearch->ViewValue !== null) { // Load from cache
                $this->banco_destino->EditValue = array_values($this->banco_destino->lookupOptions());
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = SearchFilter($this->banco_destino->Lookup->getTable()->Fields["id"]->searchExpression(), "=", $this->banco_destino->AdvancedSearch->SearchValue, $this->banco_destino->Lookup->getTable()->Fields["id"]->searchDataType(), "");
                }
            ...
            $this->tasa_usd->EditValue = $this->tasa_usd->AdvancedSearch->SearchValue;
            ...
            $this->fecha_resgistro->EditValue = HtmlEncode(FormatDateTime(UnFormatDateTime($this->fecha_resgistro->AdvancedSearch->SearchValue, $this->fecha_resgistro->formatPattern()), $this->fecha_resgistro->formatPattern()));
            $this->fecha_resgistro->PlaceHolder = RemoveHtml($this->fecha_resgistro->caption());
            $this->fecha_resgistro->setupEditAttributes();
            $this->fecha_resgistro->EditValue2 = HtmlEncode(FormatDateTime(UnFormatDateTime($this->fecha_resgistro->AdvancedSearch->SearchValue2, $this->fecha_resgistro->formatPattern()), $this->fecha_resgistro->formatPattern()));
            $this->fecha_resgistro->PlaceHolder = RemoveHtml($this->fecha_resgistro->caption());
        }
    ...
    protected function validateSearch()
    {
        ...
        if (!CheckDate($this->fecha_pago->AdvancedSearch->SearchValue, $this->fecha_pago->formatPattern())) {
            $this->fecha_pago->addErrorMessage($this->fecha_pago->getErrorMessage(false));
        }
        if (!CheckDate($this->fecha_pago->AdvancedSearch->SearchValue2, $this->fecha_pago->formatPattern())) {
            $this->fecha_pago->addErrorMessage($this->fecha_pago->getErrorMessage(false));
        }
        if (!CheckDate($this->fecha_resgistro->AdvancedSearch->SearchValue, $this->fecha_resgistro->formatPattern())) {
            $this->fecha_resgistro->addErrorMessage($this->fecha_resgistro->getErrorMessage(false));
        }
        if (!CheckDate($this->fecha_resgistro->AdvancedSearch->SearchValue2, $this->fecha_resgistro->formatPattern())) {
            $this->fecha_resgistro->addErrorMessage($this->fecha_resgistro->getErrorMessage(false));
        }
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fview_pagossrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
