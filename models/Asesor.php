...
/**
 * Table class for asesor
 */
class Asesor extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->activo->Lookup = new Lookup($this->activo, 'asesor', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
    ...
    public function getDetailUrl()
    {
        // Detail url
        $detailUrl = "";
        if ($this->getCurrentDetailTable() == "asesor_cliente") {
            $detailUrl = Container("asesor_cliente")->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
            $detailUrl .= "&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue);
        }
        if ($this->getCurrentDetailTable() == "asesor_fabricante") {
            $detailUrl = Container("asesor_fabricante")->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
            $detailUrl .= "&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue);
        }
        if ($detailUrl == "") {
            $detailUrl = "AsesorList";
        }
        return $detailUrl;
    }
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "asesor";
    }
    ...
    {
        // Cascade Update detail table 'asesor_cliente'
        $cascadeUpdate = false;
        $rscascade = [];
        if ($rsold && (isset($rs['id']) && $rsold['id'] != $rs['id'])) { // Update detail field 'asesor'
            $cascadeUpdate = true;
            $rscascade['asesor'] = $rs['id'];
        }
        if ($cascadeUpdate) {
            $rswrk = Container("asesor_cliente")->loadRs("`asesor` = " . QuotedValue($rsold['id'], DataType::NUMBER, 'DB'))->fetchAllAssociative();
            foreach ($rswrk as $rsdtlold) {
                ...
                $success = Container("asesor_cliente")->rowUpdating($rsdtlold, $rsdtlnew);
                if ($success) {
                    $success = Container("asesor_cliente")->update($rscascade, $rskey, $rsdtlold);
                }
                if (!$success) {
                    return false;
                }
                // Call Row_Updated event
                Container("asesor_cliente")->rowUpdated($rsdtlold, $rsdtlnew);
            }
        }

        // Cascade Update detail table 'asesor_fabricante'
        $cascadeUpdate = false;
        $rscascade = [];
        if ($rsold && (isset($rs['id']) && $rsold['id'] != $rs['id'])) { // Update detail field 'asesor'
            $cascadeUpdate = true;
        ...
        if ($cascadeUpdate) {
            $rswrk = Container("asesor_fabricante")->loadRs("`asesor` = " . QuotedValue($rsold['id'], DataType::NUMBER, 'DB'))->fetchAllAssociative();
            foreach ($rswrk as $rsdtlold) {
                $rskey = [];
                $fldname = 'id';
                $rskey[$fldname] = $rsdtlold[$fldname];
                $rsdtlnew = array_merge($rsdtlold, $rscascade);
                // Call Row_Updating event
                $success = Container("asesor_fabricante")->rowUpdating($rsdtlold, $rsdtlnew);
                if ($success) {
                    $success = Container("asesor_fabricante")->update($rscascade, $rskey, $rsdtlold);
                }
                if (!$success) {
                    return false;
                }
                // Call Row_Updated event
                Container("asesor_fabricante")->rowUpdated($rsdtlold, $rsdtlnew);
            }
        }
    ...
    public function delete(&$rs, $where = "", $curfilter = false)
    {
        ...
        // Cascade delete detail table 'asesor_cliente'
        $dtlrows = Container("asesor_cliente")->loadRs("`asesor` = " . QuotedValue($rs['id'], DataType::NUMBER, "DB"))->fetchAllAssociative();
        // Call Row Deleting event
        foreach ($dtlrows as $dtlrow) {
            $success = Container("asesor_cliente")->rowDeleting($dtlrow);
            if (!$success) {
                break;
            }
        }
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                $success = Container("asesor_cliente")->delete($dtlrow); // Delete
                if (!$success) {
                    break;
                }
            }
        ...
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                Container("asesor_cliente")->rowDeleted($dtlrow);
            }
        }

        // Cascade delete detail table 'asesor_fabricante'
        $dtlrows = Container("asesor_fabricante")->loadRs("`asesor` = " . QuotedValue($rs['id'], DataType::NUMBER, "DB"))->fetchAllAssociative();
        // Call Row Deleting event
        foreach ($dtlrows as $dtlrow) {
            $success = Container("asesor_fabricante")->rowDeleting($dtlrow);
            if (!$success) {
                break;
            }
        }
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                $success = Container("asesor_fabricante")->delete($dtlrow); // Delete
                if (!$success) {
                    break;
                }
            }
        ...
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                Container("asesor_fabricante")->rowDeleted($dtlrow);
            }
        }
        if ($success) {
            try {
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("AsesorList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "AsesorView" => $Language->phrase("View"),
            "AsesorEdit" => $Language->phrase("Edit"),
            "AsesorAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "AsesorView",
            Config("API_ADD_ACTION") => "AsesorAdd",
            Config("API_EDIT_ACTION") => "AsesorEdit",
            Config("API_DELETE_ACTION") => "AsesorDelete",
            Config("API_LIST_ACTION") => "AsesorList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("AsesorView", $parm);
        } else {
            $url = $this->keyUrl("AsesorView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("AsesorEdit", $parm);
        } else {
            $url = $this->keyUrl("AsesorEdit", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("AsesorList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("AsesorAdd", $parm);
        } else {
            $url = $this->keyUrl("AsesorAdd", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("AsesorList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("AsesorDelete", $parm);
        }
    }
    ...
    {
        WriteAuditLog(CurrentUserIdentifier(), $typ, 'asesor');
    }
    ...
    public function writeAuditTrailOnAdd(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "A", 'asesor', $fldname, $key, "", $newvalue);
            }
        }
    }
        ...
        foreach (array_keys($rsnew) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && array_key_exists($fldname, $rsold) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                if ($modified) {
                    ...
                    WriteAuditLog($usr, "U", 'asesor', $fldname, $key, $oldvalue, $newvalue);
                }
            }
        }
    }
    ...
    public function writeAuditTrailOnDelete(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "D", 'asesor', $fldname, $key, $oldvalue);
            }
        }
    }
    ...
    public function rowDeleting(&$rs)
    {
        // Enter your code here
        // To cancel, set return value to False
        $sql = "SELECT * FROM asesor_cliente WHERE asesor = " . $rs["id"] . ";";
        if($row = ExecuteRow($sql)) {
    		$this->CancelMessage = "El asesor est&aacute; asociado a uno o m&aacute;s clientes; no se puede eliminar.";
    		return FALSE;
        }
        $sql = "SELECT * FROM usuario WHERE asesor = " . $rs["id"] . ";";
        if($row = ExecuteRow($sql)) {
    		$this->CancelMessage = "El asesor est&aacute; asociado a un usuario; no se puede eliminar.";
    		return FALSE;
        }
        return true;
    }
