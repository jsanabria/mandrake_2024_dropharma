 ...
 * Page class
 */
class ContLotesPagosDetalleList extends ContLotesPagosDetalle
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ContLotesPagosDetalleList";
    ...
    public $FormName = "fcont_lotes_pagos_detallelist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "ContLotesPagosDetalleList";
    ...
    public function __construct()
    {
        ...
        $this->MultiDeleteUrl = "ContLotesPagosDetalleDelete";
        $this->MultiUpdateUrl = "ContLotesPagosDetalleUpdate";
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                    ...
                    $result["view"] = SameString($pageName, "ContLotesPagosDetalleView"); // If View page, no primary button
                } else { // List page
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->tipodoc->AdvancedSearch->toJson(), ","); // Field tipodoc
        $filterList = Concat($filterList, $this->nro_documento->AdvancedSearch->toJson(), ","); // Field nro_documento
        $filterList = Concat($filterList, $this->monto_a_pagar->AdvancedSearch->toJson(), ","); // Field monto_a_pagar
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fcont_lotes_pagos_detallesrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field cont_lotes_pago
        $this->cont_lotes_pago->AdvancedSearch->SearchValue = @$filter["x_cont_lotes_pago"];
        $this->cont_lotes_pago->AdvancedSearch->SearchOperator = @$filter["z_cont_lotes_pago"];
        $this->cont_lotes_pago->AdvancedSearch->SearchCondition = @$filter["v_cont_lotes_pago"];
        $this->cont_lotes_pago->AdvancedSearch->SearchValue2 = @$filter["y_cont_lotes_pago"];
        $this->cont_lotes_pago->AdvancedSearch->SearchOperator2 = @$filter["w_cont_lotes_pago"];
        $this->cont_lotes_pago->AdvancedSearch->save();
        ...
        // Field monto_a_pagar
        $this->monto_a_pagar->AdvancedSearch->SearchValue = @$filter["x_monto_a_pagar"];
        $this->monto_a_pagar->AdvancedSearch->SearchOperator = @$filter["z_monto_a_pagar"];
        $this->monto_a_pagar->AdvancedSearch->SearchCondition = @$filter["v_monto_a_pagar"];
        $this->monto_a_pagar->AdvancedSearch->SearchValue2 = @$filter["y_monto_a_pagar"];
        $this->monto_a_pagar->AdvancedSearch->SearchOperator2 = @$filter["w_monto_a_pagar"];
        $this->monto_a_pagar->AdvancedSearch->save();

        // Field monto_pagdo
        $this->monto_pagdo->AdvancedSearch->SearchValue = @$filter["x_monto_pagdo"];
        $this->monto_pagdo->AdvancedSearch->SearchOperator = @$filter["z_monto_pagdo"];
        $this->monto_pagdo->AdvancedSearch->SearchCondition = @$filter["v_monto_pagdo"];
        $this->monto_pagdo->AdvancedSearch->SearchValue2 = @$filter["y_monto_pagdo"];
        $this->monto_pagdo->AdvancedSearch->SearchOperator2 = @$filter["w_monto_pagdo"];
        $this->monto_pagdo->AdvancedSearch->save();
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->monto_a_pagar); // monto_a_pagar
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fcont_lotes_pagos_detallelist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fcont_lotes_pagos_detallelist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<button type=\"button\" class=\"ew-action ew-multi-delete\" title=\"" .
            ...
            HtmlTitle($Language->phrase("DeleteSelectedLink")) . "\" form=\"fcont_lotes_pagos_detallelist\"" .
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "tipodoc");
            $this->createColumnOption($option, "nro_documento");
            $this->createColumnOption($option, "monto_a_pagar");
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fcont_lotes_pagos_detallesrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fcont_lotes_pagos_detallesrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions as $listAction) {
            if ($listAction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fcont_lotes_pagos_detallelist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            if ($custom) {
                return "<button type=\"button\" class=\"btn btn-default ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcel", true)) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcel", true)) . "\" form=\"fcont_lotes_pagos_detallelist\" data-url=\"$exportUrl\" data-ew-action=\"export\" data-export=\"excel\" data-custom=\"true\" data-export-selected=\"false\">" . $Language->phrase("ExportToExcel") . "</button>";
            } else {
                return "<a href=\"$exportUrl\" class=\"btn btn-default ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcel", true)) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcel", true)) . "\">" . $Language->phrase("ExportToExcel") . "</a>";
            }
        } elseif (SameText($type, "word")) {
            if ($custom) {
                return "<button type=\"button\" class=\"btn btn-default ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWord", true)) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWord", true)) . "\" form=\"fcont_lotes_pagos_detallelist\" data-url=\"$exportUrl\" data-ew-action=\"export\" data-export=\"word\" data-custom=\"true\" data-export-selected=\"false\">" . $Language->phrase("ExportToWord") . "</button>";
            } else {
                return "<a href=\"$exportUrl\" class=\"btn btn-default ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWord", true)) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWord", true)) . "\">" . $Language->phrase("ExportToWord") . "</a>";
            }
        } elseif (SameText($type, "pdf")) {
            if ($custom) {
                return "<button type=\"button\" class=\"btn btn-default ew-export-link ew-pdf\" title=\"" . HtmlEncode($Language->phrase("ExportToPdf", true)) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToPdf", true)) . "\" form=\"fcont_lotes_pagos_detallelist\" data-url=\"$exportUrl\" data-ew-action=\"export\" data-export=\"pdf\" data-custom=\"true\" data-export-selected=\"false\">" . $Language->phrase("ExportToPdf") . "</button>";
            } else {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmail", true) . '" data-caption="' . $Language->phrase("ExportToEmail", true) . '" form="fcont_lotes_pagos_detallelist" data-ew-action="email" data-custom="false" data-hdr="' . $Language->phrase("ExportToEmail", true) . '" data-exported-selected="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fcont_lotes_pagos_detallesrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    */
    public function exportData($doc)
    {
        ...
        if (Config("EXPORT_MASTER_RECORD") && $this->DbMasterFilter != "" && $this->getCurrentMasterTable() == "cont_lotes_pagos") {
            $cont_lotes_pagos = new ContLotesPagosList();
            $rsmaster = $cont_lotes_pagos->loadRs($this->DbMasterFilter); // Load master record
            if ($rsmaster) {
                $exportStyle = $doc->Style;
                $doc->setStyle("v"); // Change to vertical
                if (!$this->isExport("csv") || Config("EXPORT_MASTER_RECORD_FOR_CSV")) {
                    $doc->setTable($cont_lotes_pagos);
                    $cont_lotes_pagos->exportDocument($doc, $rsmaster);
                    $doc->exportEmptyRow();
                    $doc->setTable($this);
                }
    ...
    protected function setupMasterParms()
    {
        ...
        if (($master = Get(Config("TABLE_SHOW_MASTER"), Get(Config("TABLE_MASTER")))) !== null) {
            ...
            if ($masterTblVar == "cont_lotes_pagos") {
                $validMaster = true;
                $masterTbl = Container("cont_lotes_pagos");
                if (($parm = Get("fk_id", Get("cont_lotes_pago"))) !== null) {
                    $masterTbl->id->setQueryStringValue($parm);
                    $this->cont_lotes_pago->QueryStringValue = $masterTbl->id->QueryStringValue; // DO NOT change, master/detail key data type can be different
                    $this->cont_lotes_pago->setSessionValue($this->cont_lotes_pago->QueryStringValue);
                    $foreignKeys["cont_lotes_pago"] = $this->cont_lotes_pago->QueryStringValue;
            ...
            if ($masterTblVar == "cont_lotes_pagos") {
                $validMaster = true;
                $masterTbl = Container("cont_lotes_pagos");
                if (($parm = Post("fk_id", Post("cont_lotes_pago"))) !== null) {
                    $masterTbl->id->setFormValue($parm);
                    $this->cont_lotes_pago->FormValue = $masterTbl->id->FormValue;
                    $this->cont_lotes_pago->setSessionValue($this->cont_lotes_pago->FormValue);
                    $foreignKeys["cont_lotes_pago"] = $this->cont_lotes_pago->FormValue;
        ...
        if ($validMaster) {
            ...
            if ($masterTblVar != "cont_lotes_pagos") {
                if (!array_key_exists("cont_lotes_pago", $foreignKeys)) { // Not current foreign key
                    $this->cont_lotes_pago->setSessionValue("");
                }
            }
        }
