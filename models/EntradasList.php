 ...
 * Page class
 */
class EntradasList extends Entradas
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "EntradasList";
    ...
    public $FormName = "fentradaslist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "EntradasList";
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "EntradasAdd?" . Config("TABLE_SHOW_DETAIL") . "=";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "EntradasDelete";
        $this->MultiUpdateUrl = "EntradasUpdate";
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fentradassrch", $filters);
    ...
    public function renderListOptions()
    {
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"entradas\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $Language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($Security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"entradas\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-btn=\"SaveBtn\">" . $Language->phrase("EditLink") . "</a>";
                } else {
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fentradaslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fentradaslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
        ...
        if ($Security->allowList(CurrentProjectID() . 'entradas_salidas')) {
            ...
            $body = "<a class=\"btn btn-default ew-row-link ew-detail" . ($this->ListOptions->UseDropDownButton ? " dropdown-toggle" : "") . "\" data-action=\"list\" href=\"" . HtmlEncode("EntradasSalidasList?" . Config("TABLE_SHOW_MASTER") . "=entradas&" . GetForeignKeyUrl("fk_tipo_documento", $this->tipo_documento->CurrentValue) . "&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("EntradasSalidasGrid");
            if ($detailPage->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'entradas')) {
            ...
            if ($detailPage->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'entradas')) {
    ...
    protected function renderListOptionsExt()
    {
        ...
        if ($this->DetailPages?->getItem("entradas_salidas")?->Visible && $Security->allowList(CurrentProjectID() . 'entradas_salidas')) {
            ...
            $url = "EntradasSalidasPreview?t=entradas&f=" . Encrypt($detailFilter . "|" . implode("|", array_values($masterKeys)));
            $btngrp = "<div data-table=\"entradas_salidas\" data-url=\"" . $url . "\" class=\"ew-detail-btn-group d-none\">";
            if ($Security->allowList(CurrentProjectID() . 'entradas')) {
                ...
                $detaillnk = GetUrl("EntradasSalidasList?" . Config("TABLE_SHOW_MASTER") . "=entradas");
            ...
            if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'entradas')) {
            ...
            if ($detailPageObj->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'entradas')) {
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
            $item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"entradas\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("AddLink") . "</a>";
        } else {
        ...
        $item->Visible = ($detailPage->DetailAdd && $Security->allowAdd(CurrentProjectID() . 'entradas') && $Security->canAdd());
        if ($item->Visible) {
            if ($detailTableLink != "") {
                $detailTableLink .= ",";
            }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fentradassrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fentradassrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions as $listAction) {
            if ($listAction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fentradaslist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_entradas", "data-rowtype" => RowType::ADD]);
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fentradassrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    */
    ...
    public function listOptionsRendered() {
    	...
    	//switch($this->tipo_documento->CurrentValue) {
    	//switch($_REQUEST["tipo"]) {
    	$sql = "SELECT tipo_documento FROM username_tipo_documento WHERE username = '" . CurrentUserName() . "';";
    	$tipo = ExecuteScalar($sql);
    	switch($tipo) {
    	case "TDCPDC":
    		...
    		$url = "reportes/pedido_de_compra.php?id=$id&tipo=TDCPDC";
    		$this->ListOptions->Items["print"]->Body = '<a class="fas fa-print" target="_blank" href="' . $url . '" data-original-title="Procesar"><span data-phrase="Imprimir" class="glyphicon glyphicon-print ewIcon" data-caption="Imprimir"></span></a>';
    		...
    		if(ExecuteScalar($sql) > 0) {
    			if(VerificaFuncion('002')) {
    				if($this->estatus->CurrentValue == "NUEVO") {
    					$url = "ConfirmPage?page=TDCPDC&id=$id";
    	...
    	case "TDCNRP":
            /*
    		$url = "EntradasView?showdetail=entradas_salidas&id=$id&tipo=TDCNRP";
            $this->ListOptions->Items["view"]->Body = '<a class="fas fa-eye" href="' . $url . '" data-toggle="tooltip" title="Ver" data-placement="bottom"></a>';
    		$url = "EntradasEdit?showdetail=entradas_salidas&id=$id&tipo=TDCNRP";
            $this->ListOptions->Items["edit"]->Body = '<a class="fas fa-edit" href="' . $url . '" data-toggle="tooltip" title="Editar" data-placement="bottom"></a>';
    		$this->ListOptions->Items["copy"]->Body = '';
    		$url = "EntradasDelete?id=$id&tipo=TDCNRP";
            $this->ListOptions->Items["delete"]->Body ='<a class="fas fa-trash" href="' . $url . '" data-toggle="tooltip" title="Borrar" data-placement="bottom"></a>';
    		$url = "reportes/nota_de_recepcion.php?id=$id&tipo=TDCNRP";
            $this->ListOptions->Items["print"]->Body = '<a class="fas fa-print" target="_blank" href="' . $url . '" data-original-title="Procesar"><span data-phrase="Imprimir" class="glyphicon glyphicon-print ewIcon" data-caption="Imprimir"></span></a>';
            */
    		$url = "reportes/nota_de_recepcion.php?id=$id&tipo=TDCNRP";
            $this->ListOptions->Items["print"]->Body = '<a class="fas fa-print" target="_blank" href="' . $url . '" data-original-title="Procesar"><span data-phrase="Imprimir" class="glyphicon glyphicon-print ewIcon" data-caption="Imprimir"></span></a>';
    	...
    	case "TDCFCC":
            /*
    		$url = "EntradasView?showdetail=entradas_salidas&id=$id&tipo=TDCFCC";
            $this->ListOptions->Items["view"]->Body = '<a class="fas fa-eye" href="' . $url . '" data-toggle="tooltip" title="Ver" data-placement="bottom"></a>';
    		$url = "EntradasEdit?showdetail=entradas_salidas&id=$id&tipo=TDCFCC";
            $this->ListOptions->Items["edit"]->Body = '<a class="fas fa-edit" href="' . $url . '" data-toggle="tooltip" title="Editar" data-placement="bottom"></a>';
    		$url = "FacturaDeCompraDetalleCopia?id=$id";
            $this->ListOptions->Items["copy"]->Body = '<a class="fas fa-copy" href="' . $url . '" data-toggle="tooltip" title="Copiar" data-placement="bottom"></a>';
    		$url = "EntradasDelete?id=$id&tipo=TDCFCC";
            $this->ListOptions->Items["delete"]->Body ='<a class="fas fa-trash" href="' . $url . '" data-toggle="tooltip" title="Borrar" data-placement="bottom"></a>';
    		$url = "reportes/factura_de_compra.php?id=$id&tipo=TDCFCC";
            $this->ListOptions->Items["print"]->Body = '<a class="fas fa-print" target="_blank" href="' . $url . '" data-original-title="Procesar"><span data-phrase="Imprimir" class="glyphicon glyphicon-print ewIcon" data-caption="Imprimir"></span></a>';
            */
    		$url = "reportes/factura_de_compra.php?id=$id&tipo=TDCFCC";
            $this->ListOptions->Items["print"]->Body = '<a class="fas fa-print" target="_blank" href="' . $url . '" data-original-title="Procesar"><span data-phrase="Imprimir" class="glyphicon glyphicon-print ewIcon" data-caption="Imprimir"></span></a>';
    	...
    	case "TDCAEN":
            /*
    		$url = "EntradasView?showdetail=entradas_salidas&id=$id&tipo=TDCAEN";
            $this->ListOptions->Items["view"]->Body = '<a class="fas fa-eye" href="' . $url . '" data-toggle="tooltip" title="Ver" data-placement="bottom"></a>';
    		$url = "EntradasEdit?showdetail=entradas_salidas&id=$id&tipo=TDCAEN";
            $this->ListOptions->Items["edit"]->Body = '<a class="fas fa-edit" href="' . $url . '" data-toggle="tooltip" title="Editar" data-placement="bottom"></a>';
    		$url = "AjusteDeEntradaDetalleCopia?id=$id";
            $this->ListOptions->Items["copy"]->Body = '<a class="fas fa-copy" href="' . $url . '" data-toggle="tooltip" title="Copiar" data-placement="bottom"></a>';
    		$url = "EntradasDelete?id=$id&tipo=TDCAEN";
            $this->ListOptions->Items["delete"]->Body ='<a class="fas fa-trash" href="' . $url . '" data-toggle="tooltip" title="Borrar" data-placement="bottom"></a>';
    		$url = "reportes/ajuste_de_entrada.php?id=$id&tipo=TDCAEN";
