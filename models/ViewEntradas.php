...
/**
 * Table class for view_entradas
 */
class ViewEntradas extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        // Update Table
        $this->UpdateTable = "view_entradas";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // nombre_proveedor
        $this->nombre_proveedor = new DbField(
            $this, // Table
            'x_nombre_proveedor', // Variable name
            'nombre_proveedor', // Name
            '`nombre_proveedor`', // Expression
            '`nombre_proveedor`', // Basic search expression
            200, // Type
            80, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`nombre_proveedor`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->nombre_proveedor->InputTextType = "text";
        $this->nombre_proveedor->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY", "IS NULL", "IS NOT NULL"];
        ...
        $this->consignacion->Lookup = new Lookup($this->consignacion, 'view_entradas', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
        ...
        $this->consignacion_reportada->Lookup = new Lookup($this->consignacion_reportada, 'view_entradas', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "view_entradas";
    }
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->nombre_proveedor->DbValue = $row['nombre_proveedor'];
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("ViewEntradasList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "ViewEntradasView" => $Language->phrase("View"),
            "ViewEntradasEdit" => $Language->phrase("Edit"),
            "ViewEntradasAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "ViewEntradasView",
            Config("API_ADD_ACTION") => "ViewEntradasAdd",
            Config("API_EDIT_ACTION") => "ViewEntradasEdit",
            Config("API_DELETE_ACTION") => "ViewEntradasDelete",
            Config("API_LIST_ACTION") => "ViewEntradasList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("ViewEntradasView", $parm);
        } else {
            $url = $this->keyUrl("ViewEntradasView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("ViewEntradasEdit", $parm);
        } else {
            $url = $this->keyUrl("ViewEntradasEdit", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ViewEntradasList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("ViewEntradasAdd", $parm);
        } else {
            $url = $this->keyUrl("ViewEntradasAdd", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ViewEntradasList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("ViewEntradasDelete", $parm);
        }
    }
    ...
    public function renderEditRow()
    {
        ...
        // nombre_proveedor
        $this->nombre_proveedor->setupEditAttributes();
        if (!$this->nombre_proveedor->Raw) {
            $this->nombre_proveedor->CurrentValue = HtmlDecode($this->nombre_proveedor->CurrentValue);
        }
        $this->nombre_proveedor->EditValue = $this->nombre_proveedor->CurrentValue;
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->nombre_documento);
                    $doc->exportCaption($this->proveedor);
                    $doc->exportCaption($this->nombre_proveedor);
                    $doc->exportCaption($this->nro_documento);
                    $doc->exportCaption($this->fecha);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->nombre_documento);
                        $doc->exportField($this->proveedor);
                        $doc->exportField($this->nombre_proveedor);
                        $doc->exportField($this->nro_documento);
                        $doc->exportField($this->fecha);
