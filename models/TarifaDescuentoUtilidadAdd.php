 ...
 * Page class
 */
class TarifaDescuentoUtilidadAdd extends TarifaDescuentoUtilidad
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "TarifaDescuentoUtilidadAdd";
    ...
    // CSS class/style
    public $CurrentPageName = "TarifaDescuentoUtilidadAdd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$rsold) { // Record not loaded
                    ...
                    $this->terminate("TarifaDescuentoUtilidadList"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($rsold)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "TarifaDescuentoUtilidadList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "TarifaDescuentoUtilidadView") {
                    ...
                    if ($this->IsModal && $this->UseAjaxActions && !$this->getCurrentMasterTable()) {
                        ...
                        if (GetPageName($returnUrl) != "TarifaDescuentoUtilidadList") {
                            ...
                            $returnUrl = "TarifaDescuentoUtilidadList"; // Return list page content
                        }
                    }
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'utilidad' first before field var 'x_utilidad'
        $val = $CurrentForm->hasValue("utilidad") ? $CurrentForm->getValue("utilidad") : $CurrentForm->getValue("x_utilidad");
        if (!$this->utilidad->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->utilidad->Visible = false; // Disable update for API request
            } else {
                $this->utilidad->setFormValue($val, true, $validate);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->utilidad->CurrentValue = $this->utilidad->FormValue;
    }
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            ...
            if ($this->utilidad->Visible && $this->utilidad->Required) {
                if (!$this->utilidad->IsDetailKey && EmptyValue($this->utilidad->FormValue)) {
                    $this->utilidad->addErrorMessage(str_replace("%s", $this->utilidad->caption(), $this->utilidad->RequiredErrorMessage));
                }
            }
            if (!CheckNumber($this->utilidad->FormValue)) {
                $this->utilidad->addErrorMessage($this->utilidad->getErrorMessage(false));
            }
    ...
    protected function getAddRow()
    {
        ...
        // utilidad
        $this->utilidad->setDbValueDef($rsnew, $this->utilidad->CurrentValue, false);

        // tarifa
        if ($this->tarifa->getSessionValue() != "") {
            $rsnew['tarifa'] = $this->tarifa->getSessionValue();
        }
        return $rsnew;
    }
    ...
    protected function restoreAddFormFromRow($row)
    {
        ...
        if (isset($row['utilidad'])) { // utilidad
            $this->utilidad->setFormValue($row['utilidad']);
        }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("TarifaDescuentoUtilidadList"), "", $this->TableVar, true);
