 ...
 * Page class
 */
class HistoriaArticuloAdd extends HistoriaArticulo
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'historia_articulo';

    // Page object name
    public $PageObjName = "HistoriaArticuloAdd";
    ...
    public function __construct()
    {
        ...
        if (!isset($GLOBALS["historia_articulo"]) || get_class($GLOBALS["historia_articulo"]) == PROJECT_NAMESPACE . "historia_articulo") {
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $doc = new $class(Container("historia_articulo"));
                $doc->Text = @$content;
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if ($pageName != $this->getListUrl()) { // Not List page
                    ...
                    if ($pageName == "HistoriaArticuloView") {
                        $row["view"] = "1";
                    }
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("HistoriaArticuloList"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "HistoriaArticuloList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "HistoriaArticuloView") {
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->entradas->CurrentValue = null;
        $this->entradas->OldValue = $this->entradas->CurrentValue;
        $this->salidas->CurrentValue = null;
        $this->salidas->OldValue = $this->salidas->CurrentValue;
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->fecha_vencimiento->IsDetailKey) {
            ...
            $this->fecha_vencimiento->CurrentValue = UnFormatDateTime($this->fecha_vencimiento->CurrentValue, 0);
        }
        ...
        // Check field name 'salidas' first before field var 'x_salidas'
        $val = $CurrentForm->hasValue("salidas") ? $CurrentForm->getValue("salidas") : $CurrentForm->getValue("x_salidas");
        if (!$this->salidas->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->salidas->Visible = false; // Disable update for API request
            } else {
                $this->salidas->setFormValue($val);
            }
        }

        // Check field name 'existencia' first before field var 'x_existencia'
        $val = $CurrentForm->hasValue("existencia") ? $CurrentForm->getValue("existencia") : $CurrentForm->getValue("x_existencia");
        if (!$this->existencia->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->existencia->Visible = false; // Disable update for API request
            } else {
                $this->existencia->setFormValue($val);
            }
        }
    }
    ...
    public function restoreFormValues()
    {
        ...
        $this->salidas->CurrentValue = $this->salidas->FormValue;
    ...
    protected function newRow()
    {
        ...
        $row['entradas'] = $this->entradas->CurrentValue;
        $row['salidas'] = $this->salidas->CurrentValue;
        $row['existencia'] = $this->existencia->CurrentValue;
        return $row;
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->entradas->ViewCustomAttributes = "";

            // salidas
            $this->salidas->ViewValue = $this->salidas->CurrentValue;
            $this->salidas->ViewCustomAttributes = "";
            ...
            // entradas
            $this->entradas->LinkCustomAttributes = "";
            $this->entradas->HrefValue = "";
            $this->entradas->TooltipValue = "";

            // salidas
            $this->salidas->LinkCustomAttributes = "";
            ...
            if ($curVal != "") {
                ...
                $this->articulo->ViewValue = $this->articulo->Lookup !== null && is_array($this->articulo->Lookup->Options) ? $curVal : null;
            }
            if ($this->articulo->ViewValue !== null) { // Load from cache
                $this->articulo->EditValue = array_values($this->articulo->Lookup->Options);
                ...
                if ($curVal == "") {
                    ...
                    $filterWrk = "`id`" . SearchString("=", $this->articulo->CurrentValue, DATATYPE_NUMBER, "");
                }
            ...
            if ($curVal != "") {
                ...
                $this->almacen->ViewValue = $this->almacen->Lookup !== null && is_array($this->almacen->Lookup->Options) ? $curVal : null;
            }
            if ($this->almacen->ViewValue !== null) { // Load from cache
                $this->almacen->EditValue = array_values($this->almacen->Lookup->Options);
                if ($this->almacen->ViewValue == "") {
                    $this->almacen->ViewValue = $Language->phrase("PleaseSelect");
                }
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = "`codigo`" . SearchString("=", $this->almacen->CurrentValue, DATATYPE_STRING, "");
                }
            ...
            if ($curVal != "") {
                ...
                $this->tipo_documento->ViewValue = $this->tipo_documento->Lookup !== null && is_array($this->tipo_documento->Lookup->Options) ? $curVal : null;
            }
            if ($this->tipo_documento->ViewValue !== null) { // Load from cache
                $this->tipo_documento->EditValue = array_values($this->tipo_documento->Lookup->Options);
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = "`codigo`" . SearchString("=", $this->tipo_documento->CurrentValue, DATATYPE_STRING, "");
                }
                $sqlWrk = $this->tipo_documento->Lookup->getSql(true, $filterWrk, '', $this, false, true);
                $rswrk = Conn()->executeQuery($sqlWrk)->fetchAll(\PDO::FETCH_BOTH);
                $ari = count($rswrk);
                $arwrk = $rswrk;
            ...
            $this->fecha_vencimiento->EditValue = HtmlEncode(FormatDateTime($this->fecha_vencimiento->CurrentValue, 8));
            ...
            if ($curVal != "") {
                ...
                $this->usuario->ViewValue = $this->usuario->Lookup !== null && is_array($this->usuario->Lookup->Options) ? $curVal : null;
            }
            if ($this->usuario->ViewValue !== null) { // Load from cache
                $this->usuario->EditValue = array_values($this->usuario->Lookup->Options);
                if ($this->usuario->ViewValue == "") {
                    $this->usuario->ViewValue = $Language->phrase("PleaseSelect");
                }
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = "`username`" . SearchString("=", $this->usuario->CurrentValue, DATATYPE_STRING, "");
                }
                $sqlWrk = $this->usuario->Lookup->getSql(true, $filterWrk, '', $this, false, true);
                $rswrk = Conn()->executeQuery($sqlWrk)->fetchAll(\PDO::FETCH_BOTH);
                $ari = count($rswrk);
                if ($ari > 0) { // Lookup values found
                    $arwrk = $this->usuario->Lookup->renderViewRow($rswrk[0]);
            ...
            // entradas
            $this->entradas->EditAttrs["class"] = "form-control";
            $this->entradas->EditCustomAttributes = "";
            if (!$this->entradas->Raw) {
                $this->entradas->CurrentValue = HtmlDecode($this->entradas->CurrentValue);
            }
            $this->entradas->EditValue = HtmlEncode($this->entradas->CurrentValue);
            $this->entradas->PlaceHolder = RemoveHtml($this->entradas->caption());

            // salidas
            $this->salidas->EditAttrs["class"] = "form-control";
            $this->salidas->EditCustomAttributes = "";
            if (!$this->salidas->Raw) {
                $this->salidas->CurrentValue = HtmlDecode($this->salidas->CurrentValue);
            }
            $this->salidas->EditValue = HtmlEncode($this->salidas->CurrentValue);
            ...
            if (!$this->existencia->Raw) {
                $this->existencia->CurrentValue = HtmlDecode($this->existencia->CurrentValue);
            }
            $this->existencia->EditValue = HtmlEncode($this->existencia->CurrentValue);
    ...
    protected function validateForm()
    {
        ...
        if (!CheckDate($this->fecha_vencimiento->FormValue)) {
        ...
        if ($this->salidas->Required) {
            if (!$this->salidas->IsDetailKey && EmptyValue($this->salidas->FormValue)) {
                $this->salidas->addErrorMessage(str_replace("%s", $this->salidas->caption(), $this->salidas->RequiredErrorMessage));
            }
        }
        if ($this->existencia->Required) {
            if (!$this->existencia->IsDetailKey && EmptyValue($this->existencia->FormValue)) {
                $this->existencia->addErrorMessage(str_replace("%s", $this->existencia->caption(), $this->existencia->RequiredErrorMessage));
            }
        }
    ...
    protected function addRow($rsold = null)
    {
        ...
        // almacen
        $this->almacen->setDbValueDef($rsnew, $this->almacen->CurrentValue, null, false);
        ...
        // fecha_vencimiento
        $this->fecha_vencimiento->setDbValueDef($rsnew, UnFormatDateTime($this->fecha_vencimiento->CurrentValue, 0), null, false);

        // usuario
        $this->usuario->setDbValueDef($rsnew, $this->usuario->CurrentValue, null, false);

        // entradas
        $this->entradas->setDbValueDef($rsnew, $this->entradas->CurrentValue, null, false);

        // salidas
        $this->salidas->setDbValueDef($rsnew, $this->salidas->CurrentValue, null, false);

        // existencia
        $this->existencia->setDbValueDef($rsnew, $this->existencia->CurrentValue, null, false);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("HistoriaArticuloList"), "", $this->TableVar, true);
