...
class AbonoView extends Abono
{
    ...
    // CSS class/style
    public $CurrentPageName = "AbonoView";
    ...
    public function run()
    {
        ...
        if (($keyValue = Get("id") ?? Route("id")) !== null) {
            ...
            $returnUrl = "AbonoList"; // Return to list
        }
            ...
            case "show": // Get a record to display
                if (!$this->IsModal && !IsApi()) { // Normal view page
                    ...
                    if ($this->TotalRecords <= 0) { // No record found
                        ...
                        $this->terminate("AbonoList"); // Return to list page
                        return;
                    ...
                    if (!$matchRecord) {
                        ...
                        $returnUrl = "AbonoList"; // No matching record, return to list
                    } else {
    ...
    protected function setupOtherOptions()
    {
        ...
        */
        ...
        // "detail_recarga"
        $item = &$option->add("detail_recarga");
        $body = $Language->phrase("ViewPageDetailLink") . $Language->tablePhrase("recarga", "TblCaption");
        $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode(GetUrl("RecargaList?" . Config("TABLE_SHOW_MASTER") . "=abono&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "")) . "\">" . $body . "</a>";
        $links = "";
        ...
        if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'abono')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailViewLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=recarga"))) . "\">" . $Language->phrase("MasterDetailViewLink", null) . "</a></li>";
            if ($detailViewTblVar != "") {
                $detailViewTblVar .= ",";
            }
    ...
    public function pageDataRendering(&$header)
    {
        ...
        if($this->metodo_pago->CurrentValue == "IMPRIMIR")
        	$header = '<a class="btn btn-primary" target="_blank" href="../reportes/rptRecibo.php?id=' . $this->id->CurrentValue . '"><span class="fas fa-print"></span> Imprimir Recibo</a>';
    }
