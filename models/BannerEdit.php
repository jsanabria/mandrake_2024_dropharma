 ...
 * Page class
 */
class BannerEdit extends Banner
{
    use MessagesTrait;
    ...
    // CSS class/style
    public $CurrentPageName = "BannerEdit";
    ...
    public function run()
    {
            ...
            if ($this->isShow()) {
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if ($this->TotalRecords <= 0) { // No record found
                        ...
                        $this->terminate("BannerList"); // Return to list page
                        return;
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("BannerList"); // No matching record, return to list
                        return;
                    }
                } // End modal checking
                break;
            case "update": // Update
                ...
                if (GetPageName($returnUrl) == "BannerList") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "BannerList") {
                            ...
                            $returnUrl = "BannerList"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'subtitulo' first before field var 'x_subtitulo'
        $val = $CurrentForm->hasValue("subtitulo") ? $CurrentForm->getValue("subtitulo") : $CurrentForm->getValue("x_subtitulo");
        if (!$this->subtitulo->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->subtitulo->Visible = false; // Disable update for API request
            } else {
                $this->subtitulo->setFormValue($val);
            }
        }
    ...
    public function renderRow()
    {
        ...
        // subtitulo
        $this->subtitulo->RowCssClass = "row";
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            // subtitulo
            $this->subtitulo->setupEditAttributes();
            if (!$this->subtitulo->Raw) {
                $this->subtitulo->CurrentValue = HtmlDecode($this->subtitulo->CurrentValue);
            }
            $this->subtitulo->EditValue = HtmlEncode($this->subtitulo->CurrentValue);
            $this->subtitulo->PlaceHolder = RemoveHtml($this->subtitulo->caption());
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            ...
            if ($this->subtitulo->Visible && $this->subtitulo->Required) {
                if (!$this->subtitulo->IsDetailKey && EmptyValue($this->subtitulo->FormValue)) {
                    $this->subtitulo->addErrorMessage(str_replace("%s", $this->subtitulo->caption(), $this->subtitulo->RequiredErrorMessage));
                }
            }
    ...
    protected function getEditRow($rsold)
    {
        ...
        // subtitulo
        $this->subtitulo->setDbValueDef($rsnew, $this->subtitulo->CurrentValue, $this->subtitulo->ReadOnly);

        // imagen
        $this->imagen->setDbValueDef($rsnew, $this->imagen->CurrentValue, $this->imagen->ReadOnly);
        ...
        $this->activo->setDbValueDef($rsnew, $tmpBool, $this->activo->ReadOnly);
        return $rsnew;
    }
    ...
    protected function restoreEditFormFromRow($row)
    {
        ...
        if (isset($row['subtitulo'])) { // subtitulo
            $this->subtitulo->CurrentValue = $row['subtitulo'];
        }
        if (isset($row['imagen'])) { // imagen
            $this->imagen->CurrentValue = $row['imagen'];
        }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("BannerList"), "", $this->TableVar, true);
        $pageId = "edit";
