...
class Compania extends DbTable
{
    ...
    public function __construct()
    {
        ...
        // agente_retencion
        $this->agente_retencion = new DbField(
            $this, // Table
            'x_agente_retencion', // Variable name
            'agente_retencion', // Name
            '`agente_retencion`', // Expression
            '`agente_retencion`', // Basic search expression
            200, // Type
            1, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`agente_retencion`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->agente_retencion->addMethod("getDefault", fn() => "S");
        $this->agente_retencion->InputTextType = "text";
        $this->agente_retencion->Raw = true;
        $this->agente_retencion->Required = true; // Required field
        $this->agente_retencion->setSelectMultiple(false); // Select one
        ...
        $this->agente_retencion->Lookup = new Lookup($this->agente_retencion, 'compania', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
        $this->agente_retencion->OptionCount = 2;
        $this->agente_retencion->SearchOperators = ["=", "<>", "IS NULL", "IS NOT NULL"];
        $this->Fields['agente_retencion'] = &$this->agente_retencion;

        // logo
        $this->logo = new DbField(
            $this, // Table
            'x_logo', // Variable name
            'logo', // Name
            '`logo`', // Expression
            '`logo`', // Basic search expression
            200, // Type
            250, // Size
            -1, // Date/Time format
            true, // Is upload field
            '`logo`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'IMAGE', // View Tag
        ...
        $this->logo->SearchOperators = ["=", "<>", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY", "IS NULL", "IS NOT NULL"];
        ...
        $this->activo->Lookup = new Lookup($this->activo, 'compania', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "CompaniaView" => $Language->phrase("View"),
            "CompaniaEdit" => $Language->phrase("Edit"),
            "CompaniaAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "CompaniaView",
            Config("API_ADD_ACTION") => "CompaniaAdd",
            Config("API_EDIT_ACTION") => "CompaniaEdit",
            Config("API_DELETE_ACTION") => "CompaniaDelete",
            Config("API_LIST_ACTION") => "CompaniaList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("CompaniaView", $parm);
        } else {
            $url = $this->keyUrl("CompaniaView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("CompaniaEdit", $parm);
        } else {
            $url = $this->keyUrl("CompaniaEdit", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("CompaniaList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("CompaniaAdd", $parm);
        } else {
            $url = $this->keyUrl("CompaniaAdd", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("CompaniaList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("CompaniaDelete", $parm);
        }
    }
    ...
    {
        WriteAuditLog(CurrentUserIdentifier(), $typ, 'compania');
    }
    ...
    public function writeAuditTrailOnAdd(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "A", 'compania', $fldname, $key, "", $newvalue);
            }
        }
    }
        ...
        foreach (array_keys($rsnew) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && array_key_exists($fldname, $rsold) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                if ($modified) {
                    ...
                    WriteAuditLog($usr, "U", 'compania', $fldname, $key, $oldvalue, $newvalue);
                }
            }
        }
    }
    ...
    public function writeAuditTrailOnDelete(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "D", 'compania', $fldname, $key, $oldvalue);
            }
        }
    }
