 ...
 * Page class
 */
class TarifaEdit extends Tarifa
{
    use MessagesTrait;
    ...
    // CSS class/style
    public $CurrentPageName = "TarifaEdit";
    ...
    public function run()
    {
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("TarifaList"); // No matching record, return to list
                        return;
                    }
                } // End modal checking
            ...
            case "update": // Update
                ...
                if (GetPageName($returnUrl) == "TarifaList") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "TarifaList") {
                            ...
                            $returnUrl = "TarifaList"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'patron' first before field var 'x_patron'
        $val = $CurrentForm->hasValue("patron") ? $CurrentForm->getValue("patron") : $CurrentForm->getValue("x_patron");
        if (!$this->patron->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->patron->Visible = false; // Disable update for API request
            } else {
                $this->patron->setFormValue($val);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->patron->CurrentValue = $this->patron->FormValue;
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            ...
            if ($this->patron->Visible && $this->patron->Required) {
                if (!$this->patron->IsDetailKey && EmptyValue($this->patron->FormValue)) {
                    $this->patron->addErrorMessage(str_replace("%s", $this->patron->caption(), $this->patron->RequiredErrorMessage));
                }
            }
        ...
        if (in_array("tarifa_descuento_utilidad", $detailTblVar) && $detailPage->DetailEdit) {
            $detailPage->run();
            $validateForm = $validateForm && $detailPage->validateGridForm();
        }
        $detailPage = Container("TarifaArticuloGrid");
        if (in_array("tarifa_articulo", $detailTblVar) && $detailPage->DetailEdit) {
            $detailPage->run();
            $validateForm = $validateForm && $detailPage->validateGridForm();
        }
    ...
    protected function editRow()
    {
        ...
        if ($updateRow) {
            ...
            $detailTblVar = explode(",", $this->getCurrentDetailTable());
            $detailPage = Container("TarifaDescuentoUtilidadGrid");
            if (in_array("tarifa_descuento_utilidad", $detailTblVar) && $detailPage->DetailEdit && $editRow) {
                $Security->loadCurrentUserLevel($this->ProjectID . "tarifa_descuento_utilidad"); // Load user level of detail table
                $editRow = $detailPage->gridUpdate();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
            }
            $detailPage = Container("TarifaArticuloGrid");
            if (in_array("tarifa_articulo", $detailTblVar) && $detailPage->DetailEdit && $editRow) {
                $Security->loadCurrentUserLevel($this->ProjectID . "tarifa_articulo"); // Load user level of detail table
                $editRow = $detailPage->gridUpdate();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
            }
    ...
    protected function getEditRow($rsold)
    {
        ...
        // patron
        $this->patron->setDbValueDef($rsnew, $this->patron->CurrentValue, $this->patron->ReadOnly);
    ...
    protected function restoreEditFormFromRow($row)
    {
        ...
        if (isset($row['patron'])) { // patron
            $this->patron->CurrentValue = $row['patron'];
        }
