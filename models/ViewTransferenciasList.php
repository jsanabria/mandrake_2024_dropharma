 ...
 * Page class
 */
class ViewTransferenciasList extends ViewTransferencias
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ViewTransferenciasList";
    ...
    public $FormName = "fview_transferenciaslist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "ViewTransferenciasList";
    ...
    public function __construct()
    {
        ...
        // Table object (view_transferencias)
        if (!isset($GLOBALS["view_transferencias"]) || $GLOBALS["view_transferencias"]::class == PROJECT_NAMESPACE . "view_transferencias") {
            $GLOBALS["view_transferencias"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "ViewTransferenciasAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "ViewTransferenciasDelete";
        $this->MultiUpdateUrl = "ViewTransferenciasUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'view_transferencias');
        }
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                    ...
                    $result["view"] = SameString($pageName, "ViewTransferenciasView"); // If View page, no primary button
                } else { // List page
    ...
    public function run()
    {
        ...
        if ($this->IsModal) {
            $this->FormName = "fview_transferenciasgrid";
        }
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server") {
            $savedFilterList = Profile()->getSearchFilters("fview_transferenciassrch");
        }
        ...
        $filterList = Concat($filterList, $this->iddoc->AdvancedSearch->toJson(), ","); // Field iddoc
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fview_transferenciassrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field iddoc
        $this->iddoc->AdvancedSearch->SearchValue = @$filter["x_iddoc"];
        $this->iddoc->AdvancedSearch->SearchOperator = @$filter["z_iddoc"];
        $this->iddoc->AdvancedSearch->SearchCondition = @$filter["v_iddoc"];
        $this->iddoc->AdvancedSearch->SearchValue2 = @$filter["y_iddoc"];
        $this->iddoc->AdvancedSearch->SearchOperator2 = @$filter["w_iddoc"];
        $this->iddoc->AdvancedSearch->save();
    ...
    public function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->iddoc, $default, false); // iddoc
        ...
        $this->buildSearchSql($where, $this->cantidad_articulo, $default, false); // cantidad_articulo
        ...
        if (!$default && $this->Command == "search") {
            ...
            $this->iddoc->AdvancedSearch->save(); // iddoc
            ...
            $this->cantidad_articulo->AdvancedSearch->save(); // cantidad_articulo
    ...
    public function showFilterList()
    {
        ...
        // Field cantidad_articulo
        $filter = $this->queryBuilderWhere("cantidad_articulo");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->cantidad_articulo, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->cantidad_articulo->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->iddoc->AdvancedSearch->issetSession()) {
            return true;
        }
        ...
        if ($this->cantidad_articulo->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    protected function resetAdvancedSearchParms()
    {
        ...
        $this->iddoc->AdvancedSearch->unsetSession();
        ...
        $this->cantidad_articulo->AdvancedSearch->unsetSession();
    ...
    protected function restoreSearchParms()
    {
        ...
        $this->iddoc->AdvancedSearch->load();
        ...
        $this->cantidad_articulo->AdvancedSearch->load();
    ...
    protected function setupSortOrder()
    {
        ...
        if ($this->Command != "json") {
            $defaultSort = $this->nro_documento->Expression . " DESC" . ", " . $this->tipo_documento->Expression . " DESC"; // Set up default sort
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fview_transferenciaslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fview_transferenciaslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fview_transferenciassrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fview_transferenciassrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions as $listAction) {
            if ($listAction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fview_transferenciaslist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_view_transferencias", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_view_transferencias",
    ...
    protected function loadSearchValues()
    {
        ...
        // iddoc
        if ($this->iddoc->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->iddoc->AdvancedSearch->SearchValue != "" || $this->iddoc->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        // cantidad_articulo
        if ($this->cantidad_articulo->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->cantidad_articulo->AdvancedSearch->SearchValue != "" || $this->cantidad_articulo->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
    ...
    protected function newRow()
    {
        ...
        $row['iddoc'] = $this->iddoc->DefaultValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            $curVal = trim(strval($this->tipo_documento->AdvancedSearch->SearchValue));
            if ($curVal != "") {
                $this->tipo_documento->AdvancedSearch->ViewValue = $this->tipo_documento->lookupCacheOption($curVal);
            } else {
                $this->tipo_documento->AdvancedSearch->ViewValue = $this->tipo_documento->Lookup !== null && is_array($this->tipo_documento->lookupOptions()) && count($this->tipo_documento->lookupOptions()) > 0 ? $curVal : null;
            }
            if ($this->tipo_documento->AdvancedSearch->ViewValue !== null) { // Load from cache
                $this->tipo_documento->EditValue = array_values($this->tipo_documento->lookupOptions());
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = SearchFilter($this->tipo_documento->Lookup->getTable()->Fields["codigo"]->searchExpression(), "=", $this->tipo_documento->AdvancedSearch->SearchValue, $this->tipo_documento->Lookup->getTable()->Fields["codigo"]->searchDataType(), "");
                }
                $lookupFilter = $this->tipo_documento->getSelectFilter($this); // PHP
            ...
            $this->fecha_vencimiento->EditValue = HtmlEncode(FormatDateTime(UnFormatDateTime($this->fecha_vencimiento->AdvancedSearch->SearchValue, $this->fecha_vencimiento->formatPattern()), $this->fecha_vencimiento->formatPattern()));
            $this->fecha_vencimiento->PlaceHolder = RemoveHtml($this->fecha_vencimiento->caption());

            // cantidad_articulo
            $this->cantidad_articulo->setupEditAttributes();
            $this->cantidad_articulo->EditValue = $this->cantidad_articulo->AdvancedSearch->SearchValue;
            $this->cantidad_articulo->PlaceHolder = RemoveHtml($this->cantidad_articulo->caption());

            // almacen
            $this->almacen->setupEditAttributes();
            $curVal = trim(strval($this->almacen->AdvancedSearch->SearchValue));
            if ($curVal != "") {
                $this->almacen->AdvancedSearch->ViewValue = $this->almacen->lookupCacheOption($curVal);
            } else {
                $this->almacen->AdvancedSearch->ViewValue = $this->almacen->Lookup !== null && is_array($this->almacen->lookupOptions()) && count($this->almacen->lookupOptions()) > 0 ? $curVal : null;
            }
            if ($this->almacen->AdvancedSearch->ViewValue !== null) { // Load from cache
                $this->almacen->EditValue = array_values($this->almacen->lookupOptions());
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = SearchFilter($this->almacen->Lookup->getTable()->Fields["codigo"]->searchExpression(), "=", $this->almacen->AdvancedSearch->SearchValue, $this->almacen->Lookup->getTable()->Fields["codigo"]->searchDataType(), "");
                }
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmail", true) . '" data-caption="' . $Language->phrase("ExportToEmail", true) . '" form="fview_transferenciaslist" data-ew-action="email" data-custom="false" data-hdr="' . $Language->phrase("ExportToEmail", true) . '" data-exported-selected="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fview_transferenciassrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
