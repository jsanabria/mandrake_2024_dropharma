...
class Usuario extends DbTable
{
    ...
    public function __construct()
    {
        ...
        $this->userlevelid->setSelectMultiple(false); // Select one
        ...
        $this->userlevelid->Lookup = new Lookup($this->userlevelid, 'userlevels', false, 'userlevelid', ["userlevelname","","",""], '', '', [], [], [], [], [], [], false, '`userlevelname`', '', "`userlevelname`");
        $this->userlevelid->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->userlevelid->SearchOperators = ["=", "<>", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN", "IS NULL", "IS NOT NULL"];
        ...
        $this->asesor->Lookup = new Lookup($this->asesor, 'asesor', false, 'id', ["nombre","","",""], '', '', [], [], [], [], [], [], false, '`nombre`', '', "`nombre`");
        $this->asesor->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->asesor->SearchOperators = ["=", "<>", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN", "IS NULL", "IS NOT NULL"];
        ...
        $this->activo->Lookup = new Lookup($this->activo, 'usuario', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
        $this->activo->OptionCount = 2;
        $this->activo->SearchOperators = ["=", "<>", "IS NULL", "IS NOT NULL"];
        $this->Fields['activo'] = &$this->activo;

        // userlevelid2
        $this->userlevelid2 = new DbField(
            $this, // Table
            'x_userlevelid2', // Variable name
            'userlevelid2', // Name
            '`userlevelid2`', // Expression
            '`userlevelid2`', // Basic search expression
            3, // Type
            10, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`userlevelid2`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->userlevelid2->InputTextType = "text";
        $this->userlevelid2->Raw = true;
    ...
    public function getDetailUrl()
    {
        // Detail url
        $detailUrl = "";
        if ($this->getCurrentDetailTable() == "usuario_master2") {
            $detailUrl = Container("usuario_master2")->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
            $detailUrl .= "&" . GetForeignKeyUrl("fk__username", $this->_username->CurrentValue);
        }
        if ($detailUrl == "") {
            $detailUrl = "UsuarioList";
        }
        return $detailUrl;
    }
    ...
    public function getEditUrl($parm = "")
    {
        if ($parm != "") {
            ...
            $url = $this->keyUrl("UsuarioEdit", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    public function getCopyUrl($parm = "")
    {
        if ($parm != "") {
            ...
            $url = $this->keyUrl("UsuarioAdd", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    public function renderEditRow()
    {
        ...
        // userlevelid2
        $this->userlevelid2->setupEditAttributes();
        $this->userlevelid2->EditValue = $this->userlevelid2->CurrentValue;
        $this->userlevelid2->PlaceHolder = RemoveHtml($this->userlevelid2->caption());
        if (strval($this->userlevelid2->EditValue) != "" && is_numeric($this->userlevelid2->EditValue)) {
    ...
    {
        WriteAuditLog(CurrentUserIdentifier(), $typ, 'usuario');
    }
    ...
    public function writeAuditTrailOnAdd(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "A", 'usuario', $fldname, $key, "", $newvalue);
            }
        }
    }
        ...
        foreach (array_keys($rsnew) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && array_key_exists($fldname, $rsold) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                if ($modified) {
                    ...
                    WriteAuditLog($usr, "U", 'usuario', $fldname, $key, $oldvalue, $newvalue);
                }
            }
        }
    }
    ...
    public function writeAuditTrailOnDelete(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "D", 'usuario', $fldname, $key, $oldvalue);
            }
        }
    }
    ...
    public function rowInserting($rsold, &$rsnew) {
    	...
    	/*if() {
    		$this->CancelMessage = "El usuario debe ser cliente, proveedor o asesor,  .";
    		return FALSE;
    	}*/
    	return TRUE;
    }
