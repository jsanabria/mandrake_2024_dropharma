 ...
 * Page class
 */
class ContTerceroMdkEdit extends ContTerceroMdk
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ContTerceroMdkEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "ContTerceroMdkEdit";
    ...
    public function run()
    {
            ...
            if ($this->isShow()) {
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if ($this->TotalRecords <= 0) { // No record found
                        ...
                        $this->terminate("ContTerceroMdkList"); // Return to list page
                        return;
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("ContTerceroMdkList"); // No matching record, return to list
                        return;
                    }
                } // End modal checking
                break;
            case "update": // Update
                ...
                if (GetPageName($returnUrl) == "ContTerceroMdkList") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "ContTerceroMdkList") {
                            ...
                            $returnUrl = "ContTerceroMdkList"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'origen_tabla' first before field var 'x_origen_tabla'
        $val = $CurrentForm->hasValue("origen_tabla") ? $CurrentForm->getValue("origen_tabla") : $CurrentForm->getValue("x_origen_tabla");
        if (!$this->origen_tabla->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->origen_tabla->Visible = false; // Disable update for API request
            } else {
                $this->origen_tabla->setFormValue($val);
            }
        }

        // Check field name 'origen_id' first before field var 'x_origen_id'
        $val = $CurrentForm->hasValue("origen_id") ? $CurrentForm->getValue("origen_id") : $CurrentForm->getValue("x_origen_id");
        if (!$this->origen_id->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->origen_id->Visible = false; // Disable update for API request
            } else {
                $this->origen_id->setFormValue($val, true, $validate);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->origen_tabla->CurrentValue = $this->origen_tabla->FormValue;
        $this->origen_id->CurrentValue = $this->origen_id->FormValue;
    ...
    public function renderRow()
    {
        ...
        // origen_tabla
        $this->origen_tabla->RowCssClass = "row";

        // origen_id
        $this->origen_id->RowCssClass = "row";
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            $this->origen_tabla->EditValue = HtmlEncode($this->origen_tabla->CurrentValue);
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            ...
            if ($this->origen_tabla->Visible && $this->origen_tabla->Required) {
                if (!$this->origen_tabla->IsDetailKey && EmptyValue($this->origen_tabla->FormValue)) {
                    $this->origen_tabla->addErrorMessage(str_replace("%s", $this->origen_tabla->caption(), $this->origen_tabla->RequiredErrorMessage));
                }
            }
            if ($this->origen_id->Visible && $this->origen_id->Required) {
                if (!$this->origen_id->IsDetailKey && EmptyValue($this->origen_id->FormValue)) {
                    $this->origen_id->addErrorMessage(str_replace("%s", $this->origen_id->caption(), $this->origen_id->RequiredErrorMessage));
                }
            }
            if (!CheckInteger($this->origen_id->FormValue)) {
                $this->origen_id->addErrorMessage($this->origen_id->getErrorMessage(false));
            }
    ...
    protected function getEditRow($rsold)
    {
        ...
        // origen_tabla
        $this->origen_tabla->setDbValueDef($rsnew, $this->origen_tabla->CurrentValue, $this->origen_tabla->ReadOnly);

        // origen_id
        $this->origen_id->setDbValueDef($rsnew, $this->origen_id->CurrentValue, $this->origen_id->ReadOnly);
    ...
    protected function restoreEditFormFromRow($row)
    {
        ...
        if (isset($row['origen_tabla'])) { // origen_tabla
            $this->origen_tabla->CurrentValue = $row['origen_tabla'];
        }
        if (isset($row['origen_id'])) { // origen_id
            $this->origen_id->CurrentValue = $row['origen_id'];
        }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("ContTerceroMdkList"), "", $this->TableVar, true);
        $pageId = "edit";
