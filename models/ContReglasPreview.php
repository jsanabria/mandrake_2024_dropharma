 ...
 * Page class
 */
class ContReglasPreview extends ContReglas
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ContReglasPreview";
    ...
    // CSS class/style
    public $CurrentPageName = "ContReglasPreview";
    ...
    protected function masterKeyUrl()
    {
        ...
        if ($masterTblVar == "cont_reglas_hr") {
            $url = "" . Config("TABLE_SHOW_MASTER") . "=cont_reglas_hr&" . GetForeignKeyUrl("fk_id", $this->regla->QueryStringValue) . "";
        }
        return $url;
    }
    ...
    protected function setupForeignKeys($keys)
    {
        $masterTblVar = Get("t", "");
        if ($masterTblVar == "cont_reglas_hr") {
            $this->regla->setQueryStringValue($keys[0] ?? "");
        }
    }
