...
class SalidasList extends Salidas
{
    ...
    // CSS class/style
    public $CurrentPageName = "SalidasList";
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server") {
            $savedFilterList = Profile()->getSearchFilters("fsalidassrch");
        }
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fsalidassrch", $filters);
    ...
    public function showFilterList()
    {
        ...
        // Field iva
        $filter = $this->queryBuilderWhere("iva");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->iva, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->iva->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
        ...
        // Field nota
        $filter = $this->queryBuilderWhere("nota");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->nota, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->nota->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
        ...
        // Field asesor
        $filter = $this->queryBuilderWhere("asesor");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->asesor, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->asesor->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
        ...
        // Field consignacion
        $filter = $this->queryBuilderWhere("consignacion");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->consignacion, false, false);
        }
        if ($filter != "") {
        ...
        // Field comprobante
        $filter = $this->queryBuilderWhere("comprobante");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->comprobante, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->comprobante->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }

        // Field nro_despacho
        $filter = $this->queryBuilderWhere("nro_despacho");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->nro_despacho, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->nro_despacho->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
        ...
        if (!$filter) {
            $this->buildSearchSql($filter, $this->id_documento_padre, false, false);
        }
        if ($filter != "") {
        ...
        // Field archivo_pedido
        $filter = $this->queryBuilderWhere("archivo_pedido");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->archivo_pedido, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->archivo_pedido->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }

        // Field checker
        $filter = $this->queryBuilderWhere("checker");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->checker, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->checker->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }

        // Field checker_date
        $filter = $this->queryBuilderWhere("checker_date");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->checker_date, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->checker_date->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }

        // Field packer
        $filter = $this->queryBuilderWhere("packer");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->packer, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->packer->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }

        // Field packer_date
        $filter = $this->queryBuilderWhere("packer_date");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->packer_date, false, false);
        }
        if ($filter != "") {
        ...
        // Field descuento3
        $filter = $this->queryBuilderWhere("descuento3");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->descuento3, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->descuento3->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
        ...
        // Field nacionalizacion
        $filter = $this->queryBuilderWhere("nacionalizacion");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->nacionalizacion, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->nacionalizacion->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }

        // Field arquitecto
        $filter = $this->queryBuilderWhere("arquitecto");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->arquitecto, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->arquitecto->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }

        // Field ganancia
        $filter = $this->queryBuilderWhere("ganancia");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->ganancia, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->ganancia->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }

        // Field intermediario
        $filter = $this->queryBuilderWhere("intermediario");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->intermediario, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->intermediario->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }

        // Field telefeno_intermediario
        $filter = $this->queryBuilderWhere("telefeno_intermediario");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->telefeno_intermediario, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->telefeno_intermediario->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }

        // Field email_intermediario
        $filter = $this->queryBuilderWhere("email_intermediario");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->email_intermediario, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->email_intermediario->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }

        // Field genera_ne
        $filter = $this->queryBuilderWhere("genera_ne");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->genera_ne, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->genera_ne->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }

        // Field visita
        $filter = $this->queryBuilderWhere("visita");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->visita, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->visita->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
        if ($this->BasicSearch->Keyword != "") {
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->descuento3); // descuento3
            $this->updateSort($this->impreso); // impreso
            $this->updateSort($this->nacionalizacion); // nacionalizacion
            $this->updateSort($this->arquitecto); // arquitecto
            $this->updateSort($this->ganancia); // ganancia
            $this->updateSort($this->intermediario); // intermediario
            $this->updateSort($this->telefeno_intermediario); // telefeno_intermediario
            $this->updateSort($this->email_intermediario); // email_intermediario
            $this->updateSort($this->genera_ne); // genera_ne
            $this->updateSort($this->visita); // visita
    ...
    public function renderListOptions()
    {
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"salidas\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $Language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($Security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"salidas\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-btn=\"SaveBtn\">" . $Language->phrase("EditLink") . "</a>";
                } else {
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fsalidaslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fsalidaslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
        ...
        if ($Security->allowList(CurrentProjectID() . 'entradas_salidas')) {
            ...
            $body = "<a class=\"btn btn-default ew-row-link ew-detail" . ($this->ListOptions->UseDropDownButton ? " dropdown-toggle" : "") . "\" data-action=\"list\" href=\"" . HtmlEncode("EntradasSalidasList?" . Config("TABLE_SHOW_MASTER") . "=salidas&" . GetForeignKeyUrl("fk_tipo_documento", $this->tipo_documento->CurrentValue) . "&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
        ...
        if ($Security->allowList(CurrentProjectID() . 'pagos')) {
            ...
            $body = "<a class=\"btn btn-default ew-row-link ew-detail" . ($this->ListOptions->UseDropDownButton ? " dropdown-toggle" : "") . "\" data-action=\"list\" href=\"" . HtmlEncode("PagosList?" . Config("TABLE_SHOW_MASTER") . "=salidas&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue) . "&" . GetForeignKeyUrl("fk_tipo_documento", $this->tipo_documento->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
    ...
    protected function renderListOptionsExt()
    {
        ...
        if ($this->DetailPages?->getItem("entradas_salidas")?->Visible && $Security->allowList(CurrentProjectID() . 'entradas_salidas')) {
            $link = "";
            $option = $this->ListOptions["detail_entradas_salidas"];
            $detailTbl = Container("entradas_salidas");
            $detailFilter = $detailTbl->getDetailFilter($this);
            $detailTbl->setCurrentMasterTable($this->TableVar);
            $detailFilter = $detailTbl->applyUserIDFilters($detailFilter);
            $url = "EntradasSalidasPreview?t=salidas&f=" . Encrypt($detailFilter . "|" . implode("|", array_values($masterKeys)));
            $btngrp = "<div data-table=\"entradas_salidas\" data-url=\"" . $url . "\" class=\"ew-detail-btn-group d-none\">";
            if ($Security->allowList(CurrentProjectID() . 'salidas')) {
                $label = $Language->tablePhrase("entradas_salidas", "TblCaption");
                $link = "<button class=\"nav-link\" data-bs-toggle=\"tab\" data-table=\"entradas_salidas\" data-url=\"" . $url . "\" type=\"button\" role=\"tab\" aria-selected=\"false\">" . $label . "</button>";
                $detaillnk = GetUrl("EntradasSalidasList?" . Config("TABLE_SHOW_MASTER") . "=salidas");
                foreach ($masterKeys as $key => $value) {
                    $detaillnk .= "&" . GetForeignKeyUrl("fk_$key", $value);
                }
                $title = $Language->tablePhrase("entradas_salidas", "TblCaption");
            ...
            if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'salidas')) {
                ...
                $viewurl = GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=entradas_salidas"));
            ...
            if ($detailPageObj->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'salidas')) {
                ...
                $editurl = GetUrl($this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=entradas_salidas"));
        ...
        if ($this->DetailPages?->getItem("pagos")?->Visible && $Security->allowList(CurrentProjectID() . 'pagos')) {
            ...
            $detailTbl->setCurrentMasterTable($this->TableVar);
            $detailFilter = $detailTbl->applyUserIDFilters($detailFilter);
            $url = "PagosPreview?t=salidas&f=" . Encrypt($detailFilter . "|" . implode("|", array_values($masterKeys)));
            $btngrp = "<div data-table=\"pagos\" data-url=\"" . $url . "\" class=\"ew-detail-btn-group d-none\">";
            if ($Security->allowList(CurrentProjectID() . 'salidas')) {
                ...
                $detaillnk = GetUrl("PagosList?" . Config("TABLE_SHOW_MASTER") . "=salidas");
                foreach ($masterKeys as $key => $value) {
                    $detaillnk .= "&" . GetForeignKeyUrl("fk_$key", $value);
                }
            ...
            if ($detailPageObj->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'salidas')) {
                $caption = $Language->phrase("MasterDetailEditLink");
                $editurl = GetUrl($this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=pagos"));
                $btngrp .= "<a href=\"#\" class=\"me-2\" title=\"" . HtmlTitle($caption) . "\" data-ew-action=\"redirect\" data-url=\"" . HtmlEncode($editurl) . "\">" . $caption . "</a>";
            }
            $btngrp .= "</div>";
            if ($link != "") {
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
            $item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"salidas\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("AddLink") . "</a>";
        } else {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "iva");
            ...
            $this->createColumnOption($option, "asesor");
            ...
            $this->createColumnOption($option, "nro_despacho");
            ...
            $this->createColumnOption($option, "checker");
            ...
            $this->createColumnOption($option, "descuento3");
            $this->createColumnOption($option, "impreso");
            $this->createColumnOption($option, "nacionalizacion");
            $this->createColumnOption($option, "arquitecto");
            $this->createColumnOption($option, "ganancia");
            $this->createColumnOption($option, "intermediario");
            $this->createColumnOption($option, "telefeno_intermediario");
            $this->createColumnOption($option, "email_intermediario");
            $this->createColumnOption($option, "genera_ne");
            $this->createColumnOption($option, "visita");
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fsalidassrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fsalidassrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions as $listAction) {
            if ($listAction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fsalidaslist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_salidas", "data-rowtype" => RowType::ADD]);
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            $this->iva->EditValue = $this->iva->AdvancedSearch->SearchValue;
            ...
            $this->nota->EditValue = HtmlEncode($this->nota->AdvancedSearch->SearchValue);
            ...
            if ($curVal != "") {
                ...
                $this->asesor->AdvancedSearch->ViewValue = $this->asesor->Lookup !== null && is_array($this->asesor->lookupOptions()) && count($this->asesor->lookupOptions()) > 0 ? $curVal : null;
            }
            if ($this->asesor->AdvancedSearch->ViewValue !== null) { // Load from cache
                $this->asesor->EditValue = array_values($this->asesor->lookupOptions());
                if ($this->asesor->AdvancedSearch->ViewValue == "") {
                    $this->asesor->AdvancedSearch->ViewValue = $Language->phrase("PleaseSelect");
                }
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = SearchFilter($this->asesor->Lookup->getTable()->Fields["username"]->searchExpression(), "=", $this->asesor->AdvancedSearch->SearchValue, $this->asesor->Lookup->getTable()->Fields["username"]->searchDataType(), "");
                }
                ...
                if ($ari > 0) { // Lookup values found
                    $arwrk = $this->asesor->Lookup->renderViewRow($rswrk[0]);
                    $this->asesor->AdvancedSearch->ViewValue = $this->asesor->displayValue($arwrk);
                } else {
                    $this->asesor->AdvancedSearch->ViewValue = $Language->phrase("PleaseSelect");
                }
                $arwrk = $rswrk;
            ...
            $this->comprobante->EditValue = $this->comprobante->AdvancedSearch->SearchValue;
            ...
            if (!$this->archivo_pedido->Raw) {
                $this->archivo_pedido->AdvancedSearch->SearchValue = HtmlDecode($this->archivo_pedido->AdvancedSearch->SearchValue);
            }
            $this->archivo_pedido->EditValue = HtmlEncode($this->archivo_pedido->AdvancedSearch->SearchValue);
            $this->archivo_pedido->PlaceHolder = RemoveHtml($this->archivo_pedido->caption());

            // checker
            $this->checker->setupEditAttributes();
            if (!$this->checker->Raw) {
                $this->checker->AdvancedSearch->SearchValue = HtmlDecode($this->checker->AdvancedSearch->SearchValue);
            }
            $this->checker->EditValue = HtmlEncode($this->checker->AdvancedSearch->SearchValue);
            $this->checker->PlaceHolder = RemoveHtml($this->checker->caption());

            // checker_date
            $this->checker_date->setupEditAttributes();
            $this->checker_date->EditValue = HtmlEncode(FormatDateTime(UnFormatDateTime($this->checker_date->AdvancedSearch->SearchValue, $this->checker_date->formatPattern()), $this->checker_date->formatPattern()));
            $this->checker_date->PlaceHolder = RemoveHtml($this->checker_date->caption());

            // packer
            $this->packer->setupEditAttributes();
            if (!$this->packer->Raw) {
                $this->packer->AdvancedSearch->SearchValue = HtmlDecode($this->packer->AdvancedSearch->SearchValue);
            }
            $this->packer->EditValue = HtmlEncode($this->packer->AdvancedSearch->SearchValue);
            ...
            $this->descuento3->EditValue = $this->descuento3->AdvancedSearch->SearchValue;
            ...
            $this->nacionalizacion->EditValue = $this->nacionalizacion->AdvancedSearch->SearchValue;
            $this->nacionalizacion->PlaceHolder = RemoveHtml($this->nacionalizacion->caption());

            // arquitecto
            $this->arquitecto->setupEditAttributes();
            $this->arquitecto->EditValue = $this->arquitecto->AdvancedSearch->SearchValue;
            $this->arquitecto->PlaceHolder = RemoveHtml($this->arquitecto->caption());

            // ganancia
            $this->ganancia->setupEditAttributes();
            $this->ganancia->EditValue = $this->ganancia->AdvancedSearch->SearchValue;
            ...
            if (!$this->intermediario->Raw) {
                $this->intermediario->AdvancedSearch->SearchValue = HtmlDecode($this->intermediario->AdvancedSearch->SearchValue);
            }
            $this->intermediario->EditValue = HtmlEncode($this->intermediario->AdvancedSearch->SearchValue);
            ...
            if (!$this->telefeno_intermediario->Raw) {
                $this->telefeno_intermediario->AdvancedSearch->SearchValue = HtmlDecode($this->telefeno_intermediario->AdvancedSearch->SearchValue);
            }
            $this->telefeno_intermediario->EditValue = HtmlEncode($this->telefeno_intermediario->AdvancedSearch->SearchValue);
            ...
            if (!$this->email_intermediario->Raw) {
                $this->email_intermediario->AdvancedSearch->SearchValue = HtmlDecode($this->email_intermediario->AdvancedSearch->SearchValue);
            }
            $this->email_intermediario->EditValue = HtmlEncode($this->email_intermediario->AdvancedSearch->SearchValue);
            ...
            $this->visita->EditValue = $this->visita->AdvancedSearch->SearchValue;
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmail", true) . '" data-caption="' . $Language->phrase("ExportToEmail", true) . '" form="fsalidaslist" data-ew-action="email" data-custom="false" data-hdr="' . $Language->phrase("ExportToEmail", true) . '" data-exported-selected="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fsalidassrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    */
    ...
    public function pageRender() {
    	...
    	case "TDCPDV":
    		...
    		// $this->asesor_asignado->Visible = TRUE;
    		break;
    	case "TDCNET":
    ...
    public function pageDataRendering(&$header) {
    	...
    	case "TDCPDV":
    		...
    		$header .= '<div class="btn-group btn-group-justified" role="group" aria-label="...">
    			<div class="btn-group" role="group">
    				<button type="button" class="btn btn-warning">NUEVO</button>
    			</div>
    			<div class="btn-group" role="group">
    			  	<button type="button" class="btn btn-info">RECIBIDO</button>
    			</div>
    			<div class="btn-group" role="group">
    				<button type="button" class="btn btn-success">PROCESADO</button>
    			</div>
    			<div class="btn-group" role="group">
    	...
    	case "TDCNET":
    		$header .= '<div class="btn-group btn-group-justified" role="group" aria-label="...">
    			...
    			<div class="btn-group" role="group">
    			  	<button type="button" class="btn" style="background-color: #6c757d; color: #ffffff;">EMBALADO</button>
    			</div>
    			<div class="btn-group" role="group">
    			  	<button type="button" class="btn" style="background-color: #343a40; color: #ffffff;">ENTREGADO</button>
    			</div>
    			<div class="btn-group" role="group">
    				<button type="button" class="btn btn-danger">ANULADO</button>
    			</div>
    		...
    		break;
    	default: 
    		$header = '<button class="btn btn-primary" id="btnResumen2">Resumen de Facturaci&oacute;n Detal</button>';
    		$header .= '<div class="btn-group btn-group-justified" role="group" aria-label="...">
    			<div class="btn-group" role="group">
    				<button type="button" class="btn btn-warning">NUEVO</button>
    			</div>
    	...
    	case "TDCPDV":
    		...
    		$recibido = '<div class="btn-group" role="group">
    			  	<button type="button" class="btn btn-info">RECIBIDO</button>
    				</div>';
    		break;
    	case "TDCFCV":
    		$header = '<button class="btn btn-primary" id="btnResumen">Resumen de Facturaci&oacute;n</button> <button class="btn btn-primary" id="btnGanancia">Resumen Utilidad</button>';
    		$pagado = '<div class="btn-group" role="group">
    			  	<button type="button" class="btn btn-info">PAGADO</button>
    				</div>';
    		break;
    	}
    	...
    	*/
        ...
        if($tipo == "TDCFCV") {    
            if(VerificaFuncion("021")) {
    			...
    			$header .= '<a class="btn btn-primary" id="btnNuevo" href="SalidasAdd?showdetail="><span class="fas fa-plus"></span> Nuevo Documento</a>';
            }
        }
        else {
    		$header .= '<a class="btn btn-primary" id="btnNuevo" href="SalidasAdd?showdetail="><span class="fas fa-plus"></span> Nuevo Documento</a>';
    ...
    public function listOptionsRendered() {
        ...
        case "TDCPDV":
        	...
        	$url = "PedidoDeVentaDetalleCopia?id=$id";
        ...
        case "TDCNET":
            /*
        	$url = "SalidasView?showdetail=entradas_salidas&id=$id&tipo=TDCNET";
            $this->ListOptions->Items["view"]->Body = '<a class="fas fa-eye" href="' . $url . '" data-toggle="tooltip" title="Ver" data-placement="bottom"></a>';
        	...
        	if(VerificaFuncion('011')) {
        		if($this->estatus->CurrentValue == "NUEVO") {
    				$url = "ConfirmPage?page=TDCNET2&id=$id";
    				$this->ListOptions->Items["new"]->Body = '<a class="fas fa-cog" href=" ' . $url . '" data-caption="View" data-toggle="tooltip" title="Modificar Nota de Entrega" data-placement="bottom" ></a>';
        		}
        		else
        			$this->ListOptions->Items["new2"]->Body ='';
        	}
        	if($this->estatus->CurrentValue == "PROCESADO" or $this->estatus->CurrentValue == "NUEVO") {
        		if(intval($this->bultos->CurrentValue) > 0) {
        			$url = "reportes/bultos_qr.php?id=$id";
        			...
        			$url = "SalidasEdit?showdetail=&id=$id";
        			$this->ListOptions->Items["qr"]->Body = '<a class="fas fa-qrcode" href="' . $url . '" target="_blank" data-toggle="tooltip" title="QR" data-placement="bottom"></a>';
        		}
        	} else $this->ListOptions->Items["qr"]->Body = '';
       		*/
        	$url = "reportes/nota_de_entrega.php?id=$id&tipo=TDCNET";
            $this->ListOptions->Items["print"]->Body ='<a class="fas fa-print" target="_blank" href="' . $url . '" data-toggle="tooltip" title="Imprimir" data-placement="bottom"></a>';
        ...
        case "TDCFCV":
            ...
            $this->ListOptions->Items["delete"]->Body ='<a class="fas fa-trash" href="' . $url . '" data-toggle="tooltip" title="Borrar" data-placement="bottom"></a>';
        	$url = "reportes/factura_de_venta.php?id=$id&tipo=TDCFCV";
            $this->ListOptions->Items["print"]->Body ='<a target="_blank" class="fas fa-print" href="' . $url . '" data-toggle="tooltip" title="Imprimir" data-placement="bottom"></a>';
            if(VerificaFuncion("021")) {
            	$this->ListOptions->Items["view"]->Body = '';
            	$this->ListOptions->Items["edit"]->Body = '';
            	$this->ListOptions->Items["edit2"]->Body = '';
            	$this->ListOptions->Items["copy"]->Body = '';
