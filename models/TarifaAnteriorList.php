 ...
 * Page class
 */
class TarifaAnteriorList extends TarifaAnterior
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "TarifaAnteriorList";
    ...
    public $FormName = "ftarifa_anteriorlist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "TarifaAnteriorList";
    ...
    public function setVisibility()
    {
        ...
        $this->precio_anterior->setVisibility();
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'tarifa_anterior';
        $this->TableName = 'tarifa_anterior';
        ...
        // Table object (tarifa_anterior)
        if (!isset($GLOBALS["tarifa_anterior"]) || $GLOBALS["tarifa_anterior"]::class == PROJECT_NAMESPACE . "tarifa_anterior") {
            $GLOBALS["tarifa_anterior"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "TarifaAnteriorAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "TarifaAnteriorDelete";
        $this->MultiUpdateUrl = "TarifaAnteriorUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'tarifa_anterior');
        }
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                    ...
                    $result["view"] = SameString($pageName, "TarifaAnteriorView"); // If View page, no primary button
                } else { // List page
    ...
    public function run()
    {
        ...
        if ($this->IsModal) {
            $this->FormName = "ftarifa_anteriorgrid";
        }
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server") {
            $savedFilterList = Profile()->getSearchFilters("ftarifa_anteriorsrch");
        }
        ...
        $filterList = Concat($filterList, $this->precio_anterior->AdvancedSearch->toJson(), ","); // Field precio_anterior
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("ftarifa_anteriorsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        $this->precio_anterior->AdvancedSearch->SearchValue = @$filter["x_precio_anterior"];
        $this->precio_anterior->AdvancedSearch->SearchOperator = @$filter["z_precio_anterior"];
        $this->precio_anterior->AdvancedSearch->SearchCondition = @$filter["v_precio_anterior"];
        $this->precio_anterior->AdvancedSearch->SearchValue2 = @$filter["y_precio_anterior"];
        $this->precio_anterior->AdvancedSearch->SearchOperator2 = @$filter["w_precio_anterior"];
        $this->precio_anterior->AdvancedSearch->save();
    ...
    public function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->precio_anterior, $default, false); // precio_anterior
        ...
        if (!$default && $this->Command == "search") {
            ...
            $this->precio_anterior->AdvancedSearch->save(); // precio_anterior
    ...
    public function showFilterList()
    {
        ...
        // Field precio_anterior
        $filter = $this->queryBuilderWhere("precio_anterior");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->precio_anterior, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->precio_anterior->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->precio_anterior->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    protected function resetAdvancedSearchParms()
    {
        ...
        $this->precio_anterior->AdvancedSearch->unsetSession();
    ...
    protected function restoreSearchParms()
    {
        ...
        $this->precio_anterior->AdvancedSearch->load();
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->precio_anterior); // precio_anterior
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"ftarifa_anteriorlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"ftarifa_anteriorlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "precio_anterior");
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"ftarifa_anteriorsrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"ftarifa_anteriorsrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions as $listAction) {
            if ($listAction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="ftarifa_anteriorlist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_tarifa_anterior", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_tarifa_anterior",
    ...
    protected function loadSearchValues()
    {
        ...
        // precio_anterior
        if ($this->precio_anterior->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->precio_anterior->AdvancedSearch->SearchValue != "" || $this->precio_anterior->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
    ...
    public function loadRowValues($row = null)
    {
        ...
        $this->precio_anterior->setDbValue($row['precio_anterior']);
    ...
    protected function newRow()
    {
        ...
        $row['precio_anterior'] = $this->precio_anterior->DefaultValue;
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            if ($curVal != "") {
                ...
                if ($this->tarifa->ViewValue === null) { // Lookup from database
                    ...
                    $sqlWrk = $this->tarifa->Lookup->getSql(false, $filterWrk, '', $this, true, true);
                    $conn = Conn();
            ...
            // precio_anterior
            $this->precio_anterior->ViewValue = $this->precio_anterior->CurrentValue;
            $this->precio_anterior->ViewValue = FormatNumber($this->precio_anterior->ViewValue, $this->precio_anterior->formatPattern());
            ...
            // precio_anterior
            $this->precio_anterior->HrefValue = "";
            $this->precio_anterior->TooltipValue = "";
            ...
            $this->tarifa->EditValue = $this->tarifa->AdvancedSearch->SearchValue;
            $curVal = strval($this->tarifa->AdvancedSearch->SearchValue);
            if ($curVal != "") {
                $this->tarifa->EditValue = $this->tarifa->lookupCacheOption($curVal);
                if ($this->tarifa->EditValue === null) { // Lookup from database
                    $filterWrk = SearchFilter($this->tarifa->Lookup->getTable()->Fields["id"]->searchExpression(), "=", $curVal, $this->tarifa->Lookup->getTable()->Fields["id"]->searchDataType(), "");
                    $sqlWrk = $this->tarifa->Lookup->getSql(false, $filterWrk, '', $this, true, true);
                    $conn = Conn();
                    $config = $conn->getConfiguration();
                    ...
                    if ($ari > 0) { // Lookup values found
                        $arwrk = $this->tarifa->Lookup->renderViewRow($rswrk[0]);
                        $this->tarifa->EditValue = $this->tarifa->displayValue($arwrk);
                    } else {
                        $this->tarifa->EditValue = HtmlEncode($this->tarifa->AdvancedSearch->SearchValue);
                    }
                }
            } else {
                $this->tarifa->EditValue = null;
            }
            ...
            $curVal = strval($this->fabricante->AdvancedSearch->SearchValue);
            if ($curVal != "") {
                ...
                if ($this->fabricante->EditValue === null) { // Lookup from database
                    ...
                    if ($ari > 0) { // Lookup values found
                        ...
                        $this->fabricante->EditValue = HtmlEncode($this->fabricante->AdvancedSearch->SearchValue);
                    }
                }
            } else {
            ...
            // precio_anterior
            $this->precio_anterior->setupEditAttributes();
            $this->precio_anterior->EditValue = $this->precio_anterior->AdvancedSearch->SearchValue;
            $this->precio_anterior->PlaceHolder = RemoveHtml($this->precio_anterior->caption());
    ...
    protected function validateSearch()
    {
        ...
        if (!CheckInteger($this->tarifa->AdvancedSearch->SearchValue)) {
            $this->tarifa->addErrorMessage($this->tarifa->getErrorMessage(false));
        }
        if (!CheckInteger($this->fabricante->AdvancedSearch->SearchValue)) {
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmail", true) . '" data-caption="' . $Language->phrase("ExportToEmail", true) . '" form="ftarifa_anteriorlist" data-ew-action="email" data-custom="false" data-hdr="' . $Language->phrase("ExportToEmail", true) . '" data-exported-selected="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"ftarifa_anteriorsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
