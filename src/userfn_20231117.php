...
function Menu_Rendering($menu) {
	...
	if($grupo == "CLIENTE") {
		if ($menu->IsRoot) { // Root menu
			...
			//$menu->AddMenuItem(2, "Articulo", "Consulta de Articulos", "ArticuloList");
...
class PdvLineaGuardar {
  ...
  function insertar_articulo($tipo_documento="TDCPDV", $salida, $cliente, $cod_articulo, $lista_pedido, $cantidad, $tarifa_default) 
  { 
...
class PdcLineaGuardar {
  ...
  function insertar_articulo($tipo_documento, $entrada, $proveedor, $cod_articulo, $cantidad) 
  { 
    $this->tipo_documento = $tipo_documento;
...
class FctLineaGuardar {
  ...
  function insertar_articulo($tipo_documento, $salida, $cod_articulo, $cantidad, $precio, $lote, $fecha) 
  { 
    $this->tipo_documento = $tipo_documento;
    $this->salida = $salida;
    $this->cod_articulo = $cod_articulo;
    ...
    $fecha = validar_fecha_vencimiento($fecha) == "NULL" ? "NULL" : "'" . validar_fecha_vencimiento($fecha) . "'";
    $sql = "INSERT INTO entradas_salidas
          (id, tipo_documento, id_documento, 
          fabricante, articulo, almacen, 
          cantidad_articulo, articulo_unidad_medida, cantidad_unidad_medida, cantidad_movimiento, 
          precio_unidad, precio, alicuota, descuento, precio_unidad_sin_desc, costo_unidad, costo, lote, fecha_vencimiento)
        VALUES 
          (NULL, '$this->tipo_documento', $this->salida, 
          $this->fabricante, $this->articulo, '$this->almacen', 
          $this->cantidad, '$this->unidad_medida', $this->cantidad_unidad, $this->cantidad_movimiento, 
          $this->precio, $this->total, $this->alicuota, $this->descuento, $this->precio_ful, $this->costo, $this->total_costo, '$lote', $fecha);";
    Execute($sql);
    return true;
  }
  function ActualizarCabecera() {
