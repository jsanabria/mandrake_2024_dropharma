...
$sql_header = "SELECT 
                    ...
                    a.nro_documento, 
                    DATE_FORMAT(a.fecha, '%d/%m/%Y') AS FECHA, a.nro_documento AS NO_NOTA_ENTREGA, b.ci_rif AS CLIENTE_RIF, b.nombre AS CLIENTE_NOMBRE 
                FROM 
                    salidas AS a 
                    JOIN cliente AS b ON b.id = a.cliente 
                WHERE a.id = $id_pedido AND a.tipo_documento = 'TDCNET'; ";
...
if ($row_header = $result_header->fetch_assoc()) {
    ...
    $sheet->setCellValue('F' . $current_row, "FECHA");
    $sheet->setCellValue('G' . $current_row, $row_header["FECHA"]);
    ...
    $sheet->setCellValue('F' . $current_row, "NO PEDIDO");
    $sheet->setCellValue('G' . $current_row, $row_header["NO_NOTA_ENTREGA"]);
    $sheet->getStyle('A' . $current_row . ':E' . $current_row)->applyFromArray($headerStyle);
    ...
    $sheet->setCellValue('G' . $current_row, "");
    $sheet->getStyle('A' . $current_row . ':F' . $current_row)->applyFromArray($headerStyle);
    $current_row += 1; // Dejar una fila en blanco antes del detalle
} 
$result_header->free();
...
$sql_detail = "SELECT 
                    -- CAST(IF(LTRIM(IFNULL(b.codigo, ''))='', b.codigo_de_barra, b.codigo) AS CHAR) AS COD_ART,  
                    -- CONCAT(LPAD(LTRIM(RTRIM(IFNULL(a.lote, ''))), 10, '0'), '-', LTRIM(RTRIM(IFNULL(b.codigo, '')))) AS COD_ART, 
                    CONCAT(LTRIM(RTRIM(IFNULL(a.lote, ''))), '-', LTRIM(RTRIM(IFNULL(b.codigo, '')))) AS COD_ART, 
                    CONCAT(IFNULL(b.nombre_comercial, ''), IFNULL(b.principio_activo, ''), IFNULL(b.presentacion, '')) AS DESCRIPCION, 
                    LPAD(a.lote, 10, '0') AS LOTE, 
                    CONCAT( LPAD(CONVERT(MONTH(a.fecha_vencimiento), CHAR), 2, '0'), '/', CONVERT(YEAR(a.fecha_vencimiento), CHAR)) AS FEC_VEN,
                    CAST(ABS(a.cantidad_movimiento) AS UNSIGNED) AS CANT, a.precio_unidad AS P_UNT_$, a.precio AS P_TOTAL_$, 
                    a.descuento AS desc1, 
                    (SELECT descuento FROM salidas WHERE id = a.id_documento) AS desc2, 
                    (SELECT descuento2 FROM salidas WHERE id = a.id_documento)  AS desc3 
                FROM 
                    entradas_salidas AS a 
                    JOIN articulo AS b ON b.id = a.articulo  
                WHERE a.id_documento = $id_pedido AND a.tipo_documento = 'TDCNET';"; 
...
$sheet->setCellValue('G' . $current_row, $total);
...
$filename = $nro_documento . "_nota_de_entrega.xls"; // Cambiado a .xlsx
...
$sql = "UPDATE salidas SET estatus = 'PROCESADO' WHERE id = $id_pedido AND tipo_documento = 'TDCNET';";
$conn->query($sql);
