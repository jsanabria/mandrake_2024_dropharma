...
$sql = "SELECT a.proveedor FROM entradas AS a WHERE a.id = $pedido AND a.tipo_documento = 'TDCPDC';";
...
$sql = "SELECT 
        ...
        WHERE 
         z.id_documento = $pedido AND z.tipo_documento = 'TDCPDC'"; 
$result = mysqli_query($link, $sql);
$row = mysqli_fetch_array($result);
...
$sql = "SELECT 
          ...
          z.descuento, z.costo_unidad AS costo, z.costo as total     
        FROM 
          entradas_salidas AS z 
          INNER JOIN articulo AS a ON a.id = z.articulo 
          LEFT OUTER JOIN fabricante AS b ON b.Id = a.fabricante 
        WHERE 
         z.id_documento = $pedido AND tipo_documento = 'TDCPDC' 
...
// die(json_encode($sql3, JSON_UNESCAPED_UNICODE)); 
      ...
      $html .= '</td>';
      $html .= '<td class="text-center">';  
                  $html .= '<input type="text" class="form-control" id="x' . $i . '_costoFull" name="x' . $i . '_costoFull" size="4" onkeyup="myCalc(' . $i . ');" onchange="myCalc(' . $i . ');" value="' . ($xultimo_costo_ful==0 ? "" : number_format($xultimo_costo_ful, 2, ".", "")) . '" style="width: 120px;" ' . ($xCant==0 ? '' : 'disabled="disabled"') . '>';
      $html .= '</td>';
      $html .= '<td class="text-center">';  
                  $html .= '<input type="number" class="form-control" id="x' . $i . '_descuento" name="x' . $i . '_descuento" size="4" onkeyup="myCalc(' . $i . ');" onchange="myCalc(' . $i . ');" value="' . $xdescuentoG . '" style="width: 80px;" ' . ($xCant==0 ? '' : 'disabled="disabled"') . '>';
      $html .= '</td>';
      $html .= '<td class="text-center">';  
                  $html .= '<input type="number" class="form-control" id="x' . $i . '_costo" name="x' . $i . '_costo" size="4" readonly="yes" value="' . ($xultimo_costo==0 ? "" : number_format($xultimo_costo, 2, ".", "")) . '" style="width: 120px;" ' . ($xCant==0 ? '' : 'disabled="disabled"') . '>';
      $html .= '</td>';
      $html .= '<td class="text-center">';  
                  $html .= '<input type="number" class="form-control" id="x' . $i . '_total" name="x' . $i . '_total" size="4" readonly="yes" value="' . ($costo==0 ? "" : $costo) . '" style="width: 150px;" ' . ($xCant==0 ? '' : 'disabled="disabled"') . '>';
      $html .= '</td>';
      $html .= '<td class="text-center">';  
                  if($xCant == 0) {
    ...
    $i++;
      ...
      $html .= '<tr><td colspan="8"> <center><b>Registros ' . $i-1 . ' de ' . $cantidad . '</b></center> </td></tr>';
