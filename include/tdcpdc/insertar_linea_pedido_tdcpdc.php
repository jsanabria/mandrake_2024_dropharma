...
if($pedido == 0) {
	...
	$sql = "INSERT INTO entradas 
				...
				nota, estatus, moneda, consignacion, descuento) 
			VALUES 
				(NULL, '$tipo_documento', '$username', '" . date("Y-m-d H:i:s") . "', $proveedor, '$nro_documento', '$almacen', 
				'$nota', '$estatus', '$moneda', '$consignacion', $descuentoG);";
	...
	$sql = "INSERT INTO audittrail
		...
		VALUES (NULL, '" . date("Y-m-d H:i:s") . "', 'Crea Pedido de Compra NRO $nro_documento', '$username', 'I', 'view_in_tdcpdc', 'id', '$pedido', '$nro_documento', '');";
	mysqli_query($link, $sql);
}
else {
...
$sql = "INSERT INTO audittrail
	(id, datetime, script, `user`, `action`, `table`, `field`, keyvalue, oldvalue, newvalue)
	VALUES (NULL, '" . date("Y-m-d H:i:s") . "', 'Inserta Articulo en Pedido de Compra NRO $nro_documento', '$username', 'I', 'view_in_tdcpdc', 'id', '$pedido', '$articulo', '');";
mysqli_query($link, $sql);
