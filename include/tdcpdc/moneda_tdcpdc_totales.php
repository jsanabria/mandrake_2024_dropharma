...
$sql = "SELECT 
	    ...
	    FROM 
	      entradas_salidas 
	    WHERE 
	      id_documento = '$pedido' AND tipo_documento = 'TDCPDC';";
$rs = mysqli_query($link, $sql);
$row = mysqli_fetch_array($rs);
...
$sql = "SELECT
	    ...
	    FROM 
	      entradas_salidas 
	    WHERE id_documento = '$pedido' AND tipo_documento = 'TDCPDC';"; 
$rs = mysqli_query($link, $sql);
if($row = mysqli_fetch_array($rs)) {
	...
	$sql = "INSERT INTO audittrail
		...
		VALUES (NULL, '" . date("Y-m-d H:i:s") . "', 'Actualiza moneda y/o tasa del dia en Pedido de Compra NRO $nro_documento', '$username', 'U', 'view_in_tdcpdc', 'id', '$pedido', '$moneda', '$tasa_usd');";
	mysqli_query($link, $sql);
