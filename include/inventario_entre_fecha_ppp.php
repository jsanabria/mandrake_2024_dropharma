...
$reporte = $id;
    ...
    $tipo_documento = ($row = mysqli_fetch_array($rs)) ? $row["tipo_documento"] : 'TDCNET';
...
$sql = "SELECT 
            ...
            LEFT OUTER JOIN (
                SELECT a.articulo, SUM(a.cantidad_movimiento) AS cantidad 
                ...
                WHERE ((a.tipo_documento IN ('TDCPDV') AND b.estatus = 'NUEVO') OR 
                       (a.tipo_documento IN ('$tipo_documento', 'TDCASA') AND b.estatus <> 'ANULADO')) 
                      AND b.fecha < '$fecha_hasta 23:59:59' AND a.newdata = 'S' $where 
                GROUP BY a.articulo
            ...
            ORDER BY laboratorio, art.principio_activo, art.presentacion;"; 

    $rs = mysqli_query($link, $sql);
    while($row = mysqli_fetch_array($rs)) {
        // Obtenemos los valores directamente de la fila sin consultar de nuevo
