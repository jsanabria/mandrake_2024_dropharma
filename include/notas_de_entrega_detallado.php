<?php 
    ...
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h4 class="text-secondary m-0">Notas de Entrega Detallado</h4>
    </div>
...
HTML;
    ...
    // 3. Consulta de datos fiel a tu estructura original (Filtrando por TDCNET)
    $sql = "SELECT
                a.id, 
                a.nro_documento, 
    ...
    // 4. Renderizado dinámico de filas con resguardo de nulos para PHP 8.x
    while ($row = mysqli_fetch_array($rs)) {
        ...
        // Conversión entera segura de cantidad
        $val_cantidad = intval($row["cantidad_movimiento"] ?? 0);

        // Clases de Bootstrap dinámicas según el estatus (incluye control para ANULADO)
...
HTML;
        ...
        // Umbral de protección de desbordamiento en vista HTML (Top 20)
        if ($contar >= 20) {
            $out .= <<<HTML
            <tr>
                <th colspan="11" class="text-center table-warning py-3 text-secondary" style="font-size: 0.85rem;">
                    ⚠️ Se visualizan las primeras {$contar} filas en pantalla. Para revisar el histórico completo, por favor usa el botón superior de exportación.
                </th>
            </tr>
HTML;
            break;
    ...
    // 5. Cierre de tabla con sumatoria totalizadora
