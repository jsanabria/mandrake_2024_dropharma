	...
	function __construct($purga, $username, $link) {
		...
		while($row = mysqli_fetch_array($rs)) { 
			...
			if($sw2 == false) {
				...
				$sql3 = "UPDATE entradas_salidas SET check_ne='N' WHERE id_documento = " . $this->head_as . " AND tipo_documento='TDCASA';";
				mysqli_query($this->link, $sql3);
			}
		}
	...
	function Crear_detail_as($head, $id_compra, $cantidad) { 
		$sql = "INSERT INTO entradas_salidas
				...
				SELECT 
					...
					(-1)*$cantidad AS cantidad_movimiento, costo_unidad, costo, $id_compra AS id_compra, 'S' 
				FROM 
