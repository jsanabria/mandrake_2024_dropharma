<?php 
	...
	$fecha_hasta = '2025-10-31'; // date("Y-m-d");
	...
	$sql = "SELECT 
				inventario.articulo, 
				-- CAST(LPAD(LTRIM(RTRIM(IFNULL(inventario.lote, ''))), 10, '0') AS CHAR) AS lote, 
				CAST(LTRIM(RTRIM(IFNULL(inventario.lote, ''))) AS CHAR) AS lote, 
				DATE_FORMAT(inventario.fecha_vencimiento, '%m/%Y') AS fecha_vencimiento, SUM(inventario.cantidad) AS cantidad 
			FROM 
				(
					SELECT 
	...
	while( $row = mysqli_fetch_assoc($rs) ) {
		...
		$sql = "SELECT 
					...
					a.ultimo_costo, a.precio, (SELECT alicuota FROM alicuota WHERE codigo = a.alicuota AND activo = 'S' LIMIT 0, 1) AS alicuota 
				FROM 
					articulo AS a 
	    ...
	    // Creamos un nuevo array temporal para la fila con el orden deseado
	    $newRow = array(
	        'lote' => $row['lote'],
	        'fecha_vencimiento' => $row['fecha_vencimiento'],
	        'codigo' => trim($row['lote']) . "-" . trim($row2['codigo']),
