...
class PurgaProcesar {
	...
	function __construct($purga, $username, $link) {
		...
		while($row = mysqli_fetch_array($rs)) { 
			...
			$sql2 = "SELECT 
					...
					(IFNULL(a.cantidad_movimiento, 0) + IFNULL(d.cantidad_movimiento, 0)) AS cantidad, c.descripcion AS almacen, check_ne  
				FROM 
	...
	function Crear_detail_as($head, $id_compra, $cantidad) { 
		$sql = "INSERT INTO entradas_salidas
					...
					cantidad_movimiento, costo_unidad, costo, id_compra, check_ne) 
				SELECT 
					...
					(-1)*$cantidad AS cantidad_movimiento, costo_unidad, costo, $id_compra AS id_compra, 'N' 
				FROM 
	...
	function Crear_detail_ae($head, $id_compra) { 
		$sql = "INSERT INTO entradas_salidas
				...
				SELECT 
					...
					" . $this->cantidad_articulo . " AS cantidad_movimiento, costo_unidad, (" . $this->cantidad_articulo . "*costo_unidad) AS costo, NULL AS id_compra, 'S' 
				FROM 
					entradas_salidas WHERE id = $id_compra;";
		mysqli_query($this->link, $sql);
	}
	...
	function Crear_detail_ae2($head, $articulo) { 
		$sql = "INSERT INTO entradas_salidas
				...
				SELECT 
					...
					" . $this->cantidad_articulo . " AS cantidad_movimiento, ultimo_costo, (" . $this->cantidad_articulo . "*ultimo_costo) AS costo, NULL AS id_compra, 'S' 
				FROM 
