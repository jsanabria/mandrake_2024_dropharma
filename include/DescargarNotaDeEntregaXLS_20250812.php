...
if ($row_header = $result_header->fetch_assoc()) {
    ...
    $sheet->setCellValue('E' . $current_row, "FECHA");
    $sheet->setCellValue('F' . $current_row, $row_header["FECHA"]);
    ...
    $sheet->setCellValue('E' . $current_row, "NO PEDIDO");
    $sheet->setCellValue('F' . $current_row, $row_header["NO_NOTA_ENTREGA"]);
    $sheet->getStyle('A' . $current_row . ':E' . $current_row)->applyFromArray($headerStyle);
...
$sql_detail = "SELECT 
                    ...
                    CONCAT(a.lote, ' | ', CONVERT(MONTH(a.fecha_vencimiento), CHAR), '/', CONVERT(YEAR(a.fecha_vencimiento), CHAR)) AS lote,
