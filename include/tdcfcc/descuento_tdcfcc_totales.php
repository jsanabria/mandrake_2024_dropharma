...
$sql = "INSERT INTO audittrail
	...
	VALUES (NULL, '" . date("Y-m-d H:i:s") . "', 'Actualiza porcentaje descuento en Factura de Compra NRO $nro_documento', '$username', 'U', 'view_in_tdcfcc', 'id', '$pedido', '$descuentoG', '');";
mysqli_query($link, $sql);
...
$sql = "SELECT 
	    ...
	    FROM 
	      entradas_salidas 
	    WHERE 
	      id_documento = '$pedido' AND tipo_documento = 'TDCFCC';";
$rs = mysqli_query($link, $sql);
$row = mysqli_fetch_array($rs);
...
$sql = "SELECT
	    ...
	    FROM 
	      entradas_salidas 
	    WHERE id_documento = '$pedido' AND tipo_documento = 'TDCFCC';"; 
$rs = mysqli_query($link, $sql);
