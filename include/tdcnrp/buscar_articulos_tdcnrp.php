...
while ($row = mysqli_fetch_array($result)) { 
    ...
    $html .= '<tr>';
      ...
      $html .= '<td class="text-center">'; 
                  ...
                  if($hubb == "SI") {
                  $sql3 = "SELECT 
                              cantidad_articulo AS cantidad, precio_unidad_sin_desc, descuento, costo_unidad, costo, lote, fecha_vencimiento, check_ne, id AS id_item     
...
// die(json_encode($sql3, JSON_UNESCAPED_UNICODE)); 
      ...
      $html .= '<td class="text-center">'; 
                  if($id_item != 0) {
                    $html .= '<br><input type="checkbox" id="x' . $i . '_checkne" name="x' . $i . '_checkne" ' . ($check_ne=="S" ? "checked" : "") . ' onclick="js:check_item(' . $id_item . ')"><br> ';
                  }
                  if($xCant == 0) {
