...
$sql = "INSERT INTO audittrail
	...
	VALUES (NULL, '" . date("Y-m-d H:i:s") . "', 'Actualiza porcentaje descuento en Nota de Recepcion NRO $nro_documento', '$username', 'U', 'view_in_tdcnrp', 'id', '$pedido', '$descuentoG', '');";
mysqli_query($link, $sql);
...
$sql = "SELECT 
	    ...
	    FROM 
	      entradas_salidas 
	    WHERE 
	      id_documento = '$pedido' AND tipo_documento = 'TDCNRP';";
$rs = mysqli_query($link, $sql);
$row = mysqli_fetch_array($rs);
...
$sql = "SELECT
	    ...
	    FROM 
	      entradas_salidas 
	    WHERE id_documento = '$pedido' AND tipo_documento = 'TDCNRP';"; 
$rs = mysqli_query($link, $sql);
