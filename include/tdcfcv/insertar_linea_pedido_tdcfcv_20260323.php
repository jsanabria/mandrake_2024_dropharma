...
if($row = mysqli_fetch_array($rs)) { 
	$tipo_documento_inventario = $row["tipo_documento"];
...
if($pedido == 0) {
	$sql = "INSERT INTO salidas 
				(id, tipo_documento, username, fecha, cliente, 
				nota, estatus, moneda, consignacion, descuento, descuento2, documento, doc_afectado) 
			VALUES 
				(NULL, '$tipo_documento', '$username', '" . date("Y-m-d H:i:s") . "', $cliente, 
				'$nota', '$estatus', '$moneda', 'N', $descuentoG, $descTransferencista, '$consignacion', '$doc_afectado');";
	...
	$sql = "INSERT INTO audittrail
		...
		VALUES (NULL, '" . date("Y-m-d H:i:s") . "', 'Crea Factura de Venta', '$username', 'I', 'view_out_tdcfcv', 'id', '$pedido', 'Nuevo nro de factura', '');";
	mysqli_query($link, $sql);
	$nro_documento = "";
}
else {
...
$sql = "INSERT INTO entradas_salidas
			...
			costo_unidad, costo, 
			precio_unidad, precio, alicuota, descuento, precio_unidad_sin_desc, newdata, lote, fecha_vencimiento)
		VALUES 
			(NULL, '$tipo_documento', $pedido, $fabricante, $articulo, 
			'$almacen', $cantidad, '$unidad_medida', $cantidad_unidad_medida, $cantidad_movimiento, 
			$costo_unidad, $costo, 
			$precio, $total, $alicuota, $descuento, $precioFull,'S', '$lote', '$vence');"; 
...
$sql = "INSERT INTO audittrail
	(id, datetime, script, `user`, `action`, `table`, `field`, keyvalue, oldvalue, newvalue)
	VALUES (NULL, '" . date("Y-m-d H:i:s") . "', 'Inserta Articulo en Factura de Ventas NRO/ID $nro_documento/($pedido)', '$username', 'I', 'view_out_tdcfcc', 'id', '$pedido', '$articulo', '');";
mysqli_query($link, $sql);
